...
[[ "$V" == "2.22.0" ]] || { echo "ERROR: expected Workspace v2.22.0 baseline, found $V" >&2; exit 1; }
cp -a "$HERE/patch/." "$REPO/"
python3 scripts/validate_reliability_survival_v2230.py
...
git add -A
git commit -m "Workspace v2.23.0 — Reliability, Survival & Failure-Time Analysis"
git tag -a v2.23.0 -m "Workspace v2.23.0 — Reliability, Survival & Failure-Time Analysis"
git push origin HEAD:main
git push origin v2.23.0
echo "PASS: Workspace v2.23.0 applied, validated, committed, pushed, and tagged."
