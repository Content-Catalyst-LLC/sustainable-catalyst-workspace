/* Workspace v3.4.0 typed browser client. Service credentials remain server-side in WordPress. */
...
class WorkspaceTypedApiClient {
  ...
  executionProvenance():Promise<WorkspaceApiEnvelope>{return this.request('/execution-provenance');}
  projectExecutionProvenance(projectId:string,limit=250):Promise<WorkspaceApiEnvelope>{return this.request('/execution-provenance/projects/'+encodeURIComponent(projectId)+'?limit='+encodeURIComponent(String(limit)));}
  executionRunProvenance(projectId:string,runId:string):Promise<WorkspaceApiEnvelope>{return this.request('/execution-provenance/projects/'+encodeURIComponent(projectId)+'/runs/'+encodeURIComponent(runId));}
  createExecutionProvenanceSnapshot(projectId:string,request:ScientificExecutionProvenanceSnapshotRequest):Promise<WorkspaceApiEnvelope>{return this.request('/execution-provenance/projects/'+encodeURIComponent(projectId)+'/snapshots',{method:'POST',body:request});}
  executionProvenanceSnapshots(projectId:string,limit=100):Promise<WorkspaceApiEnvelope>{return this.request('/execution-provenance/projects/'+encodeURIComponent(projectId)+'/snapshots?limit='+encodeURIComponent(String(limit)));}
