...
APP=(R/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v2.0.2.js').read_text()
EXP=(R/'wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-experience-v1.js').read_text()
CSS=(R/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v2.0.2.css').read_text()
class WorkspaceReleaseCandidateIContract(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.80.0','0.79.0','Workspace Release Candidate I')); self.assertIn('Version: 2.0.2',MAIN)
 ...
 def test_07_runtime_stage_and_ui(self): self.assertIn('data-release-stage="work-mode-cards-repair"',PHP); self.assertIn('data-scw-release-candidate',PHP); self.assertIn('Run RC gate',PHP); self.assertIn('Export field checklist',PHP); self.assertIn('data-scw-workspace-view="release-candidate"',PHP)
 ...
 def test_09_assets(self): self.assertIn("'sc-workspace-v202'",PHP); self.assertIn('workspace-v2.0.2.js',PHP); self.assertIn('workspace-v2.0.2.css',PHP); self.assertIn('sc-workspace-release-candidate-i-v1',PHP); self.assertIn('sc-workspace-release-candidate-i-ui-v1',PHP); self.assertIn('/* v0.80.0 — Workspace Release Candidate I */',CSS); self.assertIn("'release-candidate'",APP); self.assertIn("id:'release-candidate'",EXP)
 def test_10_registry(self): self.assertEqual((REG['public_version'],REG['previous_version'],REG['release_name']),('0.80.0','0.79.0','Workspace Release Candidate I')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v202'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0790',REGPHP); self.assertTrue((R/'history/release-manifest-v0.79.0.json').exists()); self.assertTrue((R/'history/workspace-product-record-v0.79.0.json').exists())
