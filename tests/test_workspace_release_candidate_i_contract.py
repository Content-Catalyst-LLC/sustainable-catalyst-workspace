...
class WorkspaceReleaseCandidateIContract(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.80.0','0.79.0','Workspace Release Candidate I')); self.assertIn('Version: 1.5.0',MAIN)
 ...
 def test_07_runtime_stage_and_ui(self): self.assertIn('data-release-stage="lab-scientific-integration"',PHP); self.assertIn('data-scw-release-candidate',PHP); self.assertIn('Run RC gate',PHP); self.assertIn('Export field checklist',PHP); self.assertIn('data-scw-workspace-view="release-candidate"',PHP)
 ...
 def test_09_assets(self): self.assertIn("'sc-workspace-v150'",PHP); self.assertIn('workspace-v1.5.0.js',PHP); self.assertIn('workspace-v1.5.0.css',PHP); self.assertIn('sc-workspace-release-candidate-i-v1',PHP); self.assertIn('sc-workspace-release-candidate-i-ui-v1',PHP); self.assertIn('/* v0.80.0 — Workspace Release Candidate I */',CSS); self.assertIn("'release-candidate'",APP); self.assertIn("id:'release-candidate'",EXP)
 def test_10_registry(self): self.assertEqual((REG['public_version'],REG['previous_version'],REG['release_name']),('0.80.0','0.79.0','Workspace Release Candidate I')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v150'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0790',REGPHP); self.assertTrue((R/'history/release-manifest-v0.79.0.json').exists()); self.assertTrue((R/'history/workspace-product-record-v0.79.0.json').exists())
 ...
 def test_12_docs(self): self.assertTrue((R/'docs/WORKSPACE_RELEASE_CANDIDATE_I_V0800.md').exists()); self.assertTrue((R/'RELEASE_NOTES_0.80.0.md').exists()); self.assertIn('# Sustainable Catalyst Workspace v1.5.0',(R/'README.md').read_text()); self.assertIn('General Availability',(R/'README.md').read_text())
