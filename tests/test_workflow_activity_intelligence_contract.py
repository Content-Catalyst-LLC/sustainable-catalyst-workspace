...
JS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.35.0.js';PHP=ROOT/'wordpress/sustainable-catalyst-workspace/includes/class-sc-workspace.php';CSS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.35.0.css';MANIFEST=ROOT/'release-manifest-v0.35.0.json'
class WorkflowActivityIntelligenceContract(unittest.TestCase):
 def test_manifest(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.35.0');self.assertEqual(m['previous_version'],'0.34.0');self.assertEqual(m['storage_schema_version'],31);self.assertEqual(m['project_schema'],'sc-workspace-project/16.0');self.assertEqual(m['activity_intelligence_schema'],'sc-workspace-activity-intelligence/1.0')
 ...
 def test_storage_migration_only(self):
  j=JS.read_text();self.assertIn('const STORAGE_VERSION = 31',j);self.assertIn('function migrateV17(raw)',j);self.assertIn('if (raw.schemaVersion === 17) return migrateV17(raw)',j);self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/16.0'",j)
