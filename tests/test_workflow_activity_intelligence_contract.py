...
class WorkflowActivityIntelligenceContract(unittest.TestCase):
 def test_manifest(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.44.0');self.assertEqual(m['previous_version'],'0.43.0');self.assertEqual(m['storage_schema_version'],35);self.assertEqual(m['project_schema'],'sc-workspace-project/20.0');self.assertEqual(m['activity_intelligence_schema'],'sc-workspace-activity-intelligence/1.0')
