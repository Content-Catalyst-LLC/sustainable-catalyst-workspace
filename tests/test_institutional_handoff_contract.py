...
class InstitutionalHandoffContract(unittest.TestCase):
 def test_release_metadata(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.19.0');self.assertEqual(m['previous_version'],'0.18.0');self.assertEqual(m['release_name'],'Institutional Handoff');self.assertEqual(m['storage_schema_version'],20);self.assertEqual(m['project_schema'],'sc-workspace-project/11.0')
 ...
 def test_registry_lineage(self):
  r=REG.read_text();self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0190'",r);self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0190'",r);self.assertIn("LEGACY_PENDING_KEY_V0180 = 'sc_workspace_registry_pending_v0180'",r)
 def test_css(self):
  c=CSS.read_text();self.assertIn('/* v0.19.0 — Institutional Handoff */',c);self.assertIn('.scw-institutional-boundary',c);self.assertIn('.scw-institutional-readiness',c)
