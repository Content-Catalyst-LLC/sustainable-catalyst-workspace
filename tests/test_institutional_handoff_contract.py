...
MANIFEST=ROOT/'release-manifest-v0.64.1.json'
JS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.64.1.js'
PHP=ROOT/'wordpress/sustainable-catalyst-workspace/includes/class-sc-workspace.php'
CSS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.64.1.css'
...
class InstitutionalHandoffContract(unittest.TestCase):
 def test_release_metadata(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.64.1');self.assertEqual(m['previous_version'],'0.64.0');self.assertEqual(m['release_name'],'Accessibility Runtime & Desktop Layout Recovery');self.assertEqual(m['storage_schema_version'],35);self.assertEqual(m['project_schema'],'sc-workspace-project/20.0')
 ...
 def test_registry_lineage(self):
  r=REG.read_text();self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0641'",r);self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0641'",r);self.assertIn("LEGACY_PENDING_KEY_V0240 = 'sc_workspace_registry_pending_v0240'",r);self.assertIn("LEGACY_PENDING_KEY_V0220 = 'sc_workspace_registry_pending_v0220'",r)
 def test_css(self):
