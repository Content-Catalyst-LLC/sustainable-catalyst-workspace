...
class InstitutionalAuditStudio(unittest.TestCase):
 ...
 def test_06_wordpress(self): self.assertIn('Version: 1.10.0',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '1.10.0'",MAIN);self.assertIn("'/institutional-audit-studio-contract'",PHP);self.assertIn('Institutional Audit Studio',PHP);self.assertIn('Auditability, not automatic compliance',PHP);self.assertIn('data-release-stage="research-operations"',PHP)
 def test_07_deployment(self): self.assertIn("PREVIOUS_RELEASE = '1.9.0'",DEP);self.assertIn("ROLLBACK_RELEASE = '1.9.0'",DEP);self.assertIn('workspace-v1.10.0.js',DEP);self.assertIn('class-sc-workspace-institutional-audit-studio.php',DEP)
 def test_08_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('1.9.0','1.8.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v1100'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V180',REGPHP)
