...
class WorkspaceContract(unittest.TestCase):
 def test_manifest(self):
  m=json.loads((ROOT/'release-manifest-v0.44.0.json').read_text()); self.assertEqual(m['version'],'0.44.0'); self.assertEqual(m['previous_version'],'0.43.0'); self.assertEqual(m['release_name'],'Citation Library & Reference Management'); self.assertFalse(m['account_required']); self.assertTrue(m['anonymous_access']); self.assertEqual(m['cloud_sync'],'explicit-project-enrollment'); self.assertEqual(m['server_project_storage'],'manual-backup-plus-explicit-sync-head'); self.assertEqual(m['canvas_schema'],'sc-workspace-canvas/1.0')
 def test_registry(self):
  r=json.loads((ROOT/'registry/workspace-product-record-v0.44.0.json').read_text()); self.assertEqual(r['public_version'],'0.44.0'); self.assertEqual(r['previous_version'],'0.43.0'); self.assertEqual(r['console_screen'],'commercial'); self.assertEqual(r['family'],'commercial'); self.assertEqual(r['product_url'],'/platform/')
