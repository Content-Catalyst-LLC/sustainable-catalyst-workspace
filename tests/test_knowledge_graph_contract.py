...
class KnowledgeGraphContract(unittest.TestCase):
 def test_manifest(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.20.0');self.assertEqual(m['previous_version'],'0.19.0');self.assertEqual(m['storage_schema_version'],20);self.assertEqual(m['project_schema'],'sc-workspace-project/11.0');self.assertEqual(m['knowledge_graph_schema'],'sc-workspace-knowledge-graph/1.0')
 ...
 def test_storage_migration(self):
  j=JS.read_text();self.assertIn('const STORAGE_VERSION = 20',j);self.assertIn('function migrateV16(raw)',j);self.assertIn('if (raw.schemaVersion === 16) return migrateV16(raw)',j);self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/11.0'",j)
 def test_ui(self):
