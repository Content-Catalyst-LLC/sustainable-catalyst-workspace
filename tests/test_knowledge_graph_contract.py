...
class KnowledgeGraphContract(unittest.TestCase):
 def test_manifest(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.19.0');self.assertEqual(m['previous_version'],'0.18.0');self.assertEqual(m['storage_schema_version'],20);self.assertEqual(m['project_schema'],'sc-workspace-project/11.0');self.assertEqual(m['knowledge_graph_schema'],'sc-workspace-knowledge-graph/1.0')
