...
class ContinuityHardening(unittest.TestCase):
 def test_01_lineage_and_header(self):
  ...
  self.assertIn('Version: 0.67.0',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '0.67.0",MAIN)
  for label,value in [('Plugin Name','Sustainable Catalyst Workspace'),('Version','0.67.0'),('Author','Content Catalyst LLC'),('Requires at least','6.4'),('Requires PHP','8.0')]:
 ...
 def test_03_assets_and_contract_route(self):
  ...
  self.assertIn("'sc-workspace-v0670'",PHP); self.assertIn('workspace-v0.67.0.js',PHP); self.assertIn('workspace-v0.67.0.css',PHP)
 ...
 def test_12_schemas_registry_history(self):
  ...
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0670'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0661',REGPHP)
