...
class PublicExperienceTests(unittest.TestCase):
    ...
    def test_traceability_schema_migration_preserves_platform_boundary(self):
        m=json.loads((ROOT/'release-manifest-v0.21.0.json').read_text())
        self.assertEqual(m['storage_schema_version'],21)
        self.assertEqual(m['project_schema'],'sc-workspace-project/11.0')
        self.assertEqual(m['version'],'0.21.0')
        self.assertEqual(m['previous_version'],'0.20.0')
        self.assertFalse(m['cloud_sync'])
        self.assertEqual(m['server_project_storage'],'manual-account-backup-only')
