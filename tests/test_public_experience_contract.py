...
class PublicExperienceTests(unittest.TestCase):
    ...
    def test_traceability_schema_migration_preserves_platform_boundary(self):
        m=json.loads((ROOT/'release-manifest-v0.32.0.json').read_text())
        self.assertEqual(m['storage_schema_version'],28)
        self.assertEqual(m['project_schema'],'sc-workspace-project/13.0')
        self.assertEqual(m['version'],'0.32.0')
        self.assertEqual(m['previous_version'],'0.31.0')
