...
class PublicExperienceTests(unittest.TestCase):
    ...
    def test_traceability_schema_migration_preserves_platform_boundary(self):
        m=json.loads((ROOT/'release-manifest-v0.35.0.json').read_text())
        self.assertEqual(m['storage_schema_version'],31)
        self.assertEqual(m['project_schema'],'sc-workspace-project/16.0')
        self.assertEqual(m['version'],'0.35.0')
        self.assertEqual(m['previous_version'],'0.34.0')
