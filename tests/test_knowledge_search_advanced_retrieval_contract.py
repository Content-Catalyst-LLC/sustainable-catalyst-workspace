...
CSS=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.43.0.css').read_text()
class KnowledgeSearchAdvancedRetrieval(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.43.0','0.42.0','Research Collections & Dynamic Views')); self.assertIn('Version: 0.43.0',MAIN)
 ...
 def test_11_registry_history(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.43.0','0.42.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0430'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0420',REGPHP); self.assertTrue((ROOT/'history/release-manifest-v0.42.0.json').exists()); self.assertTrue((ROOT/'history/workspace-product-record-v0.42.0.json').exists())
 def test_12_v041_navigation_retained(self): self.assertIn('data-scw-workspace-area="research"',PHP); self.assertIn('sc-workspace-research-navigation-v1.js',PHP); self.assertTrue(all(x in (ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-research-navigation-v1.js').read_text() for x in ("start:{label:'Start'","projects:{label:'Projects'","research:{label:'Research'","review:{label:'Review'","exchange:{label:'Exchange'")))
