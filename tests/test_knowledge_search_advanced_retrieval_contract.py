...
class KnowledgeSearchAdvancedRetrieval(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.53.0','0.52.0','Collaboration Architecture Foundation')); self.assertIn('Version: 0.53.0',MAIN)
 ...
 def test_11_registry_history(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.53.0','0.52.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0530'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0420',REGPHP); self.assertTrue((ROOT/'history/release-manifest-v0.43.0.json').exists()); self.assertTrue((ROOT/'history/workspace-product-record-v0.43.0.json').exists())
