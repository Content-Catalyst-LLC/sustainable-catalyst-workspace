...
class KnowledgeSearchAdvancedRetrieval(unittest.TestCase):
 ...
 def test_11_registry_history(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.64.0','0.63.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0640'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0420',REGPHP); self.assertTrue((ROOT/'history/release-manifest-v0.43.0.json').exists()); self.assertTrue((ROOT/'history/workspace-product-record-v0.43.0.json').exists())
