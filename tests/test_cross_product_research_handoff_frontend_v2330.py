...
PHP=(WP/'includes/class-sc-workspace.php').read_text(); PLUGIN=(WP/'sustainable-catalyst-workspace.php').read_text(); MAIN=(WP/'assets/js/workspace-v2.35.0.js').read_text(); TYPED=(WP/'assets/js/sc-workspace-typed-client-v2350.js').read_text(); TS=(ROOT/'frontend/typed-client/src/50-cross-product-handoffs.ts').read_text()
