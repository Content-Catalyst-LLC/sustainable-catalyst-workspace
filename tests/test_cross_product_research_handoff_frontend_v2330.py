...
PHP=(WP/'includes/class-sc-workspace.php').read_text(); PLUGIN=(WP/'sustainable-catalyst-workspace.php').read_text(); MAIN=(WP/'assets/js/workspace-v3.1.0.js').read_text(); TYPED=(WP/'assets/js/sc-workspace-typed-client-v3100.js').read_text(); TS=(ROOT/'frontend/typed-client/src/50-cross-product-handoffs.ts').read_text()
def test_release_and_interaction_lineage_preserved():
    assert 'Version: 3.1.0' in PLUGIN and "const WORKSPACE_RELEASE = '3.1.0';" in MAIN; assert 'SCWorkspaceInteractionRuntime' in MAIN and 'SCWorkspaceInteractionRepair' in MAIN
