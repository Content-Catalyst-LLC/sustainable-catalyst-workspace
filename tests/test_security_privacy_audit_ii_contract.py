...
class SecurityPrivacyAuditIIContract(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.77.0','0.76.0','Security & Privacy Audit II')); self.assertIn('Version: 0.77.0',MAIN)
 ...
 def test_10_assets(self): self.assertIn("'sc-workspace-v0770'",PHP); self.assertIn('workspace-v0.77.0.js',PHP); self.assertIn('workspace-v0.77.0.css',PHP); self.assertIn('sc-workspace-security-privacy-audit-ii-v1',PHP); self.assertIn('sc-workspace-security-privacy-audit-ii-ui-v1',PHP); self.assertIn('/* v0.77.0 — Security & Privacy Audit II */',CSS)
 def test_11_registry(self): self.assertEqual((REG['public_version'],REG['previous_version'],REG['release_name']),('0.77.0','0.76.0','Security & Privacy Audit II')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0770'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0760',REGPHP); self.assertTrue((R/'history/release-manifest-v0.76.0.json').exists()); self.assertTrue((R/'history/workspace-product-record-v0.76.0.json').exists())
