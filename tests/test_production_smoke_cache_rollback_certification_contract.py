...
class ProductionCertificationContract(unittest.TestCase):
 ...
 def test_04_server_boundaries(self):
  for t in ["PREVIOUS_RELEASE = '1.8.0'",'production_certification_requires_live_field_checks','production_certified','automatic_cache_purge','automatic_rollback','project_data_inspected']: self.assertIn(t,PROD)
 def test_05_deployment_inheritance(self): self.assertIn("PREVIOUS_RELEASE = '1.8.0'",DEP); self.assertIn('workspace-v1.9.0.js',DEP); self.assertIn('production_runtime',DEP)
 ...
 def test_09_current_assets(self): self.assertIn("'sc-workspace-v190'",PHP); self.assertIn('workspace-v1.9.0.js',PHP); self.assertIn('workspace-v1.9.0.css',PHP); self.assertNotIn('sc-workspace-v0810',PHP)
