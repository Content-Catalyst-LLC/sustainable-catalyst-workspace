...
class ProductionCertificationContract(unittest.TestCase):
 ...
 def test_04_server_boundaries(self):
  for t in ["PREVIOUS_RELEASE = '2.0.3'",'production_certification_requires_live_field_checks','production_certified','automatic_cache_purge','automatic_rollback','project_data_inspected']: self.assertIn(t,PROD)
 def test_05_deployment_inheritance(self): self.assertIn("PREVIOUS_RELEASE = '2.0.3'",DEP); self.assertIn('workspace-v2.0.4.js',DEP); self.assertIn('production_runtime',DEP)
 ...
 def test_09_current_assets(self): self.assertIn("'sc-workspace-v204'",PHP); self.assertIn('workspace-v2.0.4.js',PHP); self.assertIn('workspace-v2.0.4.css',PHP); self.assertNotIn('sc-workspace-v0810',PHP)
