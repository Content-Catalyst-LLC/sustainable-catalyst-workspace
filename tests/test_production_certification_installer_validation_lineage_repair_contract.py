...
class InstallerValidationLineageRepair(unittest.TestCase):
    def test_01_current_release_discovery_is_not_pinned(self):
        self.assertEqual(current_release(R).version,'0.83.0')
        self.assertEqual(current_release(R).previous_version,'0.82.1')
    ...
    def test_04_current_cumulative_assets_exist(self):
        cur=current_release(R); self.assertTrue(cur.script_path.exists()); self.assertTrue(cur.style_path.exists()); self.assertEqual(cur.asset_handle,'sc-workspace-v0830')
    def test_05_lineage_gate_passes_current_tree(self):
        result=validate_current_release_lineage(R,'0.83.0','0.82.1'); self.assertTrue(result['ok'],result['errors'])
    ...
    def test_09_live_runtime_predecessors_advance(self):
        ...
        self.assertIn("PREVIOUS_RELEASE = '0.82.1'",dep); self.assertIn("PREVIOUS_RELEASE = '0.82.1'",prod)
    def test_10_registry_retry_includes_v0821_legacy_state(self):
        ...
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0830'",text); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0830'",text); self.assertIn('LEGACY_PENDING_KEY_V0821',text)
    def test_11_compact_wordpress_header(self):
        raw=(R/'wordpress/sustainable-catalyst-workspace/sustainable-catalyst-workspace.php').read_bytes(); self.assertLess(raw.find(b'Version:'),512); self.assertLess(raw.find(b'Requires PHP:'),512); self.assertIn(b'Version: 0.83.0',raw[:8192])
