...
class InteroperabilityTests(unittest.TestCase):
 def test_release(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.39.0');self.assertEqual(m['previous_version'],'0.38.0');self.assertEqual(m['storage_schema_version'],35);self.assertEqual(m['project_schema'],'sc-workspace-project/20.0')
 ...
 def test_project_schema_unchanged(self):
  j=JS.read_text();m=json.loads(MANIFEST.read_text());self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/20.0'",j);self.assertFalse(m['migration']['project_schema_unchanged'])
 def test_ui(self):
