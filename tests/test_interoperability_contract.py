...
class InteroperabilityTests(unittest.TestCase):
 ...
 def test_project_schema_unchanged(self):
  j=JS.read_text();m=json.loads(MANIFEST.read_text());self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/20.0'",j);self.assertTrue(m['migration']['project_schema_unchanged'])
 def test_ui(self):
