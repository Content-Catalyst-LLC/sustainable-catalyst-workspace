...
assert.equal(api.RELEASE_VERSION,'1.1.0');
...
const root={dataset:{version:'1.1.0',releaseStage:'workspace-home',storageVersion:'35',projectSchema:'sc-workspace-project/20.0'},querySelector:(q)=>{const m=q.match(/data-scw-workspace-section="([^"]+)"/);return m&&selectors.has(m[1])?{}:null;}};
const env={
...
const report=api.report('1.1.0',result);assert.equal(report.workspaceVersion,'1.1.0');assert.equal(report.privacy.projectContentIncluded,false);assert.equal(report.governance.featureFreeze,true);
const checklist=api.checklist('1.1.0');assert.equal(checklist.items.length,7);assert.equal(checklist.governance.automaticCompletion,false);
