...
class GuidedReconciliationContract(unittest.TestCase):
    ...
    def test_public_rest_contract(self):
        self.assertIn('/wp-json/sc-workspace/v1/reconciliation-contract',MAN['rest_routes']); self.assertIn("'/reconciliation-contract'",PHP); self.assertIn('public function reconciliation_contract()',PHP); self.assertIn("'storage_schema_version' => 29",PHP)
    ...
    def test_registry_lineage(self):
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0330'",REG); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0330'",REG); self.assertIn("LEGACY_PENDING_KEY_V0260",REG); self.assertIn("'previous_version' => '0.32.0'",REG)
