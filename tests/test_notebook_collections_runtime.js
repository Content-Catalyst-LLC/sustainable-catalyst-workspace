...
const nb=require('../wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-research-notebook-v3.js');
let n=0;const id=p=>`${p}-${++n}`;const now=()=>`2026-08-09T21:00:${String(n).padStart(2,'0')}Z`;
assert.strictEqual(nb.WORKSPACE_SCHEMA,'sc-workspace-notebook-workspace/3.0');assert.strictEqual(nb.EXPORT_SCHEMA,'sc-workspace-notebook-export/3.0');
...
const pkg=nb.exportNotebook(a,{id:'p1',title:'Project'},'0.34.0',ws);assert.strictEqual(pkg.schema,'sc-workspace-notebook-export/3.0');assert.strictEqual(pkg.links.length,1);assert.strictEqual(pkg.collections.length,1);assert.strictEqual(pkg.governance.backlinksDerivedFromExplicitLinks,true);
const migrated=nb.workspace({schema:'sc-workspace-notebook-workspace/2.0',notebooks:[a],activeNotebookId:a.id,createdAt:now(),updatedAt:now()},id,now);assert.strictEqual(migrated.schema,'sc-workspace-notebook-workspace/3.0');assert.deepStrictEqual(migrated.collections,[]);assert.deepStrictEqual(migrated.links,[]);
console.log('PASS - Workspace v0.34.0 Notebook Collections & Knowledge Linking runtime');
