...
REG=json.loads((ROOT/'registry/workspace-product-record-v0.33.0.json').read_text()) if (ROOT/'registry/workspace-product-record-v0.33.0.json').exists() else {}
class VersionHistoryContract(unittest.TestCase):
  def test_plugin_version(self): self.assertIn('Version: 0.33.0',MAIN)
  def test_storage_schema_23(self): self.assertIn('const STORAGE_VERSION = 29;',JS)
  ...
  def test_registry_previous(self): self.assertEqual(REG.get('previous_version'),'0.32.0')
