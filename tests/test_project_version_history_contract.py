...
REG=json.loads((ROOT/'registry/workspace-product-record-v0.54.0.json').read_text()) if (ROOT/'registry/workspace-product-record-v0.54.0.json').exists() else {}
class VersionHistoryContract(unittest.TestCase):
  def test_plugin_version(self): self.assertIn('Version: 0.54.0',MAIN)
  ...
  def test_registry_previous(self): self.assertEqual(REG.get('previous_version'),'0.53.0')
