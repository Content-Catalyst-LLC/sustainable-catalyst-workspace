...
MAIN=(JS/'workspace-v2.35.0.js').read_text(); COMPAT=(JS/'sc-workspace-local-project-compat-v2350.js').read_text(); PHP=(WP/'includes/class-sc-workspace.php').read_text(); DEPLOY=(WP/'includes/class-sc-workspace-deployment.php').read_text(); TYPED=(JS/'sc-workspace-typed-client-v2350.js').read_text()
...
def test_local_project_runtime_is_isolated_and_lazy():
    ...
    assert 'sc-workspace-local-project-compat-v2350.js' in DEPLOY

def test_historical_versioned_workspace_assets_are_retired():
    assert [p.name for p in JS.glob('workspace-v*.js')] == ['workspace-v2.35.0.js']
    assert [p.name for p in CSS.glob('workspace-v*.css')] == ['workspace-v2.35.0.css']
