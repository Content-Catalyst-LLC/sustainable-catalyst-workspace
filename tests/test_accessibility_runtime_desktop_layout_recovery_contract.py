...
class HotfixContract(unittest.TestCase):
 def test_01_lineage_and_schema_stability(self):
  self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.65.0','0.64.1','Responsive & Field-Use Experience'))
  self.assertIn('Version: 0.65.0',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '0.65.0",MAIN)
 ...
 def test_03_current_assets(self):
  self.assertIn("'sc-workspace-v0650'",PHP);self.assertIn('workspace-v0.65.0.css',PHP);self.assertIn('workspace-v0.65.0.js',PHP)
  self.assertIn("array('sc-workspace-v0650')",PHP);self.assertIn("wp_localize_script('sc-workspace-v0650'",PHP)
 ...
 def test_08_registry_hotfix(self):
  self.assertEqual((REG['public_version'],REG['previous_version']),('0.65.0','0.64.1'));self.assertEqual(REG['release_name'],'Responsive & Field-Use Experience')
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0650'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0640',REGPHP)
