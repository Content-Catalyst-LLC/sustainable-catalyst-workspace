...
class HotfixContract(unittest.TestCase):
 def test_01_lineage_and_schema_stability(self):
  ...
  self.assertIn('Version: 0.66.1',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '0.66.1",MAIN)
 ...
 def test_03_current_assets(self):
  self.assertIn("'sc-workspace-v0660'",PHP);self.assertIn('workspace-v0.66.0.css',PHP);self.assertIn('workspace-v0.66.0.js',PHP)
  self.assertIn("array('sc-workspace-v0660')",PHP);self.assertIn("wp_localize_script('sc-workspace-v0660'",PHP)
 ...
 def test_08_registry_hotfix(self):
  ...
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0661'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0640',REGPHP)
