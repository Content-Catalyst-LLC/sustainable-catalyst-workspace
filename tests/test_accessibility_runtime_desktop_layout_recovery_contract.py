...
class HotfixContract(unittest.TestCase):
 def test_01_lineage_and_schema_stability(self):
  self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.64.1','0.64.0','Accessibility Runtime & Desktop Layout Recovery'))
  self.assertIn('Version: 0.64.1',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '0.64.1",MAIN)
 ...
 def test_03_current_assets(self):
  self.assertIn("'sc-workspace-v0641'",PHP);self.assertIn('workspace-v0.64.1.css',PHP);self.assertIn('workspace-v0.64.1.js',PHP)
  self.assertIn("array('sc-workspace-v0641')",PHP);self.assertIn("wp_localize_script('sc-workspace-v0641'",PHP)
 ...
 def test_08_registry_hotfix(self):
  self.assertEqual((REG['public_version'],REG['previous_version']),('0.64.1','0.64.0'));self.assertEqual(REG['release_name'],'Accessibility Runtime & Desktop Layout Recovery')
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0641'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0640',REGPHP)
