...
class HotfixContract(unittest.TestCase):
 ...
 def test_03_current_assets(self):
  self.assertIn("'sc-workspace-v170'",PHP);self.assertIn('workspace-v1.7.0.css',PHP);self.assertIn('workspace-v1.7.0.js',PHP)
  self.assertIn("array('sc-workspace-v170')",PHP);self.assertIn("wp_localize_script('sc-workspace-v170'",PHP)
 ...
 def test_08_registry_hotfix(self):
  ...
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v170'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0640',REGPHP)
