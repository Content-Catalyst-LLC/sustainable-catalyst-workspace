...
class HotfixContract(unittest.TestCase):
 ...
 def test_03_current_assets(self):
  self.assertIn("'sc-workspace-v204'",PHP);self.assertIn('workspace-v2.0.4.css',PHP);self.assertIn('workspace-v2.0.4.js',PHP)
  self.assertIn("array('sc-workspace-v204')",PHP);self.assertIn("wp_localize_script('sc-workspace-v204'",PHP)
 ...
 def test_08_registry_hotfix(self):
  ...
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v204'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0640',REGPHP)
