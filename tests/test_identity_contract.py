...
ROOT=Path(__file__).resolve().parents[1]; JS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.58.0.js'
class Identity(unittest.TestCase):
 ...
 def test_no_project_upload(self):
  m=json.loads((ROOT/'release-manifest-v0.58.0.json').read_text()); self.assertFalse(m['governance']['automatic_cloud_upload']); self.assertFalse(m['governance']['account_session_uploads_project_content']); self.assertFalse(m['governance']['sign_in_changes_storage_scope'])
