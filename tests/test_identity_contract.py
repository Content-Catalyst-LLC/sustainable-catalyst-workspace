...
class Identity(unittest.TestCase):
 ...
 def test_project_persistence(self):
  s=json.loads((ROOT/'schemas/sc-workspace-project-v3-1.schema.json').read_text()); self.assertIn('persistence',s['required']); p=s['properties']['persistence']['properties']; self.assertEqual(p['scope']['const'],'device'); self.assertEqual(p['syncState']['const'],'local-only'); self.assertFalse(p['serverStored']['const'])
 ...
 def test_no_project_upload(self):
  m=json.loads((ROOT/'release-manifest-v0.4.1.json').read_text()); self.assertFalse(m['governance']['automatic_cloud_upload']); self.assertFalse(m['governance']['account_session_uploads_project_content']); self.assertFalse(m['governance']['sign_in_changes_storage_scope'])
