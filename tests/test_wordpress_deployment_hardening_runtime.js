...
function doc(script='https://example.test/wp-content/plugins/workspace/assets/js/workspace-v1.4.0.js?ver=1.4.0',style='https://example.test/wp-content/plugins/workspace/assets/css/workspace-v1.4.0.css?ver=1.4.0',extraScripts=[]){return {querySelectorAll(sel){if(sel==='script[src]')return [node(script,'src'),...extraScripts.map(x=>node(x,'src'))];if(sel==='link[rel="stylesheet"][href]')return [node(style,'href')];return [];}}}
const root={dataset:{version:'1.4.0',releaseStage:'knowledge-graph-explorer',scwDeploymentServerState:'server-ready'}};
const env={SCWorkspaceIdentity:{workspaceVersion:'1.4.0'},SCWorkspaceReleaseCandidateI:{},document:doc()};
...
let wrongServer=A.assess({dataset:{version:'1.4.0',releaseStage:'knowledge-graph-explorer',scwDeploymentServerState:'attention'}},{env,document:env.document});
assert.equal(wrongServer.automatedDeploymentGate,false);
const report=A.report('1.4.0',r);assert.equal(report.schema,'sc-workspace-wordpress-deployment-report/1.0');assert.equal(report.privacy.projectContentIncluded,false);assert.equal(report.governance.automaticCachePurge,false);
const checklist=A.checklist('1.4.0');assert.equal(checklist.rollbackRelease,'1.3.0');assert(checklist.items.length>=7);assert.equal(checklist.governance.automaticCompletion,false);
const c=A.contract();assert.equal(c.releaseVersion,'1.4.0');assert.equal(c.schemaMigrationRequired,false);assert.equal(c.projectDataMutation,false);assert.equal(c.automaticRollback,false);
