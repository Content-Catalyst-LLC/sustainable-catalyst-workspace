...
class V048(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.49.0','0.48.0','Research Templates & Reusable Workflows')); self.assertIn('Version: 0.49.0',MAIN)
 def test_02_schema_stable(self): self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(35,'sc-workspace-project/20.0','sc-workspace-project-export/20.0')); self.assertFalse(MAN['schema_migration_required']); self.assertTrue(MAN['migration']['research_templates_only_release']); self.assertFalse(MAN['migration']['cross_project_knowledge_only_release'])
