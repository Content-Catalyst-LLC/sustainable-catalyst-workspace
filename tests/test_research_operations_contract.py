...
class ResearchOperations(unittest.TestCase):
 ...
 def test_06_wordpress(self): self.assertIn('Version: 1.11.0',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '1.11.0'",MAIN);self.assertIn("'/research-operations-contract'",PHP);self.assertIn('Research Operations',PHP);self.assertIn('Schedules are declarations, not a daemon',PHP);self.assertIn('data-release-stage="developer-api"',PHP)
 def test_07_deployment(self): self.assertIn("PREVIOUS_RELEASE = '1.10.0'",DEP);self.assertIn("ROLLBACK_RELEASE = '1.10.0'",DEP);self.assertIn('workspace-v1.11.0.js',DEP);self.assertIn('class-sc-workspace-research-operations.php',DEP)
 def test_08_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('1.10.0','1.9.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v1110'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V190',REGPHP)
