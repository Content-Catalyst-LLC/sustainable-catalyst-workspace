...
class PublicBetaIIIDefectClosureContract(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.79.0','0.78.0','Public Beta III Defect Closure')); self.assertIn('Version: 1.6.0',MAIN)
 ...
 def test_10_assets(self): self.assertIn("'sc-workspace-v160'",PHP); self.assertIn('workspace-v1.6.0.js',PHP); self.assertIn('workspace-v1.6.0.css',PHP); self.assertIn('sc-workspace-public-beta-iii-defect-closure-v1',PHP); self.assertIn('sc-workspace-public-beta-iii-defect-closure-ui-v1',PHP); self.assertIn('/* v0.79.0 — Public Beta III Defect Closure */',CSS); self.assertIn("'beta-closure'",APP); self.assertIn("id:'beta-closure'",EXP)
 def test_11_registry(self): self.assertEqual((REG['public_version'],REG['previous_version'],REG['release_name']),('0.79.0','0.78.0','Public Beta III Defect Closure')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v160'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0780',REGPHP); self.assertTrue((R/'history/release-manifest-v0.78.0.json').exists()); self.assertTrue((R/'history/workspace-product-record-v0.78.0.json').exists())
