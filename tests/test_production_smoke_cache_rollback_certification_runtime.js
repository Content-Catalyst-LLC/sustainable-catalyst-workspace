...
function doc(script='https://example.test/workspace-v2.0.1.js?ver=2.0.1',style='https://example.test/workspace-v2.0.1.css?ver=2.0.1',extras=[]){return {querySelectorAll(sel){if(sel==='script[src]')return [node(script,'src'),...extras.map(x=>node(x,'src'))];if(sel==='link[rel="stylesheet"][href]')return [node(style,'href')];return [];}}}
const root={dataset:{version:'2.0.1',releaseStage:'button-system-repair',scwDeploymentServerState:'server-ready'}};
const env={SCWorkspaceIdentity:{workspaceVersion:'2.0.1'},SCWorkspaceWordPressDeploymentHardening:{},SCWorkspaceReleaseCandidateI:{},document:doc()};
...
const rep=A.report('2.0.1',r); assert.equal(rep.productionCertified,false); assert.equal(rep.rollbackRelease,'2.0.0'); assert.equal(rep.privacy.projectContentIncluded,false); assert.equal(rep.governance.automaticRollback,false);
const list=A.checklist('2.0.1'); assert.equal(list.rollbackRelease,'2.0.0'); assert.equal(list.items.length,6); assert.equal(list.governance.automaticCompletion,false);
const c=A.contract(); assert.equal(c.releaseVersion,'2.0.1'); assert.equal(c.previousRelease,'2.0.0'); assert.equal(c.liveProductionChecksManual,true); assert.equal(c.automaticProductionCertification,false);
