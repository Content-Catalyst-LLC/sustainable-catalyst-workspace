...
const rep=A.report('1.8.0',r); assert.equal(rep.productionCertified,false); assert.equal(rep.rollbackRelease,'1.7.0'); assert.equal(rep.privacy.projectContentIncluded,false); assert.equal(rep.governance.automaticRollback,false);
const list=A.checklist('1.8.0'); assert.equal(list.rollbackRelease,'1.7.0'); assert.equal(list.items.length,6); assert.equal(list.governance.automaticCompletion,false);
const c=A.contract(); assert.equal(c.releaseVersion,'1.8.0'); assert.equal(c.previousRelease,'1.7.0'); assert.equal(c.liveProductionChecksManual,true); assert.equal(c.automaticProductionCertification,false);
