...
class PublicResearchPackages(unittest.TestCase):
 ...
 def test_07_wordpress(self): self.assertIn('Version: 2.0.4',MAIN);self.assertIn("'/public-research-packages-contract'",PHP);self.assertIn('data-scw-public-research-packages',PHP);self.assertIn('data-release-stage="visual-regression-theme-isolation"',PHP)
 def test_08_deployment(self): self.assertIn("PREVIOUS_RELEASE = '2.0.3'",DEP);self.assertIn('class-sc-workspace-public-research-packages.php',DEP);self.assertIn('workspace-v2.0.4.js',DEP)
