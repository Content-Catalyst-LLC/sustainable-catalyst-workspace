...
let staged=H.stage(s,{previousRaw:a,nextRaw:b,workspaceVersion:'0.64.0',storageSchemaVersion:35});
...
let committed=H.commit(s,b,{workspaceVersion:'0.64.0',storageSchemaVersion:35});
...
staged=H.stage(s,{previousRaw:drift,nextRaw:b,workspaceVersion:'0.64.0',storageSchemaVersion:35});H.fail(s,new Error('quota'));audit=H.inspect(s);assert.strictEqual(audit.transaction.state,'failed');
const currentCandidate=H.recoveryCandidate(s,'current','0.64.0');assert.strictEqual(currentCandidate.schema,H.RECOVERY_SCHEMA);assert.strictEqual(currentCandidate.privacy.containsCanonicalWorkspaceContent,true);assert.strictEqual(currentCandidate.governance.automaticRestore,false);
const diag=H.diagnosticSnapshot(s,'0.64.0');assert.strictEqual(diag.privacy.projectContentIncluded,false);assert.strictEqual(diag.privacy.rawStateIncluded,false);assert.ok(!Object.prototype.hasOwnProperty.call(diag,'raw'));
// Prepared transaction that never wrote target can reconcile against previous bytes without mutating canonical content.
s.setItem(H.CURRENT_KEY,drift);H.stage(s,{previousRaw:drift,nextRaw:b,workspaceVersion:'0.64.0',storageSchemaVersion:35});const before=s.getItem(H.CURRENT_KEY);const rec=H.reconcileJournal(s,{workspaceVersion:'0.64.0',storageSchemaVersion:35});assert.strictEqual(rec.state,'resolved-prewrite');assert.strictEqual(s.getItem(H.CURRENT_KEY),before);assert.strictEqual(s.getItem(H.TRANSACTION_KEY),null);

// Checksum-bound last-known-good snapshots refuse to verify after envelope drift.
const integrityDamaged=new Store({sc_workspace:a});H.stage(integrityDamaged,{previousRaw:a,nextRaw:b,workspaceVersion:'0.64.0',storageSchemaVersion:35});
...
const receiptFailure=new Store({sc_workspace:a});H.stage(receiptFailure,{previousRaw:a,nextRaw:b,workspaceVersion:'0.64.0',storageSchemaVersion:35});receiptFailure.setItem(H.CURRENT_KEY,b);receiptFailure.failSetKey=H.METADATA_KEY;
const failedCommit=H.commit(receiptFailure,b,{workspaceVersion:'0.64.0',storageSchemaVersion:35});assert.strictEqual(failedCommit.ok,false);assert.strictEqual(failedCommit.reason,'integrity-metadata-write-failed');assert.ok(receiptFailure.getItem(H.TRANSACTION_KEY));

console.log('PASS - Workspace v0.64.0 Product Hardening II persistence integrity runtime');
