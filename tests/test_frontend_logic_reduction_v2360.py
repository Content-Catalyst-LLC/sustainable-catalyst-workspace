...
MAIN=(JS/'workspace-v2.36.0.js').read_text(); COMPAT=(JS/'sc-workspace-local-project-compat-v2360.js').read_text(); PHP=(WP/'includes/class-sc-workspace.php').read_text(); DEPLOY=(WP/'includes/class-sc-workspace-deployment.php').read_text(); TYPED=(JS/'sc-workspace-typed-client-v2360.js').read_text()
...
def test_historical_versioned_workspace_assets_are_retired():
    assert [p.name for p in JS.glob('workspace-v*.js')] == ['workspace-v2.36.0.js']
    assert [p.name for p in CSS.glob('workspace-v*.css')] == ['workspace-v2.36.0.css']
