...
REG=json.loads((ROOT/'registry/workspace-product-record-v0.34.0.json').read_text())
...
class NotebookCollectionsKnowledgeLinkingContract(unittest.TestCase):
    def test_01_release_lineage(self):
        self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.34.0','0.33.0','Notebook Collections & Knowledge Linking'))
        self.assertIn('Version: 0.34.0',MAIN)
    def test_02_migration(self):
        self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(30,'sc-workspace-project/15.0','sc-workspace-project-export/15.0'))
        self.assertEqual((MAN['migration']['storage_from'],MAN['migration']['storage_to']),(29,30))
        self.assertEqual((MAN['migration']['project_schema_from'],MAN['migration']['project_schema_to']),('sc-workspace-project/14.0','sc-workspace-project/15.0'))
        self.assertIn('function migrateV29(raw)',JS)
    def test_03_notebook_linking_schemas(self):
        self.assertEqual(MAN['notebook_workspace_schema'],'sc-workspace-notebook-workspace/3.0')
        ...
        self.assertEqual(MAN['notebook_export_schema'],'sc-workspace-notebook-export/3.0')
    ...
    def test_06_runtime_helper(self):
        for token in ('createCollection','addCollectionItem','createLink','backlinksForRef','findBlockLocation','refExists','sc-workspace-notebook-workspace/3.0'):
    ...
    def test_09_rest_contract(self):
        self.assertIn("'notebook_workspace_schema' => 'sc-workspace-notebook-workspace/3.0'",PHP)
    ...
    def test_10_registry_lineage(self):
        self.assertEqual((REG['public_version'],REG['previous_version']),('0.34.0','0.33.0'))
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0340'",REGPHP)
        self.assertIn('LEGACY_PENDING_KEY_V0330',REGPHP)
        self.assertTrue((ROOT/'history/release-manifest-v0.33.0.json').exists())
        self.assertTrue((ROOT/'history/workspace-product-record-v0.33.0.json').exists())
