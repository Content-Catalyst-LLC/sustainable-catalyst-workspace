...
NOTEBOOK=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-research-notebook-v4.js').read_text()

class NotebookCollectionsKnowledgeLinkingContract(unittest.TestCase):
    def test_01_release_lineage(self):
        self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.35.0','0.34.0','Notebook-to-Workspace Intelligence'))
        self.assertIn('Version: 0.35.0',MAIN)
    def test_02_migration(self):
        self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(31,'sc-workspace-project/16.0','sc-workspace-project-export/16.0'))
        self.assertEqual((MAN['migration']['storage_from'],MAN['migration']['storage_to']),(30,31))
        self.assertEqual((MAN['migration']['project_schema_from'],MAN['migration']['project_schema_to']),('sc-workspace-project/15.0','sc-workspace-project/16.0'))
        self.assertIn('function migrateV29(raw)',JS)
    def test_03_notebook_linking_schemas(self):
        self.assertEqual(MAN['notebook_workspace_schema'],'sc-workspace-notebook-workspace/4.0')
        ...
        self.assertEqual(MAN['notebook_export_schema'],'sc-workspace-notebook-export/4.0')
    ...
    def test_06_runtime_helper(self):
        for token in ('createCollection','addCollectionItem','createLink','backlinksForRef','findBlockLocation','refExists','sc-workspace-notebook-workspace/4.0'):
    ...
    def test_09_rest_contract(self):
        self.assertIn("'notebook_workspace_schema' => 'sc-workspace-notebook-workspace/4.0'",PHP)
    ...
    def test_10_registry_lineage(self):
        self.assertEqual((REG['public_version'],REG['previous_version']),('0.35.0','0.34.0'))
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0350'",REGPHP)
        self.assertIn('LEGACY_PENDING_KEY_V0340',REGPHP)
