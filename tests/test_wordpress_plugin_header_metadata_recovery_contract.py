...
class HeaderRecovery(unittest.TestCase):
 ...
 def test_03_wordpress_style_parse_window_finds_metadata(self):
  ...
  self.assertEqual(found,{'name':'Sustainable Catalyst Workspace','version':'1.0.1','author':'Content Catalyst LLC','wp':'6.4','php':'8.0'})
 ...
 def test_05_plugin_constant(self): self.assertIn("define('SC_WORKSPACE_VERSION', '1.0.1');",T)
 def test_06_registry_lineage(self):
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v101'",REGPHP)
  self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v101'",REGPHP)
  self.assertIn("LEGACY_PENDING_KEY_V0660",REGPHP)
  self.assertIn("'previous_version' => '1.0.0'",REGPHP)
 ...
 def test_08_v066_assets_retained(self):
  self.assertIn('workspace-v1.0.1.css',PHP); self.assertIn('workspace-v1.0.1.js',PHP)
