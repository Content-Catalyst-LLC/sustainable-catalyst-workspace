...
class CrossDeviceSyncContract(unittest.TestCase):
 def test_lineage(self): self.assertEqual(MAN['version'],'0.43.0');self.assertEqual(MAN['previous_version'],'0.42.0');self.assertEqual(MAN['storage_schema_version'],35);self.assertEqual(MAN['project_schema'],'sc-workspace-project/20.0')
 ...
 def test_registry_lineage(self): self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0430'",REG);self.assertIn("LEGACY_PENDING_KEY_V0240 = 'sc_workspace_registry_pending_v0240'",REG);self.assertIn("LEGACY_PENDING_KEY_V0220 = 'sc_workspace_registry_pending_v0220'",REG)
