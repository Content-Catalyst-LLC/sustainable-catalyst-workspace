...
class UniversalWorkspaceSearchContractTests(unittest.TestCase):
 def setUp(self):
  ...
  self.app=(P/'assets/js/workspace-v1.13.0.js').read_text()
 ...
 def test_wordpress_runtime_integration(self):
  self.assertIn('Version: 1.13.0',self.main);self.assertIn("define('SC_WORKSPACE_VERSION', '1.13.0')",self.main);self.assertIn('class-sc-workspace-universal-search.php',self.main)
  self.assertIn("'/universal-search-contract'",self.workspace);self.assertIn('universal_search_contract',self.workspace);self.assertIn('sc-workspace-universal-search-v1',self.workspace);self.assertIn("'sc-workspace-v1130'",self.workspace);self.assertIn('workspace-v1.13.0.js',self.workspace);self.assertIn('workspace-v1.13.0.css',self.workspace);self.assertIn('data-release-stage="connected-intelligence"',self.workspace)
 ...
 def test_release_engineering_identity(self):
  self.assertIn("PREVIOUS_RELEASE = '1.12.0'",self.dep);self.assertIn("ROLLBACK_RELEASE = '1.12.0'",self.dep);self.assertIn('workspace-v1.13.0.js',self.dep);self.assertIn('workspace-v1.13.0.css',self.dep)
  self.assertIn("PREVIOUS_RELEASE = '1.12.0'",self.prod);self.assertIn("ROLLBACK_RELEASE = '1.12.0'",self.prod);self.assertIn('workspace-v1.13.0.js',self.prod);self.assertIn('workspace-v1.13.0.css',self.prod)
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v1130'",self.regphp);self.assertIn('LEGACY_PENDING_KEY_V110',self.regphp)
