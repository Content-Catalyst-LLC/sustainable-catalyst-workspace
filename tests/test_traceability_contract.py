...
JS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.9.0.1.js'
SCHEMA=ROOT/'schemas/sc-workspace-traceability-v1.schema.json'
PROJECT=ROOT/'schemas/sc-workspace-project-v8.schema.json'
MANIFEST=ROOT/'release-manifest-v0.9.0.1.json'
class TraceabilityContract(unittest.TestCase):
    def test_release_contract(self):
        m=json.loads(MANIFEST.read_text()); self.assertEqual(m['version'],'0.9.0.1'); self.assertEqual(m['previous_version'],'0.9.0'); self.assertEqual(m['storage_schema_version'],10); self.assertEqual(m['project_schema'],'sc-workspace-project/8.0'); self.assertEqual(m['traceability_schema'],'sc-workspace-traceability/1.0')
