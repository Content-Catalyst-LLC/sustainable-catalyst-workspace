...
class PerformanceII(unittest.TestCase):
 def test_01_lineage_and_wordpress_header(self):
  ...
  for label,value in [('Plugin Name','Sustainable Catalyst Workspace'),('Version','0.71.0'),('Author','Content Catalyst LLC'),('Requires at least','6.4'),('Requires PHP','8.0')]:
 ...
 def test_03_contract_and_assets(self):
  ...
  self.assertIn("'sc-workspace-v0710'",PHP); self.assertIn('workspace-v0.71.0.js',PHP); self.assertIn('workspace-v0.71.0.css',PHP)
  self.assertIn("wp_localize_script('sc-workspace-v0710'",PHP)
 ...
 def test_10_registry_history(self):
  ...
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0710'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0670',REGPHP)
