...
APP=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v2.0.3.js').read_text()
...
class PerformanceII(unittest.TestCase):
 def test_01_lineage_and_wordpress_header(self):
  ...
  for label,value in [('Plugin Name','Sustainable Catalyst Workspace'),('Version','2.0.3'),('Author','Content Catalyst LLC'),('Requires at least','6.4'),('Requires PHP','8.0')]:
 ...
 def test_03_contract_and_assets(self):
  ...
  self.assertIn("'sc-workspace-v203'",PHP); self.assertIn('workspace-v2.0.3.js',PHP); self.assertIn('workspace-v2.0.3.css',PHP)
  self.assertIn("wp_localize_script('sc-workspace-v203'",PHP)
 ...
 def test_10_registry_history(self):
  ...
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v203'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0670',REGPHP)
