...
APP=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v1.4.0.js').read_text()
...
CSS=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v1.4.0.css').read_text()
class PerformanceII(unittest.TestCase):
 def test_01_lineage_and_wordpress_header(self):
  ...
  self.assertIn('Version: 1.4.0',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '1.4.0",MAIN)
  for label,value in [('Plugin Name','Sustainable Catalyst Workspace'),('Version','1.4.0'),('Author','Content Catalyst LLC'),('Requires at least','6.4'),('Requires PHP','8.0')]:
 ...
 def test_03_contract_and_assets(self):
  ...
  self.assertIn("'sc-workspace-v140'",PHP); self.assertIn('workspace-v1.4.0.js',PHP); self.assertIn('workspace-v1.4.0.css',PHP)
  self.assertIn("wp_localize_script('sc-workspace-v140'",PHP)
 ...
 def test_10_registry_history(self):
  ...
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v140'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0670',REGPHP)
