...
define('SC_WORKSPACE_VERSION', '0.25.0');
...
assert_true($r['sustainable-catalyst-workspace']['public_version'] === '0.25.0', 'Workspace version updated');
assert_true($r['sustainable-catalyst-workspace']['previous_version'] === '0.24.0', 'previous version recorded');
...
assert_true(get_option(SC_Workspace_Registry::BACKUP_KEY) === $before, 'pre-v0.25.0 registry backup created');
...
foreach ($r['products'] as $product) {
    if (($product['canonical_id'] ?? '') === 'sustainable-catalyst-workspace') {
        $count++;
        assert_true($product['public_version'] === '0.25.0', 'list Workspace version updated');
    }
}
...
assert_true(get_option(SC_Workspace_Registry::PENDING_KEY) === '1', 'v0.25.0 pending marker is set');

echo "PASS - Workspace v0.25.0 registry runtime migration\n";
