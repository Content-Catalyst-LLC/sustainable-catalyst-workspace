...
assert_true($r['sustainable-catalyst-workspace']['public_version'] === '2.0.4', 'Workspace version updated');
assert_true($r['sustainable-catalyst-workspace']['previous_version'] === '2.0.3', 'previous version recorded');
...
assert_true(get_option(SC_Workspace_Registry::BACKUP_KEY) === $before, 'pre-v2.0.4 registry backup created');
...
foreach ($r['products'] as $product) {
    if (($product['canonical_id'] ?? '') === 'sustainable-catalyst-workspace') {
        $count++;
        assert_true($product['public_version'] === '2.0.4', 'list Workspace version updated');
    }
}
...
assert_true(get_option(SC_Workspace_Registry::PENDING_KEY) === '1', 'v2.0.4 pending marker is set');

echo "PASS - Workspace v2.0.4 registry runtime migration\n";
