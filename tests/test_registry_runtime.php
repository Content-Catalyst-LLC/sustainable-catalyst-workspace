...
define('SC_WORKSPACE_VERSION', '0.2.0');
...
// Direct associative registry: preserve existing commercial records and update Workspace.
$before = array(
    'catalyst-intelligence' => array('canonical_id' => 'catalyst-intelligence', 'name' => 'Catalyst Intelligence Platform', 'display_order' => 410),
    'sustainable-catalyst-workspace' => array('canonical_id' => 'sustainable-catalyst-workspace', 'public_version' => '0.1.0', 'custom_preserved' => 'yes'),
);
...
assert_true($r['sustainable-catalyst-workspace']['public_version'] === '0.2.0', 'Workspace version updated');
assert_true($r['sustainable-catalyst-workspace']['previous_version'] === '0.1.0', 'previous version recorded');
...
assert_true(get_option(SC_Workspace_Registry::BACKUP_KEY) === $before, 'pre-v0.2.0 registry backup created');

// Wrapper/list registry: preserve wrapper metadata and update Workspace exactly once.
$GLOBALS['scw_options'] = array(
    SC_Workspace_Registry::OPTION_KEY => array(
        'schema' => 'scfs-canonical-product-registry/2.1',
        'products' => array(
            array('canonical_id' => 'sustainable-catalyst-workspace', 'public_version' => '0.1.0'),
...
foreach ($r['products'] as $product) {
    if (($product['canonical_id'] ?? '') === 'sustainable-catalyst-workspace') {
        $count++;
        assert_true($product['public_version'] === '0.2.0', 'list Workspace version updated');
    }
}
assert_true($count === 1, 'Workspace appears exactly once');

// Missing registry: fail closed and leave a pending marker.
$GLOBALS['scw_options'] = array();
assert_true(SC_Workspace_Registry::register_product() === false, 'missing registry does not fabricate a catalog');
assert_true(get_option(SC_Workspace_Registry::PENDING_KEY) === '1', 'v0.2.0 pending marker is set');

echo "PASS - Workspace v0.2.0 registry runtime migration\n";
