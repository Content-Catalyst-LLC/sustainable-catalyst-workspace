...
MAN=json.loads((ROOT/'release-manifest-v0.38.0.json').read_text())
REG=json.loads((ROOT/'registry/workspace-product-record-v0.38.0.json').read_text())
JS=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.38.0.js').read_text()
NB=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-research-notebook-v7.js').read_text()
...
class GroundedNotebookAssistanceContract(unittest.TestCase):
 def test_01_lineage(self):
  self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.38.0','0.37.0','Portable & Synced Notebooks')); self.assertIn('Version: 0.38.0',MAIN)
 def test_02_schema_migration(self):
  self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(34,'sc-workspace-project/19.0','sc-workspace-project-export/19.0')); self.assertEqual((MAN['migration']['storage_from'],MAN['migration']['storage_to']),(33,34)); self.assertEqual((MAN['migration']['project_schema_from'],MAN['migration']['project_schema_to']),('sc-workspace-project/18.0','sc-workspace-project/19.0')); self.assertIn('function migrateV32(raw)',JS); self.assertIn('if (raw.schemaVersion === 32) return migrateV32(raw);',JS)
 def test_03_assistance_schemas(self):
  self.assertEqual(MAN['notebook_workspace_schema'],'sc-workspace-notebook-workspace/7.0'); self.assertEqual(MAN['notebook_export_schema'],'sc-workspace-notebook-export/7.0'); self.assertEqual(MAN['notebook_assistance_schema'],'sc-workspace-notebook-assistance/1.0')
 ...
 def test_09_rest_contract(self):
  self.assertIn("'notebook_workspace_schema' => 'sc-workspace-notebook-workspace/7.0'",PHP); self.assertIn("'notebook_assistance_schema' => 'sc-workspace-notebook-assistance/1.0'",PHP); self.assertIn("'grounded_assistance_citations_required' => true",PHP); self.assertIn("'grounded_assistance_automatic_submission' => false",PHP)
 def test_10_history_registry(self):
  self.assertTrue((ROOT/'history/release-manifest-v0.36.0.json').exists()); self.assertTrue((ROOT/'history/workspace-product-record-v0.36.0.json').exists()); self.assertEqual((REG['public_version'],REG['previous_version']),('0.38.0','0.37.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0380'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0360',REGPHP)
