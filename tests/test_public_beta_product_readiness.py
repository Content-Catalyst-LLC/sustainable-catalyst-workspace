...
class PublicBetaProductReadiness(unittest.TestCase):
 def test_release_lineage(self):
  self.assertEqual(MAN['version'],'0.38.0'); self.assertEqual(MAN['previous_version'],'0.37.0'); self.assertEqual(MAN['release_name'],'Portable & Synced Notebooks')
 def test_schema_stable(self):
  self.assertEqual(MAN['storage_schema_version'],34); self.assertEqual(MAN['project_schema'],'sc-workspace-project/19.0'); self.assertTrue(MAN['schema_migration_required']); self.assertEqual(MAN['migration']['storage_from'],33); self.assertEqual(MAN['migration']['storage_to'],34); self.assertFalse(MAN['migration']['project_schema_unchanged'])
 ...
 def test_registry_lineage(self):
  self.assertEqual(REG['public_version'],'0.38.0'); self.assertEqual(REG['previous_version'],'0.37.0'); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0380'",REGPHP); self.assertIn("LEGACY_PENDING_KEY_V0290",REGPHP)
 def test_plugin_and_rest_contract(self):
  self.assertIn('Version: 0.38.0',MAIN); self.assertIn("'/public-beta-contract'",PHP); self.assertIn('public function public_beta_contract()',PHP); self.assertIn("'release_stage' => 'public-beta'",PHP)
