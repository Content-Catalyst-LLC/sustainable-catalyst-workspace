...
MAN=json.loads((ROOT/'release-manifest-v0.54.0.json').read_text())
REG=json.loads((ROOT/'registry/workspace-product-record-v0.54.0.json').read_text())
PHP=(ROOT/'wordpress/sustainable-catalyst-workspace/includes/class-sc-workspace.php').read_text()
MAIN=(ROOT/'wordpress/sustainable-catalyst-workspace/sustainable-catalyst-workspace.php').read_text()
JS=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.54.0.js').read_text()
HELPER=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-public-beta-v1.js').read_text()
CSS=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.54.0.css').read_text()
...
class PublicBetaProductReadiness(unittest.TestCase):
 def test_release_lineage(self):
  self.assertEqual(MAN['version'],'0.54.0'); self.assertEqual(MAN['previous_version'],'0.53.0'); self.assertEqual(MAN['release_name'],'Shared Review & Research Handoff')
 ...
 def test_registry_lineage(self):
  self.assertEqual(REG['public_version'],'0.54.0'); self.assertEqual(REG['previous_version'],'0.53.0'); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0540'",REGPHP); self.assertIn("LEGACY_PENDING_KEY_V0290",REGPHP)
 def test_plugin_and_rest_contract(self):
  self.assertIn('Version: 0.54.0',MAIN); self.assertIn("'/public-beta-contract'",PHP); self.assertIn('public function public_beta_contract()',PHP); self.assertIn("'release_stage' => 'public-beta'",PHP)
