...
class PublicBetaProductReadiness(unittest.TestCase):
 ...
 def test_registry_lineage(self):
  self.assertEqual(REG['public_version'],'0.66.0'); self.assertEqual(REG['previous_version'],'0.65.0'); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v160'",REGPHP); self.assertIn("LEGACY_PENDING_KEY_V0290",REGPHP)
 def test_plugin_and_rest_contract(self):
  self.assertIn('Version: 1.6.0',MAIN); self.assertIn("'/public-beta-contract'",PHP); self.assertIn('public function public_beta_contract()',PHP); self.assertIn("'release_stage' => 'public-beta'",PHP)
