...
class ChangeGatesSafeActionsContract(unittest.TestCase):
    def test_release_lineage_and_storage_migration(self):
        self.assertEqual(MAN['version'],'0.55.0'); self.assertEqual(MAN['previous_version'],'0.54.0')
        self.assertEqual(MAN['release_name'],'Workspace API & Embed Foundation')
    ...
    def test_registry_lineage(self):
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0550'",REG); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0550'",REG)
        self.assertIn("LEGACY_PENDING_KEY_V0260 = 'sc_workspace_registry_pending_v0260'",REG); self.assertIn("LEGACY_PENDING_KEY_V0240 = 'sc_workspace_registry_pending_v0240'",REG); self.assertIn("'previous_version' => '0.54.0'",REG)
