...
class ChangeGatesSafeActionsContract(unittest.TestCase):
    def test_release_lineage_and_storage_migration(self):
        self.assertEqual(MAN['version'],'0.32.0'); self.assertEqual(MAN['previous_version'],'0.31.0')
        self.assertEqual(MAN['release_name'],'Research Notebook Foundation')
        self.assertEqual(MAN['storage_schema_version'],28); self.assertEqual(MAN['project_schema'],'sc-workspace-project/13.0')
        self.assertTrue(MAN['schema_migration_required']); self.assertEqual(MAN['migration']['storage_from'],27); self.assertEqual(MAN['migration']['storage_to'],28); self.assertFalse(MAN['migration']['project_schema_unchanged'])
    ...
    def test_public_rest_contract(self):
        ...
        self.assertIn("'storage_schema_version' => 28",PHP)
    ...
    def test_registry_lineage(self):
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0320'",REG); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0320'",REG)
        self.assertIn("LEGACY_PENDING_KEY_V0260 = 'sc_workspace_registry_pending_v0260'",REG); self.assertIn("LEGACY_PENDING_KEY_V0240 = 'sc_workspace_registry_pending_v0240'",REG); self.assertIn("'previous_version' => '0.31.0'",REG)
