...
class ChangeGatesSafeActionsContract(unittest.TestCase):
    def test_release_lineage_and_storage_migration(self):
        self.assertEqual(MAN['version'],'0.43.0'); self.assertEqual(MAN['previous_version'],'0.42.0')
        self.assertEqual(MAN['release_name'],'Research Collections & Dynamic Views')
    ...
    def test_registry_lineage(self):
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0430'",REG); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0430'",REG)
        self.assertIn("LEGACY_PENDING_KEY_V0260 = 'sc_workspace_registry_pending_v0260'",REG); self.assertIn("LEGACY_PENDING_KEY_V0240 = 'sc_workspace_registry_pending_v0240'",REG); self.assertIn("'previous_version' => '0.42.0'",REG)
