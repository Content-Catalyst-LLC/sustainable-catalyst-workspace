...
class ChangeGatesSafeActionsContract(unittest.TestCase):
    def test_release_lineage_and_storage_migration(self):
        self.assertEqual(MAN['version'],'0.35.0'); self.assertEqual(MAN['previous_version'],'0.34.0')
        self.assertEqual(MAN['release_name'],'Notebook-to-Workspace Intelligence')
        self.assertEqual(MAN['storage_schema_version'],31); self.assertEqual(MAN['project_schema'],'sc-workspace-project/16.0')
        self.assertTrue(MAN['schema_migration_required']); self.assertEqual(MAN['migration']['storage_from'],30); self.assertEqual(MAN['migration']['storage_to'],31); self.assertFalse(MAN['migration']['project_schema_unchanged'])
    ...
    def test_public_rest_contract(self):
        ...
        self.assertIn("'storage_schema_version' => 31",PHP)
    ...
    def test_registry_lineage(self):
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0350'",REG); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0350'",REG)
        self.assertIn("LEGACY_PENDING_KEY_V0260 = 'sc_workspace_registry_pending_v0260'",REG); self.assertIn("LEGACY_PENDING_KEY_V0240 = 'sc_workspace_registry_pending_v0240'",REG); self.assertIn("'previous_version' => '0.34.0'",REG)
