...
class NotebookReviewProvenance(unittest.TestCase):
 def test_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.39.0','0.38.0','Notebook Review & Provenance'));self.assertIn('Version: 0.39.0',MAIN)
 def test_migration(self): self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(35,'sc-workspace-project/20.0','sc-workspace-project-export/20.0'));self.assertEqual((MAN['migration']['storage_from'],MAN['migration']['storage_to']),(34,35));self.assertIn('function migrateV34(raw)',JS);self.assertIn('if (raw.schemaVersion === 34) return migrateV34(raw);',JS)
 ...
 def test_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.39.0','0.38.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0390'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0380',REGPHP)
 def test_history(self): self.assertTrue((ROOT/'history/release-manifest-v0.38.0.json').exists());self.assertTrue((ROOT/'history/workspace-product-record-v0.38.0.json').exists())
