...
class NotebookReviewProvenance(unittest.TestCase):
 def test_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.64.0','0.63.0','Accessibility & Keyboard-First Product Audit'));self.assertIn('Version: 0.64.0',MAIN)
 ...
 def test_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.64.0','0.63.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0640'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0380',REGPHP)
