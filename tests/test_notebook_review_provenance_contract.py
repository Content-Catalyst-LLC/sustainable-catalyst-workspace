...
class NotebookReviewProvenance(unittest.TestCase):
 def test_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.63.0','0.62.0','Cross-Browser & Device Compatibility'));self.assertIn('Version: 0.63.0',MAIN)
 ...
 def test_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.63.0','0.62.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0630'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0380',REGPHP)
