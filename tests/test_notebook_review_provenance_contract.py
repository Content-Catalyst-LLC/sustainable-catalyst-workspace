...
class NotebookReviewProvenance(unittest.TestCase):
 def test_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.48.0','0.47.0','Cross-Project Knowledge'));self.assertIn('Version: 0.48.0',MAIN)
 ...
 def test_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.48.0','0.47.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0480'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0380',REGPHP)
