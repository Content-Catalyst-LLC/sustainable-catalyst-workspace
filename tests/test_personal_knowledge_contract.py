...
class PersonalKnowledgeContractTests(unittest.TestCase):
    def test_release_boundary(self):
        ...
        self.assertEqual(m['version'],'0.14.0'); self.assertEqual(m['previous_version'],'0.13.0')
        self.assertEqual(m['storage_schema_version'],15); self.assertEqual(m['project_schema'],'sc-workspace-project/11.0')
    ...
    def test_migration_from_v011(self):
        j=JS.read_text()
        for token in ("const STORAGE_VERSION = 15","const PROJECT_SCHEMA = 'sc-workspace-project/11.0'",'function migrateV14(raw)',"if (raw.schemaVersion === 14) return migrateV14(raw)",'state.knowledge = normalizeKnowledge(raw.knowledge, state.projects)'):
            self.assertIn(token,j)
