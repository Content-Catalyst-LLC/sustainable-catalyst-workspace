...
CSS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.12.0.css'
...
class PersonalKnowledgeContractTests(unittest.TestCase):
    def test_release_boundary(self):
        ...
        self.assertTrue(m['migration']['non_destructive']); self.assertFalse(m['migration']['rewrites_project_objects'])
    ...
    def test_workspace_level_view_not_project_subschema(self):
        ...
        project=json.loads((ROOT/'schemas/sc-workspace-project-v10.schema.json').read_text())
    ...
    def test_migration_from_v011(self):
        j=JS.read_text()
        for token in ("const STORAGE_VERSION = 13","const PROJECT_SCHEMA = 'sc-workspace-project/10.0'",'function migrateV12(raw)',"if (raw.schemaVersion === 12) return migrateV12(raw)",'state.knowledge = normalizeKnowledge(raw.knowledge, state.projects)'):
            self.assertIn(token,j)
