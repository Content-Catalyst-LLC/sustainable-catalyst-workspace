...
class PersonalKnowledgeContractTests(unittest.TestCase):
    ...
    def test_migration_from_v011(self):
        j=JS.read_text()
        for token in ("const STORAGE_VERSION = 14","const PROJECT_SCHEMA = 'sc-workspace-project/11.0'",'function migrateV13(raw)',"if (raw.schemaVersion === 13) return migrateV13(raw)",'state.knowledge = normalizeKnowledge(raw.knowledge, state.projects)'):
            self.assertIn(token,j)
