...
class PersonalKnowledgeContractTests(unittest.TestCase):
    def test_release_boundary(self):
        ...
        self.assertEqual(m['version'],'0.35.0'); self.assertEqual(m['previous_version'],'0.34.0')
        self.assertEqual(m['storage_schema_version'],31); self.assertEqual(m['project_schema'],'sc-workspace-project/16.0')
    ...
    def test_migration_from_v011(self):
        j=JS.read_text()
        for token in ("const STORAGE_VERSION = 31","const PROJECT_SCHEMA = 'sc-workspace-project/16.0'",'function migrateV14(raw)','function migrateV15(raw)',"if (raw.schemaVersion === 15) return migrateV15(raw)",'state.knowledge = normalizeKnowledge(raw.knowledge, state.projects)'):
            self.assertIn(token,j)
