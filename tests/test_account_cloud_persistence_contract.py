...
class AccountCloudPersistenceContract(unittest.TestCase):
  def test_release_lineage(self): self.assertEqual(MAN['version'],'0.37.0'); self.assertEqual(MAN['previous_version'],'0.36.0'); self.assertEqual(MAN['storage_schema_version'],33); self.assertEqual(MAN['project_schema'],'sc-workspace-project/18.0')
  ...
  def test_project_schema_unchanged(self): self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/18.0'",JS)
  def test_storage_migration(self): self.assertIn('function migrateV20(raw)',JS); self.assertIn('if (raw.schemaVersion === 20) return migrateV20(raw);',JS); self.assertIn('const STORAGE_VERSION = 33;',JS)
