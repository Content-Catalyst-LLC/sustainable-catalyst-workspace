...
class AccountCloudPersistenceContract(unittest.TestCase):
  def test_release_lineage(self): self.assertEqual(MAN['version'],'0.26.0'); self.assertEqual(MAN['previous_version'],'0.25.0'); self.assertEqual(MAN['storage_schema_version'],25); self.assertEqual(MAN['project_schema'],'sc-workspace-project/11.0')
  ...
  def test_storage_migration(self): self.assertIn('function migrateV20(raw)',JS); self.assertIn('if (raw.schemaVersion === 20) return migrateV20(raw);',JS); self.assertIn('const STORAGE_VERSION = 25;',JS)
