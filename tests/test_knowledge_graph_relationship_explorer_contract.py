...
class KnowledgeGraphRelationshipExplorer(unittest.TestCase):
 ...
 def test_07_ui(self): app=(P/'assets/js/workspace-v1.4.0.js').read_text();self.assertIn('Trace explicit path',PHP);self.assertIn('workspace-knowledge-graph-snapshot.json',app)
 def test_08_identity(self): self.assertIn('Version: 1.4.0',MAIN);self.assertIn('data-release-stage="knowledge-graph-explorer"',PHP)
