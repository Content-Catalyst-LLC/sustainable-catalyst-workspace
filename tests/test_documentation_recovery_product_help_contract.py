...
class ProductHelpContract(unittest.TestCase):
 def test_01_lineage(self):
  ...
  self.assertIn('Version: 0.82.1',MAIN)
 ...
 def test_09_assets(self):
  self.assertIn("'sc-workspace-v0821'",PHP); self.assertIn('workspace-v0.82.1.js',PHP); self.assertIn('workspace-v0.82.1.css',PHP); self.assertIn('sc-workspace-product-help-v1',PHP); self.assertIn('sc-workspace-product-help-ui-v1',PHP); self.assertIn('/* v0.76.0 — Documentation, Recovery Guidance & Product Help */',CSS)
 def test_10_registry_history(self):
  self.assertEqual((REG['public_version'],REG['previous_version'],REG['release_name']),('0.76.0','0.75.0','Documentation, Recovery Guidance & Product Help')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0821'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0750',REGPHP); self.assertTrue((R/'history/release-manifest-v0.75.0.json').exists()); self.assertTrue((R/'history/workspace-product-record-v0.75.0.json').exists())
