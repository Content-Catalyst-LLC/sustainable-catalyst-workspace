...
class CitationLibraryReferenceManagement(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.44.0','0.43.0','Citation Library & Reference Management')); self.assertIn('Version: 0.44.0',MAIN)
 def test_02_schema_stable(self): self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(35,'sc-workspace-project/20.0','sc-workspace-project-export/20.0')); self.assertFalse(MAN['schema_migration_required']); self.assertTrue(MAN['migration']['schema_stable']); self.assertTrue(MAN['migration']['citation_library_only_release']); self.assertFalse(MAN['migration']['canonical_data_rewrite'])
 ...
 def test_12_registry_history(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.44.0','0.43.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0440'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0430',REGPHP); self.assertTrue((ROOT/'history/release-manifest-v0.43.0.json').exists()); self.assertTrue((ROOT/'history/workspace-product-record-v0.43.0.json').exists())
