...
const nb=require('../wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-research-notebook-v8.js');
let seq=0;const id=p=>`${p}-${++seq}`;const now=()=> '2026-08-09T21:45:00.000Z';
assert.strictEqual(nb.WORKSPACE_SCHEMA,'sc-workspace-notebook-workspace/8.0');
assert.strictEqual(nb.EXPORT_SCHEMA,'sc-workspace-notebook-export/8.0');
...
assert.strictEqual(ws.schema,'sc-workspace-notebook-workspace/8.0');assert.deepStrictEqual(ws.syntheses,[]);
...
const nbexp=nb.exportNotebook(ws.notebooks[0],{id:'p1',title:'Project'},'0.36.0',ws);assert.strictEqual(nbexp.schema,'sc-workspace-notebook-export/8.0');assert.strictEqual(nbexp.syntheses.length,1);assert.strictEqual(nbexp.governance.automaticSynthesis,false);
