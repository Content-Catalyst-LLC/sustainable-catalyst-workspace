...
assert.strictEqual(nb.WORKSPACE_SCHEMA,'sc-workspace-notebook-workspace/5.0');
assert.strictEqual(nb.EXPORT_SCHEMA,'sc-workspace-notebook-export/5.0');
...
assert.strictEqual(ws.schema,'sc-workspace-notebook-workspace/5.0');assert.deepStrictEqual(ws.syntheses,[]);
...
const nbexp=nb.exportNotebook(ws.notebooks[0],{id:'p1',title:'Project'},'0.36.0',ws);assert.strictEqual(nbexp.schema,'sc-workspace-notebook-export/5.0');assert.strictEqual(nbexp.syntheses.length,1);assert.strictEqual(nbexp.governance.automaticSynthesis,false);
