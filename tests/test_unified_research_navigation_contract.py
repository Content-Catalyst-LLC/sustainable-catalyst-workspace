...
class UnifiedResearchNavigation(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.41.0','0.40.0','Unified Research Navigation & Information Architecture')); self.assertIn('Version: 0.41.0',MAIN)
 def test_02_schema_stable(self): self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(35,'sc-workspace-project/20.0','sc-workspace-project-export/20.0')); self.assertFalse(MAN['schema_migration_required']); self.assertTrue(MAN['migration']['schema_stable']); self.assertTrue(MAN['migration']['navigation_only_release']); self.assertFalse(MAN['migration']['canonical_data_rewrite'])
 ...
 def test_11_registry_history(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.41.0','0.40.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0410'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0400',REGPHP); self.assertTrue((ROOT/'history/release-manifest-v0.40.0.json').exists()); self.assertTrue((ROOT/'history/workspace-product-record-v0.40.0.json').exists())
