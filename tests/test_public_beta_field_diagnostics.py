...
class FieldDiagnosticsContract(unittest.TestCase):
 def test_lineage(self): self.assertEqual(MAN['version'],'0.39.0'); self.assertEqual(MAN['previous_version'],'0.38.0'); self.assertEqual(MAN['release_name'],'Notebook Review & Provenance')
 def test_schema_stable(self): self.assertEqual(MAN['storage_schema_version'],35); self.assertEqual(MAN['project_schema'],'sc-workspace-project/20.0'); self.assertTrue(MAN['schema_migration_required']); self.assertTrue(MAN['governance']['schema_migration_required'])
 ...
 def test_registry(self): self.assertEqual(REG['public_version'],'0.39.0'); self.assertEqual(REG['previous_version'],'0.38.0'); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0390'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0300',REGPHP)
 def test_plugin_version(self): self.assertIn('Version: 0.39.0',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '0.39.0",MAIN)
