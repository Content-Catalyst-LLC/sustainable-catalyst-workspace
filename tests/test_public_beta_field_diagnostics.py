...
class FieldDiagnosticsContract(unittest.TestCase):
 ...
 def test_schema_stable(self): self.assertEqual(MAN['storage_schema_version'],27); self.assertEqual(MAN['project_schema'],'sc-workspace-project/12.0'); self.assertFalse(MAN['schema_migration_required']); self.assertFalse(MAN['governance']['schema_migration_required'])
 ...
 def test_registry(self): self.assertEqual(REG['public_version'],'0.31.0'); self.assertEqual(REG['previous_version'],'0.30.0'); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0310'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0300',REGPHP)
 def test_plugin_version(self): self.assertIn('Version: 0.31.0',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '0.31.0",MAIN)
