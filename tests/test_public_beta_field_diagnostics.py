...
class FieldDiagnosticsContract(unittest.TestCase):
 def test_lineage(self): self.assertEqual(MAN['version'],'0.54.0'); self.assertEqual(MAN['previous_version'],'0.53.0'); self.assertEqual(MAN['release_name'],'Shared Review & Research Handoff')
 ...
 def test_registry(self): self.assertEqual(REG['public_version'],'0.54.0'); self.assertEqual(REG['previous_version'],'0.53.0'); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0540'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0300',REGPHP)
 def test_plugin_version(self): self.assertIn('Version: 0.54.0',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '0.54.0",MAIN)
