...
class FieldDiagnosticsContract(unittest.TestCase):
 def test_lineage(self): self.assertEqual(MAN['version'],'0.64.1'); self.assertEqual(MAN['previous_version'],'0.64.0'); self.assertEqual(MAN['release_name'],'Accessibility Runtime & Desktop Layout Recovery')
 ...
 def test_registry(self): self.assertEqual(REG['public_version'],'0.64.1'); self.assertEqual(REG['previous_version'],'0.64.0'); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0641'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0300',REGPHP)
 def test_plugin_version(self): self.assertIn('Version: 0.64.1',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '0.64.1",MAIN)
