...
class FieldDiagnosticsContract(unittest.TestCase):
 ...
 def test_registry(self): self.assertEqual(REG['public_version'],'0.66.0'); self.assertEqual(REG['previous_version'],'0.65.0'); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0780'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0300',REGPHP)
 def test_plugin_version(self): self.assertIn('Version: 0.78.0',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '0.78.0",MAIN)
