...
class FieldDiagnosticsContract(unittest.TestCase):
 def test_lineage(self): self.assertEqual(MAN['version'],'0.49.0'); self.assertEqual(MAN['previous_version'],'0.48.0'); self.assertEqual(MAN['release_name'],'Research Templates & Reusable Workflows')
 ...
 def test_registry(self): self.assertEqual(REG['public_version'],'0.49.0'); self.assertEqual(REG['previous_version'],'0.48.0'); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0490'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0300',REGPHP)
 def test_plugin_version(self): self.assertIn('Version: 0.49.0',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '0.49.0",MAIN)
