...
class T(unittest.TestCase):
 def test_01_lineage(self):
  self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.66.0','0.65.0','Import, Export & Backward-Compatibility Hardening'));self.assertIn('Version: 1.8.0',MAIN)
 ...
 def test_06_asset_wiring(self):
  self.assertIn("'sc-workspace-field-use-v1'",PHP);self.assertIn("array('sc-workspace-browser-compatibility-v1', 'sc-workspace-accessibility-v1')",PHP);self.assertIn("'sc-workspace-v180'",PHP);self.assertIn('workspace-v1.8.0.css',PHP);self.assertIn('workspace-v1.8.0.js',PHP);self.assertIn('SCWorkspaceFieldUse.boot(window)',APP)
 ...
 def test_12_registry_and_history(self):
  self.assertEqual((REG['public_version'],REG['previous_version'],REG['release_name']),('0.66.0','0.65.0','Import, Export & Backward-Compatibility Hardening'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v180'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0641',REGPHP);self.assertTrue((ROOT/'history/release-manifest-v0.64.1.json').exists());self.assertTrue((ROOT/'history/workspace-product-record-v0.64.1.json').exists())
