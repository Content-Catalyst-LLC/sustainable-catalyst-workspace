...
PHP=(WP/'includes/class-sc-workspace.php').read_text(); PLUGIN=(WP/'sustainable-catalyst-workspace.php').read_text(); MAIN=(WP/'assets/js/workspace-v2.36.0.js').read_text(); TYPED=(WP/'assets/js/sc-workspace-typed-client-v2360.js').read_text(); GENERATED=(ROOT/'frontend/typed-client/src/00-generated-contracts.ts').read_text()
def test_release_identity_and_assets():
    assert 'Version: 2.36.0' in PLUGIN and "const WORKSPACE_RELEASE = '2.36.0';" in MAIN
    assert 'workspace-v2.36.0.js' in PHP and 'workspace-v2.36.0.css' in PHP and 'sc-workspace-typed-client-v2360.js' in PHP
def test_typed_client_loads_before_main_runtime():
    typed=PHP.index("'sc-workspace-typed-client-v2360'"); main=PHP.index("'sc-workspace-v241'",typed); assert typed<main
    main_block=PHP[main:PHP.index('SC_WORKSPACE_VERSION',main)]; assert 'sc-workspace-typed-client-v2360' in main_block
