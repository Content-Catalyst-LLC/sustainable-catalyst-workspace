...
PHP=(WP/'includes/class-sc-workspace.php').read_text(); PLUGIN=(WP/'sustainable-catalyst-workspace.php').read_text(); MAIN=(WP/'assets/js/workspace-v3.0.0.js').read_text(); TYPED=(WP/'assets/js/sc-workspace-typed-client-v3000.js').read_text(); GENERATED=(ROOT/'frontend/typed-client/src/00-generated-contracts.ts').read_text()
def test_release_identity_and_assets():
    assert 'Version: 3.0.0' in PLUGIN and "const WORKSPACE_RELEASE = '3.0.0';" in MAIN
    assert 'workspace-v3.0.0.js' in PHP and 'workspace-v3.0.0.css' in PHP and 'sc-workspace-typed-client-v3000.js' in PHP
def test_typed_client_loads_before_main_runtime():
    typed=PHP.index("'sc-workspace-typed-client-v3000'"); main=PHP.index("'sc-workspace-v241'",typed); assert typed<main
    main_block=PHP[main:PHP.index('SC_WORKSPACE_VERSION',main)]; assert 'sc-workspace-typed-client-v3000' in main_block
