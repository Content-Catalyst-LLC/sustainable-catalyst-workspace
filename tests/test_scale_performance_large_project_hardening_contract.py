...
class TestScalePerformanceLargeProjectHardening(unittest.TestCase):
 ...
 def test_12_registry_history(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.62.0','0.61.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0620'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0570',REGPHP); self.assertTrue((ROOT/'history/release-manifest-v0.58.0.json').exists())
