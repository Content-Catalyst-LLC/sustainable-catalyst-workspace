...
APP=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.58.0.js').read_text()
...
class TestScalePerformanceLargeProjectHardening(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.58.0','0.57.0','Scale, Performance & Large-Project Hardening')); self.assertIn('Version: 0.58.0',MAIN)
 ...
 def test_12_registry_history(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.58.0','0.57.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0580'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0570',REGPHP); self.assertTrue((ROOT/'history/release-manifest-v0.57.0.json').exists())
