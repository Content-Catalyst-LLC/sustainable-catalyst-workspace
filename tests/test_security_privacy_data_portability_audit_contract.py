...
class SecurityPrivacyPortabilityContract(unittest.TestCase):
 def test_01_release_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.66.0','0.65.0','Import, Export & Backward-Compatibility Hardening')); self.assertIn('Version: 0.73.0',MAIN)
 ...
 def test_12_registry_history(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.66.0','0.65.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0730'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0580',REGPHP); self.assertTrue((ROOT/'history/release-manifest-v0.58.0.json').exists())
