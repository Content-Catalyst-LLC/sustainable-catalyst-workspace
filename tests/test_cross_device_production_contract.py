...
class CrossDeviceProduction(unittest.TestCase):
 ...
 def test_02_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('1.7.0','1.6.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v170'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V160',REGPHP)
 ...
 def test_09_runtime(self): self.assertIn('data-release-stage="cross-device-production"',PHP);self.assertIn('workspace-v1.7.0.js',PHP);self.assertIn('sc-workspace-cross-device-production-v1',PHP)
 def test_10_deployment(self): self.assertIn("PREVIOUS_RELEASE = '1.6.0'",DEP);self.assertIn("ROLLBACK_RELEASE = '1.6.0'",DEP);self.assertIn('class-sc-workspace-cross-device-production.php',DEP)
 def test_11_identity(self): self.assertIn('Version: 1.7.0',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '1.7.0'",MAIN)
