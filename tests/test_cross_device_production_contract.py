...
class CrossDeviceProduction(unittest.TestCase):
 ...
 def test_09_runtime(self): self.assertIn('data-release-stage="root-scope-cockpit-recovery"',PHP);self.assertIn('workspace-v2.0.3.js',PHP);self.assertIn('sc-workspace-cross-device-production-v1',PHP)
 def test_10_deployment(self): self.assertIn("PREVIOUS_RELEASE = '2.0.2'",DEP);self.assertIn("ROLLBACK_RELEASE = '2.0.2'",DEP);self.assertIn('class-sc-workspace-cross-device-production.php',DEP)
