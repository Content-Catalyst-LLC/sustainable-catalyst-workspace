...
class CrossDeviceProduction(unittest.TestCase):
 ...
 def test_09_runtime(self): self.assertIn('data-release-stage="connected-knowledge-workspace"',PHP);self.assertIn('workspace-v2.0.0.js',PHP);self.assertIn('sc-workspace-cross-device-production-v1',PHP)
 def test_10_deployment(self): self.assertIn("PREVIOUS_RELEASE = '1.15.0'",DEP);self.assertIn("ROLLBACK_RELEASE = '1.15.0'",DEP);self.assertIn('class-sc-workspace-cross-device-production.php',DEP)
