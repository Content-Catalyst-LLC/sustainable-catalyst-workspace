...
class CrossDeviceProduction(unittest.TestCase):
 ...
 def test_09_runtime(self): self.assertIn('data-release-stage="developer-api"',PHP);self.assertIn('workspace-v1.11.0.js',PHP);self.assertIn('sc-workspace-cross-device-production-v1',PHP)
 def test_10_deployment(self): self.assertIn("PREVIOUS_RELEASE = '1.10.0'",DEP);self.assertIn("ROLLBACK_RELEASE = '1.10.0'",DEP);self.assertIn('class-sc-workspace-cross-device-production.php',DEP)
