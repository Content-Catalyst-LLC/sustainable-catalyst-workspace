...
REG = json.loads((ROOT / 'registry/workspace-product-record-v0.33.0.json').read_text())
...
class SourceCaptureResearchClippingContract(unittest.TestCase):
    def test_01_release_lineage(self):
        self.assertEqual((MAN['version'], MAN['previous_version'], MAN['release_name']), ('0.33.0','0.32.0','Source Capture & Research Clipping'))
        self.assertIn('Version: 0.33.0', MAIN)

    def test_02_schema_migration(self):
        self.assertEqual((MAN['storage_schema_version'], MAN['project_schema'], MAN['export_schema']), (29,'sc-workspace-project/14.0','sc-workspace-project-export/14.0'))
        self.assertEqual((MAN['migration']['storage_from'], MAN['migration']['storage_to']), (28,29))
        self.assertEqual((MAN['migration']['project_schema_from'], MAN['migration']['project_schema_to']), ('sc-workspace-project/13.0','sc-workspace-project/14.0'))
    ...
    def test_12_notebook_v2_preserves_capture_context(self):
        for token in ('sc-workspace-notebook-workspace/2.0','sc-workspace-notebook-block/2.0','blockFromCapture','citationLine','bibliography','capture'):
    ...
    def test_14_capture_adapter_public_helper(self):
        ...
        self.assertIn("'sc-workspace-research-notebook-v2'", PHP)
    ...
    def test_16_registry_and_history(self):
        self.assertEqual((REG['public_version'], REG['previous_version']), ('0.33.0','0.32.0'))
        self.assertIn('Source Capture & Research Clipping', REG['change_summary'])
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0330'", REGPHP)
