...
class SourceCaptureResearchClippingContract(unittest.TestCase):
    def test_01_release_lineage(self):
        self.assertEqual((MAN['version'], MAN['previous_version'], MAN['release_name']), ('0.50.0','0.49.0','Workspace Experience Consolidation'))
        self.assertIn('Version: 0.50.0', MAIN)
    ...
    def test_16_registry_and_history(self):
        self.assertEqual((REG['public_version'], REG['previous_version']), ('0.50.0','0.49.0'))
        self.assertIn('Workspace Experience Consolidation', REG['change_summary'])
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0500'", REGPHP)
