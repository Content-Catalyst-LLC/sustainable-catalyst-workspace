...
class SourceCaptureResearchClippingContract(unittest.TestCase):
    def test_01_release_lineage(self):
        self.assertEqual((MAN['version'], MAN['previous_version'], MAN['release_name']), ('0.46.1','0.46.0','Workspace Import & Interchange'))
        self.assertIn('Version: 0.46.1', MAIN)
    ...
    def test_16_registry_and_history(self):
        self.assertEqual((REG['public_version'], REG['previous_version']), ('0.46.1','0.46.0'))
        self.assertIn('Workspace Import & Interchange', REG['change_summary'])
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0461'", REGPHP)
