...
class SourceCaptureResearchClippingContract(unittest.TestCase):
    def test_01_release_lineage(self):
        self.assertEqual((MAN['version'], MAN['previous_version'], MAN['release_name']), ('0.54.0','0.53.0','Shared Review & Research Handoff'))
    ...
    def test_16_registry_and_history(self):
        self.assertEqual((REG['public_version'], REG['previous_version']), ('0.54.0','0.53.0'))
        self.assertIn('Shared Review & Research Handoff', REG['change_summary'])
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0540'", REGPHP)
