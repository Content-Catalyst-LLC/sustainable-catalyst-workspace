...
class SourceCaptureResearchClippingContract(unittest.TestCase):
    def test_01_release_lineage(self):
        self.assertEqual((MAN['version'], MAN['previous_version'], MAN['release_name']), ('0.41.0','0.40.0','Unified Research Navigation & Information Architecture'))
        self.assertIn('Version: 0.41.0', MAIN)
    ...
    def test_16_registry_and_history(self):
        self.assertEqual((REG['public_version'], REG['previous_version']), ('0.41.0','0.40.0'))
        self.assertIn('Unified Research Navigation & Information Architecture', REG['change_summary'])
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0410'", REGPHP)
