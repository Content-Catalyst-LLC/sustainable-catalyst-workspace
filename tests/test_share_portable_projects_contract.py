...
class SharePortableProjectsTests(unittest.TestCase):
 def test_release(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.58.0');self.assertEqual(m['previous_version'],'0.57.0');self.assertEqual(m['storage_schema_version'],35);self.assertEqual(m['project_schema'],'sc-workspace-project/20.0')
 ...
 def test_ui(self):
  p=PHP.read_text();c=CSS.read_text();self.assertIn('SHARE &amp; PORTABLE PROJECTS',p);self.assertIn('data-scw-share-export',p);self.assertIn('data-scw-share-import',p);self.assertIn('.scw-share{',c);self.assertIn("function syncWorkspaceNavigation()",JS.read_text());self.assertIn('data-scw-workspace-view="share"',p);self.assertIn("shareSection.hidden = workspaceView !== 'share'",JS.read_text())
