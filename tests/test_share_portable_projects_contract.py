...
JS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.16.0.js';PHP=ROOT/'wordpress/sustainable-catalyst-workspace/includes/class-sc-workspace.php';CSS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.16.0.css';MANIFEST=ROOT/'release-manifest-v0.16.0.json'
class SharePortableProjectsTests(unittest.TestCase):
 ...
 def test_ui(self):
  p=PHP.read_text();c=CSS.read_text();self.assertIn('SHARE &amp; PORTABLE PROJECTS',p);self.assertIn('data-scw-share-export',p);self.assertIn('data-scw-share-import',p);self.assertIn('.scw-share{',c);self.assertIn("['projects','knowledge','graph','interoperability','share']",JS.read_text());self.assertIn("shareSection.hidden = workspaceView !== 'share'",JS.read_text())
