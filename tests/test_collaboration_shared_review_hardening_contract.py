...
class CollaborationSharedReviewHardeningContract(unittest.TestCase):
 ...
 def test_wordpress_current_assets_and_rest(self):
  self.assertIn('Version: 0.74.0',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '0.74.0",MAIN); self.assertIn("'/collaboration-review-hardening-contract'",PHP); self.assertIn('collaboration_review_hardening_contract',PHP)
 ...
 def test_registry_history_docs_schemas(self):
  ...
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0740'",REGPHP); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0740'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0720',REGPHP)
