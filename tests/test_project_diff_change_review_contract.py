...
REG=json.loads((ROOT/'registry/workspace-product-record-v0.25.0.json').read_text())
class ProjectDiffChangeReviewContract(unittest.TestCase):
  def test_lineage(self): self.assertEqual(MAN['version'],'0.25.0'); self.assertEqual(MAN['previous_version'],'0.24.0'); self.assertEqual(MAN['release_name'],'Change Gates & Safe Actions')
  def test_schema_stable(self): self.assertEqual(MAN['storage_schema_version'],24); self.assertEqual(MAN['project_schema'],'sc-workspace-project/11.0'); self.assertIn('const STORAGE_VERSION = 24;',JS)
  ...
  def test_registry(self): self.assertEqual(REG['public_version'],'0.25.0'); self.assertEqual(REG['previous_version'],'0.24.0')
