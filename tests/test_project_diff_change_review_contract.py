...
REG=json.loads((ROOT/'registry/workspace-product-record-v0.26.0.json').read_text())
class ProjectDiffChangeReviewContract(unittest.TestCase):
  def test_lineage(self): self.assertEqual(MAN['version'],'0.26.0'); self.assertEqual(MAN['previous_version'],'0.25.0'); self.assertEqual(MAN['release_name'],'Guided Reconciliation & Selective Apply')
  def test_schema_stable(self): self.assertEqual(MAN['storage_schema_version'],25); self.assertEqual(MAN['project_schema'],'sc-workspace-project/11.0'); self.assertIn('const STORAGE_VERSION = 25;',JS)
  ...
  def test_registry(self): self.assertEqual(REG['public_version'],'0.26.0'); self.assertEqual(REG['previous_version'],'0.25.0')
