...
MANIFEST=json.loads((ROOT/'release-manifest-v0.44.0.json').read_text())
...
class StabilityAccessibilityReleaseReadiness(unittest.TestCase):
    def test_release_metadata_and_schema_stability(self):
        ...
        self.assertEqual(MANIFEST['previous_version'],'0.43.0')
        self.assertEqual(MANIFEST['release_name'],'Citation Library & Reference Management')
    ...
    def test_registry_lineage_advances_with_safe_actions_storage_migration(self):
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0440'",REG)
        self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0440'",REG)
        ...
        self.assertIn("'previous_version' => '0.43.0'",REG)
