...
MANIFEST=json.loads((ROOT/'release-manifest-v0.61.0.json').read_text())

class StabilityAccessibilityReleaseReadiness(unittest.TestCase):
    def test_release_metadata_and_schema_stability(self):
        self.assertEqual(MANIFEST['version'],'0.61.0')
        self.assertEqual(MANIFEST['previous_version'],'0.60.0')
        self.assertEqual(MANIFEST['release_name'],'Product Hardening I: Browser, Recovery & Field-Use Resilience')
    ...
    def test_registry_lineage_advances_with_safe_actions_storage_migration(self):
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0610'",REG)
        self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0610'",REG)
        ...
        self.assertIn("'previous_version' => '0.60.0'",REG)
