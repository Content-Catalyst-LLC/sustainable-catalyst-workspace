...
MANIFEST=json.loads((ROOT/'release-manifest-v0.49.0.json').read_text())

class StabilityAccessibilityReleaseReadiness(unittest.TestCase):
    def test_release_metadata_and_schema_stability(self):
        self.assertEqual(MANIFEST['version'],'0.49.0')
        self.assertEqual(MANIFEST['previous_version'],'0.48.0')
        self.assertEqual(MANIFEST['release_name'],'Research Templates & Reusable Workflows')
        self.assertEqual(MANIFEST['storage_schema_version'],35)
        self.assertEqual(MANIFEST['project_schema'],'sc-workspace-project/20.0')
    ...
    def test_registry_lineage_advances_with_safe_actions_storage_migration(self):
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0490'",REG)
        self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0490'",REG)
        ...
        self.assertIn("'previous_version' => '0.48.0'",REG)
