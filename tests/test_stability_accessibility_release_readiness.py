...
MANIFEST=json.loads((ROOT/'release-manifest-v0.60.0.json').read_text())
...
class StabilityAccessibilityReleaseReadiness(unittest.TestCase):
    def test_release_metadata_and_schema_stability(self):
        ...
        self.assertEqual(MANIFEST['previous_version'],'0.59.1')
        self.assertEqual(MANIFEST['release_name'],'Public Product Beta II')
    ...
    def test_registry_lineage_advances_with_safe_actions_storage_migration(self):
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0600'",REG)
        self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0600'",REG)
        ...
        self.assertIn("'previous_version' => '0.59.1'",REG)
