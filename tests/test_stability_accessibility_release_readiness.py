...
class StabilityAccessibilityReleaseReadiness(unittest.TestCase):
    def test_release_metadata_and_schema_stability(self):
        ...
        self.assertEqual(MANIFEST['previous_version'],'0.33.0')
        self.assertEqual(MANIFEST['release_name'],'Notebook Collections & Knowledge Linking')
        self.assertEqual(MANIFEST['storage_schema_version'],30)
        self.assertEqual(MANIFEST['project_schema'],'sc-workspace-project/15.0')
    ...
    def test_registry_lineage_advances_with_safe_actions_storage_migration(self):
        self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0340'",REG)
        self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0340'",REG)
        ...
        self.assertIn("'previous_version' => '0.33.0'",REG)
