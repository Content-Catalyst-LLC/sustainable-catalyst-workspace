...
class LifecycleGovernanceContract(unittest.TestCase):
 ...
 def test_runtime_migration(self): self.assertIn('function migrateV26(raw)',JS); self.assertIn('if (raw.schemaVersion === 26) return migrateV26(raw);',JS); self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/14.0'",JS); self.assertIn('lifecycle: lifecycleTemplate()',JS)
 ...
 def test_registry(self): self.assertEqual(REG['public_version'],'0.33.0'); self.assertEqual(REG['previous_version'],'0.32.0')
