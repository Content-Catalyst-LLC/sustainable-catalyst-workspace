...
class ApiEmbedIntegrationHardeningContract(unittest.TestCase):
 ...
 def test_rest_and_assets(self):
  self.assertIn('Version: 1.8.0',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '1.8.0",MAIN); self.assertIn("'/api-embed-hardening-contract'",PHP); self.assertIn('api_embed_hardening_contract',PHP); self.assertIn('/wp-json/sc-workspace/v1/api-embed-hardening-contract',MAN['rest_routes']); self.assertIn("'sc-workspace-v180'",PHP); self.assertIn('workspace-v1.8.0.js',PHP); self.assertIn('workspace-v1.8.0.css',PHP)
 def test_registry_history_and_artifacts(self):
  self.assertEqual((REG['public_version'],REG['previous_version'],REG['release_name']),('0.74.0','0.73.0','API, Embed & Integration Hardening')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v180'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0730',REGPHP); self.assertTrue((R/'history/release-manifest-v0.73.0.json').exists()); self.assertTrue((R/'history/workspace-product-record-v0.73.0.json').exists())
