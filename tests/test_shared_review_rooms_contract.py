...
MAN=json.loads((R/'release-manifest-v1.8.0.json').read_text());OLD=json.loads((R/'history/release-manifest-v1.7.0.json').read_text());REG=json.loads((R/'registry/workspace-product-record-v1.8.0.json').read_text())
...
class SharedReviewRooms(unittest.TestCase):
 ...
 def test_02_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('1.8.0','1.7.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v180'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V170',REGPHP)
 ...
 def test_09_runtime(self): self.assertIn('data-release-stage="shared-review-rooms"',PHP);self.assertIn('workspace-v1.8.0.js',PHP);self.assertIn('sc-workspace-review-rooms-v1',PHP);self.assertIn('SCWorkspaceReviewRoomsUI',UI)
 def test_10_deployment(self): self.assertIn("PREVIOUS_RELEASE = '1.7.0'",DEP);self.assertIn("ROLLBACK_RELEASE = '1.7.0'",DEP);self.assertIn('class-sc-workspace-review-rooms.php',DEP)
 def test_11_identity(self): self.assertIn('Version: 1.8.0',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '1.8.0'",MAIN)
