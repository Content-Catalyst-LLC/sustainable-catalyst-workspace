...
class SharedReviewRooms(unittest.TestCase):
 ...
 def test_09_runtime(self): self.assertIn('data-release-stage="research-operations"',PHP);self.assertIn('workspace-v1.10.0.js',PHP);self.assertIn('sc-workspace-review-rooms-v1',PHP);self.assertIn('SCWorkspaceReviewRoomsUI',UI)
 def test_10_deployment(self): self.assertIn("PREVIOUS_RELEASE = '1.9.0'",DEP);self.assertIn("ROLLBACK_RELEASE = '1.9.0'",DEP);self.assertIn('class-sc-workspace-review-rooms.php',DEP)
