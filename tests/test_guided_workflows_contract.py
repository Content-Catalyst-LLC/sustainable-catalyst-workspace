...
MANIFEST=ROOT/'release-manifest-v0.11.0.json'
class GuidedWorkflowContractTests(unittest.TestCase):
    def test_release_boundary(self):
        m=json.loads(MANIFEST.read_text())
        self.assertEqual(m['version'],'0.11.0'); self.assertEqual(m['previous_version'],'0.10.0')
        self.assertEqual(m['storage_schema_version'],12); self.assertEqual(m['project_schema'],'sc-workspace-project/10.0')
    ...
    def test_migration_and_import_compatibility(self):
        j=JS.read_text()
        for token in ("const STORAGE_VERSION = 12","const PROJECT_SCHEMA = 'sc-workspace-project/10.0'","const LEGACY_PROJECT_SCHEMA_V9 = 'sc-workspace-project/9.0'",'function migrateV11(raw)',"if (raw.schemaVersion === 11) return migrateV11(raw)","LEGACY_EXPORT_SCHEMA_V9"):
            self.assertIn(token,j)
