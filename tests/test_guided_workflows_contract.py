...
class GuidedWorkflowContractTests(unittest.TestCase):
    ...
    def test_migration_and_import_compatibility(self):
        j=JS.read_text()
        for token in ("const STORAGE_VERSION = 16","const PROJECT_SCHEMA = 'sc-workspace-project/11.0'","const LEGACY_PROJECT_SCHEMA_V10 = 'sc-workspace-project/10.0'",'function migrateV14(raw)','function migrateV15(raw)',"if (raw.schemaVersion === 15) return migrateV15(raw)","LEGACY_EXPORT_SCHEMA_V9"):
            self.assertIn(token,j)
