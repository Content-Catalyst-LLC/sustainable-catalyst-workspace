...
class GuidedWorkflowContractTests(unittest.TestCase):
    ...
    def test_migration_and_import_compatibility(self):
        j=JS.read_text()
        for token in ("const STORAGE_VERSION = 21","const PROJECT_SCHEMA = 'sc-workspace-project/11.0'","const LEGACY_PROJECT_SCHEMA_V10 = 'sc-workspace-project/10.0'",'function migrateV14(raw)','function migrateV15(raw)',"if (raw.schemaVersion === 15) return migrateV15(raw)","LEGACY_EXPORT_SCHEMA_V9"):
            self.assertIn(token,j)
    ...
    def test_no_new_server_boundary(self):
        m=json.loads(MANIFEST.read_text())
        self.assertEqual(m['server_project_storage'],'manual-account-backup-only'); self.assertFalse(m['cloud_sync']); self.assertFalse(m['governance']['templates_generate_findings'])
