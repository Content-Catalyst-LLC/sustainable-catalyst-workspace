...
class InstitutionalScaleHardening(unittest.TestCase):
 ...
 def test_07_wordpress(self): self.assertIn('Version: 2.0.3',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '2.0.3'",MAIN);self.assertIn("'/institutional-scale-hardening-contract'",PHP);self.assertIn('institutional_scale_hardening_contract',PHP);self.assertIn('data-release-stage="root-scope-cockpit-recovery"',PHP);self.assertIn('Large Workspace &amp; Institutional Scale Hardening',PHP)
 ...
 def test_09_deployment(self): self.assertIn("PREVIOUS_RELEASE = '2.0.2'",DEP);self.assertIn("ROLLBACK_RELEASE = '2.0.2'",DEP);self.assertIn('workspace-v2.0.3.js',DEP);self.assertIn('class-sc-workspace-institutional-scale-hardening.php',DEP)
 def test_10_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('1.12.0','1.11.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v203'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V1120',REGPHP)
