...
with sync_playwright() as p:
 ...
 for w,h in V:
  ...
  x=page.evaluate("""() => {const panel=document.querySelector('.scw-release-candidate').getBoundingClientRect(),sections=[...document.querySelectorAll('.scw-release-candidate-grid>section')].map(x=>x.getBoundingClientRect()),buttons=[...document.querySelectorAll('.scw-release-candidate .scw-button')].map(x=>x.getBoundingClientRect());return {body:document.documentElement.scrollWidth,panel:panel.width,minSection:Math.min(...sections.map(x=>x.width)),minH:Math.min(...buttons.map(x=>x.height))}}""")
