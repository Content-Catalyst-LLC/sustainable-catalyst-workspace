...
class ResearchWorkflowGuidanceContract(unittest.TestCase):
 ...
 def test_wordpress_current_assets_and_rest(self):
  self.assertIn('Version: 1.2.0',MAIN); self.assertIn("SC_WORKSPACE_VERSION', '1.2.0",MAIN); self.assertIn("'/workflow-guidance-contract'",PHP); self.assertIn('workflow_guidance_contract',PHP)
  self.assertIn("'sc-workspace-v120'",PHP); self.assertIn('workspace-v1.2.0.js',PHP); self.assertIn('workspace-v1.2.0.css',PHP); self.assertIn('sc-workspace-workflow-guidance-v1.js',PHP); self.assertIn('sc-workspace-workflow-guidance-ui-v1.js',PHP)
 def test_registry_history_docs_schemas(self):
  ...
  self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v120'",REGPHP); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v120'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0710',REGPHP)
