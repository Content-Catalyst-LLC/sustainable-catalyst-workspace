...
class NotebookSynthesisCitationWorkspaceContract(unittest.TestCase):
 ...
 def test_08_rest_contract(self):
  self.assertIn("'notebook_workspace_schema' => 'sc-workspace-notebook-workspace/6.0'",PHP); self.assertIn("'notebook_synthesis_schema' => 'sc-workspace-notebook-synthesis/1.0'",PHP); self.assertIn("'synthesis_requires_explicit_selection' => true",PHP); self.assertIn("'citation_guessing' => false",PHP)
 ...
 def test_10_registry(self):
  self.assertEqual((REG['public_version'],REG['previous_version']),('0.37.0','0.36.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0370'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0360',REGPHP)
