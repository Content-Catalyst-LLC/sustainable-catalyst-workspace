...
class NotebookSynthesisCitationWorkspaceContract(unittest.TestCase):
 def test_01_lineage(self):
  self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.49.0','0.48.0','Research Templates & Reusable Workflows')); self.assertIn('Version: 0.49.0',MAIN)
 ...
 def test_10_registry(self):
  self.assertEqual((REG['public_version'],REG['previous_version']),('0.49.0','0.48.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0490'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0360',REGPHP)
