...
class NotebookSynthesisCitationWorkspaceContract(unittest.TestCase):
 def test_01_lineage(self):
  self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.42.0','0.41.0','Knowledge Search & Advanced Retrieval')); self.assertIn('Version: 0.42.0',MAIN)
 ...
 def test_10_registry(self):
  self.assertEqual((REG['public_version'],REG['previous_version']),('0.42.0','0.41.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0420'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0360',REGPHP)
