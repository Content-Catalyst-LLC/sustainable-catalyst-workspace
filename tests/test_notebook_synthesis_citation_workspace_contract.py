...
class NotebookSynthesisCitationWorkspaceContract(unittest.TestCase):
 def test_01_lineage(self):
  self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.36.0','0.35.0','Notebook Synthesis & Citation Workspace')); self.assertIn('Version: 0.36.0',MAIN)
 def test_02_schema_migration(self):
  self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(32,'sc-workspace-project/17.0','sc-workspace-project-export/17.0')); self.assertEqual((MAN['migration']['storage_from'],MAN['migration']['storage_to']),(31,32)); self.assertEqual((MAN['migration']['project_schema_from'],MAN['migration']['project_schema_to']),('sc-workspace-project/16.0','sc-workspace-project/17.0')); self.assertIn('function migrateV31(raw)',JS); self.assertIn('if (raw.schemaVersion === 31) return migrateV31(raw);',JS)
 def test_03_synthesis_schemas(self):
  self.assertEqual(MAN['notebook_workspace_schema'],'sc-workspace-notebook-workspace/5.0'); self.assertEqual(MAN['notebook_export_schema'],'sc-workspace-notebook-export/5.0'); self.assertEqual(MAN['notebook_synthesis_schema'],'sc-workspace-notebook-synthesis/1.0'); self.assertEqual(MAN['notebook_synthesis_export_schema'],'sc-workspace-notebook-synthesis-export/1.0')
 ...
 def test_08_rest_contract(self):
  self.assertIn("'notebook_workspace_schema' => 'sc-workspace-notebook-workspace/5.0'",PHP); self.assertIn("'notebook_synthesis_schema' => 'sc-workspace-notebook-synthesis/1.0'",PHP); self.assertIn("'synthesis_requires_explicit_selection' => true",PHP); self.assertIn("'citation_guessing' => false",PHP)
 ...
 def test_10_registry(self):
  self.assertEqual((REG['public_version'],REG['previous_version']),('0.36.0','0.35.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0360'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0350',REGPHP)
