...
class NotebookSynthesisCitationWorkspaceContract(unittest.TestCase):
 def test_01_lineage(self):
  self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.62.0','0.61.0','Product Hardening II: Persistence, Corruption & Recovery Integrity')); self.assertIn('Version: 0.62.0',MAIN)
 ...
 def test_10_registry(self):
  self.assertEqual((REG['public_version'],REG['previous_version']),('0.62.0','0.61.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0620'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0360',REGPHP)
