...
class NotebookSynthesisCitationWorkspaceContract(unittest.TestCase):
 def test_01_lineage(self):
  self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.46.1','0.46.0','Workspace Import & Interchange')); self.assertIn('Version: 0.46.1',MAIN)
 ...
 def test_10_registry(self):
  self.assertEqual((REG['public_version'],REG['previous_version']),('0.46.1','0.46.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0461'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0360',REGPHP)
