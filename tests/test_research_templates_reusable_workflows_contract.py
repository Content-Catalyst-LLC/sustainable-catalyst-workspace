...
MAN=json.loads((ROOT/'release-manifest-v0.49.0.json').read_text())
REG=json.loads((ROOT/'registry/workspace-product-record-v0.49.0.json').read_text())
PREV=json.loads((ROOT/'history/release-manifest-v0.48.0.json').read_text())
...
JS=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.49.0.js').read_text()
TPL=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-research-templates-v1.js').read_text()
CSS=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.49.0.css').read_text()
class V049Templates(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.49.0','0.48.0','Research Templates & Reusable Workflows')); self.assertIn('Version: 0.49.0',MAIN); self.assertEqual(PREV['version'],'0.48.0')
 def test_02_schema_stable(self): self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(35,'sc-workspace-project/20.0','sc-workspace-project-export/20.0')); self.assertFalse(MAN['schema_migration_required']); self.assertTrue(MAN['migration']['research_templates_only_release'])
 ...
 def test_12_registry_history(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.49.0','0.48.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0490'",REGPHP); self.assertIn("LEGACY_PENDING_KEY_V0480 = 'sc_workspace_registry_pending_v0480'",REGPHP); self.assertTrue((ROOT/'history/workspace-product-record-v0.48.0.json').exists())
