...
class AuditTrailContract(unittest.TestCase):
  ...
  def test_storage_attribute_corrected(self): self.assertIn('data-storage-version="33"',PHP); self.assertNotIn('data-storage-version="25"',PHP)
  def test_registry_lineage(self): self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0380'",REG); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0380'",REG); self.assertIn("LEGACY_PENDING_KEY_V0270",REG); self.assertIn("'previous_version' => '0.37.0'",REG)
