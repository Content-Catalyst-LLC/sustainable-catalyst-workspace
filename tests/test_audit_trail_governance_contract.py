...
class AuditTrailContract(unittest.TestCase):
  def test_lineage(self): self.assertEqual(MAN['version'],'0.29.0'); self.assertEqual(MAN['previous_version'],'0.28.0'); self.assertEqual(MAN['release_name'],'Governance Milestones & Project Lifecycle')
  def test_schema_stable(self): self.assertEqual(MAN['storage_schema_version'],27); self.assertEqual(MAN['project_schema'],'sc-workspace-project/12.0'); self.assertTrue(MAN['schema_migration_required']); self.assertEqual(MAN['migration']['storage_from'],26); self.assertEqual(MAN['migration']['storage_to'],27)
  ...
  def test_registry_lineage(self): self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0290'",REG); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0290'",REG); self.assertIn("LEGACY_PENDING_KEY_V0270",REG); self.assertIn("'previous_version' => '0.28.0'",REG)
