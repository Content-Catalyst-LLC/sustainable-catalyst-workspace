...
class AuditTrailContract(unittest.TestCase):
  ...
  def test_registry_lineage(self): self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0350'",REG); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0350'",REG); self.assertIn("LEGACY_PENDING_KEY_V0270",REG); self.assertIn("'previous_version' => '0.34.0'",REG)
