...
class AuditTrailContract(unittest.TestCase):
  def test_lineage(self): self.assertEqual(MAN['version'],'0.37.0'); self.assertEqual(MAN['previous_version'],'0.36.0'); self.assertEqual(MAN['release_name'],'Grounded Notebook Assistance')
  ...
  def test_registry_lineage(self): self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0370'",REG); self.assertIn("PENDING_KEY = 'sc_workspace_registry_pending_v0370'",REG); self.assertIn("LEGACY_PENDING_KEY_V0270",REG); self.assertIn("'previous_version' => '0.36.0'",REG)
