...
class MigrationTests(unittest.TestCase):
    def test_storage_v12_to_v13_migration(self):
        js=JS.read_text()
        for token in ("const STORAGE_VERSION = 22",'function migrateV14','function migrateV15',"if (raw.schemaVersion === 15) return migrateV15(raw)","const PROJECT_SCHEMA = 'sc-workspace-project/11.0'","const LEGACY_PROJECT_SCHEMA_V10 = 'sc-workspace-project/10.0'","guidedWorkflows: normalizeGuidedWorkflows(raw.guidedWorkflows, objects)"):
