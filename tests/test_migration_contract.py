...
class MigrationTests(unittest.TestCase):
    def test_storage_v8_to_v9_migration(self):
        js=JS.read_text()
        for token in ('const STORAGE_VERSION = 9','function migrateV8',"if (raw.schemaVersion === 8) return migrateV8(raw)","const PROJECT_SCHEMA = 'sc-workspace-project/7.0'","const LEGACY_PROJECT_SCHEMA_V6 = 'sc-workspace-project/6.0'","handoffs: normalizeHandoffs(raw.handoffs, objects, canvas)"):
