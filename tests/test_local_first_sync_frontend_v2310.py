...
PHP=(WP/'includes/class-sc-workspace.php').read_text(); PLUGIN=(WP/'sustainable-catalyst-workspace.php').read_text(); MAIN=(WP/'assets/js/workspace-v2.32.0.js').read_text(); TYPED=(WP/'assets/js/sc-workspace-typed-client-v2320.js').read_text(); TS=(ROOT/'frontend/typed-client/src/30-local-first-sync.ts').read_text()
def test_release_identity_and_interaction_runtime_preserved():
 assert 'Version: 2.32.0' in PLUGIN and "const WORKSPACE_RELEASE = '2.32.0';" in MAIN; assert 'SCWorkspaceInteractionRuntime' in MAIN and 'SCWorkspaceInteractionRepair' in MAIN
