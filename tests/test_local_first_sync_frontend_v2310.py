...
PHP=(WP/'includes/class-sc-workspace.php').read_text(); PLUGIN=(WP/'sustainable-catalyst-workspace.php').read_text(); MAIN=(WP/'assets/js/workspace-v3.0.0.js').read_text(); TYPED=(WP/'assets/js/sc-workspace-typed-client-v3000.js').read_text(); TS=(ROOT/'frontend/typed-client/src/30-local-first-sync.ts').read_text()
