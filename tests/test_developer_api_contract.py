...
class DeveloperAPI(unittest.TestCase):
 ...
 def test_06_wordpress(self): self.assertIn('Version: 1.12.0',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '1.12.0'",MAIN);self.assertIn("'/developer-api-contract'",PHP);self.assertIn('Developer API &amp; Extensions',PHP);self.assertIn('data-release-stage="institutional-scale-hardening"',PHP)
 def test_07_deployment(self): self.assertIn("PREVIOUS_RELEASE = '1.11.0'",DEP);self.assertIn("ROLLBACK_RELEASE = '1.11.0'",DEP);self.assertIn('workspace-v1.12.0.js',DEP);self.assertIn('class-sc-workspace-developer-api.php',DEP)
 def test_08_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('1.11.0','1.10.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v1120'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V1110',REGPHP)
