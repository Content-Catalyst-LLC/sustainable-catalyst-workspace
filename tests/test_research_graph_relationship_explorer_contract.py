...
class V047(unittest.TestCase):
 ...
 def test_02_schema_stable(self): self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(35,'sc-workspace-project/20.0','sc-workspace-project-export/20.0'));self.assertFalse(MAN['schema_migration_required']);self.assertFalse(MAN['migration']['relationship_explorer_only_release']);self.assertTrue(MAN['migration']['research_templates_only_release'])
 ...
 def test_12_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.49.0','0.48.0'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0490'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0461',REGPHP)
