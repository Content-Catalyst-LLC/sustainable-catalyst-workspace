...
MAN=json.loads((ROOT/'release-manifest-v0.38.0.json').read_text());PHP=(ROOT/'wordpress/sustainable-catalyst-workspace/includes/class-sc-workspace.php').read_text();JS=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.38.0.js').read_text();NB=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-research-notebook-v7.js').read_text();PORT=(ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-notebook-portability-v1.js').read_text()
class PortableSyncedNotebooks(unittest.TestCase):
 def test_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.38.0','0.37.0','Portable & Synced Notebooks'))
 def test_migration(self): self.assertEqual((MAN['storage_schema_version'],MAN['project_schema'],MAN['export_schema']),(34,'sc-workspace-project/19.0','sc-workspace-project-export/19.0'));self.assertEqual((MAN['migration']['storage_from'],MAN['migration']['storage_to']),(33,34));self.assertIn('function migrateV33(raw)',JS);self.assertIn('if (raw.schemaVersion === 33) return migrateV33(raw);',JS)
 def test_notebook_v7(self): self.assertEqual(MAN['notebook_workspace_schema'],'sc-workspace-notebook-workspace/7.0');self.assertEqual(MAN['notebook_export_schema'],'sc-workspace-notebook-export/7.0');self.assertIn("WORKSPACE_SCHEMA='sc-workspace-notebook-workspace/7.0'",NB);self.assertIn('portabilityState',NB)
