...
class T(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.60.0','0.59.1','Public Product Beta II'));self.assertIn('Version: 0.69.0',MAIN)
 ...
 def test_08_ui(self): self.assertIn('data-scw-public-beta-ii',PHP);self.assertIn('Run beta gate',PHP);self.assertIn("expectedVersion:'0.69.0'",UI)
 ...
 def test_12_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.60.0','0.59.1'));self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0690'",REGPHP);self.assertIn('LEGACY_PENDING_KEY_V0600',REGPHP)
