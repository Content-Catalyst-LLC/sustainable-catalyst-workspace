...
class ProductMaturity(unittest.TestCase):
 ...
 def test_06_wordpress(self): self.assertIn('Version: 2.0.3',MAIN);self.assertIn("'/product-maturity-contract'",PHP);self.assertIn('data-scw-product-maturity',PHP);self.assertIn('data-release-stage="root-scope-cockpit-recovery"',PHP)
 def test_07_deployment(self): self.assertIn("PREVIOUS_RELEASE = '2.0.2'",DEP);self.assertIn('class-sc-workspace-product-maturity.php',DEP);self.assertIn('workspace-v2.0.3.js',DEP)
