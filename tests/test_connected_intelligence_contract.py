...
class ConnectedIntelligence(unittest.TestCase):
 ...
 def test_06_wordpress(self): self.assertIn('Version: 1.13.0',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '1.13.0'",MAIN);self.assertIn("'/connected-intelligence-contract'",PHP);self.assertIn('Connected Intelligence Workspace',PHP);self.assertIn('data-release-stage="connected-intelligence"',PHP)
 ...
 def test_08_deployment(self): self.assertIn("PREVIOUS_RELEASE = '1.12.0'",DEP);self.assertIn("ROLLBACK_RELEASE = '1.12.0'",DEP);self.assertIn('workspace-v1.13.0.js',DEP);self.assertIn('class-sc-workspace-connected-intelligence.php',DEP)
