...
class ConnectedIntelligence(unittest.TestCase):
 ...
 def test_06_wordpress(self): self.assertIn('Version: 2.0.3',MAIN);self.assertIn("SC_WORKSPACE_VERSION', '2.0.3'",MAIN);self.assertIn("'/connected-intelligence-contract'",PHP);self.assertIn('Connected Intelligence Workspace',PHP);self.assertIn('data-release-stage="root-scope-cockpit-recovery"',PHP)
 ...
 def test_08_deployment(self): self.assertIn("PREVIOUS_RELEASE = '2.0.2'",DEP);self.assertIn("ROLLBACK_RELEASE = '2.0.2'",DEP);self.assertIn('workspace-v2.0.3.js',DEP);self.assertIn('class-sc-workspace-connected-intelligence.php',DEP)
