...
JS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.29.0.js';PHP=ROOT/'wordpress/sustainable-catalyst-workspace/includes/class-sc-workspace.php';CSS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.29.0.css';MANIFEST=ROOT/'release-manifest-v0.29.0.json'
class CollaborationFoundationContract(unittest.TestCase):
 def test_manifest(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.29.0');self.assertEqual(m['previous_version'],'0.28.0');self.assertEqual(m['storage_schema_version'],27);self.assertEqual(m['project_schema'],'sc-workspace-project/12.0');self.assertEqual(m['collaboration_schema'],'sc-workspace-collaboration/1.0')
 ...
 def test_storage_migration_only(self):
  j=JS.read_text();self.assertIn('const STORAGE_VERSION = 27',j);self.assertIn('function migrateV18(raw)',j);self.assertIn('if (raw.schemaVersion === 18) return migrateV18(raw)',j);self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/12.0'",j)
