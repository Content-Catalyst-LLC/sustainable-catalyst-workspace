...
class CollaborationFoundationContract(unittest.TestCase):
 def test_manifest(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.33.0');self.assertEqual(m['previous_version'],'0.32.0');self.assertEqual(m['storage_schema_version'],29);self.assertEqual(m['project_schema'],'sc-workspace-project/14.0');self.assertEqual(m['collaboration_schema'],'sc-workspace-collaboration/1.0')
 ...
 def test_storage_migration_only(self):
  j=JS.read_text();self.assertIn('const STORAGE_VERSION = 29',j);self.assertIn('function migrateV18(raw)',j);self.assertIn('if (raw.schemaVersion === 18) return migrateV18(raw)',j);self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/14.0'",j)
