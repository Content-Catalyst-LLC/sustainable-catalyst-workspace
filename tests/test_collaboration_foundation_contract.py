...
JS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/js/workspace-v0.40.0.js';PHP=ROOT/'wordpress/sustainable-catalyst-workspace/includes/class-sc-workspace.php';CSS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.40.0.css';MANIFEST=ROOT/'release-manifest-v0.40.0.json'
class CollaborationFoundationContract(unittest.TestCase):
 def test_manifest(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.40.0');self.assertEqual(m['previous_version'],'0.39.0');self.assertEqual(m['storage_schema_version'],35);self.assertEqual(m['project_schema'],'sc-workspace-project/20.0');self.assertEqual(m['collaboration_schema'],'sc-workspace-collaboration/1.0')
