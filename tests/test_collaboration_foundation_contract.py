...
class CollaborationFoundationContract(unittest.TestCase):
 def test_manifest(self):
  m=json.loads(MANIFEST.read_text());self.assertEqual(m['version'],'0.18.0');self.assertEqual(m['previous_version'],'0.17.0');self.assertEqual(m['storage_schema_version'],19);self.assertEqual(m['project_schema'],'sc-workspace-project/11.0');self.assertEqual(m['collaboration_schema'],'sc-workspace-collaboration/1.0')
 ...
 def test_storage_migration_only(self):
  j=JS.read_text();self.assertIn('const STORAGE_VERSION = 19',j);self.assertIn('function migrateV18(raw)',j);self.assertIn('if (raw.schemaVersion === 18) return migrateV18(raw)',j);self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/11.0'",j)
