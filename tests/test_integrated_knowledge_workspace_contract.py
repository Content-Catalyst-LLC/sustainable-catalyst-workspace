...
class IntegratedKnowledgeWorkspace(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.40.0','0.39.0','Integrated Knowledge Workspace')); self.assertIn('Version: 0.40.0',MAIN)
 ...
 def test_09_registry(self): self.assertEqual((REG['public_version'],REG['previous_version']),('0.40.0','0.39.0')); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0400'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0390',REGPHP)
 def test_10_history(self): self.assertTrue((ROOT/'history/release-manifest-v0.39.0.json').exists()); self.assertTrue((ROOT/'history/workspace-product-record-v0.39.0.json').exists())
