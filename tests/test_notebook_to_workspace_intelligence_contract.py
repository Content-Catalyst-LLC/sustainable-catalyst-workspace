...
class NotebookToWorkspaceIntelligenceContract(unittest.TestCase):
    def test_01_release_lineage(self):
        self.assertEqual((MAN['version'], MAN['previous_version'], MAN['release_name']),
                         ('0.35.0', '0.34.0', 'Notebook-to-Workspace Intelligence'))
    ...
    def test_02_storage_and_project_migration(self):
        self.assertEqual((MAN['storage_schema_version'], MAN['project_schema'], MAN['export_schema']),
                         (31, 'sc-workspace-project/16.0', 'sc-workspace-project-export/16.0'))
        ...
        self.assertEqual((MAN['migration']['project_schema_from'], MAN['migration']['project_schema_to']),
                         ('sc-workspace-project/15.0', 'sc-workspace-project/16.0'))
        self.assertIn('function migrateV30(raw)', JS)
        self.assertIn('if (raw.schemaVersion === 30) return migrateV30(raw);', JS)
    ...
    def test_06_helper_promotion_ledger(self):
        for token in (
            ...
            'promotions,governance:{automaticPromotion:false',
        ):
    ...
    def test_10_project_and_account_contracts_advance(self):
        self.assertIn("'schema' => 'sc-workspace-project-contract/16.0'", PHP)
        self.assertIn("'export_schema' => 'sc-workspace-project-export/16.0'", PHP)
        self.assertIn("array('sc-workspace-project/16.0','sc-workspace-project/15.0'", PHP)
