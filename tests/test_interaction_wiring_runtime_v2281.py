...
JS=(WP/'assets/js/workspace-v3.0.0.js').read_text()
COMPAT=(WP/'assets/js/sc-workspace-local-project-compat-v3000.js').read_text()
...
def test_release_identity_aligned():
    ...
    assert "const WORKSPACE_RELEASE = '3.0.0';" in JS
    assert 'workspace-v3.0.0.js' in PHP
    assert 'workspace-v3.0.0.css' in PHP
