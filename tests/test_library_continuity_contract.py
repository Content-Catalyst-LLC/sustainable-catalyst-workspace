...
MAN=json.loads((R/'release-manifest-v1.3.0.json').read_text());OLD=json.loads((R/'history/release-manifest-v1.2.0.json').read_text());MAIN=(P/'sustainable-catalyst-workspace.php').read_text();PHP=(P/'includes/class-sc-workspace.php').read_text();JS=(P/'assets/js/sc-workspace-library-continuity-v1.js').read_text()
class LibraryContinuity(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version']),('1.3.0','1.2.0'));self.assertIn('Version: 1.3.0',MAIN)
