...
if (!defined('SC_WORKSPACE_VERSION')) define('SC_WORKSPACE_VERSION', '1.10.0');
...
if(($s['workspace_version']??'')!=='1.10.0'){fwrite(STDERR,"state version mismatch\n");exit(1);}
...
if(empty($c['safe_bootstrap_guard'])||$c['rollback_release']!=='1.9.0'||!empty($c['schema_migration_required'])){fwrite(STDERR,"contract failed\n");exit(1);}
