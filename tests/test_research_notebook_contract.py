...
class ResearchNotebookContract(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.62.0','0.61.0','Product Hardening II: Persistence, Corruption & Recovery Integrity'))
 ...
 def test_16_registry_and_plugin(self): self.assertEqual(REG['public_version'],'0.62.0'); self.assertEqual(REG['previous_version'],'0.61.0'); self.assertIn('Product Hardening II',REG['change_summary']); self.assertIn('Version: 0.62.0',MAIN); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0620'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0320',REGPHP)
