...
class ResearchNotebookContract(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.59.1','0.59.0','Focused Application Shell & Route Isolation'))
 ...
 def test_16_registry_and_plugin(self): self.assertEqual(REG['public_version'],'0.59.1'); self.assertEqual(REG['previous_version'],'0.59.0'); self.assertIn('Focused Application Shell & Route Isolation',REG['change_summary']); self.assertIn('Version: 0.59.1',MAIN); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0591'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0320',REGPHP)
