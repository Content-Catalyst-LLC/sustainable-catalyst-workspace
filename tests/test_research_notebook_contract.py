...
class ResearchNotebookContract(unittest.TestCase):
 def test_01_lineage(self): self.assertEqual((MAN['version'],MAN['previous_version'],MAN['release_name']),('0.52.0','0.51.0','Research Tasks & Workflow State'))
 ...
 def test_16_registry_and_plugin(self): self.assertEqual(REG['public_version'],'0.52.0'); self.assertEqual(REG['previous_version'],'0.51.0'); self.assertIn('Research Tasks & Workflow State',REG['change_summary']); self.assertIn('Version: 0.52.0',MAIN); self.assertIn("BACKUP_KEY = 'sc_workspace_registry_backup_v0520'",REGPHP); self.assertIn('LEGACY_PENDING_KEY_V0320',REGPHP)
