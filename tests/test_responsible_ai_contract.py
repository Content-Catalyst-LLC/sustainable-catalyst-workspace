...
CSS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.14.0.css'
SCHEMA=ROOT/'schemas/sc-workspace-ai-assistance-v1.schema.json'
PROJECT=ROOT/'schemas/sc-workspace-project-v11.schema.json'
MANIFEST=ROOT/'release-manifest-v0.14.0.json'
...
class ResponsibleAIContractTests(unittest.TestCase):
  ...
  def test_migration(self):
    j=JS.read_text(); self.assertIn('const STORAGE_VERSION = 15',j); self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/11.0'",j); self.assertIn('function migrateV13(raw)',j); self.assertIn('Project upgraded to Responsible AI Assistance',j)
