...
CSS=ROOT/'wordpress/sustainable-catalyst-workspace/assets/css/workspace-v0.13.0.css'
...
class ResponsibleAIContractTests(unittest.TestCase):
  def test_release_boundary(self):
    m=json.loads(MANIFEST.read_text()); self.assertEqual(m['version'],'0.13.0'); self.assertEqual(m['previous_version'],'0.12.0'); self.assertEqual(m['release_name'],'Responsible AI Assistance'); self.assertEqual(m['storage_schema_version'],14); self.assertEqual(m['project_schema'],'sc-workspace-project/11.0')
  ...
  def test_migration(self):
    j=JS.read_text(); self.assertIn('const STORAGE_VERSION = 14',j); self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/11.0'",j); self.assertIn('function migrateV13(raw)',j); self.assertIn('Project upgraded to Responsible AI Assistance',j)
  ...
  def test_v10_project_export_import_compatibility(self):
    j=JS.read_text(); self.assertIn("const EXPORT_SCHEMA = 'sc-workspace-project-export/11.0'",j); self.assertIn("LEGACY_EXPORT_SCHEMA_V10",j); self.assertIn('payload.schema === LEGACY_EXPORT_SCHEMA_V10',j)
