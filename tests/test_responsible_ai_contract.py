...
class ResponsibleAIContractTests(unittest.TestCase):
  ...
  def test_migration(self):
    j=JS.read_text(); self.assertIn('const STORAGE_VERSION = 30',j); self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/15.0'",j); self.assertIn('function migrateV13(raw)',j); self.assertIn('Project upgraded to Responsible AI Assistance',j)
  ...
  def test_v10_project_export_import_compatibility(self):
    j=JS.read_text(); self.assertIn("const EXPORT_SCHEMA = 'sc-workspace-project-export/15.0'",j); self.assertIn("LEGACY_EXPORT_SCHEMA_V10",j); self.assertIn('payload.schema === LEGACY_EXPORT_SCHEMA_V10',j)
