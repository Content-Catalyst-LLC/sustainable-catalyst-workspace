...
class ResponsibleAIContractTests(unittest.TestCase):
  def test_release_boundary(self):
    m=json.loads(MANIFEST.read_text()); self.assertEqual(m['version'],'0.36.0'); self.assertEqual(m['previous_version'],'0.35.0'); self.assertEqual(m['storage_schema_version'],32); self.assertEqual(m['project_schema'],'sc-workspace-project/17.0'); self.assertEqual(m['ai_assistance_schema'],'sc-workspace-ai-assistance/1.0'); self.assertIn('responsible_ai_assistance',m['capabilities'])
  ...
  def test_migration(self):
    j=JS.read_text(); self.assertIn('const STORAGE_VERSION = 32',j); self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/17.0'",j); self.assertIn('function migrateV13(raw)',j); self.assertIn('Project upgraded to Responsible AI Assistance',j)
  ...
  def test_v10_project_export_import_compatibility(self):
    j=JS.read_text(); self.assertIn("const EXPORT_SCHEMA = 'sc-workspace-project-export/17.0'",j); self.assertIn("LEGACY_EXPORT_SCHEMA_V10",j); self.assertIn('payload.schema === LEGACY_EXPORT_SCHEMA_V10',j)
