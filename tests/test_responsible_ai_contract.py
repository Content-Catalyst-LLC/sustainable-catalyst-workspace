...
class ResponsibleAIContractTests(unittest.TestCase):
  ...
  def test_migration(self):
    j=JS.read_text(); self.assertIn('const STORAGE_VERSION = 20',j); self.assertIn("const PROJECT_SCHEMA = 'sc-workspace-project/11.0'",j); self.assertIn('function migrateV13(raw)',j); self.assertIn('Project upgraded to Responsible AI Assistance',j)
