...
const result={schema:A.SCHEMA,summary:{ready:3,attention:0,manual:2,total:5},findings:[]};const report=A.report('0.64.1',result);assert.strictEqual(report.workspaceVersion,'0.64.1');assert.strictEqual(report.privacy.projectContentIncluded,false);assert.strictEqual(report.privacy.rawUserAgentIncluded,false);assert.strictEqual(report.governance.automatedCertification,false);
