...
def check(x,l):
    if not x: raise SystemExit('FAIL - inherited v0.82.1 Installer & Validation Lineage Repair gate: '+l)
check(validate_current_release_lineage(R)['ok'],'current release lineage')
MAN=load_manifest(R,'0.82.1'); OLD=load_manifest(R,'0.82.0'); REG=load_registry(R,'0.82.1')
check((MAN['version'],MAN['previous_version'],MAN['release_name'])==('0.82.1','0.82.0','Production Certification Installer & Validation Lineage Repair'),'historical release lineage')
check((REG['public_version'],REG['previous_version'],REG['release_name'])==('0.82.1','0.82.0','Production Certification Installer & Validation Lineage Repair'),'historical registry lineage')
for k in ['storage_schema_version','project_schema','export_schema','object_schema','research_schema']:
    check(MAN[k]==OLD[k],'historical frozen '+k)
check(MAN['object_types']==OLD['object_types'],'historical frozen object types')
check(MAN['rest_routes']==OLD['rest_routes'],'historical no REST changes')
check(MAN['schema_migration_required'] is False,'historical no schema migration')
...
check(repair['rollback_release']=='0.81.0','historical rollback release')
...
for f in ['scripts/release_lineage.py','scripts/verify_release_lineage.py','docs/PRODUCTION_CERTIFICATION_INSTALLER_VALIDATION_LINEAGE_REPAIR_V0821.md','RELEASE_NOTES_0.82.1.md']:
    check((R/f).exists(),f)
...
print(f'PASS - inherited v0.82.1 installer/validation lineage repair gate under current v{CUR.version}')
