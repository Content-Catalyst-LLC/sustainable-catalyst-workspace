...
ts="export const SCW_TYPED_CONTRACT_VERSION = '3.18.0';\nexport const SCW_TYPED_ENDPOINTS = "+json.dumps(eps,indent=2)+" as const;\nexport type SCWTypedEndpointKey = keyof typeof SCW_TYPED_ENDPOINTS;\n"
js="window.SCW_TYPED_CONTRACT_VERSION='3.18.0';\nwindow.SCW_TYPED_ENDPOINTS="+json.dumps(eps,separators=(',',':'))+";\n"
if a.check:
 assert (root/'frontend/typed-client/src/00-generated-contracts.ts').read_text()==ts
 assert (root/'wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-typed-client-v31800.js').read_text()==js
 print('PASS: Workspace v3.18.0 typed client contract check endpoints=%d'%len(eps))
else:
 (root/'frontend/typed-client/src/00-generated-contracts.ts').write_text(ts);(root/'wordpress/sustainable-catalyst-workspace/assets/js/sc-workspace-typed-client-v31800.js').write_text(js);print('PASS: Workspace v3.18.0 typed client generated endpoints=%d'%len(eps))
