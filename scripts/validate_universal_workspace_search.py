...
check(('Version: '+CUR.version) in MAIN and ("SC_WORKSPACE_VERSION', '"+CUR.version+"'") in MAIN and 'class-sc-workspace-universal-search.php' in MAIN,'WordPress bootstrap identity')
...
check(CUR.script_name in PHP and CUR.style_name in PHP and CUR.asset_handle in PHP and 'sc-workspace-universal-search-v1' in PHP,'current cumulative assets and Universal Search preserved')
...
check((P/'assets/js/workspace-v1.2.0.js').exists() and (P/'assets/css/workspace-v1.2.0.css').exists(),'v1.2 historical cumulative assets preserved')
...
print(f'PASS - v1.2.0 Universal Workspace Search & Knowledge Retrieval source gate under current v{CUR.version}')
