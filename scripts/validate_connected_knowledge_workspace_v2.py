...
R=Path(__file__).resolve().parents[1];P=R/'wordpress/sustainable-catalyst-workspace';MAN=load_manifest(R,'2.0.0');OLD=load_manifest(R,'1.15.0');REG=load_registry(R,'2.0.0');MAIN=(P/'sustainable-catalyst-workspace.php').read_text();PHP=(P/'includes/class-sc-workspace.php').read_text();DEP=(P/'includes/class-sc-workspace-deployment.php').read_text();PROD=(P/'includes/class-sc-workspace-production-certification.php').read_text();JS=(P/'assets/js/sc-workspace-connected-knowledge-v2.js').read_text()
def check(x,l):
 if not x: raise SystemExit('FAIL - v2.0.0 Connected Knowledge gate: '+l)
lineage=validate_current_release_lineage(R,'2.0.0','1.15.0');check(lineage['ok'],'lineage: '+('; '.join(lineage['errors']) if lineage['errors'] else 'unknown'))
check((MAN['version'],MAN['previous_version'],MAN['release_name'])==('2.0.0','1.15.0','Connected Knowledge Workspace'),'release identity')
check((REG['public_version'],REG['previous_version'])==('2.0.0','1.15.0'),'registry identity')
...
check('Version: 2.0.0' in MAIN and "SC_WORKSPACE_VERSION', '2.0.0'" in MAIN,'WordPress identity')
check("PREVIOUS_RELEASE = '1.15.0'" in DEP and "ROLLBACK_RELEASE = '1.15.0'" in DEP and "PREVIOUS_RELEASE = '1.15.0'" in PROD,'rollback lineage')
for f in ['docs/CONNECTED_KNOWLEDGE_WORKSPACE_V200.md','RELEASE_NOTES_2.0.0.md','schemas/sc-workspace-connected-knowledge-workspace-v2.schema.json','schemas/sc-workspace-connected-knowledge-context-v2.schema.json','schemas/sc-workspace-knowledge-object-reference-v2.schema.json','schemas/sc-workspace-knowledge-route-v2.schema.json','schemas/sc-workspace-v1-compatibility-registry-v2.schema.json','schemas/sc-workspace-connected-knowledge-receipt-v2.schema.json','tests/test_connected_knowledge_workspace_v2_contract.py','tests/test_connected_knowledge_workspace_v2_runtime.js','tests/test_connected_knowledge_workspace_v2_runtime.php']:check((R/f).exists(),f)
...
print('PASS - v2.0.0 Connected Knowledge Workspace source gate')
