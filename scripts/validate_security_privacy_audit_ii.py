...
current_shell=(PLUGIN/'assets/js/workspace-v0.81.0.js').read_text(errors='ignore')
...
js=[p for p in assets.glob('sc-workspace-*.js')]+[assets/'workspace-v0.81.0.js']
for p in js:
...
check(len(fetches)==1 and fetches[0][0]=='workspace-v0.81.0.js','unexpected fetch-bearing current asset')
current=fetches[0][1]
...
check('Version: 0.81.0' in MAIN,'plugin version mismatch')
