...
def check(x,l):
    if not x: raise SystemExit('FAIL - historical v0.83.0 Live Production Certification & Release Sign-Off gate: '+l)
lineage=validate_current_release_lineage(R)
check(lineage['ok'],'current release lineage while preserving v0.83 evidence: '+('; '.join(lineage['errors']) if lineage['errors'] else 'unknown'))
check((MAN['version'],MAN['previous_version'],MAN['release_name'])==('0.83.0','0.82.1','Live Production Certification & Release Sign-Off'),'historical release lineage')
check((REG['public_version'],REG['previous_version'],REG['release_name'])==('0.83.0','0.82.1','Live Production Certification & Release Sign-Off'),'historical registry lineage')
...
check(s['rollback_release']=='0.82.1','historical rollback release')
check("SIGNOFF_RELEASE = '0.83.0'" in SIGN and "PREVIOUS_RELEASE = '0.82.1'" in SIGN and "ROLLBACK_RELEASE = '0.82.1'" in SIGN,'signoff evidence identity')
...
check('/* v0.83.0 — Live Production Certification & Release Sign-Off */' in CSS,'signoff CSS layer preserved')
...
print('PASS - historical v0.83.0 Live Production Certification & Release Sign-Off source gate')
