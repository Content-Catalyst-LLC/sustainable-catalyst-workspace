...
lineage=validate_current_release_lineage(R,'1.15.0','1.14.0');check(lineage['ok'],'lineage: '+('; '.join(lineage['errors']) if lineage['errors'] else 'unknown'))
...
check('Version: 1.15.0' in MAIN and "SC_WORKSPACE_VERSION', '1.15.0'" in MAIN,'WordPress identity')
check("'/product-maturity-contract'" in PHP and 'Product Maturity &amp; 2.0 Candidate' in PHP and 'data-release-stage="product-maturity"' in PHP,'surface')
check("PREVIOUS_RELEASE = '1.14.0'" in DEP and "ROLLBACK_RELEASE = '1.14.0'" in DEP and "PREVIOUS_RELEASE = '1.14.0'" in PROD,'predecessor')
