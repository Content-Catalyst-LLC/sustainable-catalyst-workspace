...
MAN=load_manifest(R,'2.0.1'); OLD=load_manifest(R,'2.0.0'); REG=load_registry(R,'2.0.1'); CURRENT=load_manifest(R,'2.0.4')
...
lineage=validate_current_release_lineage(R,'2.0.4','2.0.3'); check(lineage['ok'],'current lineage: '+str(lineage['errors']))
...
check(CURRENT['version']=='2.0.4' and CURRENT['previous_version']=='2.0.3','current patch lineage')
check('Version: 2.0.4' in MAIN and "SC_WORKSPACE_VERSION', '2.0.4'" in MAIN,'current WordPress identity')
...
for f in ['docs/BUTTON_SYSTEM_CONTROL_ALIGNMENT_INTERACTION_STATE_REPAIR_V201.md','RELEASE_NOTES_2.0.1.md','tests/test_button_system_control_alignment_contract.py','tests/test_button_system_control_alignment_runtime.php']:
    check((R/f).exists(),f)
print('PASS - historical v2.0.1 Button System source gate preserved under v2.0.4')
