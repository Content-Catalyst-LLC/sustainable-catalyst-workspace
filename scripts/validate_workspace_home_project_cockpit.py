...
lineage=validate_current_release_lineage(R,'1.1.0','1.0.1');check(lineage['ok'],'current release lineage: '+('; '.join(lineage['errors']) if lineage['errors'] else 'unknown'))
...
check('workspace-v1.1.0.js' in PHP and 'workspace-v1.1.0.css' in PHP and "'sc-workspace-v110'" in PHP,'current assets')
check("PREVIOUS_RELEASE = '1.0.1'" in DEP and "ROLLBACK_RELEASE = '1.0.1'" in DEP and CUR.script_name in DEP and CUR.style_name in DEP,'deployment current predecessor/assets')
check("PREVIOUS_RELEASE = '1.0.1'" in PROD and "ROLLBACK_RELEASE = '1.0.1'" in PROD and CUR.script_name in PROD and CUR.style_name in PROD,'production current predecessor/assets')
check("BACKUP_KEY = 'sc_workspace_registry_backup_v110'" in REGPHP and 'LEGACY_PENDING_KEY_V101' in REGPHP,'registry retry lineage')
...
print('PASS - v1.1.0 Workspace Home, Project Cockpit & Navigation Refinement source gate')
