...
const download=(name,data,type='application/json')=>{const body=typeof data==='string'?data:JSON.stringify(data,null,2)+'\n',c=globalThis.SCWorkspaceBrowserCompatibility;if(c&&c.downloadText)return c.downloadText(name,body,type,window);const blob=new Blob([body],{type}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove();},0);};
function init(){
 ...
 exportJson.onclick=()=>{const p=active();if(!p)return;download(`workspace-readonly-api-${p.id}.json`,A.apiEnvelope(p,document.querySelector('[data-sc-workspace]')?.dataset.version||'0.57.0'));status.textContent='Static read-only API envelope exported.';};
