...
function init(){
 ...
 exportJson.onclick=()=>{const p=active();if(!p)return;download(`workspace-readonly-api-${p.id}.json`,A.apiEnvelope(p,document.querySelector('[data-sc-workspace]')?.dataset.version||'0.56.0'));status.textContent='Static read-only API envelope exported.';};
