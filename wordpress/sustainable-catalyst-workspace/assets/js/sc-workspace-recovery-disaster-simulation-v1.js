(function(root,factory){
  ...
  function runScenario(id,deps={}){
    ...
    if(id==='interrupted-write'){
      ...
      const staged=persistence.stage(st,{previousRaw:previous,nextRaw:next,workspaceVersion:'0.72.0',storageSchemaVersion:35});
      const rec=persistence.reconcileJournal(st,{workspaceVersion:'0.72.0',storageSchemaVersion:35});
    ...
    if(id==='storage-exhaustion'){
      ...
      const out=safeCall(()=>persistence.stage(st,{previousRaw:previous,nextRaw:'{}',workspaceVersion:'0.72.0',storageSchemaVersion:35}),e=>({ok:false,error:e.name||e.message}));
