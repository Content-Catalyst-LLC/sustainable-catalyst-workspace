(function(root,factory){
  ...
  const manualItems=Object.freeze([
    ...
    ['rest-identity','Production REST identity','Verify /wp-json/sc-workspace/v1/health and /production-certification-contract both report v0.83.0.'],
    ...
    ['cache-coherence','WordPress/CDN/browser cache coherence','Verify the HTML, current cumulative JS, and current cumulative CSS all identify v0.83.0. If they do not, purge only page/asset caches—never Workspace project storage.'],
    ['rollback','v0.82.1 rollback rehearsal','Replace v0.83.0 with the bundled v0.82.1 rollback plugin, verify the public shell and existing local project, then reinstall v0.83.0 and repeat the smoke.']
  ...
  function assess(workspaceRoot,options={}){
    ...
    checks.push(finding('query','Current cache-busting version',versionQueryMatches(script.url)&&versionQueryMatches(style.url),'Current cumulative asset version queries match v0.83.0, or an offline fixture omits query strings.'));
