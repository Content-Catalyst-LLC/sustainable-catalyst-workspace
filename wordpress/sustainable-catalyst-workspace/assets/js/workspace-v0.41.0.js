(() => {
  ...
  function init(root) {
    ...
    let integratedPrefs={query:'',kind:'all',project:'all',scope:'active'}; let selectedIntegratedKey='';
    ...
    function renderIntegratedKnowledge(){
      if(!integratedSection)return;const api=integratedApi();if(!api){if(integratedResults)integratedResults.innerHTML='<div class="scw-knowledge-empty-note">Integrated Knowledge helper unavailable.</div>';return;}
      const all=api.derive(state), filtered=api.filter(all,integratedPrefs), metrics=api.stats(all);
      if(integratedTotal)integratedTotal.textContent=String(metrics.total);if(integratedObjects)integratedObjects.textContent=String(metrics.objects);if(integratedNotebooks)integratedNotebooks.textContent=String(metrics.notebooks);if(integratedResearch)integratedResearch.textContent=String(metrics.research);
      if(integratedSearch&&document.activeElement!==integratedSearch)integratedSearch.value=integratedPrefs.query;if(integratedKind)integratedKind.value=integratedPrefs.kind;
      if(integratedProject){const current=integratedPrefs.project;integratedProject.innerHTML='<option value="all">All projects</option>';state.projects.slice().sort(projectSort).forEach(p=>{const o=document.createElement('option');o.value=p.id;o.textContent=`${p.title}${p.archivedAt?' (archived)':''}`;integratedProject.appendChild(o);});integratedProject.value=state.projects.some(p=>p.id===current)?current:'all';}
      if(integratedResults){integratedResults.innerHTML='';if(!filtered.length)integratedResults.innerHTML='<div class="scw-knowledge-empty-note">No canonical research records match these filters.</div>';filtered.slice(0,160).forEach(e=>{const card=document.createElement('article');card.className=`scw-integrated-result${selectedIntegratedKey===e.key?' is-selected':''}`;card.innerHTML=`<span>${escapeHtml(e.origin)} · ${escapeHtml(e.projectTitle)}</span><strong>${escapeHtml(e.title)}</strong><p>${escapeHtml(e.summary||'No summary yet.')}</p><small>${escapeHtml(e.kind.replaceAll('-',' '))}${e.subtype?` · ${escapeHtml(e.subtype)}`:''}</small>`;const acts=document.createElement('div');acts.className='scw-knowledge-actions';const inspect=document.createElement('button');inspect.type='button';inspect.className='scw-card-action';inspect.textContent='Inspect';inspect.addEventListener('click',()=>{selectedIntegratedKey=e.key;renderIntegratedKnowledge();});const open=document.createElement('button');open.type='button';open.className='scw-card-action';open.textContent='Open origin';open.addEventListener('click',()=>openIntegratedEntry(e));acts.append(inspect,open);card.appendChild(acts);integratedResults.appendChild(card);});}
      if(integratedDetail){integratedDetail.innerHTML='';const selected=all.find(e=>e.key===selectedIntegratedKey);if(!selected){integratedDetail.innerHTML='<div class="scw-knowledge-empty-note">Select a result to inspect its canonical origin and explicit connections.</div>';}else{const con=api.connections(selected,state);integratedDetail.innerHTML=`<span>CANONICAL ORIGIN</span><strong>${escapeHtml(selected.title)}</strong><p>${escapeHtml(selected.origin)} · ${escapeHtml(selected.projectTitle)}</p><small>${escapeHtml(selected.ref.schema)} · ${escapeHtml(selected.ref.kind)} · ${escapeHtml(selected.ref.id)}</small><div class="scw-integrated-connections"><span>EXPLICIT CONNECTIONS</span>${con.length?con.map(c=>`<em>${escapeHtml(c.relation)} · ${escapeHtml(c.source)}</em>`).join(''):'<em>No explicit connection records for this item.</em>'}</div>`;}}
    }
    ...
    if(integratedSearch)integratedSearch.addEventListener('input',()=>{integratedPrefs.query=integratedSearch.value.slice(0,240);renderIntegratedKnowledge();});
