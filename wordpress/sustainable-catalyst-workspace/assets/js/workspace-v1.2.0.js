...
if(researchTemplateImportFile)researchTemplateImportFile.addEventListener('change',async()=>{const file=researchTemplateImportFile.files?.[0],api=researchTemplateApi();if(!file||!api)return;try{const pkg=JSON.parse(await readFileTextCompat(file)),verified=api.verifyPackage(pkg);if(!verified.ok)throw new Error(verified.reason);let library=researchTemplateLibrary;for(const t of verified.library.customTemplates){const added=api.addCustom(library,{...t,id:id('rt')});if(added.ok)library=added.library;}researchTemplateLibrary=library;saveResearchTemplateLibrary();renderResearchTemplates(activeProject());if(researchTemplateStatus)researchTemplateStatus.textContent='Template package imported as new custom template copies.';}catch(_){if(researchTemplateStatus)researchTemplateStatus.textContent='Template import failed integrity or schema validation.';}researchTemplateImportFile.value='';});
    ...
    if(clearSearchButton)clearSearchButton.addEventListener('click',()=>{applyIntegratedPrefs({query:'',kind:'all',subtype:'all',project:'all',tag:'',origin:'all',provenance:'all',scope:'active',sort:'relevance'});if(searchStatus)searchStatus.textContent='Retrieval fields cleared.';});
