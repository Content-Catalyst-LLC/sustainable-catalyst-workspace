/* v2.35 browser-local compatibility runtime. Loaded lazily only when local project mode is required. */
(() => {
  'use strict';

  const INTERACTION_RUNTIME_VERSION = '2.30.0';
  const interactionRuntime = window.SCWorkspaceInteractionRuntime = window.SCWorkspaceInteractionRuntime || {
  ...
  interactionRuntime.version = INTERACTION_RUNTIME_VERSION;
  interactionRuntime.recordIssue = interactionRuntime.recordIssue || function(type, detail){
    this.issues.push({type:String(type||'runtime'), detail:String(detail||''), at:new Date().toISOString()});
    if(this.issues.length > 100) this.issues = this.issues.slice(-100);
  };
  if (!interactionRuntime._globalErrorBoundaryInstalled) {
    interactionRuntime._globalErrorBoundaryInstalled = true;
    window.addEventListener('error', function(event){
      interactionRuntime.unhandledErrors.push({type:'error', message:String(event.message||'Workspace client error'), at:new Date().toISOString()});
      if(interactionRuntime.unhandledErrors.length > 50) interactionRuntime.unhandledErrors = interactionRuntime.unhandledErrors.slice(-50);
    });
    window.addEventListener('unhandledrejection', function(event){
      ...
      interactionRuntime.unhandledErrors.push({type:'unhandledrejection', message:reason, at:new Date().toISOString()});
      if(interactionRuntime.unhandledErrors.length > 50) interactionRuntime.unhandledErrors = interactionRuntime.unhandledErrors.slice(-50);
    });
  }
  ...
  const LEGACY_PROJECT_SCHEMA_V19 = 'sc-workspace-project/19.0';
  const LEGACY_PROJECT_SCHEMA_V18 = 'sc-workspace-project/18.0';
  const LEGACY_PROJECT_SCHEMA_V17 = 'sc-workspace-project/17.0';
  const LEGACY_PROJECT_SCHEMA_V16 = 'sc-workspace-project/16.0';
  const LEGACY_PROJECT_SCHEMA_V15 = 'sc-workspace-project/15.0';
  ...
  const LEGACY_EXPORT_SCHEMA_V19 = 'sc-workspace-project-export/19.0';
  const LEGACY_EXPORT_SCHEMA_V18 = 'sc-workspace-project-export/18.0';
  const LEGACY_EXPORT_SCHEMA_V17 = 'sc-workspace-project-export/17.0';
  const LEGACY_EXPORT_SCHEMA_V16 = 'sc-workspace-project-export/16.0';
  const LEGACY_EXPORT_SCHEMA_V15 = 'sc-workspace-project-export/15.0';
  ...
  const NOTEBOOK_WORKSPACE_SCHEMA = 'sc-workspace-notebook-workspace/8.0';
  const NOTEBOOK_SCHEMA = 'sc-workspace-notebook/3.0';
  const NOTEBOOK_BLOCK_SCHEMA = 'sc-workspace-notebook-block/3.0';
  const NOTEBOOK_EXPORT_SCHEMA = 'sc-workspace-notebook-export/8.0';
  const NOTEBOOK_ASSISTANCE_SCHEMA = 'sc-workspace-notebook-assistance/1.0';
  const NOTEBOOK_ASSISTANCE_REQUEST_EXPORT_SCHEMA = 'sc-workspace-notebook-assistance-request-export/1.0';
  const NOTEBOOK_ASSISTANCE_RESPONSE_EXPORT_SCHEMA = 'sc-workspace-notebook-assistance-response-export/1.0';
  const NOTEBOOK_ASSISTANCE_REQUEST_KEY = 'sc_workspace_notebook_assistance_request_v1';
  const NOTEBOOK_ASSISTANCE_RESPONSE_KEY = 'sc_workspace_notebook_assistance_response_v1';
  ...
  const NOTEBOOK_PROMOTION_SCHEMA = 'sc-workspace-notebook-promotion/1.0';
  const NOTEBOOK_SYNTHESIS_SCHEMA = 'sc-workspace-notebook-synthesis/1.0';
  const NOTEBOOK_SYNTHESIS_EXPORT_SCHEMA = 'sc-workspace-notebook-synthesis-export/1.0';
  ...
  const NOTEBOOK_AUDIT_EVENT_SCHEMA = 'sc-workspace-notebook-audit-event/1.0';
  const NOTEBOOK_LINEAGE_SCHEMA = 'sc-workspace-notebook-lineage/1.0';
  ...
  const CONTINUITY_SCHEMA = 'sc-workspace-cross-device-continuity/1.0';
  const SYNC_OPERATION_SCHEMA = 'sc-workspace-sync-operation/1.0';
  const DEVICE_MIGRATION_SCHEMA = 'sc-workspace-device-migration/1.0';
  ...
  const RECONCILIATION_SCHEMA = 'sc-workspace-reconciliation/1.0';
  const RECONCILIATION_PLAN_SCHEMA = 'sc-workspace-reconciliation-plan/1.0';
  const MAX_RECONCILIATION_HISTORY = 80;
  const RECONCILIATION_RECEIPT_SCHEMA = 'sc-workspace-reconciliation-receipt/1.0';
  const RECONCILIATION_RECEIPT_EXPORT_SCHEMA = 'sc-workspace-reconciliation-receipt-export/1.0';
  const MAX_RECONCILIATION_RECEIPTS = 80;
  const PROJECT_LIFECYCLE_SCHEMA = 'sc-workspace-project-lifecycle/1.0';
  const GOVERNANCE_MILESTONE_SCHEMA = 'sc-workspace-governance-milestone/1.0';
  const LIFECYCLE_STAGES = new Set(['draft','evidence-ready','analysis-ready','decision-ready','review-ready','publication-ready','institutional-ready']);
  const MAX_LIFECYCLE_MILESTONES = 80;
  ...
  const MAX_SYNC_OPERATIONS = 80;
  const MAX_MIGRATION_HISTORY = 40;
  const SYNC_STATUS = new Set(['disabled','not-uploaded','up-to-date','local-ahead','remote-ahead','conflict','remote-missing','interrupted','error']);
  ...
  const INTERCHANGE_EXPORT_SCHEMA = 'sc-workspace-interchange/2.0';
  ...
  const INTEROP_FORMATS = new Set(['json','csv','tsv','markdown','html','text','workspace-json','obsidian-markdown','notion-csv','zotero-csl-json','portable-project']);
  ...
  const GRAPH_RELATIONS = new Set(['contains','sourced-from','same-source','evidence-from','uses','informs','supports','contrasts','extends','related','references','contradicts','derived-from','produced-by','supersedes','cites','cited-as','promoted-to','synthesized-into','supports-claim','captured-from']);
  ...
  function normalizeKnowledgeGraph(raw, projects=[]) {
    ...
    const projectIds=new Set(projects.map(p=>p.id));
    const allowedTypes=new Set(['all','project','provenance','library-record','notebook','notebook-block','research-question','research-claim','reference','synthesis','canvas-node',...OBJECT_TYPES]);
    base.preferences={
      query:String(pref.query||'').slice(0,240),
  ...
  function buildKnowledgeGraph() {
    const relationshipExplorer=(typeof window!=='undefined'&&(window.SCWorkspaceKnowledgeGraphExplorer||window.SCWorkspaceRelationshipExplorer))||null;
    if(relationshipExplorer&&typeof relationshipExplorer.build==='function'){
      return relationshipExplorer.build(state,{references:Array.isArray(referenceLibrary?.references)?referenceLibrary.references:[],crossProjectReferences:Array.isArray(crossProjectKnowledge?.references)?crossProjectKnowledge.references:[],libraryContinuityItems:Array.isArray(libraryContinuityItems)?libraryContinuityItems:[],maxNodes:MAX_GRAPH_NODES,maxEdges:MAX_GRAPH_EDGES});
    }
  ...
  function interchangeApi(){return window.SCWorkspaceInterchange||null;}
  function specializedInteropObjects(profile,text,fileName,fingerprint){const api=interchangeApi();if(!api||profile==='generic'||profile==='workspace-json')return null;const descriptors=api.importDescriptors(profile,text,fileName);const objects=descriptors.map(d=>{const obj=importObjectDraft(d.type,d.title,d.content,d.summary,fileName,fingerprint,[profile,...(Array.isArray(d.tags)?d.tags:[])]);if(d.sourceUrl)obj.provenance.sourceUrl=String(d.sourceUrl).slice(0,2000);obj._interchangeProfile=profile;obj._interchangeMeta=d.meta&&typeof d.meta==='object'?d.meta:{};return obj;});objects._profile=profile;objects._importReport=api.importReport(profile,fileName,fingerprint,descriptors);return objects;}
  function stageExternalContent(format,text,fileName,fingerprint){
    const stem=String(fileName||'Imported file').replace(/\.[^.]+$/,'');const api=interchangeApi();const profile=api?api.detectProfile(format,text,fileName):'generic';const specialized=specializedInteropObjects(profile,text,fileName,fingerprint);if(specialized&&specialized.length)return specialized;
    if(format==='csv'||format==='tsv'){const rows=csvRows(text,format==='tsv'?'\t':',');const headers=rows[0]||[];const summary=`Imported ${format.toUpperCase()} dataset · ${Math.max(0,rows.length-1)} data row(s) · ${headers.length} column(s)${headers.length?` · ${headers.slice(0,8).join(', ')}`:''}`;const out=[importObjectDraft('dataset',stem,text,summary,fileName,fingerprint,[format])];out._profile=profile;return out;}
    if(format==='markdown'){const out=[importObjectDraft('document',stem,text,'Imported Markdown document.',fileName,fingerprint,['markdown'])];out._profile=profile;return out;}
    if(format==='html'){const out=[importObjectDraft('document',stem,htmlToText(text),'Imported HTML document; text content preserved for local review.',fileName,fingerprint,['html'])];out._profile=profile;return out;}
    if(format==='text'){const out=[importObjectDraft('document',stem,text,'Imported plain-text document.',fileName,fingerprint,['text'])];out._profile=profile;return out;}
    if(format==='json'){
      let payload; try{payload=JSON.parse(text);}catch(_){throw new Error('The selected JSON file is not valid JSON.');}
      if(payload&&['sc-workspace-interchange/1.0','sc-workspace-interchange/2.0','sc-workspace-object-export/1.0'].includes(payload.schema)){
        const objs=payload.schema==='sc-workspace-object-export/1.0'?[payload.object]:stageInterchangePayload(payload,fileName,fingerprint);const out=payload.schema==='sc-workspace-object-export/1.0'?objs.map(raw=>{const n=normalizeObject(raw);n._sourceObjectId=String(raw&&raw.id||'').slice(0,160);n.id=id('scwo');n.status='draft';n.tags=normalizeTags(['imported',...(n.tags||[])]);n.provenance={sourceType:'imported',sourceTitle:fileName,sourceUrl:'',capturedAt:nowIso()};n._importFingerprint=fingerprint;return n;}):objs;out._profile=payload.schema==='sc-workspace-interchange/2.0'?'workspace-json':profile;return out;
      }
      if(payload&&payload.project&&Array.isArray(payload.project.objects)){const out=stageInterchangePayload({objects:payload.project.objects,relationships:payload.relationships||[]},fileName,fingerprint);out._profile=profile;return out;}
      const pretty=JSON.stringify(payload,null,2); const shape=Array.isArray(payload)?`${payload.length} top-level item(s)`:`${Object.keys(payload||{}).length} top-level field(s)`;const out=[importObjectDraft('dataset',stem,pretty,`Imported generic JSON dataset · ${shape}.`,fileName,fingerprint,['json'])];out._profile=profile;return out;
    }
    throw new Error('Unsupported import format.');
  }
  function interchangePackage(project){const api=interchangeApi();if(api)return api.workspaceJson(project,rootVersion());const objectIds=new Set(project.objects.filter(o=>!o.archivedAt).map(o=>o.id));return {schema:INTERCHANGE_EXPORT_SCHEMA,workspaceVersion:rootVersion(),exportedAt:nowIso(),project:{id:project.id,title:project.title,description:project.description},objects:project.objects.filter(o=>!o.archivedAt).map(o=>JSON.parse(JSON.stringify(o))),relationships:(project.traceability&&Array.isArray(project.traceability.lineage)?project.traceability.lineage:[]).filter(r=>objectIds.has(r.fromObjectId)&&objectIds.has(r.toObjectId)).map(r=>({relation:r.relation,fromObjectId:r.fromObjectId,toObjectId:r.toObjectId,note:r.note||''})),boundary:{portableCopy:true,canonicalObjectOverwrite:false,serverUpload:false,automaticTrustElevation:false}};}
  ...
  function importExportCompatibilityApi(){ return globalThis.SCWorkspaceImportExportCompatibility || null; }
  function continuityApi(){ return globalThis.SCWorkspaceCrossDeviceContinuity || null; }
  function crossDeviceProductionApi(){ return globalThis.SCWorkspaceCrossDeviceProduction || null; }
  function compatibilityApi(){ return globalThis.SCWorkspaceBrowserCompatibility || null; }
  async function readFileTextCompat(file){
    const api=compatibilityApi();
    if(api?.readFileText)return api.readFileText(file, window);
    if(file&&typeof file.text==='function')return String(await file.text());
    if(typeof FileReader==='function')return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result||''));reader.onerror=()=>reject(reader.error||new Error('Workspace could not read this file.'));reader.readAsText(file);});
    throw new Error('This browser does not provide a supported text-file reader.');
  }
  function downloadText(filename,text,type='text/plain'){const api=compatibilityApi();if(api?.downloadText){const out=api.downloadText(filename,text,type,window);if(out?.ok)return out;}const blob=new Blob([text],{type});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=filename;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);return {ok:true,mode:'legacy-object-url'};}
  ...
  function crossDeviceSyncTemplate() {
    return { schema: CROSS_DEVICE_SYNC_SCHEMA, continuitySchema: CONTINUITY_SCHEMA, enrollments: [], operations: [], migrations: [], history: [], updatedAt: nowIso() };
  }
  ...
  function normalizeCrossDeviceSync(raw, projects) {
    ...
    next.enrollments = (Array.isArray(value.enrollments) ? value.enrollments : []).map(item => ({
      ...
      lastOperationId: String(item?.lastOperationId || '').slice(0,160),
    ...
    next.operations=(api?.normalizeOperations ? api.normalizeOperations(value.operations) : (Array.isArray(value.operations)?value.operations:[])).filter(item=>item&&projectIds.has(String(item.projectId||''))).slice(0,MAX_SYNC_OPERATIONS);
    next.migrations=(Array.isArray(value.migrations)?value.migrations:[]).map(item=>({id:String(item?.id||id('sym')).slice(0,160),direction:['export','import'].includes(item?.direction)?item.direction:'import',sourceProjectId:String(item?.sourceProjectId||'').slice(0,160),outputProjectId:String(item?.outputProjectId||'').slice(0,160),projectFingerprint:String(item?.projectFingerprint||'').slice(0,128),at:validIso(item?.at)?item.at:nowIso()})).slice(0,MAX_MIGRATION_HISTORY);
    next.history = (Array.isArray(value.history) ? value.history : []).map(item => ({
      id: String(item?.id || id('syh')).slice(0,160),
      action: ['enable','disable','check','push','pull','conflict','remote-copy','resolve-local','resolve-cloud','remote-missing','interrupted','retry','migration-export','migration-import','backup-blocked'].includes(item?.action) ? item.action : 'check',
      projectId: String(item?.projectId || '').slice(0,160),
      projectTitle: String(item?.projectTitle || '').slice(0,200),
      revision: Math.max(0, Number(item?.revision) || 0),
      at: validIso(item?.at) ? item.at : nowIso(),
  ...
  function reconciliationTemplate() { return { schema: RECONCILIATION_SCHEMA, history: [], receipts: [], updatedAt: nowIso() }; }

  function normalizeReconciliation(raw, projects=[]) {
    const next=reconciliationTemplate(), value=raw&&typeof raw==='object'?raw:{}, projectIds=new Set(projects.map(project=>project.id));
    next.history=(Array.isArray(value.history)?value.history:[]).map(item=>{
      if(!item||typeof item!=='object')return null;
      return {schema:RECONCILIATION_SCHEMA,id:String(item.id||id('rch')).slice(0,160),receiptId:String(item.receiptId||'').slice(0,160),reviewerLabel:String(item.reviewerLabel||'').slice(0,160),sourceProjectId:String(item.sourceProjectId||'').slice(0,160),sourceProjectTitle:String(item.sourceProjectTitle||'Workspace project').slice(0,200),baseLabel:String(item.baseLabel||'Base state').slice(0,200),targetLabel:String(item.targetLabel||'Target state').slice(0,200),availableChangeCount:Math.max(0,Number(item.availableChangeCount)||0),selectedChangeCount:Math.max(0,Number(item.selectedChangeCount)||0),blockerCount:Math.max(0,Number(item.blockerCount)||0),outputProjectId:String(item.outputProjectId||'').slice(0,160),outputProjectTitle:String(item.outputProjectTitle||'').slice(0,200),at:validIso(item.at)?item.at:nowIso()};
    }).filter(Boolean).slice(0,MAX_RECONCILIATION_HISTORY);
    next.receipts=(Array.isArray(value.receipts)?value.receipts:[]).filter(item=>item&&item.schema===RECONCILIATION_RECEIPT_SCHEMA).map(item=>JSON.parse(JSON.stringify(item))).slice(0,MAX_RECONCILIATION_RECEIPTS);
    next.updatedAt=validIso(value.updatedAt)?value.updatedAt:nowIso();return next;
  }

  function notebookHelper() { return window.SCWorkspaceResearchNotebook || null; }

  function notebookWorkspaceTemplate() {
    const helper = notebookHelper();
    if (helper && typeof helper.workspace === 'function') return helper.workspace(null, id, nowIso);
    const stamp = nowIso();
    return { schema: NOTEBOOK_WORKSPACE_SCHEMA, notebooks: [], activeNotebookId: null, collections: [], activeCollectionId: null, links: [], promotions: [], syntheses: [], activeSynthesisId: null, assistances: [], activeAssistanceId: null, portability:{restorePoints:[],importHistory:[],syncEnrollments:[],updatedAt:stamp}, governance:{schema:NOTEBOOK_GOVERNANCE_SCHEMA,reviews:[],reconciliations:[],updatedAt:stamp}, createdAt: stamp, updatedAt: stamp };
  }

  function normalizeNotebooks(raw, objects = [], canvas = null) {
    const helper = notebookHelper();
    const next = helper && typeof helper.workspace === 'function' ? helper.workspace(raw, id, nowIso) : notebookWorkspaceTemplate();
    const objectIds = new Set(objects.map(object => object.id));
    const canvasNodeIds = new Set(Array.isArray(canvas?.nodes) ? canvas.nodes.map(node => node.id) : []);
    next.notebooks.forEach(notebook => notebook.sections.forEach(section => section.blocks.forEach(block => {
      if (block.sourceObjectId && !objectIds.has(block.sourceObjectId)) block.sourceObjectId = '';
      if (block.referenceObjectId && !objectIds.has(block.referenceObjectId)) block.referenceObjectId = '';
      if (block.promotion?.targetKind === 'object' && block.promotion.targetId && !objectIds.has(block.promotion.targetId)) {
        block.promotion = { status: 'none', targetKind: '', targetType: '', targetId: '', promotedAt: null };
      }
      if (block.promotion?.targetKind === 'canvas-node' && block.promotion.targetId && !canvasNodeIds.has(block.promotion.targetId)) {
        block.promotion = { status: 'none', targetKind: '', targetType: '', targetId: '', promotedAt: null };
      }
    })));
    if (helper?.refExists) {
    ...
    next.promotions = (Array.isArray(next.promotions) ? next.promotions : []).filter(item => {
      if (!helper?.refExists || !helper.refExists(item.source, next, objectIds)) return false;
      if (item.targetKind === 'object') return objectIds.has(item.targetId);
      if (item.targetKind === 'canvas-node') return canvasNodeIds.has(item.targetId);
      return item.targetKind === 'research-question' || item.targetKind === 'research-claim';
    });
    next.syntheses = (Array.isArray(next.syntheses) ? next.syntheses : []).map(item => ({...item, selectedRefs:(Array.isArray(item.selectedRefs)?item.selectedRefs:[]).filter(ref => helper?.refExists ? helper.refExists(ref, next, objectIds) : true)})).filter(item => item.selectedRefs.length);
    if (!next.syntheses.some(item => item.id === next.activeSynthesisId)) next.activeSynthesisId = next.syntheses[0]?.id || null;
    next.assistances = (Array.isArray(next.assistances) ? next.assistances : []).map(item => ({...item, selectedRefs:(Array.isArray(item.selectedRefs)?item.selectedRefs:[]).filter(ref => helper?.refExists ? helper.refExists(ref, next, objectIds) : true)})).filter(item => item.selectedRefs.length && String(item.question||'').trim());
    next.assistances.forEach(item => { if (item.acceptedDocumentObjectId && !objectIds.has(item.acceptedDocumentObjectId)) item.acceptedDocumentObjectId=''; });
    if (!next.assistances.some(item => item.id === next.activeAssistanceId)) next.activeAssistanceId = next.assistances[0]?.id || null;
    return next;
  }

  function touchNotebooks(project) {
    if (!project) return;
    project.notebooks = normalizeNotebooks(project.notebooks, project.objects, project.canvas);
    project.notebooks.updatedAt = nowIso();
    project.updatedAt = project.notebooks.updatedAt;
  }

  function activeNotebookFor(project) {
    if (!project) return null;
    project.notebooks = normalizeNotebooks(project.notebooks, project.objects, project.canvas);
    return project.notebooks.notebooks.find(notebook => notebook.id === project.notebooks.activeNotebookId) || project.notebooks.notebooks[0] || null;
  }

  function activeNotebookSectionFor(project, notebook = null) {
    const nb = notebook || activeNotebookFor(project);
    if (!nb) return null;
  ...
  function rootVersion(){ const el=document.querySelector('[data-sc-workspace]'); return el&&el.dataset.version?el.dataset.version:'0.54.0'; }
  ...
  function lifecycleTemplate() {
    return { schema: PROJECT_LIFECYCLE_SCHEMA, state: 'draft', milestones: [], updatedAt: nowIso() };
  }

  function normalizeLifecycleReadiness(raw) {
    const value=raw&&typeof raw==='object'?raw:{};
    return {
      schema: PROJECT_LIFECYCLE_SCHEMA,
      stage: LIFECYCLE_STAGES.has(value.stage)?value.stage:'draft',
      label: String(value.label||'').slice(0,120),
      generatedAt: validIso(value.generatedAt)?value.generatedAt:nowIso(),
      totalCount: Math.max(0,Number(value.totalCount)||0),
      metCount: Math.max(0,Number(value.metCount)||0),
      unmetCount: Math.max(0,Number(value.unmetCount)||0),
      ready: Boolean(value.ready),
      checks: (Array.isArray(value.checks)?value.checks:[]).map(c=>({id:String(c?.id||'').slice(0,120),label:String(c?.label||'').slice(0,240),met:Boolean(c?.met),detail:String(c?.detail||'').slice(0,1200),source:String(c?.source||'').slice(0,120),required:c?.required!==false})).filter(c=>c.id).slice(0,30),
      governance: { score:null, automaticAdvance:false, humanDeclarationRequired:true, readinessIsCertification:false }
    };
  }

  function normalizeLifecycle(raw) {
    const value=raw&&typeof raw==='object'?raw:{}, base=lifecycleTemplate();
    base.state=LIFECYCLE_STAGES.has(value.state)?value.state:'draft';
    base.milestones=(Array.isArray(value.milestones)?value.milestones:[]).map(item=>{
      if(!item||typeof item!=='object')return null;
      const milestone={schema:GOVERNANCE_MILESTONE_SCHEMA,id:String(item.id||id('glm')).slice(0,160),fromState:LIFECYCLE_STAGES.has(item.fromState)?item.fromState:'draft',toState:LIFECYCLE_STAGES.has(item.toState)?item.toState:'draft',rationale:String(item.rationale||'').trim().slice(0,4000),acknowledged:Boolean(item.acknowledged),readiness:normalizeLifecycleReadiness(item.readiness),declaredAt:validIso(item.declaredAt)?item.declaredAt:nowIso()};
      return milestone;
    }).filter(Boolean).slice(0,MAX_LIFECYCLE_MILESTONES);
    base.updatedAt=validIso(value.updatedAt)?value.updatedAt:base.updatedAt;
    return base;
  }

  function lifecycleAssessment(project,stage){
    const helper=window.SCWorkspaceProjectLifecycle;
    if(!helper||typeof helper.assess!=='function')return {stage,label:stage,checks:[],totalCount:0,metCount:0,unmetCount:0,ready:false,governance:{score:null,automaticAdvance:false,humanDeclarationRequired:true,readinessIsCertification:false}};
    return helper.assess(project,state,stage);
  }

  function lifecycleLabel(stage){
    const helper=window.SCWorkspaceProjectLifecycle;
    return helper&&typeof helper.labelFor==='function'?helper.labelFor(stage):String(stage||'draft').replaceAll('-',' ').replace(/\b\w/g,m=>m.toUpperCase());
  }

  function recordLifecycleMilestone(project,targetState,rationale,assessment){
    if(!project||!LIFECYCLE_STAGES.has(targetState))return null;
    project.lifecycle=normalizeLifecycle(project.lifecycle);
    const milestone={schema:GOVERNANCE_MILESTONE_SCHEMA,id:id('glm'),fromState:project.lifecycle.state,toState:targetState,rationale:String(rationale||'').trim().slice(0,4000),acknowledged:true,readiness:normalizeLifecycleReadiness(assessment),declaredAt:nowIso()};
    project.lifecycle.state=targetState;
    project.lifecycle.milestones.unshift(milestone);
    project.lifecycle.milestones=project.lifecycle.milestones.slice(0,MAX_LIFECYCLE_MILESTONES);
    project.lifecycle.updatedAt=milestone.declaredAt;
    project.updatedAt=milestone.declaredAt;
    addActivity(project,'lifecycle',`Lifecycle declared ${lifecycleLabel(targetState)}${assessment?.ready?'':' with unmet readiness conditions'}`);
    return milestone;
  }
  ...
  function projectTemplate(title, description = '') {
    ...
    const project = {
      ...
      notebooks: notebookWorkspaceTemplate()
    };
  ...
  function normalizeProject(raw) {
    ...
    return {
      ...
      notebooks: normalizeNotebooks(raw.notebooks, objects, canvas)
    };
  }
  ...
  function migrateV24(raw) {
    ...
    next.reconciliation=reconciliationTemplate();
    next.knowledge=normalizeKnowledge(raw.knowledge,next.projects);next.knowledgeGraph=normalizeKnowledgeGraph(raw.knowledgeGraph,next.projects);next.activityIntelligence=normalizeActivityIntelligence(raw.activityIntelligence,next.projects);next.interoperability=normalizeInteroperability(raw.interoperability);next.share=normalizeShare(raw.share);next.collaboration=normalizeCollaboration(raw.collaboration,next.projects);next.institutional=normalizeInstitutional(raw.institutional,next.projects);next.createdAt=validIso(raw.createdAt)?raw.createdAt:next.createdAt;next.updatedAt=nowIso();return next;
  }

  function migrateV25(raw) {
    const next=defaultState();
    next.projects=Array.isArray(raw.projects)?raw.projects.map(normalizeProject).filter(Boolean):[];
    next.recentTools=Array.isArray(raw.recentTools)?raw.recentTools.map(normalizeRecentTool).filter(Boolean).slice(0,MAX_RECENT_TOOLS):[];
    next.activeProjectId=next.projects.some(p=>p.id===raw.activeProjectId&&!p.archivedAt)?raw.activeProjectId:null;
  ...
  function migrateV26(raw) {
    const next=defaultState();
    next.projects=Array.isArray(raw.projects)?raw.projects.map(project=>{const normalized=normalizeProject(project);if(normalized&&project?.schema===LEGACY_PROJECT_SCHEMA_V11)addActivity(normalized,'migrated','Project upgraded to Governance Milestones & Project Lifecycle');return normalized;}).filter(Boolean):[];
    next.recentTools=Array.isArray(raw.recentTools)?raw.recentTools.map(normalizeRecentTool).filter(Boolean).slice(0,MAX_RECENT_TOOLS):[];
    next.activeProjectId=next.projects.some(p=>p.id===raw.activeProjectId&&!p.archivedAt)?raw.activeProjectId:null;
    next.identity=normalizeIdentity(raw.identity);
    next.accountPersistence=normalizeAccountPersistence(raw.accountPersistence,next.projects);
  ...
  function migrateV27(raw) {
    const next = defaultState();
    next.projects = Array.isArray(raw.projects) ? raw.projects.map(project => {
      const normalized = normalizeProject(project);
      if (normalized && project?.schema === LEGACY_PROJECT_SCHEMA_V12) addActivity(normalized, 'migrated', 'Project upgraded to Research Notebook Foundation');
      return normalized;
    }).filter(Boolean) : [];
    next.recentTools = Array.isArray(raw.recentTools) ? raw.recentTools.map(normalizeRecentTool).filter(Boolean).slice(0, MAX_RECENT_TOOLS) : [];
    next.activeProjectId = next.projects.some(p => p.id === raw.activeProjectId && !p.archivedAt) ? raw.activeProjectId : null;
  ...
  function migrateV30(raw) {
    const next = defaultState();
    next.projects = Array.isArray(raw.projects) ? raw.projects.map(project => {
      const normalized = normalizeProject(project);
      if (normalized && project?.schema === LEGACY_PROJECT_SCHEMA_V15) addActivity(normalized, 'migrated', 'Project upgraded to Notebook-to-Workspace Intelligence');
      return normalized;
    }).filter(Boolean) : [];
    next.recentTools = Array.isArray(raw.recentTools) ? raw.recentTools.map(normalizeRecentTool).filter(Boolean).slice(0, MAX_RECENT_TOOLS) : [];
    next.activeProjectId = next.projects.some(p => p.id === raw.activeProjectId && !p.archivedAt) ? raw.activeProjectId : null;
  ...
  function migrateV31(raw) {
    const next = defaultState();
    next.projects = Array.isArray(raw.projects) ? raw.projects.map(project => {
      const normalized = normalizeProject(project);
      if (normalized && project?.schema === LEGACY_PROJECT_SCHEMA_V16) addActivity(normalized, 'migrated', 'Project upgraded to Notebook Synthesis & Citation Workspace');
      return normalized;
    }).filter(Boolean) : [];
    next.recentTools = Array.isArray(raw.recentTools) ? raw.recentTools.map(normalizeRecentTool).filter(Boolean).slice(0, MAX_RECENT_TOOLS) : [];
    next.activeProjectId = next.projects.some(p => p.id === raw.activeProjectId && !p.archivedAt) ? raw.activeProjectId : null;
    ...
    next.activityIntelligence = normalizeActivityIntelligence(raw.activityIntelligence, next.projects);
    next.interoperability = normalizeInteroperability(raw.interoperability);
    next.share = normalizeShare(raw.share, next.projects);
    next.collaboration = normalizeCollaboration(raw.collaboration, next.projects);
    next.institutional = normalizeInstitutional(raw.institutional, next.projects);
    next.sourceCapture = normalizeSourceCapture(raw.sourceCapture);
    next.createdAt = validIso(raw.createdAt) ? raw.createdAt : next.createdAt;
  ...
  function migrateV32(raw) {
    const next = defaultState();
    next.projects = Array.isArray(raw.projects) ? raw.projects.map(project => {
      const normalized = normalizeProject(project);
      if (normalized && project?.schema === LEGACY_PROJECT_SCHEMA_V17) addActivity(normalized, 'migrated', 'Project upgraded to Grounded Notebook Assistance');
      return normalized;
    }).filter(Boolean) : [];
    next.recentTools = Array.isArray(raw.recentTools) ? raw.recentTools.map(normalizeRecentTool).filter(Boolean).slice(0, MAX_RECENT_TOOLS) : [];
    next.activeProjectId = next.projects.some(p => p.id === raw.activeProjectId && !p.archivedAt) ? raw.activeProjectId : null;
  ...
  function migrateV33(raw) {
    const next = defaultState();
    next.projects = Array.isArray(raw.projects) ? raw.projects.map(project => {
      const normalized = normalizeProject(project);
      if (normalized && project?.schema === LEGACY_PROJECT_SCHEMA_V18) addActivity(normalized, 'migrated', 'Project upgraded to Portable & Synced Notebooks');
      return normalized;
    }).filter(Boolean) : [];
    next.recentTools = Array.isArray(raw.recentTools) ? raw.recentTools.map(normalizeRecentTool).filter(Boolean).slice(0, MAX_RECENT_TOOLS) : [];
    next.activeProjectId = next.projects.some(p => p.id === raw.activeProjectId && !p.archivedAt) ? raw.activeProjectId : null;
  ...
  function migrateV34(raw) {
    const next = defaultState();
    next.projects = Array.isArray(raw.projects) ? raw.projects.map(project => {
      const normalized = normalizeProject(project);
      if (normalized && project?.schema === LEGACY_PROJECT_SCHEMA_V19) addActivity(normalized, 'migrated', 'Project upgraded to Notebook Review & Provenance');
      return normalized;
    }).filter(Boolean) : [];
    next.recentTools = Array.isArray(raw.recentTools) ? raw.recentTools.map(normalizeRecentTool).filter(Boolean).slice(0, MAX_RECENT_TOOLS) : [];
    next.activeProjectId = next.projects.some(p => p.id === raw.activeProjectId && !p.archivedAt) ? raw.activeProjectId : null;
  ...
  function normalizeState(raw) {
    ...
    if (raw.schemaVersion === 24) return migrateV24(raw);
    ...
    if (raw.schemaVersion === 30) return migrateV30(raw);
    if (raw.schemaVersion === 31) return migrateV31(raw);
    if (raw.schemaVersion === 32) return migrateV32(raw);
    if (raw.schemaVersion === 33) return migrateV33(raw);
    if (raw.schemaVersion === 34) return migrateV34(raw);
    const state = defaultState();
    state.identity = normalizeIdentity(raw.identity);
    state.projects = Array.isArray(raw.projects) ? raw.projects.map(normalizeProject).filter(Boolean) : [];
    state.recentTools = Array.isArray(raw.recentTools) ? raw.recentTools.map(normalizeRecentTool).filter(Boolean).slice(0, MAX_RECENT_TOOLS) : [];
  ...
  function readLastGoodState() {
    try {
      const integrity = window.SCWorkspacePersistenceIntegrity || null;
      if (integrity?.lastGoodState) {
        const checked = integrity.lastGoodState(window.localStorage);
        if (checked?.state === 'integrity-drift') {
          recoveryNotice = 'Workspace found a last-known-good snapshot whose checksum no longer matches its recorded integrity receipt. That snapshot was not loaded automatically; recovery evidence was preserved for review.';
          return null;
        }
        if (!checked?.available || !checked.envelope || !localStateLooksValid(checked.envelope.raw)) return null;
        return normalizeState(JSON.parse(checked.envelope.raw));
      }
  ...
  function readState() {
    try {
      const integrity = window.SCWorkspacePersistenceIntegrity || null;
      if (integrity?.reconcileJournal) {
        const journal = integrity.reconcileJournal(window.localStorage, { workspaceVersion: rootVersion(), storageSchemaVersion: STORAGE_VERSION });
        if (journal.state === 'interrupted' || journal.state === 'failed') {
          recoveryNotice = 'Workspace detected an unresolved or failed local save transaction. Canonical research was not rewritten automatically; review Persistence Integrity before making destructive changes.';
        }
      }
      const current = window.localStorage.getItem(STORAGE_KEY);
      if (current) {
        try {
          const parsed = normalizeState(JSON.parse(current));
          const audit = integrity?.inspect ? integrity.inspect(window.localStorage) : null;
          if (audit?.metadata?.state === 'integrity-drift') {
            recoveryNotice = 'Workspace detected persistence integrity drift: current browser-local bytes no longer match the last verified-save receipt. The readable state was left untouched for review.';
          }
          return parsed;
        } catch (_) {
          ...
          if (recovered) {
            recoveryNotice = 'Workspace isolated a damaged local state and restored the last-known-good local snapshot in memory. The damaged canonical bytes were not overwritten; review Persistence Integrity before continuing.';
            return recovered;
          }
          return defaultState();
        }
  ...
  function writeState(state) {
    ...
    state.projects.forEach((project) => { project.persistence = normalizeProjectPersistence(project.persistence); project.lifecycle = normalizeLifecycle(project.lifecycle); project.notebooks = normalizeNotebooks(project.notebooks, project.objects, project.canvas); project.schema = PROJECT_SCHEMA; });
    ...
    try {
      ...
      if (integrity?.stage) {
        const staged = integrity.stage(window.localStorage, { previousRaw, nextRaw: serialized, workspaceVersion: rootVersion(), storageSchemaVersion: STORAGE_VERSION });
        if (!staged?.ok) throw new Error('write transaction journal unavailable');
      } else captureLastGoodSnapshot();
      ...
      if (integrity?.commit) {
        const receipt = integrity.commit(window.localStorage, serialized, { workspaceVersion: rootVersion(), storageSchemaVersion: STORAGE_VERSION });
        if (!receipt.ok) throw new Error(receipt.reason || 'persistence integrity commit failed');
      }
      return true;
    } catch (error) {
      if (integrity?.fail) integrity.fail(window.localStorage, error);
      recoveryNotice = 'Workspace could not complete and verify the browser-local save transaction. The previous last-known-good snapshot and write journal were preserved when possible; current in-memory changes may only last for this page.';
      return false;
    }
  }
  ...
  function downloadJson(filename, payload) {
    const api=compatibilityApi();
    if(api?.downloadJson){const out=api.downloadJson(filename,payload,window);if(out?.ok)return out;}
    return downloadText(filename,JSON.stringify(payload,null,2)+'\n','application/json;charset=utf-8');
  }


  function downloadText(filename, text, type='text/plain;charset=utf-8') {
    const api=compatibilityApi();
    if(api?.downloadText){const out=api.downloadText(filename,String(text || ''),type,window);if(out?.ok)return out;}
    const blob = new Blob([String(text || '')], { type }); const url=URL.createObjectURL(blob); const anchor=document.createElement('a'); anchor.href=url; anchor.download=filename; document.body.appendChild(anchor); anchor.click(); anchor.remove(); window.setTimeout(()=>URL.revokeObjectURL(url),1500); return {ok:true,mode:'legacy-object-url'};
  }
  ...
  function cloneProject(project) {
    ...
    copy.notebooks = notebookHelper()?.workspace(copy.notebooks, id, nowIso) || notebookWorkspaceTemplate();
    copy.notebooks.notebooks.forEach(notebook => {
      notebook.id = id('nb'); notebook.createdAt = copy.createdAt; notebook.updatedAt = copy.createdAt;
      notebook.sections.forEach(section => {
        section.id = id('nbs'); section.createdAt = copy.createdAt; section.updatedAt = copy.createdAt;
        section.blocks.forEach(block => {
          block.id = id('nbb');
          block.sourceObjectId = objectMap.get(block.sourceObjectId) || '';
          block.referenceObjectId = objectMap.get(block.referenceObjectId) || '';
          block.promotion = { status: 'none', targetKind: '', targetId: '', promotedAt: null };
          block.createdAt = copy.createdAt; block.updatedAt = copy.createdAt;
        });
      });
      notebook.activeSectionId = notebook.sections[0]?.id || null;
    });
    copy.notebooks.activeNotebookId = copy.notebooks.notebooks[0]?.id || null;
    copy.notebooks.createdAt = copy.createdAt; copy.notebooks.updatedAt = copy.createdAt;
    copy.notebooks = normalizeNotebooks(copy.notebooks, copy.objects);
  ...
  function init(root) {
    ...
    root.dataset.scwRuntimeVersion = INTERACTION_RUNTIME_VERSION;
    interactionRuntime.roots += 1;
    const bindingIssues = [];
    const bindControl = (selector, type, handler, options) => {
      const element = root.querySelector(selector);
      if (!element) {
        bindingIssues.push(selector);
        interactionRuntime.recordIssue('missing-control', selector);
        return null;
      }
      element.addEventListener(type, handler, options);
      element.dataset.scwBound = '1';
      return element;
    };

    let state = readState();
    const longSessionApi = window.SCWorkspaceLongSessionPerformance || null;
    const longSessionMonitor = longSessionApi && typeof longSessionApi.createSessionMonitor === 'function' ? longSessionApi.createSessionMonitor() : null;
    if (longSessionMonitor) {
      window.SCWorkspacePerformanceSession = longSessionMonitor;
      longSessionMonitor.startLongTaskObserver();
      window.addEventListener('pagehide', (event) => { if (!event.persisted) longSessionMonitor.dispose(); });
    }
    recoverInterruptedSyncOperations();
    ...
    const projectImportStage = root.querySelector('[data-scw-project-import-stage]');
    const projectImportCommit = root.querySelector('[data-scw-project-import-commit]');
    const projectImportClear = root.querySelector('[data-scw-project-import-clear]');
    ...
    const syncMigrationExportButton = root.querySelector('[data-scw-sync-migration-export]');
    const syncMigrationImportButton = root.querySelector('[data-scw-sync-migration-import]');
    const syncMigrationFile = root.querySelector('[data-scw-sync-migration-file]');
    const syncMigrationStatus = root.querySelector('[data-scw-sync-migration-status]');
    const syncPlanState = root.querySelector('[data-scw-sync-plan-state]');
    const syncPlanReason = root.querySelector('[data-scw-sync-plan-reason]');
    const syncPlanExport = root.querySelector('[data-scw-sync-plan-export]');
    let lastCrossDeviceProductionPlan = null;
...
const researchTemplateSelect = root.querySelector('[data-scw-research-template-select]');
const researchTemplateProjectTitle = root.querySelector('[data-scw-research-template-project-title]');
const researchTemplateIncludeWorkflow = root.querySelector('[data-scw-research-template-workflow]');
const researchTemplateIncludeNotebook = root.querySelector('[data-scw-research-template-notebook]');
const researchTemplateIncludeObjects = root.querySelector('[data-scw-research-template-objects]');
const researchTemplateApply = root.querySelector('[data-scw-research-template-apply]');
const researchTemplateCreateProject = root.querySelector('[data-scw-research-template-create-project]');
const researchTemplateSaveWorkflow = root.querySelector('[data-scw-research-template-save-workflow]');
const researchTemplateExport = root.querySelector('[data-scw-research-template-export]');
const researchTemplateImport = root.querySelector('[data-scw-research-template-import]');
const researchTemplateImportFile = root.querySelector('[data-scw-research-template-import-file]');
const researchTemplateStatus = root.querySelector('[data-scw-research-template-status]');
const researchTemplatePreview = root.querySelector('[data-scw-research-template-preview]');
const researchTemplateCustomList = root.querySelector('[data-scw-research-template-custom-list]');
    ...
    const integratedKind = root.querySelector('[data-scw-integrated-kind]');
    const integratedProject = root.querySelector('[data-scw-integrated-project]');
    const retrievalSubtype = root.querySelector('[data-scw-retrieval-subtype]');
    const retrievalTag = root.querySelector('[data-scw-retrieval-tag]');
    const retrievalOrigin = root.querySelector('[data-scw-retrieval-origin]');
    const retrievalProvenance = root.querySelector('[data-scw-retrieval-provenance]');
    const retrievalScope = root.querySelector('[data-scw-retrieval-scope]');
    const retrievalSort = root.querySelector('[data-scw-retrieval-sort]');
    const retrievalCount = root.querySelector('[data-scw-retrieval-count]');
    const savedSearchSelect = root.querySelector('[data-scw-saved-search]');
    const saveSearchButton = root.querySelector('[data-scw-save-search]');
    const deleteSearchButton = root.querySelector('[data-scw-delete-search]');
    const clearSearchButton = root.querySelector('[data-scw-clear-search]');
    const searchStatus = root.querySelector('[data-scw-search-status]');
    const researchCollectionsSection = root.querySelector('[data-scw-research-collections]');
    const smartCollectionSelect = root.querySelector('[data-scw-smart-collection]');
    const saveCollectionButton = root.querySelector('[data-scw-save-collection]');
    const deleteCollectionButton = root.querySelector('[data-scw-delete-collection]');
    const researchViewSelect = root.querySelector('[data-scw-research-view]');
    const saveViewButton = root.querySelector('[data-scw-save-view]');
    const deleteViewButton = root.querySelector('[data-scw-delete-view]');
    const researchViewGroup = root.querySelector('[data-scw-view-group]');
    const researchViewDensity = root.querySelector('[data-scw-view-density]');
    const researchCollectionStatus = root.querySelector('[data-scw-collection-status]');
    const dynamicPreview = root.querySelector('[data-scw-dynamic-preview]');
    const researchCollectionCount = root.querySelector('[data-scw-collection-count]');
    const researchViewCount = root.querySelector('[data-scw-view-count]');
    const dashboardSources = root.querySelector('[data-scw-dashboard-sources]');
    const dashboardEvidence = root.querySelector('[data-scw-dashboard-evidence]');
    const dashboardDecisions = root.querySelector('[data-scw-dashboard-decisions]');
    const dashboardDocumented = root.querySelector('[data-scw-dashboard-documented]');
    const dashboardProjects = root.querySelector('[data-scw-dashboard-projects]');
    const dashboardRecords = root.querySelector('[data-scw-dashboard-records]');
    const referenceLibrarySection = root.querySelector('[data-scw-reference-library]');
    const libraryContinuitySection=root.querySelector('[data-scw-library-continuity]'), libraryContinuityList=root.querySelector('[data-scw-library-continuity-list]'), libraryContinuityStatus=root.querySelector('[data-scw-library-continuity-status]'), libraryContinuityImport=root.querySelector('[data-scw-library-continuity-import]'), libraryContinuityImportFile=root.querySelector('[data-scw-library-continuity-import-file]'), libraryContinuityClear=root.querySelector('[data-scw-library-continuity-clear]'), libraryContinuityOpen=root.querySelector('[data-scw-library-continuity-open]');
    const referenceCount = root.querySelector('[data-scw-reference-count]');
    const referenceDuplicates = root.querySelector('[data-scw-reference-duplicates]');
    const citationStyle = root.querySelector('[data-scw-citation-style]');
    const referenceSearch = root.querySelector('[data-scw-reference-search]');
    const referenceAddSelected = root.querySelector('[data-scw-reference-add-selected]');
    const referenceExport = root.querySelector('[data-scw-reference-export]');
    const referenceImport = root.querySelector('[data-scw-reference-import]');
    const referenceImportFile = root.querySelector('[data-scw-reference-import-file]');
    const referenceStatus = root.querySelector('[data-scw-reference-status]');
    const referenceForm = root.querySelector('[data-scw-reference-form]');
    const referenceFormClear = root.querySelector('[data-scw-reference-form-clear]');
    const referenceList = root.querySelector('[data-scw-reference-list]');
    const referenceDetail = root.querySelector('[data-scw-reference-detail]');
    const compositionSection = root.querySelector('[data-scw-composition-studio]');
    const compositionDraftSelect = root.querySelector('[data-scw-composition-draft]');
    const compositionNew = root.querySelector('[data-scw-composition-new]');
    const compositionDelete = root.querySelector('[data-scw-composition-delete]');
    const compositionExport = root.querySelector('[data-scw-composition-export]');
    const compositionImport = root.querySelector('[data-scw-composition-import]');
    const compositionImportFile = root.querySelector('[data-scw-composition-import-file]');
    const compositionTitle = root.querySelector('[data-scw-composition-title]');
    const compositionAbstract = root.querySelector('[data-scw-composition-abstract]');
    const compositionStatusInput = root.querySelector('[data-scw-composition-status]');
    const compositionSectionForm = root.querySelector('[data-scw-composition-section-form]');
    const compositionAttachResearch = root.querySelector('[data-scw-composition-attach-research]');
    const compositionAttachReference = root.querySelector('[data-scw-composition-attach-reference]');
    const compositionSectionList = root.querySelector('[data-scw-composition-section-list]');
    const compositionPreview = root.querySelector('[data-scw-composition-preview]');
    const compositionMaterialize = root.querySelector('[data-scw-composition-materialize]');
    const compositionDraftCount = root.querySelector('[data-scw-composition-draft-count]');
    const compositionInputCount = root.querySelector('[data-scw-composition-input-count]');
    const compositionStatus = root.querySelector('[data-scw-composition-status-message]');
    const crossProjectSection = root.querySelector('[data-scw-cross-project-knowledge]');
    const crossProjectTarget = root.querySelector('[data-scw-cross-project-target]');
    const crossProjectRelation = root.querySelector('[data-scw-cross-project-relation]');
    const crossProjectNote = root.querySelector('[data-scw-cross-project-note]');
    const crossProjectAdd = root.querySelector('[data-scw-cross-project-add]');
    const crossProjectList = root.querySelector('[data-scw-cross-project-list]');
    const crossProjectStatus = root.querySelector('[data-scw-cross-project-status]');
    const crossProjectCount = root.querySelector('[data-scw-cross-project-count]');
    const crossProjectProjects = root.querySelector('[data-scw-cross-project-projects]');
    const crossProjectUnresolved = root.querySelector('[data-scw-cross-project-unresolved]');
    ...
    const groundedClearScope = root.querySelector('[data-scw-grounded-clear-scope]');
    const groundedDelete = root.querySelector('[data-scw-grounded-delete]');
    const groundedStatus = root.querySelector('[data-scw-grounded-status]');
    const groundedScope = root.querySelector('[data-scw-grounded-scope]');
    const groundedForm = root.querySelector('[data-scw-grounded-form]');
    const groundedPrompt = root.querySelector('[data-scw-grounded-prompt]');
    const groundedExportRequest = root.querySelector('[data-scw-grounded-export-request]');
    const groundedCopy = root.querySelector('[data-scw-grounded-copy]');
    const groundedResponseSource = root.querySelector('[data-scw-grounded-response-source]');
    const groundedResponse = root.querySelector('[data-scw-grounded-response]');
    const groundedSaveResponse = root.querySelector('[data-scw-grounded-save-response]');
    const groundedExportResponse = root.querySelector('[data-scw-grounded-export-response]');
    const groundedReview = root.querySelector('[data-scw-grounded-review]');
    const groundedReject = root.querySelector('[data-scw-grounded-reject]');
    const groundedTargetProject = root.querySelector('[data-scw-grounded-target-project]');
    const groundedMaterialize = root.querySelector('[data-scw-grounded-materialize]');
    const groundedCitations = root.querySelector('[data-scw-grounded-citations]');
    const groundedSessionCount = root.querySelector('[data-scw-grounded-session-count]');
    const groundedScopeCount = root.querySelector('[data-scw-grounded-scope-count]');
    const integratedResults = root.querySelector('[data-scw-integrated-results]');
    const integratedDetail = root.querySelector('[data-scw-integrated-detail]');
    const integratedTotal = root.querySelector('[data-scw-integrated-total]');
    const integratedObjects = root.querySelector('[data-scw-integrated-objects]');
    const integratedNotebooks = root.querySelector('[data-scw-integrated-notebooks]');
    const integratedResearch = root.querySelector('[data-scw-integrated-research]');
    const integratedOpenNotebook = root.querySelector('[data-scw-integrated-open-notebook]');
    const integratedOpenKnowledge = root.querySelector('[data-scw-integrated-open-knowledge]');
    const integratedOpenProject = root.querySelector('[data-scw-integrated-open-project]');
    const integratedLoadMore = root.querySelector('[data-scw-integrated-load-more]');
    let integratedRenderLimit=120; let integratedPrefs={query:'',kind:'all',subtype:'all',project:'all',tag:'',origin:'all',provenance:'all',scope:'active',sort:'relevance'}; let selectedIntegratedKey=''; let savedKnowledgeSearches=[]; let researchCollections=[]; let researchViews=[]; let activeResearchCollectionId=''; let activeResearchViewId=''; let dynamicGroupBy='project'; let dynamicDensity='comfortable';
    ...
    const cockpit = root.querySelector('[data-scw-project-cockpit]');
    const cockpitSummary = root.querySelector('[data-scw-cockpit-summary]');
    const cockpitTitle = root.querySelector('[data-scw-cockpit-title]');
    const cockpitUpdated = root.querySelector('[data-scw-cockpit-updated]');
    const cockpitProjects = root.querySelector('[data-scw-cockpit-projects]');
    const cockpitObjects = root.querySelector('[data-scw-cockpit-objects]');
    const cockpitResearch = root.querySelector('[data-scw-cockpit-research]');
    const cockpitNotebooks = root.querySelector('[data-scw-cockpit-notebooks]');
    const cockpitNextTitle = root.querySelector('[data-scw-cockpit-next-title]');
    const cockpitNextDetail = root.querySelector('[data-scw-cockpit-next-detail]');
    const cockpitOpenProject = root.querySelector('[data-scw-cockpit-open-project]');
    const cockpitModeCards = Array.from(root.querySelectorAll('[data-scw-cockpit-mode]'));
    ...
    const firstRunPanel = root.querySelector('[data-scw-first-run]');
    const firstRunForm = root.querySelector('[data-scw-first-run-form]');
    const firstRunBoundary = root.querySelector('[data-scw-first-run-boundary]');
    const firstRunStatus = root.querySelector('[data-scw-first-run-status]');
    const firstRunStandard = root.querySelector('[data-scw-first-run-blank]');
    const projectsSection = root.querySelector('[data-scw-workspace-section="projects"]');
    const notebookSection = root.querySelector('[data-scw-workspace-section="notebook"]');
    const notebookProject = root.querySelector('[data-scw-notebook-project]');
    const notebookMetricNotebooks = root.querySelector('[data-scw-notebook-metric-notebooks]');
    const notebookMetricSections = root.querySelector('[data-scw-notebook-metric-sections]');
    const notebookMetricBlocks = root.querySelector('[data-scw-notebook-metric-blocks]');
    const notebookMetricPromoted = root.querySelector('[data-scw-notebook-metric-promoted]');
    const notebookCreateForm = root.querySelector('[data-scw-notebook-create-form]');
    const notebookList = root.querySelector('[data-scw-notebook-list]');
    const notebookActive = root.querySelector('[data-scw-notebook-active]');
    const notebookSectionForm = root.querySelector('[data-scw-notebook-section-form]');
    const notebookSectionList = root.querySelector('[data-scw-notebook-section-list]');
    const notebookBlockForm = root.querySelector('[data-scw-notebook-block-form]');
    const notebookBlockObject = root.querySelector('[data-scw-notebook-block-object]');
    ...
    const notebookPromotionList = root.querySelector('[data-scw-notebook-promotion-list]');
    const notebookMetricSyntheses = root.querySelector('[data-scw-notebook-metric-syntheses]');
    const notebookSynthesisForm = root.querySelector('[data-scw-notebook-synthesis-form]');
    const notebookSynthesisMaterial = root.querySelector('[data-scw-notebook-synthesis-material]');
    const notebookSynthesisList = root.querySelector('[data-scw-notebook-synthesis-list]');
    const notebookMetricAssistances = root.querySelector('[data-scw-notebook-metric-assistances]');
    const notebookAssistanceForm = root.querySelector('[data-scw-notebook-assistance-form]');
    const notebookAssistanceMaterial = root.querySelector('[data-scw-notebook-assistance-material]');
    const notebookAssistanceList = root.querySelector('[data-scw-notebook-assistance-list]');
    const notebookPortableImport = root.querySelector('[data-scw-notebook-portable-import]');
    const notebookPortableFile = root.querySelector('[data-scw-notebook-portable-file]');
    const notebookRestoreCreate = root.querySelector('[data-scw-notebook-restore-create]');
    const notebookRestoreList = root.querySelector('[data-scw-notebook-restore-list]');
    const notebookBackup = root.querySelector('[data-scw-notebook-backup]');
    const notebookSyncToggle = root.querySelector('[data-scw-notebook-sync-toggle]');
    const notebookSyncCheck = root.querySelector('[data-scw-notebook-sync-check]');
    const notebookSyncNow = root.querySelector('[data-scw-notebook-sync-now]');
    const notebookSyncCloudCopy = root.querySelector('[data-scw-notebook-sync-cloud-copy]');
    const notebookSyncKeepLocal = root.querySelector('[data-scw-notebook-sync-keep-local]');
    const notebookPortabilityStatus = root.querySelector('[data-scw-notebook-portability-status]');
    const notebookReviewBaseline = root.querySelector('[data-scw-notebook-review-baseline]');
    const notebookReviewCreate = root.querySelector('[data-scw-notebook-review-create]');
    const notebookReviewSummary = root.querySelector('[data-scw-notebook-review-summary]');
    const notebookReviewList = root.querySelector('[data-scw-notebook-review-list]');
    const notebookReconcileReviewer = root.querySelector('[data-scw-notebook-reconcile-reviewer]');
    const notebookReconcileRationale = root.querySelector('[data-scw-notebook-reconcile-rationale]');
    const notebookReconcileAck = root.querySelector('[data-scw-notebook-reconcile-ack]');
    const notebookReconcileCreate = root.querySelector('[data-scw-notebook-reconcile-create]');
    const notebookReconcileHistory = root.querySelector('[data-scw-notebook-reconcile-history]');
    const notebookLineageRef = root.querySelector('[data-scw-notebook-lineage-ref]');
    const notebookLineageInspect = root.querySelector('[data-scw-notebook-lineage-inspect]');
    const notebookLineageResult = root.querySelector('[data-scw-notebook-lineage-result]');
    const notebookAuditList = root.querySelector('[data-scw-notebook-audit-list]');
    ...
    const lifecycleSection = root.querySelector('[data-scw-workspace-section="lifecycle"]');
    const lifecycleProject = root.querySelector('[data-scw-lifecycle-project]');
    const lifecycleCurrent = root.querySelector('[data-scw-lifecycle-current]');
    const lifecycleCurrentMeta = root.querySelector('[data-scw-lifecycle-current-meta]');
    const lifecycleStages = root.querySelector('[data-scw-lifecycle-stages]');
    const lifecycleTarget = root.querySelector('[data-scw-lifecycle-target]');
    const lifecycleReadiness = root.querySelector('[data-scw-lifecycle-readiness]');
    const lifecycleStatus = root.querySelector('[data-scw-lifecycle-status]');
    const lifecycleForm = root.querySelector('[data-scw-lifecycle-form]');
    const lifecycleHistory = root.querySelector('[data-scw-lifecycle-history]');
    ...
    const reconciliationSection = root.querySelector('[data-scw-workspace-section="reconcile"]');
    const reconcileProject = root.querySelector('[data-scw-reconcile-project]');
    const reconcileBase = root.querySelector('[data-scw-reconcile-base]');
    const reconcileTarget = root.querySelector('[data-scw-reconcile-target]');
    const reconcileLoad = root.querySelector('[data-scw-reconcile-load]');
    const reconcileStatus = root.querySelector('[data-scw-reconcile-status]');
    const reconcileList = root.querySelector('[data-scw-reconcile-list]');
    const reconcilePreview = root.querySelector('[data-scw-reconcile-preview]');
    const reconcileSelectAll = root.querySelector('[data-scw-reconcile-select-all]');
    const reconcileClear = root.querySelector('[data-scw-reconcile-clear]');
    const reconcileExport = root.querySelector('[data-scw-reconcile-export]');
    const reconcileAck = root.querySelector('[data-scw-reconcile-ack]');
    const reconcileCreate = root.querySelector('[data-scw-reconcile-create]');
    const reconcileHistory = root.querySelector('[data-scw-reconcile-history]');
    const reconcileReviewer = root.querySelector('[data-scw-reconcile-reviewer]');
    const reconcileRationale = root.querySelector('[data-scw-reconcile-rationale]');
    const reconcileReceipts = root.querySelector('[data-scw-reconcile-receipts]');
    const reconcileMetricAvailable = root.querySelector('[data-scw-reconcile-metric-available]');
    const reconcileMetricSelected = root.querySelector('[data-scw-reconcile-metric-selected]');
    const reconcileMetricBlockers = root.querySelector('[data-scw-reconcile-metric-blockers]');
    const reconcileMetricCopies = root.querySelector('[data-scw-reconcile-metric-copies]');
    const changeReconcile = root.querySelector('[data-scw-change-reconcile]');
    ...
    const persistenceIntegritySection = root.querySelector('[data-scw-workspace-section="integrity"]');
    ...
    const collabArchitectureSection = root.querySelector('[data-scw-collaboration-architecture]');
    const collabActorForm = root.querySelector('[data-scw-collab-actor-form]');
    const collabActorList = root.querySelector('[data-scw-collab-actor-list]');
    const collabPolicyForm = root.querySelector('[data-scw-collab-policy-form]');
    const collabArchProject = root.querySelector('[data-scw-collab-arch-project]');
    const collabOwner = root.querySelector('[data-scw-collab-owner]');
    const collabGrantForm = root.querySelector('[data-scw-collab-grant-form]');
    const collabGrantActor = root.querySelector('[data-scw-collab-grant-actor]');
    const collabPolicySummary = root.querySelector('[data-scw-collab-policy-summary]');
    const collabCommentForm = root.querySelector('[data-scw-collab-comment-form]');
    const collabCommentActor = root.querySelector('[data-scw-collab-comment-actor]');
    const collabCommentObject = root.querySelector('[data-scw-collab-comment-object]');
    const collabCommentList = root.querySelector('[data-scw-collab-comment-list]');
    const collabProposalForm = root.querySelector('[data-scw-collab-proposal-form]');
    const collabProposalActor = root.querySelector('[data-scw-collab-proposal-actor]');
    const collabProposalObject = root.querySelector('[data-scw-collab-proposal-object]');
    const collabProposalList = root.querySelector('[data-scw-collab-proposal-list]');
    const collabContractPreview = root.querySelector('[data-scw-collab-contract-preview]');
    const collabContractExport = root.querySelector('[data-scw-collab-contract-export]');
    const collabArchitectureExport = root.querySelector('[data-scw-collab-architecture-export]');
    const collabArchitectureImport = root.querySelector('[data-scw-collab-architecture-import]');
    const collabArchitectureFile = root.querySelector('[data-scw-collab-architecture-file]');
    const collabContractOutput = root.querySelector('[data-scw-collab-contract-output]');
    const collabArchStatus = root.querySelector('[data-scw-collab-arch-status]');
    const collabArchActors = root.querySelector('[data-scw-collab-arch-actors]');
    const collabArchPolicies = root.querySelector('[data-scw-collab-arch-policies]');
    const collabArchComments = root.querySelector('[data-scw-collab-arch-comments]');
    const collabArchProposals = root.querySelector('[data-scw-collab-arch-proposals]');
    let collaborationArchitecture={schema:'sc-workspace-collaboration-architecture/1.0',actors:[],policies:[],comments:[],proposals:[],updatedAt:''};
    ...
    const interoperabilityExportProfile = root.querySelector('[data-scw-interoperability-export-profile]');
    ...
    const compatibilityMatrixEl = root.querySelector('[data-scw-backward-compatibility-matrix]');
    const compatibilityMatrixExport = root.querySelector('[data-scw-backward-compatibility-export]');
    ...
    const cockpitUniversalSearch = root.querySelector('[data-scw-cockpit-universal-search]');
    ...
    const graphPathFrom=root.querySelector('[data-scw-graph-path-from]'),graphPathTo=root.querySelector('[data-scw-graph-path-to]'),graphPathTrace=root.querySelector('[data-scw-graph-path-trace]'),graphPathClear=root.querySelector('[data-scw-graph-path-clear]'),graphExport=root.querySelector('[data-scw-graph-export]'),graphPathStatus=root.querySelector('[data-scw-graph-path-status]'),graphPath=root.querySelector('[data-scw-graph-path]');
    let activeGraphPath=null;
    ...
    const labWorkflow=root.querySelector('[data-scw-lab-workflow]');
    const labObjectScope=root.querySelector('[data-scw-lab-object-scope]');
    const labMethodology=root.querySelector('[data-scw-lab-methodology]');
    const labUncertainty=root.querySelector('[data-scw-lab-uncertainty]');
    const labOpen=root.querySelector('[data-scw-lab-open]');
    const labExportContext=root.querySelector('[data-scw-lab-export-context]');
    const labExportReturnTemplate=root.querySelector('[data-scw-lab-export-return-template]');
    const labImportReturn=root.querySelector('[data-scw-lab-import-return]');
    const labImportReturnFile=root.querySelector('[data-scw-lab-import-return-file]');
    const labStatus=root.querySelector('[data-scw-lab-status]');
    let lastLabContextPackage=null;
    const specialistDestination=root.querySelector('[data-scw-specialist-destination]');
    const specialistWorkflow=root.querySelector('[data-scw-specialist-workflow]');
    const specialistObjectScope=root.querySelector('[data-scw-specialist-object-scope]');
    const specialistPurpose=root.querySelector('[data-scw-specialist-purpose]');
    const specialistConstraints=root.querySelector('[data-scw-specialist-constraints]');
    const specialistScenario=root.querySelector('[data-scw-specialist-scenario]');
    const specialistOpen=root.querySelector('[data-scw-specialist-open]');
    const specialistExportContext=root.querySelector('[data-scw-specialist-export-context]');
    const specialistExportReturnTemplate=root.querySelector('[data-scw-specialist-export-return-template]');
    const specialistImportReturn=root.querySelector('[data-scw-specialist-import-return]');
    const specialistImportReturnFile=root.querySelector('[data-scw-specialist-import-return-file]');
    const specialistStatus=root.querySelector('[data-scw-specialist-status]');
    let lastSpecialistContextPackage=null;


    function labIntegrationApi(){return window.SCWorkspaceLabIntegration||null;}
    function selectedLabObjectIds(){if(!labObjectScope)return[];return [...labObjectScope.selectedOptions].map(o=>String(o.value||'')).filter(Boolean);}
    function renderLabIntegration(project=activeProject()){
      if(!labObjectScope)return;const selected=new Set(selectedLabObjectIds());labObjectScope.innerHTML='';
      if(!project){const o=document.createElement('option');o.textContent='Choose an active project first';o.disabled=true;labObjectScope.appendChild(o);if(labStatus)labStatus.textContent='Choose a project to prepare scientific context.';return;}
      const allowed=new Set(['source','evidence','dataset','analysis','document','export']);
      project.objects.filter(o=>!o.archivedAt&&allowed.has(o.type)).forEach(o=>{const opt=document.createElement('option');opt.value=o.id;opt.textContent=`${(o.type||'object').toUpperCase()} · ${o.title}`;opt.selected=selected.has(o.id)||(!selected.size&&o.id===project.activeObjectId);labObjectScope.appendChild(opt);});
      if(!labObjectScope.options.length){const o=document.createElement('option');o.textContent='No scientific context objects in this project';o.disabled=true;labObjectScope.appendChild(o);}
    }
    function createLabHandoff(project,objectIds){if(!project)return null;const first=project.objects.find(o=>objectIds.includes(o.id))||null;const h=createHandoff(project,'lab','Lab',first);h.objectIds=[...new Set(objectIds)].slice(0,80);touchHandoffs(project);return h;}
    function buildLabContext(project,handoff){const api=labIntegrationApi();if(!api||!project||!handoff)return null;return api.buildContextPackage(project,handoff,handoff.objectIds,{workspaceVersion:rootVersion(),workflow:String(labWorkflow?.value||'model-studio'),methodology:String(labMethodology?.value||''),uncertainty:String(labUncertainty?.value||''),units:String(labUncertainty?.value||'')});}
    function addLabTraceability(project,sourceIds,createdIds,handoffId){if(!project||!createdIds.length)return;const valid=new Set(project.objects.filter(o=>!o.archivedAt).map(o=>o.id));project.traceability=normalizeTraceability(project.traceability,project.objects);for(const from of sourceIds){if(!valid.has(from))continue;for(const to of createdIds){if(!valid.has(to)||from===to)continue;if(project.traceability.lineage.some(e=>e.fromObjectId===from&&e.toObjectId===to&&e.relation==='derived-from'))continue;project.traceability.lineage.unshift({id:id('tl'),fromObjectId:from,toObjectId:to,relation:'derived-from',note:`Explicit Lab round trip · ${handoffId}`,createdAt:nowIso()});}}project.traceability.lineage=project.traceability.lineage.slice(0,MAX_LINEAGE_RELATIONS);touchTraceability(project);}
    // Explicit Workbench round trip / Explicit Decision Studio round trip are human-controlled specialist handoffs.
    function specialistApi(){return window.SCWorkspaceWorkbenchDecisionRoundtrip||null;}
    function specialistDestinationKey(){return String(specialistDestination?.value||'workbench')==='decision-studio'?'decision-studio':'workbench';}
    function specialistLabel(){return specialistDestinationKey()==='decision-studio'?'Decision Studio':'Workbench';}
    function specialistRoute(){return specialistDestinationKey()==='decision-studio'?'/lab/decision-studio/':'/lab/workbench/';}
    function specialistWorkflows(){const api=specialistApi();if(!api)return[];return specialistDestinationKey()==='decision-studio'?api.DS_WORKFLOWS:api.WB_WORKFLOWS;}
    function specialistTypes(){return specialistDestinationKey()==='decision-studio'?new Set(['decision','evidence','source','analysis','document','export']):new Set(['dataset','analysis','evidence','document','export']);}
    function selectedSpecialistObjectIds(){if(!specialistObjectScope)return[];return [...specialistObjectScope.selectedOptions].map(o=>String(o.value||'')).filter(Boolean);}
    function renderSpecialistIntegration(project=activeProject()){
      if(specialistWorkflow){const current=specialistWorkflow.value,supported=specialistWorkflows();specialistWorkflow.innerHTML='';supported.forEach(w=>{const o=document.createElement('option');o.value=w;o.textContent=w.split('-').map(x=>x.charAt(0).toUpperCase()+x.slice(1)).join(' ');specialistWorkflow.appendChild(o);});if(supported.includes(current))specialistWorkflow.value=current;}
      if(!specialistObjectScope)return;const selected=new Set(selectedSpecialistObjectIds());specialistObjectScope.innerHTML='';
      if(!project){const o=document.createElement('option');o.textContent='Choose an active project first';o.disabled=true;specialistObjectScope.appendChild(o);if(specialistStatus)specialistStatus.textContent='Choose a project to prepare specialist context.';return;}
      const allowed=specialistTypes();project.objects.filter(o=>!o.archivedAt&&allowed.has(o.type)).forEach(o=>{const opt=document.createElement('option');opt.value=o.id;opt.textContent=`${(o.type||'object').toUpperCase()} · ${o.title}`;opt.selected=selected.has(o.id)||(!selected.size&&o.id===project.activeObjectId);specialistObjectScope.appendChild(opt);});
      if(!specialistObjectScope.options.length){const o=document.createElement('option');o.textContent=`No compatible ${specialistLabel()} context objects in this project`;o.disabled=true;specialistObjectScope.appendChild(o);}
    }
    function createSpecialistHandoff(project,objectIds){if(!project)return null;const destination=specialistDestinationKey(),first=project.objects.find(o=>objectIds.includes(o.id))||null,h=createHandoff(project,destination,specialistLabel(),first);h.objectIds=[...new Set(objectIds)].slice(0,80);touchHandoffs(project);return h;}
    function buildSpecialistContext(project,handoff){const api=specialistApi();if(!api||!project||!handoff)return null;const raw=String(specialistConstraints?.value||'');return api.buildContextPackage(project,handoff,handoff.objectIds,{workspaceVersion:rootVersion(),destination:handoff.destination,workflow:String(specialistWorkflow?.value||''),purpose:String(specialistPurpose?.value||''),method:String(specialistPurpose?.value||''),parameters:raw,constraints:raw,assumptions:raw,scenarioNotes:String(specialistScenario?.value||'')});}
    function addSpecialistTraceability(project,sourceIds,createdIds,handoffId,destination){if(!project||!createdIds.length)return;const valid=new Set(project.objects.filter(o=>!o.archivedAt).map(o=>o.id));project.traceability=normalizeTraceability(project.traceability,project.objects);for(const from of sourceIds){if(!valid.has(from))continue;for(const to of createdIds){if(!valid.has(to)||from===to)continue;if(project.traceability.lineage.some(e=>e.fromObjectId===from&&e.toObjectId===to&&e.relation==='derived-from'))continue;project.traceability.lineage.unshift({id:id('tl'),fromObjectId:from,toObjectId:to,relation:'derived-from',note:`Explicit ${destination==='decision-studio'?'Decision Studio':'Workbench'} round trip · ${handoffId}`,createdAt:nowIso()});}}project.traceability.lineage=project.traceability.lineage.slice(0,MAX_LINEAGE_RELATIONS);touchTraceability(project);}



    function renderProjectImportStage() {
      if (!projectImportStage) return;
      projectImportStage.innerHTML = '';
      if (!stagedProjectImport) {
        projectImportStage.hidden = true;
        if (projectImportCommit) projectImportCommit.disabled = true;
        if (projectImportClear) projectImportClear.disabled = true;
        return;
      }
      projectImportStage.hidden = false;
      const a = stagedProjectImport.assessment || {};
      const box = document.createElement('div');
      box.className = `scw-project-import-assessment ${a.status === 'ready' ? 'is-ready' : 'is-blocked'}`;
      const title = document.createElement('strong');
      title.textContent = a.status === 'ready' ? `Staged: ${a.title || stagedProjectImport.fileName}` : 'Import blocked';
      const meta = document.createElement('span');
      meta.textContent = a.status === 'ready'
        ? `${a.sourceProjectSchema || 'Unknown schema'} · ${a.objectCount || 0} object(s)${a.upgradeRequired ? ` · normalize to ${a.currentProjectSchema}` : ' · current schema'}`
        : (a.errors && a.errors[0]) || 'Workspace could not safely classify this project file.';
      box.append(title, meta);
      (Array.isArray(a.warnings) ? a.warnings : []).forEach(message => { const note=document.createElement('small'); note.textContent=message; box.appendChild(note); });
      projectImportStage.appendChild(box);
      if (projectImportCommit) projectImportCommit.disabled = a.status !== 'ready';
      if (projectImportClear) projectImportClear.disabled = false;
    }

    function renderBackwardCompatibilityMatrix() {
      if (!compatibilityMatrixEl) return;
      const api=importExportCompatibilityApi(); if(!api)return;
      const matrix=api.compatibilityMatrix();
      compatibilityMatrixEl.innerHTML='';
      const rows=[
        ['Project JSON', `${matrix.projectImport.supportedProjectSchemas[0]} → ${matrix.current.projectSchema}`, 'Stage → review → new local copy'],
        ['Project export', `${matrix.projectImport.supportedExportSchemas[0]} → ${matrix.current.exportSchema}`, 'Legacy exports normalized only after commit'],
        ['Browser storage', `Storage 1 → ${matrix.current.storageSchema}`, 'Existing-state migration path; not accepted as Project Import'],
        ['Portable project', matrix.portableProject.schema, 'Verified through Share & portable projects'],
        ['Interchange', matrix.interchange.schema, 'Staged through Import & interoperability'],
        ['Future project schemas', `Above ${matrix.current.projectSchema}`, 'Blocked; Workspace does not downgrade newer schemas']
      ];
      rows.forEach(([label,versions,mode])=>{const row=document.createElement('div');row.className='scw-compatibility-matrix-row';const a=document.createElement('strong');a.textContent=label;const b=document.createElement('span');b.textContent=versions;const c=document.createElement('small');c.textContent=mode;row.append(a,b,c);compatibilityMatrixEl.appendChild(row);});
    }
    ...
    function syncPushPayload(project, enrollment, expectedRevision, operationId='') {
      return { schema: SYNC_PUSH_SCHEMA, workspaceVersion: rootVersion(), createdAt: nowIso(), operationId: String(operationId||'').slice(0,160), sourceProjectId: project.id, projectTitle: project.title, clientUpdatedAt: project.updatedAt, expectedRevision: Math.max(0, Number(expectedRevision) || 0), privacy: { deviceIdentityIncluded: false, accountProfileIncluded: false, recentToolsIncluded: false }, transport: { automaticUpload: false, backgroundSync: false, explicitEnrollment: true, conflictStrategy: 'revision-precondition', retryStrategy: 'idempotent-operation-id' }, project: syncProjectSnapshot(project) };
    }

    function replaceSyncOperation(updated){
      if(!updated)return;state.crossDeviceSync=normalizeCrossDeviceSync(state.crossDeviceSync,state.projects);const i=state.crossDeviceSync.operations.findIndex(item=>item.id===updated.id);if(i>=0)state.crossDeviceSync.operations[i]=updated;else state.crossDeviceSync.operations.unshift(updated);state.crossDeviceSync.operations=state.crossDeviceSync.operations.slice(0,MAX_SYNC_OPERATIONS);state.crossDeviceSync.updatedAt=nowIso();
    }
    async function beginSyncOperation(kind,project,enrollment,expectedRevision,remote=null){
      const api=continuityApi(),localFingerprint=await syncProjectFingerprint(project);if(!localFingerprint)throw new Error('This browser cannot calculate the SHA-256 project fingerprint required for safe synchronization.');
      const op=api?.operation?api.operation({projectId:project.id,kind,state:'pending',expectedRevision,remoteRevision:remote?.revision||0,localFingerprint,remoteFingerprint:remote?.projectFingerprint||''},()=>id('syo'),nowIso):{schema:SYNC_OPERATION_SCHEMA,id:id('syo'),projectId:project.id,kind,state:'pending',expectedRevision:Math.max(0,Number(expectedRevision)||0),remoteRevision:Math.max(0,Number(remote?.revision)||0),localFingerprint,remoteFingerprint:String(remote?.projectFingerprint||''),startedAt:nowIso(),updatedAt:nowIso(),completedAt:'',message:''};
      enrollment.lastOperationId=op.id;enrollment.lastError='';replaceSyncOperation(op);writeState(state);return op;
    }
    function finishSyncOperation(op,message=''){
      const api=continuityApi(),done=api?.complete?api.complete(op,message,nowIso):{...op,state:'completed',updatedAt:nowIso(),completedAt:nowIso(),message};replaceSyncOperation(done);const e=syncEnrollment(op.projectId,false);if(e){e.lastOperationId=op.id;e.lastError='';}return done;
    }
    function interruptSyncOperation(op,error){
      const msg=String(error?.message||error||'Synchronization stopped before Workspace could confirm the remote result.').slice(0,500),api=continuityApi(),uncertain=/network|fetch|timeout|failed to fetch|load failed|connection/i.test(msg),failed=api?.fail?api.fail(op,msg,uncertain,nowIso):{...op,state:uncertain?'interrupted':'failed',updatedAt:nowIso(),message:msg};replaceSyncOperation(failed);const e=syncEnrollment(op.projectId,false);if(e){e.status=uncertain?'interrupted':(/revision conflict/i.test(msg)?'conflict':'error');e.lastError=msg;}return failed;
    }
    function recoverInterruptedSyncOperations(){
      state.crossDeviceSync=normalizeCrossDeviceSync(state.crossDeviceSync,state.projects);const api=continuityApi();const before=JSON.stringify(state.crossDeviceSync.operations);state.crossDeviceSync.operations=api?.markInterrupted?api.markInterrupted(state.crossDeviceSync.operations,nowIso):state.crossDeviceSync.operations.map(op=>op.state==='pending'?{...op,state:'interrupted',updatedAt:nowIso(),message:op.message||'Workspace restarted before this sync operation recorded a final result.'}:op);if(JSON.stringify(state.crossDeviceSync.operations)!==before){state.crossDeviceSync.operations.forEach(op=>{if(op.state==='interrupted'){const e=syncEnrollment(op.projectId,false);if(e&&e.enabled){e.status='interrupted';e.lastOperationId=op.id;}}});writeState(state);}
    }
    async function createDeviceMigrationPackage(project){
      const enrollment=syncEnrollment(project.id,false),fingerprint=await syncProjectFingerprint(project);if(!fingerprint)throw new Error('This browser cannot calculate the SHA-256 project fingerprint required for a migration package.');const api=continuityApi();if(!api?.migrationPackage)throw new Error('Cross-device continuity helper is unavailable.');return api.migrationPackage(syncProjectSnapshot(project),enrollment,rootVersion(),fingerprint,nowIso);
    }
    async function inspectDeviceMigrationPackage(pkg){
      const api=continuityApi(),assessment=api?.inspectMigration?.(pkg)||{ok:false,reason:'Cross-device continuity helper is unavailable.'};if(!assessment.ok)return assessment;const normalized=normalizeProject(pkg.project);if(!normalized)return {ok:false,reason:'The migration project is not compatible with this Workspace version.'};const fingerprint=await syncProjectFingerprint(normalized);if(!fingerprint||!assessment.projectFingerprint||fingerprint!==assessment.projectFingerprint)return {ok:false,reason:'Migration project fingerprint does not match the package metadata.'};const key=api.migrationKey(assessment.sourceProjectId,assessment.projectFingerprint),duplicate=state.crossDeviceSync.migrations.some(item=>item.direction==='import'&&api.migrationKey(item.sourceProjectId,item.projectFingerprint)===key);return {...assessment,project:normalized,duplicate};
    }
    ...
    async function evaluateSync(project, refreshRemote = true) {
      ...
      const pending=continuityApi()?.pendingFor?.(state.crossDeviceSync.operations,project.id)||null;
      if(pending&&status==='up-to-date'&&remote&&remote.projectFingerprint===localFingerprint){finishSyncOperation(pending,`Status check confirmed cloud revision ${remote.revision} matches the local project.`);recordSync('retry',project,remote.revision);}
      return {status,enrollment,remote,localFingerprint,message:(pending&&pending.state==='interrupted'&&status!=='up-to-date')?`${syncStatusMessage(status,remote)} An interrupted operation ${pending.id.slice(0,18)}… remains preserved for review.`:syncStatusMessage(status,remote)};
    }
    ...
    async function pushSyncHead(project,enrollment,expectedRevision,operation=null) {
      const op=operation||await beginSyncOperation('push',project,enrollment,expectedRevision,remoteCloudRecord(project.id));
      try { const data=await cloudRequest('cloud-projects',{method:'POST',body:JSON.stringify(syncPushPayload(project,enrollment,expectedRevision,op.id))}); if(!data?.item)throw new Error('The server did not return synchronized project metadata.');
      const localFingerprint=await syncProjectFingerprint(project); enrollment.serverRevision=Math.max(0,Number(data.item.revision)||0); enrollment.remoteRevision=enrollment.serverRevision; enrollment.lastSyncedFingerprint=localFingerprint||String(data.item.projectFingerprint||''); enrollment.remoteFingerprint=String(data.item.projectFingerprint||enrollment.lastSyncedFingerprint||''); enrollment.lastSyncedAt=nowIso(); enrollment.lastCheckedAt=enrollment.lastSyncedAt; enrollment.status='up-to-date'; enrollment.updatedAt=enrollment.lastSyncedAt; state.accountPersistence.cloudRecords=[data.item,...state.accountPersistence.cloudRecords.filter(record=>record.projectId!==data.item.projectId)]; state.crossDeviceSync.updatedAt=enrollment.updatedAt; finishSyncOperation(op,data.replayed?'Server confirmed an earlier completed retry-safe push.':`Cloud revision ${enrollment.serverRevision} confirmed.`); return data.item; } catch(error){ interruptSyncOperation(op,error); throw error; }
    }

    async function applyRemoteInPlace(project,enrollment,remote,preserveLocalConflictCopy=false,operation=null) {
      const op=operation||await beginSyncOperation(preserveLocalConflictCopy?'resolve-cloud':'pull',project,enrollment,enrollment.serverRevision,remote);
      try { await createRestorePoint(project,`Sync safety · ${nowIso().slice(0,16).replace('T',' ')}`,`Automatic safety point before applying cloud revision ${remote?.revision||''}.`,'sync-safety');
      const {source,item}=await fetchRemoteSyncProject(project.id); if(preserveLocalConflictCopy){const safety=cloneProject(project);safety.title=`${project.title} (Local conflict copy)`.slice(0,120);safety.activity.unshift({id:id('act'),type:'sync-conflict-copy',summary:`Preserved local changes before accepting cloud revision ${remote?.revision||item?.revision||''}`.trim(),at:nowIso()});state.projects.unshift(safety);} source.id=project.id; source.persistence=projectPersistenceTemplate(); source.activity=Array.isArray(source.activity)?source.activity:[]; source.activity.unshift({id:id('act'),type:'sync-pull',summary:`Applied cloud revision ${remote?.revision||item?.revision||''}`.trim(),at:nowIso()}); const index=state.projects.findIndex(candidate=>candidate.id===project.id);if(index<0)throw new Error('Local project is no longer available.');state.projects[index]=source; const fingerprint=await syncProjectFingerprint(source); enrollment.serverRevision=Math.max(0,Number(remote?.revision||item?.revision)||0);enrollment.remoteRevision=enrollment.serverRevision;enrollment.lastSyncedFingerprint=fingerprint||String(remote?.projectFingerprint||item?.projectFingerprint||'');enrollment.remoteFingerprint=String(remote?.projectFingerprint||item?.projectFingerprint||enrollment.lastSyncedFingerprint||'');enrollment.lastSyncedAt=nowIso();enrollment.lastCheckedAt=enrollment.lastSyncedAt;enrollment.status='up-to-date';enrollment.updatedAt=enrollment.lastSyncedAt;state.crossDeviceSync.updatedAt=enrollment.updatedAt;finishSyncOperation(op,`Cloud revision ${enrollment.serverRevision} applied after creating a local sync-safety restore point.`);return source; } catch(error){interruptSyncOperation(op,error);throw error;}
    }
    ...
    function syncCockpitModeCards(project = activeProject()) {
      cockpitModeCards.forEach((button) => {
        const mode = button.dataset.scwCockpitMode || 'overview';
        const available = Boolean(project);
        const selected = available && activeProjectMode === mode;
        button.disabled = !available;
        button.classList.toggle('is-active', selected);
        button.setAttribute('aria-pressed', selected ? 'true' : 'false');
        if (selected) button.setAttribute('aria-current', 'page'); else button.removeAttribute('aria-current');
        const action = button.querySelector('[data-scw-work-mode-action]');
        if (action) action.textContent = available ? 'Open →' : 'Choose project';
      });
    }
    ...
    function setProjectMode(mode) {
      ...
      syncCockpitModeCards(activeProject());
    ...
    function projectCard(project) {
      ...
      meta.textContent = project.archivedAt ? 'ARCHIVED' : `${project.status.toUpperCase()} · ${lifecycleLabel(project.lifecycle?.state||'draft').toUpperCase()}`;
    ...
    function renderResearch(project) {
      ...
      if (!research.questions.length) {
        const emptyItem = document.createElement('div'); emptyItem.className = 'scw-research-empty'; emptyItem.textContent = 'No research questions yet. Frame one explicit question so sources and evidence have a visible purpose.'; researchQuestionList.appendChild(emptyItem);
      } else {
      ...
      if (!queue.length) {
        const emptyItem = document.createElement('div'); emptyItem.className = 'scw-research-empty'; emptyItem.textContent = 'No sources in the reading queue. Capture a source after the question is clear; provenance stays attached to the source object.'; researchReadingList.appendChild(emptyItem);
      } else {
      ...
      if (!research.claims.length) {
        const emptyItem = document.createElement('div'); emptyItem.className = 'scw-research-empty'; emptyItem.textContent = 'No research claims yet. Create a claim only after there is evidence to connect, support, contest, or revise it.'; researchClaimList.appendChild(emptyItem);
      } else {
    ...
    function publicationPackage(project,draft){ const refs=new Set(draft.objectIds); draft.sections.forEach(sec=>sec.objectIds.forEach(v=>refs.add(v))); return {schema:PUBLICATION_EXPORT_SCHEMA,workspaceVersion:rootVersion(),exportedAt:nowIso(),automaticPublication:false,project:{id:project.id,title:project.title},draft:JSON.parse(JSON.stringify(draft)),referencedObjects:project.objects.filter(o=>refs.has(o.id)).map(o=>({id:o.id,type:o.type,title:o.title,summary:o.summary,status:o.status,tags:o.tags,provenance:o.provenance}))}; }

function researchTemplateApi(){ return window.SCWorkspaceResearchTemplates||null; }
function loadResearchTemplateLibrary(){ const api=researchTemplateApi();if(!api)return{schema:'sc-workspace-research-template-library/1.0',customTemplates:[],updatedAt:nowIso()};try{return api.normalizeLibrary(JSON.parse(localStorage.getItem(api.STORAGE_KEY)||'{"customTemplates":[]}'));}catch(_){return api.normalizeLibrary({customTemplates:[]});} }
let researchTemplateLibrary=loadResearchTemplateLibrary();
function saveResearchTemplateLibrary(){const api=researchTemplateApi();if(!api)return false;researchTemplateLibrary=api.normalizeLibrary(researchTemplateLibrary);try{localStorage.setItem(api.STORAGE_KEY,JSON.stringify(researchTemplateLibrary));return true;}catch(_){return false;}}
function selectedResearchTemplate(){const api=researchTemplateApi();if(!api)return null;const idValue=researchTemplateSelect?.value||'';return api.allTemplates(researchTemplateLibrary).find(t=>t.id===idValue)||null;}
function researchTemplateOptions(){return{projectTitle:String(researchTemplateProjectTitle?.value||'').trim(),includeWorkflow:researchTemplateIncludeWorkflow?.checked!==false,includeNotebook:researchTemplateIncludeNotebook?.checked!==false,includeStarterObjects:Boolean(researchTemplateIncludeObjects?.checked)};}
function installResearchTemplate(project,template,options){const api=researchTemplateApi(),plan=api?.plan(template,options);if(!project||!plan)return{ok:false,message:'Template could not be prepared.'};let workflows=0,notebooks=0,objects=0;
  if(plan.workflow&&project.guidedWorkflows&&project.guidedWorkflows.runs.length<MAX_WORKFLOW_RUNS){const stamp=nowIso(),steps=plan.workflow.steps.slice(0,MAX_WORKFLOW_STEPS).map((s,index)=>({id:id('ws'),key:s.key,title:s.title,description:s.description,mode:s.mode,status:index===0?'in-progress':'todo',note:'',objectIds:[],createdAt:stamp,updatedAt:stamp,completedAt:null}));const run={id:id('wr'),templateId:plan.workflow.templateId,title:plan.workflow.title,status:'active',currentStepId:steps[0]?.id||null,steps,createdAt:stamp,updatedAt:stamp,completedAt:null};project.guidedWorkflows.runs.unshift(run);project.guidedWorkflows.activeRunId=run.id;touchGuidedWorkflows(project);workflows=1;}
  if(plan.notebook){const helper=notebookHelper();if(helper&&project.notebooks.notebooks.length<helper.MAX_NOTEBOOKS){const n=helper.createNotebook(plan.notebook.title,template.description||'',id,nowIso);n.sections=plan.notebook.sections.slice(0,helper.MAX_SECTIONS).map(title=>helper.createSection(title,id,nowIso));n.activeSectionId=n.sections[0]?.id||null;project.notebooks.notebooks.unshift(n);project.notebooks.activeNotebookId=n.id;project.notebooks.updatedAt=nowIso();notebooks=1;}}
  plan.starterObjects.forEach(spec=>{if(project.objects.length>=MAX_OBJECTS)return;const o=objectTemplate(spec.type,spec.title);project.objects.unshift(o);project.activeObjectId=o.id;objects+=1;});
  project.updatedAt=nowIso();addActivity(project,'template-instantiated',`Research template instantiated: ${template.title} · ${workflows} workflow · ${notebooks} notebook · ${objects} starter object(s)`);return{ok:true,workflows,notebooks,objects,message:`${template.title} applied: ${workflows} workflow · ${notebooks} notebook · ${objects} starter object(s).`};}
function renderResearchTemplates(project){const api=researchTemplateApi();if(!api||!researchTemplateSelect)return;const templates=api.allTemplates(researchTemplateLibrary),current=researchTemplateSelect.value;researchTemplateSelect.innerHTML='';templates.forEach(t=>{const o=document.createElement('option');o.value=t.id;o.textContent=`${t.builtIn?'Built-in':'Custom'} · ${t.title}`;researchTemplateSelect.appendChild(o);});if(templates.some(t=>t.id===current))researchTemplateSelect.value=current;const chosen=selectedResearchTemplate()||templates[0];if(chosen&&researchTemplateSelect.value!==chosen.id)researchTemplateSelect.value=chosen.id;if(researchTemplatePreview){researchTemplatePreview.innerHTML='';if(chosen){const h=document.createElement('strong');h.textContent=chosen.title;const p=document.createElement('p');p.textContent=chosen.description;const m=document.createElement('small');m.textContent=`${chosen.category} · ${chosen.steps.length} workflow steps · ${chosen.notebook.sections.length} notebook sections · ${chosen.starterObjects.length} optional starter object(s)`;researchTemplatePreview.append(h,p,m);}}
  if(researchTemplateProjectTitle&&!researchTemplateProjectTitle.value&&chosen)researchTemplateProjectTitle.placeholder=`${chosen.title} Project`;
  if(researchTemplateCustomList){researchTemplateCustomList.innerHTML='';const custom=researchTemplateLibrary.customTemplates;if(!custom.length)researchTemplateCustomList.innerHTML='<div class="scw-workflow-empty">No custom reusable templates saved. Saving an active workflow captures structure only—never notes, object contents, evidence, findings, or completion state.</div>';custom.forEach(t=>{const row=document.createElement('article');row.className='scw-research-template-custom-row';const body=document.createElement('div');body.innerHTML=`<strong>${escapeHtml(t.title)}</strong><p>${escapeHtml(t.description)}</p><small>${t.steps.length} steps · ${t.notebook.sections.length} notebook sections</small>`;const actions=document.createElement('div');actions.className='scw-workflow-actions';const use=document.createElement('button');use.type='button';use.className='scw-card-action';use.textContent='Select';use.addEventListener('click',()=>{researchTemplateSelect.value=t.id;renderResearchTemplates(project);});const remove=document.createElement('button');remove.type='button';remove.className='scw-card-action scw-card-action-muted';remove.textContent='Delete template';remove.addEventListener('click',()=>{if(!window.confirm(`Delete reusable template “${t.title}”? No project content will be deleted.`))return;researchTemplateLibrary=api.removeCustom(researchTemplateLibrary,t.id).library;saveResearchTemplateLibrary();renderResearchTemplates(project);});actions.append(use,remove);row.append(body,actions);researchTemplateCustomList.appendChild(row);});}
}
    ...
    function renderGuidedWorkflows(project){
      renderResearchTemplates(project);
    ...
    function renderChangeReview(review=null){
      ...
      if(changeReconcile)changeReconcile.disabled=!r||!r.summary.total;
    }
    ...
    function reconciliationPoint(idValue){return state.versionHistory?.restorePoints.find(point=>point.id===idValue)||null;}
    function populateReconciliationSelectors(preferredProject=''){
      if(!reconcileProject||!reconcileBase||!reconcileTarget)return;
      state.reconciliation=normalizeReconciliation(state.reconciliation,state.projects);
      const current=preferredProject||reconcileProject.value||activeProject()?.id||'';
      reconcileProject.innerHTML='<option value="">Choose project</option>';state.projects.filter(p=>!p.archivedAt).forEach(project=>{const o=document.createElement('option');o.value=project.id;o.textContent=project.title;reconcileProject.appendChild(o);});if(state.projects.some(p=>p.id===current&&!p.archivedAt))reconcileProject.value=current;
      const projectId=reconcileProject.value,points=(state.versionHistory?.restorePoints||[]).filter(point=>point.projectId===projectId).sort((a,b)=>Date.parse(b.createdAt)-Date.parse(a.createdAt));
      const baseValue=reconcileBase.value,targetValue=reconcileTarget.value||'current';reconcileBase.innerHTML='<option value="">Choose a restore point</option>';points.forEach(point=>{const o=document.createElement('option');o.value=point.id;o.textContent=`${point.label} · ${formatTime(point.createdAt)}`;reconcileBase.appendChild(o);});reconcileBase.disabled=!projectId||!points.length;if(points.some(p=>p.id===baseValue))reconcileBase.value=baseValue;
      reconcileTarget.innerHTML='<option value="current">Current project</option>';points.forEach(point=>{const o=document.createElement('option');o.value=point.id;o.textContent=`Restore: ${point.label}`;reconcileTarget.appendChild(o);});reconcileTarget.disabled=!projectId||!points.length;if(targetValue==='current'||points.some(p=>p.id===targetValue))reconcileTarget.value=targetValue;
      if(reconcileLoad)reconcileLoad.disabled=!projectId||!reconcileBase.value||reconcileBase.value===reconcileTarget.value;
    }
    function reconciliationSources(){const project=state.projects.find(p=>p.id===reconcileProject?.value)||null,basePoint=reconciliationPoint(reconcileBase?.value||'');if(!project||!basePoint)return null;let target=project,targetMeta={kind:'current-project',id:project.id,label:'Current project',updatedAt:project.updatedAt};if(reconcileTarget?.value&&reconcileTarget.value!=='current'){const point=reconciliationPoint(reconcileTarget.value);if(!point)return null;target=point.snapshot;targetMeta={kind:'restore-point',id:point.id,restorePointId:point.id,label:point.label,capturedAt:point.createdAt,fingerprint:point.fingerprint};}return {project,base:basePoint.snapshot,baseMeta:{kind:'restore-point',id:basePoint.id,restorePointId:basePoint.id,label:basePoint.label,capturedAt:basePoint.createdAt,fingerprint:basePoint.fingerprint},target,targetMeta};}
    function buildActiveReconciliation(){const src=reconciliationSources(),diff=window.SCWorkspaceProjectDiff,helper=window.SCWorkspaceReconciliation;if(!src||!diff||!helper)return null;const review=diff.compareProjects(src.base,src.target,{base:src.baseMeta,target:src.targetMeta});activeReconciliation={...src,review,selected:new Set(),plan:null,result:null};return activeReconciliation;}
    function refreshReconciliationPlan(){const ctx=activeReconciliation,helper=window.SCWorkspaceReconciliation;if(!ctx||!helper)return;const keys=[...ctx.selected];ctx.plan=helper.buildPlan(ctx.review,keys,{id:id('rcp'),projectId:ctx.project.id,projectTitle:ctx.project.title,base:ctx.baseMeta,target:ctx.targetMeta,createdAt:nowIso()});ctx.result=keys.length?helper.applyPlan(ctx.base,ctx.target,ctx.plan):null;if(reconcileMetricAvailable)reconcileMetricAvailable.textContent=String(ctx.review.summary.total);if(reconcileMetricSelected)reconcileMetricSelected.textContent=String(keys.length);if(reconcileMetricBlockers)reconcileMetricBlockers.textContent=String(ctx.result?.blockers?.length||0);if(reconcileAck){reconcileAck.disabled=!keys.length||Boolean(ctx.result&&!ctx.result.canCreate);reconcileAck.checked=false;}if(reconcileCreate)reconcileCreate.disabled=true;if(reconcileExport)reconcileExport.disabled=!keys.length;
      if(reconcilePreview){if(!keys.length)reconcilePreview.innerHTML='<div class="scw-reconciliation-empty">Select one or more changes to build a preview.</div>';else if(ctx.result?.canCreate)reconcilePreview.innerHTML=`<div class="scw-reconciliation-preview-ok"><strong>${escapeHtml(keys.length)} selected change${keys.length===1?'':'s'} ready</strong><span>No blocking object/relationship dependency was detected. The result will be cloned to a new local project with fresh local IDs.</span></div>`;else{const errors=[...(ctx.result?.errors||[]),...(ctx.result?.blockers||[]).map(x=>x.message)];reconcilePreview.innerHTML=`<div class="scw-reconciliation-preview-blocked"><strong>${escapeHtml(errors.length)} blocking issue${errors.length===1?'':'s'}</strong><ul>${errors.slice(0,12).map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul><span>Select the related dependency changes or clear the incompatible removal before creating a copy.</span></div>`;}}
    }
    function renderReconciliation(){if(!reconciliationSection)return;state.reconciliation=normalizeReconciliation(state.reconciliation,state.projects);populateReconciliationSelectors();if(reconcileMetricCopies)reconcileMetricCopies.textContent=String(state.reconciliation.history.length);const ctx=activeReconciliation;if(reconcileList){reconcileList.innerHTML='';if(!ctx){reconcileList.innerHTML='<div class="scw-reconciliation-empty">No comparison loaded yet.</div>';}else if(!ctx.review.summary.total){reconcileList.innerHTML='<div class="scw-reconciliation-empty">No meaningful differences are available to reconcile.</div>';}else{ctx.review.categories.forEach(category=>{const group=document.createElement('section');group.className='scw-reconciliation-category';const head=document.createElement('div');head.className='scw-reconciliation-category-head';head.innerHTML=`<strong>${escapeHtml(category.category)}</strong><span>${category.changes.length} change${category.changes.length===1?'':'s'}</span>`;const list=document.createElement('div');category.changes.forEach(change=>{const k=window.SCWorkspaceReconciliation.key(category.category,change),label=document.createElement('label');label.className=`scw-reconciliation-row is-${change.change}`;const input=document.createElement('input');input.type='checkbox';input.checked=ctx.selected.has(k);input.addEventListener('change',()=>{if(input.checked)ctx.selected.add(k);else ctx.selected.delete(k);refreshReconciliationPlan();});const body=document.createElement('span');body.innerHTML=`<em>${escapeHtml(change.change.toUpperCase())}</em><strong>${escapeHtml(change.label)}</strong><small>${change.fields?.length?`Fields: ${escapeHtml(change.fields.join(', '))}`:'Record '+escapeHtml(change.change)}</small>`;label.append(input,body);list.appendChild(label);});group.append(head,list);reconcileList.appendChild(group);});}}
      if(reconcileSelectAll)reconcileSelectAll.disabled=!ctx||!ctx.review.summary.total;if(reconcileClear)reconcileClear.disabled=!ctx||!ctx.review.summary.total;if(reconcileStatus){if(!ctx)reconcileStatus.textContent='Choose a project and base restore point to begin.';else reconcileStatus.textContent=`${ctx.review.summary.total} explicit changes available. Nothing is selected automatically.`;}refreshReconciliationPlan();if(reconcileHistory){reconcileHistory.innerHTML='';const rows=state.reconciliation.history;if(!rows.length)reconcileHistory.innerHTML='<div class="scw-reconciliation-empty">No reconciled copies have been created yet.</div>';rows.slice(0,24).forEach(row=>{const el=document.createElement('article');el.className='scw-reconciliation-history-row';el.innerHTML=`<span>${escapeHtml(row.selectedChangeCount)} OF ${escapeHtml(row.availableChangeCount)} CHANGES</span><strong>${escapeHtml(row.outputProjectTitle)}</strong><small>${escapeHtml(row.baseLabel)} → ${escapeHtml(row.targetLabel)} · ${escapeHtml(formatTime(row.at))}</small>`;reconcileHistory.appendChild(el);});}renderReconciliationReceipts();}
    function recordReconciliation(ctx,copy,receipt){state.reconciliation=normalizeReconciliation(state.reconciliation,state.projects);state.reconciliation.history.unshift({schema:RECONCILIATION_SCHEMA,id:id('rch'),receiptId:receipt?.id||'',reviewerLabel:receipt?.reviewerLabel||'',sourceProjectId:ctx.project.id,sourceProjectTitle:ctx.project.title,baseLabel:ctx.baseMeta.label,targetLabel:ctx.targetMeta.label,availableChangeCount:ctx.review.summary.total,selectedChangeCount:ctx.plan.selectedChangeCount,blockerCount:0,outputProjectId:copy.id,outputProjectTitle:copy.title,at:receipt?.createdAt||nowIso()});state.reconciliation.history=state.reconciliation.history.slice(0,MAX_RECONCILIATION_HISTORY);if(receipt){state.reconciliation.receipts.unshift(receipt);state.reconciliation.receipts=state.reconciliation.receipts.slice(0,MAX_RECONCILIATION_RECEIPTS);}state.reconciliation.updatedAt=nowIso();}
    function renderReconciliationReceipts(){if(!reconcileReceipts)return;state.reconciliation=normalizeReconciliation(state.reconciliation,state.projects);reconcileReceipts.innerHTML='';const rows=state.reconciliation.receipts||[];if(!rows.length){reconcileReceipts.innerHTML='<div class="scw-reconciliation-empty">No decision receipts have been created yet.</div>';return;}rows.slice(0,24).forEach(receipt=>{const el=document.createElement('article');el.className='scw-reconciliation-receipt-row';const d=receipt.decision||{};el.innerHTML=`<div><span>RECEIPT</span><strong>${escapeHtml(receipt.outputProjectTitle||'Reconciled project')}</strong><p>${escapeHtml(receipt.reviewerLabel||'Workspace owner')} · ${escapeHtml(d.acceptedCount||0)} accepted · ${escapeHtml(d.declinedCount||0)} declined</p><small>${escapeHtml(receipt.base?.label||'Base')} → ${escapeHtml(receipt.target?.label||'Target')} · ${escapeHtml(formatTime(receipt.createdAt))}</small><blockquote>${escapeHtml(receipt.rationale||'No rationale recorded.')}</blockquote></div>`;const actions=document.createElement('div');actions.className='scw-reconciliation-receipt-actions';const exportBtn=document.createElement('button');exportBtn.type='button';exportBtn.className='scw-button';exportBtn.textContent='Export receipt';exportBtn.addEventListener('click',()=>{const h=window.SCWorkspaceReconciliationReceipt;if(!h)return;downloadJson(`${safeFileName(receipt.outputProjectTitle)}-reconciliation-receipt.json`,h.exportPackage(receipt));});const verifyBtn=document.createElement('button');verifyBtn.type='button';verifyBtn.className='scw-button';verifyBtn.textContent='Verify';verifyBtn.addEventListener('click',async()=>{const h=window.SCWorkspaceReconciliationReceipt;if(!h)return;const fp=await sha256Text(h.fingerprintPayload(receipt));window.alert(fp&&fp===receipt.integrity?.fingerprint?'Receipt integrity verified.':'Receipt fingerprint does not match its recorded contents.');});actions.append(exportBtn,verifyBtn);el.append(actions);reconcileReceipts.appendChild(el);});}
    ...
    function renderProjectLifecycle(){
      ...
      const helper=window.SCWorkspaceProjectLifecycle;
      const projects=state.projects.filter(project=>!project.archivedAt).sort(projectSort);
      if(lifecycleProject){const current=lifecycleProject.value||activeProject()?.id||'';lifecycleProject.innerHTML='<option value="">Choose project</option>';projects.forEach(project=>{const option=document.createElement('option');option.value=project.id;option.textContent=project.title;lifecycleProject.appendChild(option);});if(projects.some(project=>project.id===current))lifecycleProject.value=current;}
      const project=state.projects.find(item=>item.id===(lifecycleProject?.value||''))||null;
      if(!project){if(lifecycleCurrent)lifecycleCurrent.textContent='Draft';if(lifecycleCurrentMeta)lifecycleCurrentMeta.textContent='Choose a project to inspect its lifecycle.';if(lifecycleStages)lifecycleStages.innerHTML='';if(lifecycleReadiness)lifecycleReadiness.innerHTML='<div class="scw-project-lifecycle-empty">Choose a project to inspect lifecycle readiness.</div>';if(lifecycleHistory)lifecycleHistory.innerHTML='<div class="scw-project-lifecycle-empty">No project selected.</div>';if(lifecycleForm)Array.from(lifecycleForm.elements).forEach(el=>el.disabled=true);return;}
      project.lifecycle=normalizeLifecycle(project.lifecycle);
      if(lifecycleForm)Array.from(lifecycleForm.elements).forEach(el=>el.disabled=false);
      if(lifecycleCurrent)lifecycleCurrent.textContent=lifecycleLabel(project.lifecycle.state);
      if(lifecycleCurrentMeta){const latest=project.lifecycle.milestones[0];lifecycleCurrentMeta.textContent=latest?`Declared ${formatTime(latest.declaredAt)} · ${latest.readiness?.metCount||0}/${latest.readiness?.totalCount||0} visible conditions met`:'Default state; no explicit milestone declaration yet.';}
      const stages=helper?.STAGES||[];
      if(lifecycleStages){lifecycleStages.innerHTML='';stages.forEach((stage,index)=>{const assessment=lifecycleAssessment(project,stage.key),card=document.createElement('button');card.type='button';card.className=`scw-project-lifecycle-stage${project.lifecycle.state===stage.key?' is-current':''}${lifecycleTarget?.value===stage.key?' is-target':''}`;card.innerHTML=`<span>${String(index+1).padStart(2,'0')} / ${escapeHtml(stage.label.toUpperCase())}</span><strong>${escapeHtml(stage.label)}</strong><small>${assessment.metCount}/${assessment.totalCount} conditions met${assessment.ready?' · ready conditions visible':' · attention remains'}</small>`;card.addEventListener('click',()=>{if(lifecycleTarget)lifecycleTarget.value=stage.key;renderProjectLifecycle();});lifecycleStages.appendChild(card);});}
      const target=LIFECYCLE_STAGES.has(lifecycleTarget?.value)?lifecycleTarget.value:project.lifecycle.state;
      if(lifecycleTarget&&!LIFECYCLE_STAGES.has(lifecycleTarget.value))lifecycleTarget.value=target;
      const assessment=lifecycleAssessment(project,target);
      if(lifecycleReadiness){lifecycleReadiness.innerHTML='';const summary=document.createElement('div');summary.className=`scw-project-lifecycle-readiness-summary${assessment.ready?' is-ready':''}`;summary.innerHTML=`<strong>${assessment.ready?'All listed conditions are currently met':'Readiness conditions need review'}</strong><span>${assessment.metCount} of ${assessment.totalCount} visible condition(s) met for ${escapeHtml(lifecycleLabel(target))}.</span><small>This is a checklist, not a score or certification.</small>`;lifecycleReadiness.appendChild(summary);assessment.checks.forEach(item=>{const row=document.createElement('article');row.className=`scw-project-lifecycle-check${item.met?' is-met':' is-unmet'}`;row.innerHTML=`<span>${item.met?'MET':'NEEDS ATTENTION'} · ${escapeHtml(item.source||'Workspace')}</span><strong>${escapeHtml(item.label)}</strong><p>${escapeHtml(item.detail||'')}</p>`;lifecycleReadiness.appendChild(row);});}
      if(lifecycleStatus)lifecycleStatus.textContent=assessment.ready?'Readiness conditions are currently satisfied. Workspace will still wait for an explicit human declaration.':`${assessment.unmetCount} condition(s) remain unmet. You may still declare the milestone after reviewing them and recording your rationale.`;
      if(lifecycleHistory){lifecycleHistory.innerHTML='';if(!project.lifecycle.milestones.length)lifecycleHistory.innerHTML='<div class="scw-project-lifecycle-empty">No lifecycle milestones have been recorded yet.</div>';project.lifecycle.milestones.forEach(milestone=>{const row=document.createElement('article');row.className='scw-project-lifecycle-event';row.innerHTML=`<div><span>${escapeHtml(lifecycleLabel(milestone.fromState))} → ${escapeHtml(lifecycleLabel(milestone.toState))}</span><strong>${escapeHtml(lifecycleLabel(milestone.toState))}</strong><p>${escapeHtml(milestone.rationale||'No rationale recorded.')}</p><small>${escapeHtml(formatTime(milestone.declaredAt))} · ${Number(milestone.readiness?.metCount||0)}/${Number(milestone.readiness?.totalCount||0)} conditions met at declaration${milestone.readiness?.ready?'':' · unmet conditions retained in receipt'}</small></div>`;lifecycleHistory.appendChild(row);});}
    }
    ...
    function renderPublicBeta() {
      ...
      const homeApi = window.SCWorkspaceHome;
      if (homeApi) {
        const home = homeApi.summarize(state);
        const project = home.activeProject;
        const next = homeApi.nextAction(project);
        if (cockpitProjects) cockpitProjects.textContent = String(home.projectCount);
        if (cockpitObjects) cockpitObjects.textContent = String(home.objectCount);
        if (cockpitResearch) cockpitResearch.textContent = String(home.researchCount);
        if (cockpitNotebooks) cockpitNotebooks.textContent = String(home.notebookCount);
        if (cockpitTitle) cockpitTitle.textContent = project ? project.title : 'None selected';
        if (cockpitUpdated) cockpitUpdated.textContent = project?.updatedAt ? `Updated ${formatTime(project.updatedAt)}` : 'Local workspace';
        if (cockpitSummary) cockpitSummary.textContent = project ? (project.description || `Continue ${project.title} or move directly into the work mode you need.`) : 'No active project yet. Create or open a project to populate the cockpit.';
        if (cockpitNextTitle) cockpitNextTitle.textContent = next.title;
        if (cockpitNextDetail) cockpitNextDetail.textContent = next.detail;
        if (cockpitOpenProject) { cockpitOpenProject.disabled = !project; cockpitOpenProject.dataset.projectId = project?.id || ''; }
        syncCockpitModeCards(project);
      }
      ...
      const onboarding = window.SCWorkspaceFirstRunOnboarding;
      if (firstRunPanel && onboarding) {
        const env = onboarding.environment({projectCount:summary.projectCount,activeProject:Boolean(activeProject()),storageWritable:Boolean(caps.localStorage),authenticated:Boolean(window.SCWorkspaceIdentity?.authenticated)});
        firstRunPanel.hidden = !env.firstRun;
        firstRunPanel.setAttribute('aria-hidden', env.firstRun ? 'false' : 'true');
        if (firstRunBoundary) firstRunBoundary.textContent = onboarding.boundary(Boolean(window.SCWorkspaceIdentity?.authenticated));
        if (firstRunStatus) {
          firstRunStatus.textContent = env.firstRun && !env.storageWritable
            ? 'Browser-local storage is unavailable or blocked. Resolve storage access before creating a first project you intend to keep.'
            : '';
        }
      }
    ...
    function notebookSelectedProject() {
      const selectedId = String(notebookProject?.value || state.activeProjectId || '');
      return state.projects.find(project => project.id === selectedId && !project.archivedAt) || activeProject() || null;
    }

    function notebookAllBlocks(project) {
      if (!project) return [];
      project.notebooks = normalizeNotebooks(project.notebooks, project.objects, project.canvas);
    ...
    function notebookRefOptions(project) {
      ...
      project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas); const out=[];
    ...
    function renderNotebookSynthesis(project) {
      const helper=notebookHelper();
      if(!helper||!project)return;
      project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);
      const ws=project.notebooks, syntheses=Array.isArray(ws.syntheses)?ws.syntheses:[];
      if(notebookMetricSyntheses)notebookMetricSyntheses.textContent=String(syntheses.length);
      if(notebookSynthesisMaterial){const selected=new Set([...notebookSynthesisMaterial.selectedOptions].map(o=>o.value));notebookSynthesisMaterial.innerHTML='';notebookRefOptions(project).filter(item=>item.ref.kind==='block'||item.ref.kind==='object').forEach(item=>{const o=document.createElement('option');o.value=notebookRefValue(item.ref);o.textContent=item.label;o.selected=selected.has(o.value);notebookSynthesisMaterial.appendChild(o);});}
      if(!notebookSynthesisList)return;notebookSynthesisList.innerHTML='';
      if(!syntheses.length){notebookSynthesisList.innerHTML='<div class="scw-notebook-empty">No synthesis outputs yet. Select research material above to create an outline, citation pack, source matrix, evidence summary, or synthesis draft.</div>';return;}
      syntheses.forEach(item=>{const card=document.createElement('article');card.className='scw-notebook-synthesis-card';const head=document.createElement('div');head.className='scw-notebook-synthesis-card-head';const body=document.createElement('div');body.innerHTML=`<span>${escapeHtml(String(item.kind||'synthesis').replaceAll('-',' ').toUpperCase())}</span><strong>${escapeHtml(item.title||'Notebook synthesis')}</strong><small>${item.selectedRefs?.length||0} selected item${(item.selectedRefs?.length||0)===1?'':'s'} · ${item.citations?.length||0} carried citation${(item.citations?.length||0)===1?'':'s'} · ${escapeHtml(formatTime(item.updatedAt||item.createdAt))}</small>`;const actions=document.createElement('div');actions.className='scw-notebook-block-actions';const exp=document.createElement('button');exp.type='button';exp.className='scw-card-action';exp.textContent='Export';exp.addEventListener('click',()=>{const pkg=helper.exportSynthesis?.(item,project,rootVersion(),project.notebooks,project.objects);if(!pkg)return;downloadJson(`${safeFileName(project.title)}-${safeFileName(item.title)}.sc-workspace-synthesis.json`,pkg);addActivity(project,'notebook-synthesis-export',`Synthesis exported: ${item.title}`);persist('Notebook synthesis export recorded');});const doc=document.createElement('button');doc.type='button';doc.className='scw-card-action';doc.textContent='Create Document';doc.addEventListener('click',()=>{if(project.objects.length>=MAX_OBJECTS){window.alert(`This project has reached the ${MAX_OBJECTS}-object local limit.`);return;}const object=objectTemplate('document',item.title||'Notebook synthesis');object.summary=`${String(item.kind||'research-synthesis').replaceAll('-',' ')} assembled from ${item.selectedRefs?.length||0} explicitly selected research item(s).`;object.content=String(item.content||'').slice(0,50000);object.tags=['notebook-synthesis',String(item.kind||'synthesis')];object.status='working';object.provenance.sourceType='manual';object.provenance.sourceTitle='Research Notebook synthesis';object.provenance.capturedAt=nowIso();project.objects.push(object);project.activeObjectId=object.id;addActivity(project,'notebook-synthesis-document',`Synthesis materialized as Document: ${item.title}`);persist('Notebook synthesis materialized');render();setWorkspaceView('notebook');});const remove=document.createElement('button');remove.type='button';remove.className='scw-card-action scw-card-action-muted';remove.textContent='Remove';remove.addEventListener('click',()=>{if(!window.confirm(`Remove synthesis “${item.title}”? Source notes and any Documents created from it will remain.`))return;ws.syntheses=ws.syntheses.filter(s=>s.id!==item.id);ws.activeSynthesisId=ws.syntheses[0]?.id||null;touchNotebooks(project);persist('Notebook synthesis removed');renderNotebook();});actions.append(exp,doc,remove);head.append(body,actions);const preview=document.createElement('pre');preview.className='scw-notebook-synthesis-preview';preview.textContent=String(item.content||'').slice(0,6000);card.append(head,preview);notebookSynthesisList.appendChild(card);});
    }


    function consumeNotebookAssistanceAdapterResponse(project) {
      if(!project||!window.sessionStorage)return;
      let raw='';try{raw=window.sessionStorage.getItem(NOTEBOOK_ASSISTANCE_RESPONSE_KEY)||'';}catch(_){return;}
      if(!raw)return;
      try{
        const packet=JSON.parse(raw),helper=notebookHelper();
        if(!packet||packet.schema!==NOTEBOOK_ASSISTANCE_RESPONSE_EXPORT_SCHEMA||String(packet.project?.id||'')!==project.id)return;
        const item=(project.notebooks.assistances||[]).find(a=>a.id===String(packet.requestId||''));
        if(!item||!helper?.applyAssistanceResponse)return;
        const applied=helper.applyAssistanceResponse(item,packet.response,packet.responseSource||'adapter',id,nowIso);
        if(applied.ok){const index=project.notebooks.assistances.findIndex(a=>a.id===item.id);project.notebooks.assistances[index]=applied.record;project.notebooks.activeAssistanceId=applied.record.id;touchNotebooks(project);addActivity(project,'notebook-assistance-response','Grounded Notebook Assistance response returned for review');persist('Notebook assistance response returned');}
        window.sessionStorage.removeItem(NOTEBOOK_ASSISTANCE_RESPONSE_KEY);
      }catch(_){try{window.sessionStorage.removeItem(NOTEBOOK_ASSISTANCE_RESPONSE_KEY);}catch(__){}}
    }

    function renderNotebookAssistance(project) {
      const helper=notebookHelper();if(!helper||!project)return;
      project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);consumeNotebookAssistanceAdapterResponse(project);
      const ws=project.notebooks,items=Array.isArray(ws.assistances)?ws.assistances:[];
      if(notebookMetricAssistances)notebookMetricAssistances.textContent=String(items.length);
      if(notebookAssistanceMaterial){const selected=new Set([...notebookAssistanceMaterial.selectedOptions].map(o=>o.value));notebookAssistanceMaterial.innerHTML='';notebookRefOptions(project).filter(item=>item.ref.kind==='block'||item.ref.kind==='object').forEach(item=>{const o=document.createElement('option');o.value=notebookRefValue(item.ref);o.textContent=item.label;o.selected=selected.has(o.value);notebookAssistanceMaterial.appendChild(o);});}
      if(!notebookAssistanceList)return;notebookAssistanceList.innerHTML='';
      if(!items.length){notebookAssistanceList.innerHTML='<div class="scw-notebook-empty">No grounded Notebook questions yet. Select local research material and prepare a question above.</div>';return;}
      items.forEach(item=>{
        const card=document.createElement('article');card.className='scw-notebook-assistance-card';
        const head=document.createElement('div');head.className='scw-notebook-assistance-card-head';
        const body=document.createElement('div');body.innerHTML=`<span>${escapeHtml(String(item.status||'prepared').replaceAll('-',' ').toUpperCase())}</span><strong>${escapeHtml(item.title||'Grounded notebook question')}</strong><small>${item.selectedRefs?.length||0} selected item${(item.selectedRefs?.length||0)===1?'':'s'} · ${item.citations?.length||0} cited material${(item.citations?.length||0)===1?'':'s'} · ${escapeHtml(formatTime(item.updatedAt||item.createdAt))}</small><p>${escapeHtml(item.question||'')}</p>`;
        const actions=document.createElement('div');actions.className='scw-notebook-block-actions';
        const copy=document.createElement('button');copy.type='button';copy.className='scw-card-action';copy.textContent='Copy grounded prompt';copy.addEventListener('click',async()=>{const prompt=helper.assistancePrompt?.(item)||'';if(!prompt)return;try{await navigator.clipboard.writeText(prompt);copy.textContent='Copied';setTimeout(()=>copy.textContent='Copy grounded prompt',1200);}catch(_){window.prompt('Copy grounded prompt',prompt);}});
        const expReq=document.createElement('button');expReq.type='button';expReq.className='scw-card-action';expReq.textContent='Export request';expReq.addEventListener('click',()=>{const pkg=helper.exportAssistanceRequest?.(item,project,rootVersion(),project.notebooks,project.objects);if(!pkg)return;downloadJson(`${safeFileName(project.title)}-${safeFileName(item.title)}.sc-workspace-notebook-assistance-request.json`,pkg);addActivity(project,'notebook-assistance-export','Grounded Notebook Assistance request exported');persist('Notebook assistance request export recorded');});
        const open=document.createElement('button');open.type='button';open.className='scw-card-action';open.textContent='Open Research Librarian';open.addEventListener('click',()=>{const pkg=helper.exportAssistanceRequest?.(item,project,rootVersion(),project.notebooks,project.objects);if(!pkg)return;try{window.sessionStorage.setItem(NOTEBOOK_ASSISTANCE_REQUEST_KEY,JSON.stringify(pkg));}catch(_){}const prompt=helper.assistancePrompt?.(item)||'';try{navigator.clipboard?.writeText(prompt);}catch(_){}window.open('/research-librarian/','_blank');item.status='prepared';item.updatedAt=nowIso();touchNotebooks(project);addActivity(project,'notebook-assistance-open','Grounded Notebook Assistance request opened in Research Librarian');persist('Notebook assistance request prepared for Research Librarian');});
        const remove=document.createElement('button');remove.type='button';remove.className='scw-card-action scw-card-action-muted';remove.textContent='Remove';remove.addEventListener('click',()=>{if(!window.confirm(`Remove grounded question “${item.title}”? Selected research and any Documents created from the answer will remain.`))return;ws.assistances=ws.assistances.filter(a=>a.id!==item.id);ws.activeAssistanceId=ws.assistances[0]?.id||null;touchNotebooks(project);persist('Notebook assistance removed');renderNotebook();});
        actions.append(copy,expReq,open,remove);head.append(body,actions);card.appendChild(head);
        const materials=document.createElement('div');materials.className='scw-notebook-assistance-materials';item.entries?.forEach(entry=>{const row=document.createElement('div');row.innerHTML=`<span>[${entry.number}]</span><strong>${escapeHtml(entry.label||entry.type||'Selected material')}</strong><small>${escapeHtml(entry.citation||'Citation metadata not recorded; the numbered reference still resolves to this selected local item.')}</small>`;materials.appendChild(row);});card.appendChild(materials);
        const review=document.createElement('div');review.className='scw-notebook-assistance-review';const label=document.createElement('label');label.innerHTML='<span>Reviewable answer draft</span>';const textarea=document.createElement('textarea');textarea.rows=7;textarea.maxLength=30000;textarea.placeholder='Paste or return an answer that cites selected material with [1], [2], etc. Answers without valid citation markers are rejected.';textarea.value=item.response||'';label.appendChild(textarea);review.appendChild(label);
        const status=document.createElement('div');status.className='scw-notebook-assistance-status';status.textContent=item.response?`${item.citations?.length||0} citation marker${(item.citations?.length||0)===1?'':'s'} resolved to the selected grounding set.`:'No answer saved yet.';review.appendChild(status);
        const reviewActions=document.createElement('div');reviewActions.className='scw-notebook-block-actions';
        const save=document.createElement('button');save.type='button';save.className='scw-button scw-button-primary';save.textContent='Save cited draft';save.addEventListener('click',()=>{const applied=helper.applyAssistanceResponse?.(item,textarea.value,'manual',id,nowIso);if(!applied?.ok){window.alert(applied?.message||'The answer could not be validated against the selected material.');return;}const index=ws.assistances.findIndex(a=>a.id===item.id);ws.assistances[index]=applied.record;ws.activeAssistanceId=applied.record.id;touchNotebooks(project);addActivity(project,'notebook-assistance-response',`Grounded answer saved for review: ${item.title}`);persist('Cited notebook assistance draft saved');renderNotebook();});reviewActions.appendChild(save);
        if(item.response){const expResp=document.createElement('button');expResp.type='button';expResp.className='scw-card-action';expResp.textContent='Export response';expResp.addEventListener('click',()=>{const pkg=helper.exportAssistanceResponse?.(item,project,rootVersion());if(pkg)downloadJson(`${safeFileName(project.title)}-${safeFileName(item.title)}.sc-workspace-notebook-assistance-response.json`,pkg);});const accept=document.createElement('button');accept.type='button';accept.className='scw-card-action';accept.textContent='Mark reviewed';accept.addEventListener('click',()=>{item.status='accepted';item.reviewedAt=nowIso();item.updatedAt=item.reviewedAt;touchNotebooks(project);addActivity(project,'notebook-assistance-reviewed',`Grounded answer reviewed: ${item.title}`);persist('Notebook assistance draft reviewed');renderNotebook();});const doc=document.createElement('button');doc.type='button';doc.className='scw-card-action';doc.textContent='Create Document';doc.addEventListener('click',()=>{if(project.objects.length>=MAX_OBJECTS){window.alert(`This project has reached the ${MAX_OBJECTS}-object local limit.`);return;}let object=item.acceptedDocumentObjectId?objectById(project,item.acceptedDocumentObjectId):null;if(!object){object=objectTemplate('document',`${item.title} — grounded draft`);project.objects.push(object);item.acceptedDocumentObjectId=object.id;}const citationLines=(item.citations||[]).map(c=>`[${c.number}] ${c.citation||c.label}`).join('\n');object.summary=`Grounded Notebook Assistance draft based on ${item.selectedRefs?.length||0} explicitly selected research item(s).`;object.content=[String(item.response||''),citationLines?`\n\n## Grounding references\n${citationLines}`:''].join('').slice(0,50000);object.tags=normalizeTags([...object.tags,'notebook-assistance','grounded-draft']);object.status='working';object.provenance={sourceType:'tool',sourceTitle:'Grounded Notebook Assistance',sourceUrl:'',capturedAt:nowIso()};object.updatedAt=nowIso();project.activeObjectId=object.id;item.status='accepted';item.reviewedAt=nowIso();item.updatedAt=item.reviewedAt;touchNotebooks(project);addActivity(project,'notebook-assistance-document',`Grounded Notebook Assistance draft materialized as Document: ${item.title}`);persist('Grounded notebook draft materialized');render();setWorkspaceView('notebook');});const reject=document.createElement('button');reject.type='button';reject.className='scw-card-action scw-card-action-muted';reject.textContent='Reject draft';reject.addEventListener('click',()=>{item.status='rejected';item.reviewedAt=nowIso();item.updatedAt=item.reviewedAt;touchNotebooks(project);addActivity(project,'notebook-assistance-rejected',`Grounded answer rejected: ${item.title}`);persist('Notebook assistance draft rejected');renderNotebook();});reviewActions.append(expResp,accept,doc,reject);}
        review.appendChild(reviewActions);card.appendChild(review);notebookAssistanceList.appendChild(card);
      });
    }


    function notebookPortability(){return window.SCWorkspaceNotebookPortability||null;}
    function notebookPortabilityState(project){project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);project.notebooks.portability=notebookHelper()?.portabilityState?.(project.notebooks.portability)||{restorePoints:[],importHistory:[],syncEnrollments:[],updatedAt:nowIso()};return project.notebooks.portability;}
    function notebookSyncEnrollment(project,nb,create=true){const p=notebookPortabilityState(project),api=notebookPortability();let e=p.syncEnrollments.find(x=>x.notebookId===nb.id)||null;if(!e&&create&&api){e=api.enrollment(null,nb.id);p.syncEnrollments.push(e);}return e;}
    function renderNotebookPortability(project){if(!notebookRestoreList)return;const nb=activeNotebookFor(project),api=notebookPortability();if(!project||!nb||!api){notebookRestoreList.innerHTML='<div class="scw-notebook-empty">Choose a notebook to manage portability.</div>';return;}const p=notebookPortabilityState(project),points=p.restorePoints.filter(x=>x.notebookId===nb.id),e=notebookSyncEnrollment(project,nb,false);notebookRestoreList.innerHTML=points.length?'': '<div class="scw-notebook-empty">No notebook restore points yet.</div>';points.forEach(point=>{const row=document.createElement('div');row.className='scw-notebook-portability-row';row.innerHTML=`<div><strong>${escapeHtml(point.label||'Restore point')}</strong><span>${escapeHtml(formatDate(point.createdAt))}</span></div><button class="scw-button" type="button">Restore as copy</button>`;row.querySelector('button').addEventListener('click',async()=>{const result=await api.restoreAsCopy(point,id,nowIso);if(!result.ok){window.alert(result.reason||'Restore point is invalid.');return;}project.notebooks.notebooks.unshift(result.notebook);project.notebooks.activeNotebookId=result.notebook.id;touchNotebooks(project);persist('Notebook restored as copy');renderNotebook();});notebookRestoreList.appendChild(row);});if(notebookSyncToggle)notebookSyncToggle.textContent=e?.enabled?'Disable sync':'Enable sync';if(notebookSyncCheck)notebookSyncCheck.disabled=!IDENTITY_CONFIG.authenticated||!e?.enabled;if(notebookSyncNow)notebookSyncNow.disabled=!IDENTITY_CONFIG.authenticated||!e?.enabled;if(notebookBackup)notebookBackup.disabled=!IDENTITY_CONFIG.authenticated;if(notebookPortabilityStatus)notebookPortabilityStatus.textContent=e?.enabled?`Notebook sync: ${e.status||'enabled'} · revision ${e.serverRevision||0}`:'Notebook remains local unless you explicitly export, back up, or enable sync.';}
    async function notebookRemoteIndex(){const data=await cloudRequest('cloud-notebooks');return Array.isArray(data?.items)?data.items:[];}
    async function evaluateNotebookSync(project,nb){const api=notebookPortability(),e=notebookSyncEnrollment(project,nb,true);if(!e.enabled)return {status:'disabled',e};const local=await api.sha256(nb),items=await notebookRemoteIndex(),remote=items.find(x=>x.notebookId===nb.id)||null,status=api.classify(local,e,remote);e.remoteRevision=remote?.revision||0;e.remoteFingerprint=remote?.notebookFingerprint||'';e.status=status;e.lastCheckedAt=nowIso();e.updatedAt=e.lastCheckedAt;return {status,e,remote,local};}
    function notebookReviewApi(){return window.SCWorkspaceNotebookReviewProvenance||null;}
    function notebookGovernanceState(project){project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);project.notebooks.governance=notebookHelper()?.governanceState?.(project.notebooks.governance)||{schema:NOTEBOOK_GOVERNANCE_SCHEMA,reviews:[],reconciliations:[],updatedAt:nowIso()};return project.notebooks.governance;}
    function notebookReviewLatest(project,nb){const g=notebookGovernanceState(project);return g.reviews.find(r=>r.notebookId===nb?.id)||null;}
    function notebookReviewRefOptions(project,nb){const out=[];if(nb)out.push({value:`notebook:${nb.id}`,label:`Notebook · ${nb.title}`});for(const sec of nb?.sections||[])for(const b of sec.blocks||[])out.push({value:`block:${b.id}`,label:`${sec.title} · ${b.title||b.type}`});for(const o of project?.objects||[])if(!o.archivedAt)out.push({value:`object:${o.id}`,label:`Workspace ${o.type} · ${o.title}`});return out;}
    function renderNotebookLineageView(lineage){if(!notebookLineageResult)return;notebookLineageResult.innerHTML='';if(!lineage){notebookLineageResult.innerHTML='<div class="scw-notebook-empty">Choose material to inspect lineage.</div>';return;}const head=document.createElement('div');head.className='scw-notebook-review-summary';head.innerHTML=`<strong>${escapeHtml(lineage.subject?.label||lineage.subject?.id||'Selected material')}</strong><span> · ${lineage.events.length} event${lineage.events.length===1?'':'s'} · ${lineage.edges.length} relationship${lineage.edges.length===1?'':'s'}</span>`;notebookLineageResult.appendChild(head);lineage.events.slice(0,30).forEach(e=>{const row=document.createElement('div');row.className='scw-notebook-lineage-row';row.innerHTML=`<div><strong>${escapeHtml(e.label)}</strong><small>${escapeHtml(e.source)}${e.at?` · ${escapeHtml(formatTime(e.at))}`:''}</small></div>`;notebookLineageResult.appendChild(row);});if(lineage.edges.length){const edgeHead=document.createElement('div');edgeHead.className='scw-notebook-review-summary';edgeHead.innerHTML='<strong>Relationships</strong>';notebookLineageResult.appendChild(edgeHead);lineage.edges.slice(0,30).forEach(e=>{const row=document.createElement('div');row.className='scw-notebook-lineage-row';row.innerHTML=`<div><strong>${escapeHtml(e.relation)}</strong><small>${escapeHtml(e.target?.label||e.target?.id||'Referenced material')}</small></div>`;notebookLineageResult.appendChild(row);});}}
    function renderNotebookReviewProvenance(project){const api=notebookReviewApi();if(!api||!notebookReviewList||!project)return;const nb=activeNotebookFor(project);if(!nb){notebookReviewList.innerHTML='<div class="scw-notebook-empty">Choose a notebook to review.</div>';if(notebookAuditList)notebookAuditList.innerHTML='<div class="scw-notebook-empty">No notebook audit events.</div>';return;}const p=notebookPortabilityState(project),points=p.restorePoints.filter(x=>x.notebookId===nb.id);if(notebookReviewBaseline){const current=notebookReviewBaseline.value;notebookReviewBaseline.innerHTML='<option value="">Choose restore point</option>';points.forEach(point=>{const o=document.createElement('option');o.value=point.id;o.textContent=`${point.label||'Restore point'} · ${formatDate(point.createdAt)}`;notebookReviewBaseline.appendChild(o);});if(points.some(x=>x.id===current))notebookReviewBaseline.value=current;}const review=notebookReviewLatest(project,nb);if(notebookReviewSummary)notebookReviewSummary.textContent=review?`${review.summary.total} reviewed change${review.summary.total===1?'':'s'} · ${review.summary.added} added · ${review.summary.removed} removed · ${review.summary.modified} modified`:'No notebook review generated yet.';notebookReviewList.innerHTML='';if(!review)notebookReviewList.innerHTML='<div class="scw-notebook-empty">Create a review from a named restore point.</div>';else if(!review.changes.length)notebookReviewList.innerHTML='<div class="scw-notebook-empty">No differences were found between the reviewed states.</div>';else review.changes.forEach(item=>{const row=document.createElement('div');row.className='scw-notebook-review-change';row.innerHTML=`<label><input type="checkbox" data-scw-notebook-review-change value="${escapeHtml(item.key)}"><span><strong>${escapeHtml(item.change.toUpperCase())} · ${escapeHtml(item.label)}</strong><small>${escapeHtml(item.category)}${item.fields?.length?` · ${escapeHtml(item.fields.join(', '))}`:''}</small></span></label>`;notebookReviewList.appendChild(row);});const g=notebookGovernanceState(project);if(notebookReconcileHistory){notebookReconcileHistory.innerHTML='';const history=g.reconciliations.filter(x=>x.notebookId===nb.id).slice(0,10);if(!history.length)notebookReconcileHistory.innerHTML='<div class="scw-notebook-empty">No notebook reconciliations yet.</div>';history.forEach(item=>{const row=document.createElement('div');row.className='scw-notebook-reconciliation-row';row.innerHTML=`<div><strong>${escapeHtml(item.outputNotebookTitle||'Reconciliation prepared')}</strong><small>${escapeHtml(item.status)} · ${item.selectedChangeCount||0} selected · ${escapeHtml(formatTime(item.appliedAt||item.createdAt))}</small></div>`;notebookReconcileHistory.appendChild(row);});}if(notebookLineageRef){const current=notebookLineageRef.value,opts=notebookReviewRefOptions(project,nb);notebookLineageRef.innerHTML='<option value="">Choose notebook material</option>';opts.forEach(item=>{const o=document.createElement('option');o.value=item.value;o.textContent=item.label;notebookLineageRef.appendChild(o);});if(opts.some(x=>x.value===current))notebookLineageRef.value=current;}if(notebookAuditList){const events=api.auditEvents(project.notebooks);notebookAuditList.innerHTML='';if(!events.length)notebookAuditList.innerHTML='<div class="scw-notebook-empty">No notebook governance events are available yet.</div>';events.slice(0,100).forEach(event=>{const row=document.createElement('div');row.className='scw-notebook-audit-row';row.innerHTML=`<div><strong>${escapeHtml(event.label)}</strong><small>${escapeHtml(event.source)} · ${escapeHtml(formatTime(event.at))}</small></div>`;notebookAuditList.appendChild(row);});}}

    function renderNotebookPromotions(project) {
      if(!notebookPromotionList)return;notebookPromotionList.innerHTML='';
      const helper=notebookHelper(),items=Array.isArray(project?.notebooks?.promotions)?project.notebooks.promotions:[];
      if(!items.length){notebookPromotionList.innerHTML='<div class="scw-notebook-empty">No notebook material has been promoted into Workspace artifacts yet.</div>';return;}
      items.slice(0,80).forEach(item=>{
        const row=document.createElement('article');row.className='scw-notebook-promotion-row';
        const loc=helper?.findBlockLocation?helper.findBlockLocation(project.notebooks,item.source?.id):null;
        const targetLabel=item.targetType==='canvas'?'Canvas node':`${String(item.targetType||'artifact').charAt(0).toUpperCase()}${String(item.targetType||'artifact').slice(1)} object`;
        const body=document.createElement('div');body.innerHTML=`<span>${escapeHtml(targetLabel.toUpperCase())}</span><strong>${escapeHtml(item.title||loc?.block?.title||'Notebook material')}</strong><small>${escapeHtml(loc?`${loc.notebook.title} / ${loc.section.title}`:'Source block retained in notebook')} · ${escapeHtml(formatTime(item.createdAt))}</small>`;
        const open=document.createElement('button');open.type='button';open.className='scw-card-action';open.textContent='Open derivative';open.addEventListener('click',()=>{if(item.targetKind==='object'&&objectById(project,item.targetId)){state.activeProjectId=project.id;project.activeObjectId=item.targetId;persist('Notebook promotion derivative opened');render();setWorkspaceView('projects');setProjectMode('objects');return;}if(item.targetKind==='canvas-node'&&project.canvas.nodes.some(node=>node.id===item.targetId)){state.activeProjectId=project.id;const node=project.canvas.nodes.find(n=>n.id===item.targetId),board=node?project.canvas.boards.find(b=>b.id===node.boardId):null;if(board)project.canvas.activeBoardId=board.id;persist('Notebook promotion Canvas derivative opened');render();setWorkspaceView('projects');setProjectMode('canvas');}});
        row.append(body,open);notebookPromotionList.appendChild(row);
      });
    }
    ...
    function renderNotebookBlock(project, notebook, section, block, index) {
      ...
      const meta = document.createElement('span'); meta.textContent = block.type.toUpperCase();
      const title = document.createElement('strong'); title.textContent = block.title || (block.type === 'divider' ? 'Divider' : 'Untitled block');
      ...
      const blockRef=notebookHelper()?.nodeRef?.({kind:'block',id:block.id,notebookId:notebook.id,sectionId:section.id,label:block.title||'Untitled block'}); const backlinks=blockRef?(notebookHelper()?.backlinksForRef?.(project.notebooks,blockRef)||[]):[]; if(backlinks.length){const back=document.createElement('div');back.className='scw-notebook-backlinks';back.textContent=`BACKLINKS · ${backlinks.length}`;body.appendChild(back);} const synthesisUses=blockRef?(notebookHelper()?.synthesesForRef?.(project.notebooks,blockRef)||[]):[];if(synthesisUses.length){const used=document.createElement('div');used.className='scw-notebook-backlinks';used.textContent=`SYNTHESIS USE · ${synthesisUses.length}`;body.appendChild(used);}
      const actions = document.createElement('div'); actions.className='scw-notebook-block-actions';
      ...
      const edit=document.createElement('button');edit.type='button';edit.className='scw-card-action';edit.textContent='Edit';
      const editor=document.createElement('form');editor.className='scw-notebook-inline-editor';editor.hidden=true;editor.innerHTML=`<label><span>Title</span><input name="title" maxlength="240"></label><label><span>Content</span><textarea name="content" rows="5" maxlength="50000"></textarea></label><label><span>Source URL</span><input name="url" type="url" maxlength="2000"></label><label><span>Tags</span><input name="tags" maxlength="500"></label><div><button class="scw-button scw-button-primary" type="submit">Save block</button><button class="scw-button" type="button" data-cancel>Cancel</button></div>`;
      editor.elements.title.value=block.title;editor.elements.content.value=block.content;editor.elements.url.value=block.sourceUrl;editor.elements.tags.value=block.tags.join(', ');
      edit.addEventListener('click',()=>{editor.hidden=false;edit.disabled=true;editor.elements.title.focus();}); editor.querySelector('[data-cancel]').addEventListener('click',()=>{editor.hidden=true;edit.disabled=false;});
      editor.addEventListener('submit',event=>{event.preventDefault();block.title=String(editor.elements.title.value||'').trim().slice(0,240);block.content=String(editor.elements.content.value||'').slice(0,50000);block.sourceUrl=String(editor.elements.url.value||'').trim().slice(0,2000);block.tags=normalizeTags(editor.elements.tags.value);block.updatedAt=nowIso();section.updatedAt=notebook.updatedAt=block.updatedAt;touchNotebooks(project);addActivity(project,'notebook-block','Notebook block edited');persist('Notebook block saved');renderNotebook();});
      const helper=notebookHelper();
      const destinations=helper?.promotionDestinations?helper.promotionDestinations(block.type):[];
      const promotionWrap=document.createElement('span');promotionWrap.className='scw-notebook-promote-control';
      const promotionSelect=document.createElement('select');promotionSelect.className='scw-notebook-promote-select';promotionSelect.setAttribute('aria-label','Promote notebook block to');
      destinations.forEach(type=>{const option=document.createElement('option');option.value=type;option.textContent=`${type.charAt(0).toUpperCase()}${type.slice(1)}`;promotionSelect.appendChild(option);});
      const suggested=helper?.suggestedPromotionType?helper.suggestedPromotionType(block.type):'';if(suggested&&destinations.includes(suggested))promotionSelect.value=suggested;
      const promote=document.createElement('button');promote.type='button';promote.className='scw-card-action';promote.textContent='Promote';promote.disabled=!destinations.length;promote.addEventListener('click',()=>promoteNotebookBlock(project,notebook,section,block,promotionSelect.value));
      if(destinations.length)promotionWrap.append(promotionSelect,promote);else promotionWrap.append(promote);
      const remove=document.createElement('button');remove.type='button';remove.className='scw-card-action scw-card-action-muted';remove.textContent='Delete';remove.addEventListener('click',()=>{if(!window.confirm('Delete this notebook block? Structured artifacts already promoted from it will not be deleted.'))return;section.blocks=section.blocks.filter(item=>item.id!==block.id);section.updatedAt=notebook.updatedAt=nowIso();touchNotebooks(project);addActivity(project,'notebook-block','Notebook block deleted');persist('Notebook block deleted');renderNotebook();});
      actions.append(up,down,edit,promotionWrap,remove); head.append(meta,title,actions);article.append(head,body,editor);return article;
    }

    function promoteNotebookBlock(project, notebook, section, block, targetType) {
      const helper=notebookHelper();if(!helper||!helper.promotionDestinations)return;
      const destinations=helper.promotionDestinations(block.type);if(!destinations.includes(targetType))return;
      if(!window.confirm(`Promote this ${block.type} to ${targetType}? The notebook block will remain in place and the derivative will be recorded in the promotion ledger.`))return;
      const stamp=nowIso(),sourceRef=helper.nodeRef({kind:'block',id:block.id,notebookId:notebook.id,sectionId:section.id,label:block.title||block.type});
      let targetId='',targetKind='';
      if(targetType==='canvas'){
        if(project.canvas.nodes.length>=MAX_CANVAS_NODES){window.alert(`This project has reached the ${MAX_CANVAS_NODES}-node Canvas limit.`);return;}
        let board=project.canvas.boards.find(item=>item.id===project.canvas.activeBoardId)||project.canvas.boards[0]||null;
        if(!board){if(project.canvas.boards.length>=MAX_CANVAS_BOARDS){window.alert('Create or select a Canvas board before promoting this notebook block.');return;}board={id:id('cb'),title:'Notebook Promotions',description:'Canvas material explicitly promoted from Research Notebook blocks.',status:'working',createdAt:stamp,updatedAt:stamp};project.canvas.boards.push(board);project.canvas.activeBoardId=board.id;}
        const existing=project.canvas.nodes.filter(node=>node.boardId===board.id).length;
        const linkedObjectId=[block.referenceObjectId,block.sourceObjectId].find(value=>value&&objectById(project,value))||'';
        const node={id:id('cn'),boardId:board.id,type:helper.canvasNodeType?helper.canvasNodeType(block.type):'note',title:String(block.title||`${block.type} from Notebook`).trim().slice(0,240)||'Notebook material',body:String(block.content||'').slice(0,4000),objectId:linkedObjectId,x:24+(existing%4)*205,y:24+(Math.floor(existing/4)%5)*95,tags:normalizeTags(block.tags),createdAt:stamp,updatedAt:stamp};
        project.canvas.nodes.push(node);board.updatedAt=stamp;touchCanvas(project);targetId=node.id;targetKind='canvas-node';
      }else{
        if(project.objects.length>=MAX_OBJECTS){window.alert(`This project has reached the ${MAX_OBJECTS}-object local limit.`);return;}
        const object=objectTemplate(targetType,block.title||`${block.type} from Notebook`);const citation=helper.citationLine?.(block)||'';
        object.summary=String(block.content||block.title||'').slice(0,1200);object.content=[String(block.content||''),citation?`\n\nSource context: ${citation}`:''].join('').slice(0,50000);object.tags=normalizeTags([...block.tags,'notebook-promoted']);object.status='working';object.provenance.sourceType=block.sourceUrl?'web':'manual';object.provenance.sourceTitle=block.sourceTitle||notebook.title;object.provenance.sourceUrl=block.sourceUrl||'';object.provenance.capturedAt=block.capture?.capturedAt||stamp;project.objects.push(object);project.activeObjectId=object.id;targetId=object.id;targetKind='object';
        if(targetType==='source'&&project.research.readingQueue.length<MAX_READING_QUEUE)project.research.readingQueue.push({id:id('rr'),objectId:object.id,status:'unread',note:'Promoted from Research Notebook',addedAt:stamp,updatedAt:stamp});
        if(targetType==='evidence'&&block.sourceObjectId&&objectById(project,block.sourceObjectId)&&project.research.evidenceLinks.length<MAX_EVIDENCE_LINKS&&!project.research.evidenceLinks.some(link=>link.evidenceObjectId===object.id&&link.sourceObjectId===block.sourceObjectId))project.research.evidenceLinks.push({id:id('rel'),evidenceObjectId:object.id,sourceObjectId:block.sourceObjectId,createdAt:stamp});
        touchResearch(project);
      }
      const record=helper.createPromotion?.(sourceRef,targetType,targetKind,targetId,block.title||`${block.type} from Notebook`,id,()=>stamp);
      if(record){project.notebooks.promotions=Array.isArray(project.notebooks.promotions)?project.notebooks.promotions:[];project.notebooks.promotions.unshift(record);project.notebooks.promotions=project.notebooks.promotions.slice(0,helper.MAX_PROMOTIONS||500);}
      block.promotion={status:'promoted',targetKind,targetType,targetId,promotedAt:stamp};block.updatedAt=stamp;section.updatedAt=notebook.updatedAt=stamp;touchNotebooks(project);addActivity(project,'notebook-promoted',`Notebook ${block.type} promoted to ${targetType}`);persist(`Notebook block promoted to ${targetType}`);render();setWorkspaceView('notebook');
    }
    ...
    function renderNotebook() {
      ...
      if(!project){if(notebookList)notebookList.innerHTML='<div class="scw-notebook-empty">Create or choose a project before starting a notebook.</div>';if(notebookActive)notebookActive.innerHTML='<div class="scw-notebook-empty">Notebook work belongs to a Workspace Project.</div>';if(notebookSectionList)notebookSectionList.innerHTML='';if(notebookBlockList)notebookBlockList.innerHTML='';[notebookMetricNotebooks,notebookMetricSections,notebookMetricBlocks,notebookMetricPromoted,notebookMetricCollections,notebookMetricLinks,notebookMetricBacklinks,notebookMetricSyntheses,notebookMetricAssistances].forEach(el=>{if(el)el.textContent='0';});if(notebookCollectionList)notebookCollectionList.innerHTML='<div class="scw-notebook-empty">Choose a project to manage research collections.</div>';if(notebookLinkList)notebookLinkList.innerHTML='<div class="scw-notebook-empty">Choose a project to manage explicit links.</div>';if(notebookPromotionList)notebookPromotionList.innerHTML='<div class="scw-notebook-empty">Choose a project to inspect promotion lineage.</div>';if(notebookSynthesisList)notebookSynthesisList.innerHTML='<div class="scw-notebook-empty">Choose a project to create synthesis outputs.</div>';if(notebookAssistanceList)notebookAssistanceList.innerHTML='<div class="scw-notebook-empty">Choose a project to ask grounded Notebook questions.</div>';if(notebookStatus)notebookStatus.textContent='No active project selected.';return;}
      project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);const all=notebookAllBlocks(project);if(notebookMetricNotebooks)notebookMetricNotebooks.textContent=String(project.notebooks.notebooks.length);if(notebookMetricSections)notebookMetricSections.textContent=String(project.notebooks.notebooks.reduce((sum,n)=>sum+n.sections.length,0));if(notebookMetricBlocks)notebookMetricBlocks.textContent=String(all.length);if(notebookMetricPromoted)notebookMetricPromoted.textContent=String((project.notebooks.promotions||[]).length);renderNotebookKnowledgeLinks(project);renderNotebookPromotions(project);renderNotebookSynthesis(project);renderNotebookAssistance(project);
      const active=activeNotebookFor(project);if(notebookList){notebookList.innerHTML='';if(!project.notebooks.notebooks.length)notebookList.innerHTML='<div class="scw-notebook-empty">No notebooks yet. Create one when you need working notes, excerpts, synthesis, or a place to connect source context before promotion.</div>';project.notebooks.notebooks.forEach((nb,index)=>{const row=document.createElement('article');row.className=`scw-notebook-card${nb.id===active?.id?' is-active':''}`;const body=document.createElement('button');body.type='button';body.className='scw-notebook-card-open';body.innerHTML=`<span>NOTEBOOK ${String(index+1).padStart(2,'0')}</span><strong>${escapeHtml(nb.title)}</strong><small>${nb.sections.length} section${nb.sections.length===1?'':'s'} · ${nb.sections.reduce((sum,sec)=>sum+sec.blocks.length,0)} block${nb.sections.reduce((sum,sec)=>sum+sec.blocks.length,0)===1?'':'s'}</small>`;body.addEventListener('click',()=>{project.notebooks.activeNotebookId=nb.id;touchNotebooks(project);persist('Active notebook saved');renderNotebook();});const del=document.createElement('button');del.type='button';del.className='scw-card-action scw-card-action-muted';del.textContent='Remove';del.addEventListener('click',()=>{if(!window.confirm(`Remove notebook “${nb.title}”? Promoted Workspace artifacts will remain.`))return;project.notebooks.notebooks=project.notebooks.notebooks.filter(item=>item.id!==nb.id);project.notebooks.activeNotebookId=project.notebooks.notebooks[0]?.id||null;touchNotebooks(project);addActivity(project,'notebook','Notebook removed');persist('Notebook removed');renderNotebook();});row.append(body,del);notebookList.appendChild(row);});}
      ...
      if(notebookActive){notebookActive.innerHTML='';const wrap=document.createElement('div');wrap.className='scw-notebook-active-head';const text=document.createElement('div');text.innerHTML=`<span>ACTIVE NOTEBOOK</span><h3>${escapeHtml(active.title)}</h3><p>${escapeHtml(active.description||'Working notes, sources, excerpts, questions, claims, and references.')}</p>`;const rename=document.createElement('button');rename.type='button';rename.className='scw-card-action';rename.textContent='Edit notebook';rename.addEventListener('click',()=>{const title=window.prompt('Notebook title',active.title);if(title===null)return;const desc=window.prompt('Notebook description',active.description||'');active.title=String(title||'Research Notebook').trim().slice(0,160)||'Research Notebook';active.description=String(desc||'').trim().slice(0,1200);active.updatedAt=nowIso();touchNotebooks(project);persist('Notebook details saved');renderNotebook();});wrap.append(text,rename);notebookActive.appendChild(wrap);}
      const activeSection=activeNotebookSectionFor(project,active);if(notebookSectionList){notebookSectionList.innerHTML='';active.sections.forEach((section,index)=>{const row=document.createElement('article');row.className=`scw-notebook-section-card${section.id===activeSection?.id?' is-active':''}`;const open=document.createElement('button');open.type='button';open.className='scw-notebook-section-open';open.innerHTML=`<strong>${escapeHtml(section.title)}</strong><small>${section.blocks.length} block${section.blocks.length===1?'':'s'}</small>`;open.addEventListener('click',()=>{active.activeSectionId=section.id;active.updatedAt=nowIso();touchNotebooks(project);persist('Notebook section selected');renderNotebook();});const acts=document.createElement('div');acts.className='scw-notebook-section-actions';const up=document.createElement('button');up.type='button';up.className='scw-card-action';up.textContent='↑';up.disabled=index===0;up.addEventListener('click',()=>{if(notebookHelper()?.move(active.sections,index,-1)){active.updatedAt=nowIso();touchNotebooks(project);persist('Notebook section moved');renderNotebook();}});const down=document.createElement('button');down.type='button';down.className='scw-card-action';down.textContent='↓';down.disabled=index===active.sections.length-1;down.addEventListener('click',()=>{if(notebookHelper()?.move(active.sections,index,1)){active.updatedAt=nowIso();touchNotebooks(project);persist('Notebook section moved');renderNotebook();}});const del=document.createElement('button');del.type='button';del.className='scw-card-action scw-card-action-muted';del.textContent='Remove';del.addEventListener('click',()=>{if(!window.confirm(`Remove section “${section.title}” and its ${section.blocks.length} block(s)?`))return;active.sections=active.sections.filter(item=>item.id!==section.id);if(!active.sections.length)active.sections.push(notebookHelper().createSection('Notes',id,nowIso));active.activeSectionId=active.sections[0].id;active.updatedAt=nowIso();touchNotebooks(project);persist('Notebook section removed');renderNotebook();});acts.append(up,down,del);row.append(open,acts);notebookSectionList.appendChild(row);});}
      if(notebookBlockObject){const cur=notebookBlockObject.value;notebookBlockObject.innerHTML='<option value="">No object reference</option>';project.objects.filter(object=>!object.archivedAt).sort(objectSort).forEach(object=>{const o=document.createElement('option');o.value=object.id;o.textContent=`${OBJECT_LABELS[object.type]||object.type} · ${object.title}`;notebookBlockObject.appendChild(o);});if(project.objects.some(object=>object.id===cur&&!object.archivedAt))notebookBlockObject.value=cur;}
      if(notebookBlockList){notebookBlockList.innerHTML='';if(!activeSection)notebookBlockList.innerHTML='<div class="scw-notebook-empty">Add a section before creating blocks.</div>';else if(!activeSection.blocks.length)notebookBlockList.innerHTML='<div class="scw-notebook-empty">This section is empty. Capture a note, source, excerpt, question, claim, reference, checklist, divider, or attachment reference.</div>';else activeSection.blocks.forEach((block,index)=>notebookBlockList.appendChild(renderNotebookBlock(project,active,activeSection,block,index)));}
      renderNotebookPortability(project);
      renderNotebookReviewProvenance(project);
      if(notebookStatus)notebookStatus.textContent=`${project.title} · ${active.title} · ${activeSection?.title||'No section'} · local-first notebook`;
    }
    ...
    function createFirstRunProject(input) {
      const onboarding = window.SCWorkspaceFirstRunOnboarding;
      if (!onboarding) return {ok:false,message:'First-run onboarding helper is unavailable. Use the standard project form instead.'};
      const plan = onboarding.creationPlan(input || {});
      if (!plan.ok) return {ok:false,message:plan.errors?.[0]?.message || 'Review the first-project details and try again.'};
      if (state.projects.length >= MAX_PROJECTS) return {ok:false,message:'This device has reached the Workspace Project limit.'};
      const project = projectTemplate(plan.draft.title, plan.draft.description);
      let guided = false;
      if (plan.starter.workflow) {
        const run = startGuidedWorkflow(project, plan.starter.workflow);
        if (!run) return {ok:false,message:'The selected guided starter could not be created. Choose Blank project or try again.'};
        guided = true;
      }
      state.projects.unshift(project);
      state.activeProjectId = project.id;
      activeProjectMode = guided ? 'guide' : 'overview';
      workspaceView = 'projects';
      persist(`First-run project created explicitly: ${guided ? plan.starter.label : 'Blank project'}`);
      if (firstRunForm) firstRunForm.reset();
      render();
      setWorkspaceView('projects', true);
      if (guided) setProjectMode('guide');
      return {ok:true,projectId:project.id,guided};
    }
    ...
    function setWorkspaceView(view, moveFocus = false) {
      ...
      root.querySelectorAll('[data-scw-workspace-section]').forEach(section => { section.hidden = section.dataset.scwWorkspaceSection !== workspaceView; });
      ...
      if (notebookSection) notebookSection.hidden = workspaceView !== 'notebook';
      ...
      if (lifecycleSection) lifecycleSection.hidden = workspaceView !== 'lifecycle';
      ...
      if (persistenceIntegritySection) persistenceIntegritySection.hidden = workspaceView !== 'integrity';
      ...
      if (workspaceView === 'research') renderIntegratedKnowledge();
      if (workspaceView === 'notebook') renderNotebook();
      ...
      if (workspaceView === 'lifecycle') renderProjectLifecycle();
      if (workspaceView === 'history') renderVersionHistory();
      if (workspaceView === 'changes') renderChangeReview();
      if (workspaceView === 'reconcile') renderReconciliation();
    ...
    function renderKnowledgeCollectionDetail(index) {
      ...
      const exp=document.createElement('button');exp.type='button';exp.className='scw-card-action';exp.textContent='Export JSON';exp.addEventListener('click',()=>{const payload={schema:KNOWLEDGE_COLLECTION_EXPORT_SCHEMA,workspaceVersion:rootVersion(),exportedAt:nowIso(),collection:JSON.parse(JSON.stringify(collection)),objects:collection.items.map(ref=>{const e=index.find(x=>x.projectId===ref.projectId&&x.objectId===ref.objectId);return e?{project:{id:e.projectId,title:e.projectTitle},object:JSON.parse(JSON.stringify(e.object))}:null;}).filter(Boolean)};downloadJson(`${safeFileName(collection.title)}.sc-workspace-knowledge.json`,payload);});
    ...
    function collaborationArchitectureApi(){return window.SCWorkspaceCollaborationArchitecture||null;}
    function loadCollaborationArchitecture(){const api=collaborationArchitectureApi();if(!api)return collaborationArchitecture;try{return api.normalizeArchitecture(JSON.parse(localStorage.getItem(api.STORAGE_KEY)||'{}'));}catch(_){return api.normalizeArchitecture({});}}
    function persistCollaborationArchitecture(message='Collaboration architecture saved locally.'){const api=collaborationArchitectureApi();if(!api)return false;collaborationArchitecture=api.normalizeArchitecture(collaborationArchitecture);collaborationArchitecture.updatedAt=nowIso();try{localStorage.setItem(api.STORAGE_KEY,JSON.stringify(collaborationArchitecture));if(collabArchStatus)collabArchStatus.textContent=message;return true;}catch(_){if(collabArchStatus)collabArchStatus.textContent='Collaboration architecture could not be saved in this browser.';return false;}}
    function activeCollabArchitectureProject(){return state.projects.find(p=>p.id===String(collabArchProject?.value||'')&&!p.archivedAt)||null;}
    function activeCollabPolicy(){const project=activeCollabArchitectureProject();return project?collaborationArchitecture.policies.find(p=>p.projectId===project.id)||null:null;}
    function collabActorLabel(actorId){return collaborationArchitecture.actors.find(a=>a.id===actorId)?.displayName||'Unknown actor';}
    function fillCollabActorSelect(select,placeholder='Choose actor'){if(!select)return;const cur=select.value;select.innerHTML=`<option value="">${placeholder}</option>`;collaborationArchitecture.actors.slice().sort((a,b)=>a.displayName.localeCompare(b.displayName)).forEach(a=>{const o=document.createElement('option');o.value=a.id;o.textContent=`${a.displayName} · ${a.role}`;select.appendChild(o);});if([...select.options].some(o=>o.value===cur))select.value=cur;}
    function renderCollaborationArchitecture(){if(!collabArchitectureSection)return;const api=collaborationArchitectureApi();if(!api)return;collaborationArchitecture=api.normalizeArchitecture(collaborationArchitecture);const projects=state.projects.filter(p=>!p.archivedAt).sort(projectSort);const metrics=api.stats(collaborationArchitecture,projects);if(collabArchActors)collabArchActors.textContent=String(metrics.actors);if(collabArchPolicies)collabArchPolicies.textContent=String(metrics.policies);if(collabArchComments)collabArchComments.textContent=String(metrics.openComments);if(collabArchProposals)collabArchProposals.textContent=String(metrics.openProposals);
      if(collabArchProject){const cur=collabArchProject.value;collabArchProject.innerHTML='<option value="">Choose project</option>';projects.forEach(p=>{const o=document.createElement('option');o.value=p.id;o.textContent=p.title;collabArchProject.appendChild(o);});if(projects.some(p=>p.id===cur))collabArchProject.value=cur;else if(state.activeProjectId&&projects.some(p=>p.id===state.activeProjectId))collabArchProject.value=state.activeProjectId;else if(projects[0])collabArchProject.value=projects[0].id;}
      fillCollabActorSelect(collabOwner,'Choose owner');fillCollabActorSelect(collabGrantActor,'Choose collaborator');fillCollabActorSelect(collabCommentActor,'Choose commenter');fillCollabActorSelect(collabProposalActor,'Choose proposer');const project=activeCollabArchitectureProject(),policy=activeCollabPolicy();if(collabOwner&&policy&&[...collabOwner.options].some(o=>o.value===policy.ownerActorId))collabOwner.value=policy.ownerActorId;
      [collabCommentObject,collabProposalObject].forEach(select=>{if(!select)return;const cur=select.value,projectLevel=select===collabCommentObject;select.innerHTML=projectLevel?'<option value="">Project-level comment</option>':'<option value="">Choose object</option>';if(project)project.objects.filter(o=>!o.archivedAt).sort(objectSort).forEach(obj=>{const o=document.createElement('option');o.value=obj.id;o.textContent=`${OBJECT_LABELS[obj.type]||obj.type}: ${obj.title}`;select.appendChild(o);});if([...select.options].some(o=>o.value===cur))select.value=cur;});
      if(collabActorList){collabActorList.innerHTML='';if(!collaborationArchitecture.actors.length)collabActorList.innerHTML='<div class="scw-collaboration-empty">No collaboration actors yet.</div>';collaborationArchitecture.actors.forEach(actor=>{const row=document.createElement('article');row.className='scw-collab-actor-row';row.innerHTML=`<span>${escapeHtml(actor.role)} · ${escapeHtml(actor.id)}</span><strong>${escapeHtml(actor.displayName)}</strong><small>${escapeHtml(api.capabilitiesForRole(actor.role).join(' · '))}</small>`;collabActorList.appendChild(row);});}
      if(collabPolicySummary){collabPolicySummary.innerHTML='';if(!project)collabPolicySummary.innerHTML='<div class="scw-collaboration-empty">Choose a project to define ownership and role grants.</div>';else if(!policy)collabPolicySummary.innerHTML='<div class="scw-collaboration-empty">No collaboration policy exists for this project yet.</div>';else{const owner=document.createElement('article');owner.className='scw-collab-policy-row';owner.innerHTML=`<span>OWNER · MANAGE POLICY</span><strong>${escapeHtml(collabActorLabel(policy.ownerActorId))}</strong><small>Project ${escapeHtml(project.title)} · local descriptive ownership</small>`;collabPolicySummary.appendChild(owner);policy.grants.forEach(grant=>{const row=document.createElement('article');row.className='scw-collab-policy-row';row.innerHTML=`<span>${escapeHtml(grant.role)} · ${escapeHtml(api.capabilitiesForRole(grant.role).join(' · '))}</span><strong>${escapeHtml(collabActorLabel(grant.actorId))}</strong>`;const acts=document.createElement('div');acts.className='scw-collab-record-actions';const remove=document.createElement('button');remove.type='button';remove.className='scw-card-action';remove.textContent='Remove grant';remove.addEventListener('click',()=>{collaborationArchitecture.policies=collaborationArchitecture.policies.map(p=>p.id===policy.id?api.removeGrant(p,grant.actorId,nowIso):p);persistCollaborationArchitecture('Role grant removed. No server access was changed.');renderCollaborationArchitecture();});acts.appendChild(remove);row.appendChild(acts);collabPolicySummary.appendChild(row);});}}
      const comments=collaborationArchitecture.comments.filter(c=>!project||c.target.projectId===project.id).slice().sort((a,b)=>String(b.updatedAt).localeCompare(String(a.updatedAt)));if(collabCommentList){collabCommentList.innerHTML='';if(!comments.length)collabCommentList.innerHTML='<div class="scw-collaboration-empty">No collaboration comments for this project.</div>';comments.forEach(comment=>{const resolved=api.resolveTarget(comment.target,state.projects),row=document.createElement('article');row.className='scw-collab-record-row';row.innerHTML=`<span>COMMENT · ${escapeHtml(comment.status)}${resolved.resolved?'':' · UNRESOLVED TARGET'}</span><strong>${escapeHtml(collabActorLabel(comment.actorId))}</strong><p>${escapeHtml(comment.body)}</p><small>${comment.target.kind==='object'?`Object: ${escapeHtml(comment.target.objectTitle||comment.target.objectId)}`:'Project-level'} · ${escapeHtml(formatTime(comment.updatedAt))}</small>`;const acts=document.createElement('div');acts.className='scw-collab-record-actions';if(resolved.resolved){const open=document.createElement('button');open.type='button';open.className='scw-card-action';open.textContent='Open target';open.addEventListener('click',()=>openCollaborationProject(comment.target.projectId,comment.target.objectId));acts.appendChild(open);}const toggle=document.createElement('button');toggle.type='button';toggle.className='scw-card-action';toggle.textContent=comment.status==='resolved'?'Reopen':'Resolve';toggle.addEventListener('click',()=>{collaborationArchitecture.comments=collaborationArchitecture.comments.map(c=>c.id===comment.id?api.setCommentStatus(c,comment.status==='resolved'?'open':'resolved',nowIso):c);persistCollaborationArchitecture('Comment state updated. Canonical research was unchanged.');renderCollaborationArchitecture();});acts.appendChild(toggle);row.appendChild(acts);collabCommentList.appendChild(row);});}
      const proposals=collaborationArchitecture.proposals.filter(p=>!project||p.target.projectId===project.id).slice().sort((a,b)=>String(b.updatedAt).localeCompare(String(a.updatedAt)));if(collabProposalList){collabProposalList.innerHTML='';if(!proposals.length)collabProposalList.innerHTML='<div class="scw-collaboration-empty">No review proposals for this project.</div>';proposals.forEach(proposal=>{const resolved=api.resolveTarget(proposal.target,state.projects),row=document.createElement('article');row.className='scw-collab-record-row';row.innerHTML=`<span>PROPOSAL · ${escapeHtml(proposal.kind)} · ${escapeHtml(proposal.status)}${resolved.resolved?'':' · UNRESOLVED TARGET'}</span><strong>${escapeHtml(proposal.target.objectTitle||proposal.target.objectId||'Project')}</strong><p>${escapeHtml(proposal.proposedValue)}</p>${proposal.rationale?`<small>Rationale: ${escapeHtml(proposal.rationale)}</small>`:''}`;const acts=document.createElement('div');acts.className='scw-collab-record-actions';if(resolved.resolved){const open=document.createElement('button');open.type='button';open.className='scw-card-action';open.textContent='Open target';open.addEventListener('click',()=>openCollaborationProject(proposal.target.projectId,proposal.target.objectId));acts.appendChild(open);}const action=(label,status)=>{const b=document.createElement('button');b.type='button';b.className='scw-card-action';b.textContent=label;b.addEventListener('click',()=>{const actorId=policy?.ownerActorId||proposal.actorId,result=api.transitionProposal(proposal,status,actorId,status==='accepted'?'Accepted as a review decision only; canonical content not applied.':'',nowIso);if(result.ok){collaborationArchitecture.proposals=collaborationArchitecture.proposals.map(p=>p.id===proposal.id?result.proposal:p);persistCollaborationArchitecture(status==='accepted'?'Proposal accepted as a review record. Canonical project content was NOT changed.':`Proposal moved to ${status}.`);renderCollaborationArchitecture();}});acts.appendChild(b);};if(proposal.status==='draft')action('Submit','submitted');if(['draft','submitted'].includes(proposal.status)){action('Accept','accepted');action('Reject','rejected');action('Withdraw','withdrawn');}row.appendChild(acts);collabProposalList.appendChild(row);});}
      if(collabContractOutput&&!project)collabContractOutput.textContent='No project policy selected.';
    }
    ...
    function renderCollaboration(){if(!collaborationSection)return;renderCollaborationArchitecture();if(window.SCWorkspaceReviewRoomsUI&&typeof window.SCWorkspaceReviewRoomsUI.setContext==='function')window.SCWorkspaceReviewRoomsUI.setContext({projects:state.projects,activeProjectId:state.activeProjectId,actors:collaborationArchitecture.actors,policies:collaborationArchitecture.policies,workspaceVersion:rootVersion()});state.collaboration=normalizeCollaboration(state.collaboration,state.projects);const c=state.collaboration,session=activeCollaborationSession(),projects=state.projects.filter(p=>!p.archivedAt).sort(projectSort);if(collabProfileForm){const name=collabProfileForm.querySelector('[name="displayName"]'),role=collabProfileForm.querySelector('[name="role"]');if(name&&document.activeElement!==name)name.value=c.profile.displayName;if(role)role.value=c.profile.role;}
    ...
    function universalSearchApi(){return window.SCWorkspaceUniversalSearch||null;}
    function libraryContinuityApi(){return window.SCWorkspaceLibraryContinuity||null;}
    let libraryContinuityItems=[];
    function loadLibraryContinuity(){const api=libraryContinuityApi();if(!api)return[];try{return JSON.parse(localStorage.getItem(api.STORAGE_KEY)||'[]').map(api.normalizeItem).filter(Boolean);}catch(_){return[];}}
    function saveLibraryContinuity(){const api=libraryContinuityApi();if(!api)return false;try{localStorage.setItem(api.STORAGE_KEY,JSON.stringify(libraryContinuityItems));return true;}catch(_){return false;}}
    function universalEntries(){const u=universalSearchApi(),ik=integratedApi();if(!u)return derivedIntegratedEntries(ik);return u.derive(state,{integratedApi:ik,referenceLibrary,researchTaskLibrary});}
    function openUniversalEntry(entry){
      if(!entry)return;
      const project=entry.projectId?state.projects.find(p=>p.id===entry.projectId):null;
      if(project&&!project.archivedAt)state.activeProjectId=project.id;
      if(entry.kind==='project'&&project&&!project.archivedAt){activeProjectMode='overview';persist('Project opened from Universal Search');setWorkspaceView('projects');return;}
      if(entry.kind==='object'&&project&&!project.archivedAt){project.activeObjectId=entry.id;activeProjectMode='object';persist('Object opened from Universal Search');setWorkspaceView('projects');return;}
      if((entry.kind==='notebook'||entry.kind==='notebook-block')&&project&&!project.archivedAt){if(project.notebooks){project.notebooks.activeNotebookId=entry.notebookId||entry.id;if(entry.sectionId){const nb=(project.notebooks.notebooks||[]).find(n=>n.id===entry.notebookId);if(nb)nb.activeSectionId=entry.sectionId;}}persist('Notebook opened from Universal Search');setWorkspaceView('notebook');return;}
      if((entry.kind==='research-question'||entry.kind==='research-claim')&&project&&!project.archivedAt){activeProjectMode='research';persist('Research record opened from Universal Search');setWorkspaceView('projects');return;}
      if(entry.kind==='analysis-question'&&project&&!project.archivedAt){activeProjectMode='analysis';persist('Analysis opened from Universal Search');setWorkspaceView('projects');return;}
      if(entry.kind==='decision'&&project&&!project.archivedAt){activeProjectMode='decision';persist('Decision opened from Universal Search');setWorkspaceView('projects');return;}
      if(entry.kind==='briefing-draft'&&project&&!project.archivedAt){activeProjectMode='briefing';persist('Briefing opened from Universal Search');setWorkspaceView('projects');return;}
      if(entry.kind==='citation-reference'){selectedReferenceId=entry.id;setResearchSurface('citations',false);setWorkspaceView('research');renderReferenceLibrary();return;}
      if(entry.kind==='research-task'){researchTaskFilters={status:'all',type:'all',projectId:entry.projectId||'all',resolution:'all'};setResearchSurface('tasks',false);setWorkspaceView('research');renderResearchTasks();return;}
      openIntegratedEntry(entry);
    }
    function integratedApi(){return window.SCWorkspaceIntegratedKnowledge||null;}
    function scalePerformanceApi(){return window.SCWorkspaceScalePerformance||null;}
    function longSessionPerformanceApi(){return window.SCWorkspaceLongSessionPerformance||null;}
    function derivedIntegratedEntries(api){
      if(!api)return[];
      const start=window.performance&&typeof window.performance.now==='function'?window.performance.now():Date.now();
      const entries=scalePerformanceApi()?scalePerformanceApi().deriveIntegrated(state,api):api.derive(state);
      const end=window.performance&&typeof window.performance.now==='function'?window.performance.now():Date.now();
      longSessionMonitor?.markIndex(end-start,'integrated-knowledge',entries.length);
      return entries;
    }
    function researchTasksApi(){return window.SCWorkspaceResearchTasks||null;}
    function renderLibraryContinuity(){if(!libraryContinuityList)return;const api=libraryContinuityApi();libraryContinuityList.innerHTML='';if(!api){libraryContinuityList.innerHTML='<div class="scw-knowledge-empty-note">Library continuity helper unavailable.</div>';return;}if(!libraryContinuityItems.length){libraryContinuityList.innerHTML='<div class="scw-knowledge-empty-note">No Knowledge Library context is staged on this device. Export or hand off saved searches, watchlists, research queue items, source bundles, or personal recommendations from the Library.</div>';return;}const project=activeProject();libraryContinuityItems.forEach(item=>{const row=document.createElement('article');row.className='scw-library-continuity-item';const copy=document.createElement('div');copy.innerHTML=`<span>${escapeHtml(item.kind.toUpperCase().replaceAll('-',' '))}${item.private?' · PRIVATE':''}</span><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.summary||'No summary supplied by the Knowledge Library.')}</p><small>${escapeHtml(item.origin?.libraryId||item.id||'Library record')} · provenance preserved</small>`;const actions=document.createElement('div');actions.className='scw-workflow-actions';const promote=document.createElement('button');promote.type='button';promote.className='scw-card-action';promote.textContent='Add to project';promote.disabled=!project;promote.addEventListener('click',()=>{const p=activeProject();if(!p||p.objects.length>=MAX_OBJECTS)return;const src=api.toWorkspaceSource(item,p.id);if(!src)return;const obj=objectTemplate('source',src.title);obj.summary=src.summary;obj.content=src.content;obj.tags=normalizeTags(src.tags);obj.provenance={...obj.provenance,...src.provenance};obj.status='working';p.objects.push(obj);p.activeObjectId=obj.id;addActivity(p,'library-continuity-promote',`Knowledge Library context added: ${item.title}`);persist('Knowledge Library context added to project');render();if(libraryContinuityStatus)libraryContinuityStatus.textContent='Added a provenance-preserving Source copy to the active project. The canonical Library record was not changed.';});actions.append(promote);row.append(copy,actions);libraryContinuityList.appendChild(row);});}
    ...
    function activeGroundedSession(){return groundedResearchLibrary.sessions.find(s=>s.id===activeGroundedSessionId)||null;}
    function renderGroundedResearchAssistant(){if(!groundedResearchSection)return;const api=groundedResearchApi(),ik=integratedApi();if(!api||!ik)return;groundedResearchLibrary=api.normalizeLibrary(groundedResearchLibrary);const entries=derivedIntegratedEntries(ik),entryMap=new Map(entries.map(e=>[e.key,e]));groundedScopeKeys=[...new Set(groundedScopeKeys)].filter(k=>entryMap.has(k)).slice(0,api.MAX_SCOPE);const session=activeGroundedSession();if(groundedSessionCount)groundedSessionCount.textContent=String(groundedResearchLibrary.sessions.length);if(groundedScopeCount)groundedScopeCount.textContent=String(groundedScopeKeys.length);if(groundedAddSelected)groundedAddSelected.disabled=!selectedIntegratedKey||groundedScopeKeys.includes(selectedIntegratedKey);if(groundedDelete)groundedDelete.disabled=!session;if(groundedSessionSelect){const cur=activeGroundedSessionId;groundedSessionSelect.innerHTML='<option value="">Choose grounded request</option>';groundedResearchLibrary.sessions.forEach(x=>{const o=document.createElement('option');o.value=x.id;o.textContent=`${x.title} · ${String(x.status||'prepared').replaceAll('-',' ')}`;groundedSessionSelect.appendChild(o);});groundedSessionSelect.value=groundedResearchLibrary.sessions.some(x=>x.id===cur)?cur:'';}if(groundedScope){groundedScope.innerHTML='';if(!groundedScopeKeys.length)groundedScope.innerHTML='<div class="scw-knowledge-empty-note">Select research above and add it to the grounding scope.</div>';groundedScopeKeys.forEach((key,i)=>{const e=entryMap.get(key),row=document.createElement('div');row.className='scw-grounded-scope-row';row.innerHTML=`<span>[${i+1}] ${escapeHtml(e.kind.replaceAll('-',' '))}</span><strong>${escapeHtml(e.title)}</strong><small>${escapeHtml(e.projectTitle)} · ${escapeHtml(e.origin)}</small>`;const b=document.createElement('button');b.type='button';b.className='scw-card-action';b.textContent='Remove';b.addEventListener('click',()=>{groundedScopeKeys=groundedScopeKeys.filter(k=>k!==key);renderGroundedResearchAssistant();});row.appendChild(b);groundedScope.appendChild(row);});}if(groundedPrompt)groundedPrompt.textContent=session?api.promptMarkdown(session):'Select a prepared request to inspect its frozen grounding packet.';if(groundedResponse){groundedResponse.disabled=!session;if(document.activeElement!==groundedResponse)groundedResponse.value=session?.response||'';}if(groundedResponseSource){groundedResponseSource.disabled=!session;groundedResponseSource.value=session?.responseSource||'manual';}if(groundedExportRequest)groundedExportRequest.disabled=!session;if(groundedCopy)groundedCopy.disabled=!session;if(groundedSaveResponse)groundedSaveResponse.disabled=!session;if(groundedExportResponse)groundedExportResponse.disabled=!session?.response;if(groundedReview)groundedReview.disabled=!session?.response||session.status==='reviewed'||session.status==='materialized';if(groundedReject)groundedReject.disabled=!session?.response||session.status==='rejected'||session.status==='materialized';if(groundedTargetProject){const cur=groundedTargetProject.value;groundedTargetProject.innerHTML='<option value="">Choose project</option>';state.projects.filter(p=>!p.archivedAt).sort(projectSort).forEach(p=>{const o=document.createElement('option');o.value=p.id;o.textContent=p.title;groundedTargetProject.appendChild(o);});if([...groundedTargetProject.options].some(o=>o.value===cur))groundedTargetProject.value=cur;}if(groundedMaterialize)groundedMaterialize.disabled=!session||session.status!=='reviewed'||!groundedTargetProject?.value;if(groundedCitations){groundedCitations.innerHTML='';if(!session)groundedCitations.innerHTML='<div class="scw-knowledge-empty-note">Citation validation appears here after a response is saved.</div>';else{const cov=api.citationCoverage(session,session.response||'');groundedCitations.innerHTML=`<strong>${session.status.replaceAll('-',' ')}</strong><span>${session.grounding.length} frozen grounding record${session.grounding.length===1?'':'s'} · fingerprint ${escapeHtml(session.groundingFingerprint||'')}</span>${session.citations?.length?session.citations.map(c=>`<em>[${c.number}] ${escapeHtml(c.title)} · ${escapeHtml(c.projectTitle||'')}</em>`).join(''):'<em>No validated response citations saved yet.</em>'}${session.response&&!cov.valid?`<small>Citation coverage currently fails validation.</small>`:''}`;}}}
    function knowledgeSearchApi(){return window.SCWorkspaceKnowledgeSearch||null;}
    function loadSavedKnowledgeSearches(){const api=knowledgeSearchApi();if(!api)return[];try{return api.normalizeSavedSearches(JSON.parse(localStorage.getItem(api.STORAGE_KEY)||'[]'));}catch(_){return[];}}
    function persistSavedKnowledgeSearches(){const api=knowledgeSearchApi();if(!api)return false;try{localStorage.setItem(api.STORAGE_KEY,api.serializeSavedSearches(savedKnowledgeSearches));return true;}catch(_){return false;}}
    function renderSavedKnowledgeSearches(){if(!savedSearchSelect)return;const current=savedSearchSelect.value;savedSearchSelect.innerHTML='<option value="">Choose saved search</option>';savedKnowledgeSearches.forEach(item=>{const o=document.createElement('option');o.value=item.id;o.textContent=item.name;savedSearchSelect.appendChild(o);});savedSearchSelect.value=savedKnowledgeSearches.some(x=>x.id===current)?current:'';if(deleteSearchButton)deleteSearchButton.disabled=!savedSearchSelect.value;}
    function applyIntegratedPrefs(raw){const api=knowledgeSearchApi();integratedPrefs=api?api.normalizePrefs(raw):Object.assign({},integratedPrefs,raw||{});renderIntegratedKnowledge();}
    function researchCollectionsApi(){return window.SCWorkspaceResearchCollections||null;}
    function loadResearchCollections(){const api=researchCollectionsApi(),search=knowledgeSearchApi();if(!api)return[];try{return api.normalizeCollections(JSON.parse(localStorage.getItem(api.COLLECTION_STORAGE_KEY)||'[]'),search);}catch(_){return[];}}
    function loadResearchViews(){const api=researchCollectionsApi(),search=knowledgeSearchApi();if(!api)return[];try{return api.normalizeViews(JSON.parse(localStorage.getItem(api.VIEW_STORAGE_KEY)||'[]'),search);}catch(_){return[];}}
    function persistResearchCollections(){const api=researchCollectionsApi(),search=knowledgeSearchApi();if(!api)return false;try{localStorage.setItem(api.COLLECTION_STORAGE_KEY,api.serializeCollections(researchCollections,search));return true;}catch(_){return false;}}
    function persistResearchViews(){const api=researchCollectionsApi(),search=knowledgeSearchApi();if(!api)return false;try{localStorage.setItem(api.VIEW_STORAGE_KEY,api.serializeViews(researchViews,search));return true;}catch(_){return false;}}
    function renderResearchCollectionControls(){
      if(researchCollectionCount)researchCollectionCount.textContent=String(researchCollections.length);if(researchViewCount)researchViewCount.textContent=String(researchViews.length);
      if(smartCollectionSelect){smartCollectionSelect.innerHTML='<option value="">Choose collection</option>';researchCollections.forEach(item=>{const o=document.createElement('option');o.value=item.id;o.textContent=item.name;smartCollectionSelect.appendChild(o);});smartCollectionSelect.value=researchCollections.some(x=>x.id===activeResearchCollectionId)?activeResearchCollectionId:'';if(deleteCollectionButton)deleteCollectionButton.disabled=!smartCollectionSelect.value;}
      if(researchViewSelect){researchViewSelect.innerHTML='<option value="">Choose view</option>';researchViews.forEach(item=>{const o=document.createElement('option');o.value=item.id;o.textContent=item.name;researchViewSelect.appendChild(o);});researchViewSelect.value=researchViews.some(x=>x.id===activeResearchViewId)?activeResearchViewId:'';if(deleteViewButton)deleteViewButton.disabled=!researchViewSelect.value;}
      if(researchViewGroup)researchViewGroup.value=dynamicGroupBy;if(researchViewDensity)researchViewDensity.value=dynamicDensity;
    }
    function renderResearchCollections(all){
      if(!researchCollectionsSection)return;const api=researchCollectionsApi(),search=knowledgeSearchApi();if(!api||!search)return;
      const dash=api.dashboard(all,state,search,integratedPrefs);if(dashboardSources)dashboardSources.textContent=String(dash.sources);if(dashboardEvidence)dashboardEvidence.textContent=String(dash.evidence);if(dashboardDecisions)dashboardDecisions.textContent=String(dash.decisions);if(dashboardDocumented)dashboardDocumented.textContent=String(dash.documented);if(dashboardProjects)dashboardProjects.textContent=String(dash.projects);if(dashboardRecords)dashboardRecords.textContent=String(dash.records);renderResearchCollectionControls();
      if(!dynamicPreview)return;const rows=search.search(all,integratedPrefs,state).slice(0,80),groups=api.group(rows,dynamicGroupBy);dynamicPreview.classList.toggle('is-compact',dynamicDensity==='compact');dynamicPreview.innerHTML='';if(!rows.length){dynamicPreview.innerHTML='<div class="scw-knowledge-empty-note">No canonical records match this dynamic view.</div>';return;}
      groups.forEach(group=>{const section=document.createElement('section');section.className='scw-dynamic-group';const head=document.createElement('div');head.className='scw-dynamic-group-head';head.innerHTML=`<strong>${escapeHtml(group.label)}</strong><span>${group.rows.length} record${group.rows.length===1?'':'s'}</span>`;section.appendChild(head);group.rows.slice(0,40).forEach(row=>{const entry=row.entry,button=document.createElement('button');button.type='button';button.className='scw-dynamic-row';button.innerHTML=`<span>${escapeHtml(String(entry.kind||'').replaceAll('-',' '))}${entry.subtype?` · ${escapeHtml(entry.subtype)}`:''}</span><strong>${escapeHtml(entry.title)}</strong><small>${escapeHtml(entry.projectTitle)} · ${escapeHtml(entry.origin)}</small>`;button.addEventListener('click',()=>{selectedIntegratedKey=entry.key;renderIntegratedKnowledge();renderCompositionStudio();});section.appendChild(button);});dynamicPreview.appendChild(section);});
    }
    function crossProjectApi(){return window.SCWorkspaceCrossProjectKnowledge||null;}
    function loadCrossProjectKnowledge(){const api=crossProjectApi();if(!api)return{schema:'sc-workspace-cross-project-knowledge/1.0',references:[],updatedAt:''};try{return api.normalizeLedger(JSON.parse(localStorage.getItem(api.STORAGE_KEY)||'{"references":[]}'));}catch(_){return api.normalizeLedger({references:[]});}}
    function persistCrossProjectKnowledge(){const api=crossProjectApi();if(!api)return false;crossProjectKnowledge=api.normalizeLedger({...crossProjectKnowledge,updatedAt:nowIso()});try{localStorage.setItem(api.STORAGE_KEY,JSON.stringify(crossProjectKnowledge));return true;}catch(_){return false;}}
    function renderCrossProjectKnowledge(){if(!crossProjectSection)return;const api=crossProjectApi(),ik=integratedApi();if(!api||!ik)return;crossProjectKnowledge=api.normalizeLedger(crossProjectKnowledge);const entries=derivedIntegratedEntries(ik),rows=api.resolvedRows(crossProjectKnowledge,entries,state.projects),metrics=api.stats(crossProjectKnowledge,entries,state.projects),selected=entries.find(e=>e.key===selectedIntegratedKey)||null;if(crossProjectCount)crossProjectCount.textContent=String(metrics.references);if(crossProjectProjects)crossProjectProjects.textContent=String(metrics.targetProjects);if(crossProjectUnresolved)crossProjectUnresolved.textContent=String(metrics.unresolved);if(crossProjectTarget){const current=crossProjectTarget.value;crossProjectTarget.innerHTML='<option value="">Choose target project</option>';state.projects.filter(p=>!p.archivedAt).sort(projectSort).forEach(p=>{const o=document.createElement('option');o.value=p.id;o.textContent=p.title;if(selected&&p.id===selected.projectId)o.disabled=true;crossProjectTarget.appendChild(o);});if([...crossProjectTarget.options].some(o=>o.value===current&&!o.disabled))crossProjectTarget.value=current;}if(crossProjectAdd)crossProjectAdd.disabled=!selected||!crossProjectTarget?.value||selected.projectId===crossProjectTarget.value;if(crossProjectList){crossProjectList.innerHTML='';if(!rows.length)crossProjectList.innerHTML='<div class="scw-knowledge-empty-note">No cross-project references yet. Select research above, choose another project as the target context, and add an explicit relationship.</div>';rows.forEach(row=>{const r=row.reference,article=document.createElement('article');article.className=`scw-cross-project-row${row.resolved?'':' is-unresolved'}`;const sourceLabel=row.source?row.source.title:`Missing source · ${r.source.projectId}:${r.source.id}`,targetLabel=row.target?row.target.title:`Missing target · ${r.targetProjectId}`;article.innerHTML=`<span>${escapeHtml(r.relation.replaceAll('-',' '))} · ${row.resolved?'RESOLVED':'UNRESOLVED'}</span><strong>${escapeHtml(targetLabel)} ← ${escapeHtml(sourceLabel)}</strong><small>Source project: ${escapeHtml(row.source?.projectTitle||r.source.projectId)} · Target project: ${escapeHtml(targetLabel)}</small>${r.note?`<p>${escapeHtml(r.note)}</p>`:''}`;const actions=document.createElement('div');actions.className='scw-knowledge-actions';const openSource=document.createElement('button');openSource.type='button';openSource.className='scw-card-action';openSource.textContent='Open source';openSource.disabled=!row.source;openSource.addEventListener('click',()=>{if(row.source){selectedIntegratedKey=row.source.key;renderIntegratedKnowledge();openIntegratedEntry(row.source);}});const openTarget=document.createElement('button');openTarget.type='button';openTarget.className='scw-card-action';openTarget.textContent='Open target project';openTarget.disabled=!row.target||Boolean(row.target?.archivedAt);openTarget.addEventListener('click',()=>{if(!row.target||row.target.archivedAt)return;state.activeProjectId=row.target.id;persist('Target project opened from cross-project knowledge');render();setWorkspaceView('projects');});const remove=document.createElement('button');remove.type='button';remove.className='scw-card-action scw-card-action-muted';remove.textContent='Remove reference';remove.addEventListener('click',()=>{crossProjectKnowledge=api.removeReference(crossProjectKnowledge,r.id);persistCrossProjectKnowledge();renderCrossProjectKnowledge();renderKnowledgeGraph();if(crossProjectStatus)crossProjectStatus.textContent='Cross-project reference removed. Source and target projects were unchanged.';});actions.append(openSource,openTarget,remove);article.appendChild(actions);crossProjectList.appendChild(article);});}}

    function compositionApi(){return window.SCWorkspaceCompositionStudio||null;}
    function loadCompositionLibrary(){const api=compositionApi();if(!api)return{schema:'sc-workspace-composition-library/1.0',drafts:[],updatedAt:''};try{return api.normalizeLibrary(JSON.parse(localStorage.getItem(api.STORAGE_KEY)||'{"drafts":[]}'));}catch(_){return api.normalizeLibrary({drafts:[]});}}
    function persistCompositionLibrary(){const api=compositionApi();if(!api)return false;compositionLibrary=api.normalizeLibrary({...compositionLibrary,updatedAt:nowIso()});try{localStorage.setItem(api.STORAGE_KEY,JSON.stringify(compositionLibrary));return true;}catch(_){return false;}}
    function activeComposition(){return compositionLibrary.drafts.find(x=>x.id===activeCompositionId)||null;}
    function activeCompositionSection(){const d=activeComposition();return d?d.sections.find(x=>x.id===activeCompositionSectionId)||null:null;}
    function renderCompositionStudio(){if(!compositionSection)return;const api=compositionApi(),refApi=referenceLibraryApi();if(!api)return;const drafts=compositionLibrary.drafts;let draft=activeComposition();if(!draft&&drafts.length){activeCompositionId=drafts[0].id;draft=drafts[0];}
      if(compositionDraftCount)compositionDraftCount.textContent=String(drafts.length);const tr=draft?api.trace(draft):{sources:[],citations:[]};if(compositionInputCount)compositionInputCount.textContent=String(tr.sources.length+tr.citations.length);
      if(compositionDraftSelect){compositionDraftSelect.innerHTML='<option value="">Choose composition</option>';drafts.forEach(x=>{const o=document.createElement('option');o.value=x.id;o.textContent=x.title;compositionDraftSelect.appendChild(o);});compositionDraftSelect.value=draft?draft.id:'';}
      [compositionDelete,compositionExport,compositionMaterialize].forEach(b=>{if(b)b.disabled=!draft;});if(compositionTitle){compositionTitle.disabled=!draft;if(document.activeElement!==compositionTitle)compositionTitle.value=draft?.title||'';}if(compositionAbstract){compositionAbstract.disabled=!draft;if(document.activeElement!==compositionAbstract)compositionAbstract.value=draft?.abstract||'';}if(compositionStatusInput){compositionStatusInput.disabled=!draft;compositionStatusInput.value=draft?.status||'draft';}
      if(compositionAttachResearch)compositionAttachResearch.disabled=!draft||!activeCompositionSection()||!selectedIntegratedKey;if(compositionAttachReference)compositionAttachReference.disabled=!draft||!activeCompositionSection()||!selectedReferenceId;
      if(compositionSectionList){compositionSectionList.innerHTML='';if(!draft||!draft.sections.length)compositionSectionList.innerHTML='<div class="scw-knowledge-empty-note">Create a composition and add sections to begin.</div>';else draft.sections.forEach((sec,index)=>{const row=document.createElement('article');row.className=`scw-composition-section-row${sec.id===activeCompositionSectionId?' is-selected':''}`;row.innerHTML=`<button type="button" class="scw-composition-section-select"><span>${escapeHtml(sec.kind.replaceAll('-',' '))}</span><strong>${escapeHtml(sec.title)}</strong><small>${sec.sourceRefs.length} research · ${sec.citationRefs.length} citations</small></button><div class="scw-composition-section-actions"><button type="button" data-move="up" ${index===0?'disabled':''}>↑</button><button type="button" data-move="down" ${index===draft.sections.length-1?'disabled':''}>↓</button><button type="button" data-remove="1">Remove</button></div>`;row.querySelector('.scw-composition-section-select').addEventListener('click',()=>{activeCompositionSectionId=sec.id;renderCompositionStudio();});row.querySelectorAll('[data-move]').forEach(b=>b.addEventListener('click',()=>{const next=api.moveSection(draft,sec.id,b.dataset.move);compositionLibrary.drafts=compositionLibrary.drafts.map(x=>x.id===draft.id?next:x);persistCompositionLibrary();renderCompositionStudio();}));row.querySelector('[data-remove]').addEventListener('click',()=>{draft.sections=draft.sections.filter(x=>x.id!==sec.id);draft.updatedAt=nowIso();if(activeCompositionSectionId===sec.id)activeCompositionSectionId=draft.sections[0]?.id||'';persistCompositionLibrary();renderCompositionStudio();});compositionSectionList.appendChild(row);});}
      if(compositionPreview){if(!draft)compositionPreview.textContent='Select or create a composition to preview it.';else compositionPreview.textContent=api.markdown(draft,referenceLibrary,refApi?refApi.format:null,citationPreferences.style);}
    }
    function referenceLibraryApi(){return window.SCWorkspaceReferenceLibrary||null;}
    function loadReferenceLibrary(){const api=referenceLibraryApi();if(!api)return{schema:'sc-workspace-reference-library/1.0',references:[],updatedAt:''};try{return api.normalizeLibrary(JSON.parse(localStorage.getItem(api.STORAGE_KEY)||'{"references":[]}'));}catch(_){return api.normalizeLibrary({references:[]});}}
    function loadCitationPreferences(){const api=referenceLibraryApi();if(!api)return{schema:'sc-workspace-citation-preferences/1.0',style:'apa7',showUrls:true,updatedAt:''};try{return api.preferences(JSON.parse(localStorage.getItem(api.PREFS_KEY)||'{}'));}catch(_){return api.preferences({});}}
    function persistReferenceLibrary(){const api=referenceLibraryApi();if(!api)return false;try{referenceLibrary.updatedAt=nowIso();localStorage.setItem(api.STORAGE_KEY,api.serializeLibrary(referenceLibrary));return true;}catch(_){return false;}}
    function persistCitationPreferences(){const api=referenceLibraryApi();if(!api)return false;try{citationPreferences=api.preferences(citationPreferences);localStorage.setItem(api.PREFS_KEY,JSON.stringify(citationPreferences));return true;}catch(_){return false;}}
    function clearReferenceForm(){if(!referenceForm)return;referenceForm.reset();const hidden=referenceForm.querySelector('[name="id"]');if(hidden)hidden.value='';selectedReferenceId='';renderReferenceLibrary();}
    function populateReferenceForm(ref){if(!referenceForm||!ref)return;const set=(name,value)=>{const el=referenceForm.elements.namedItem(name);if(el)el.value=value==null?'':String(value);};set('id',ref.id);set('type',ref.type);set('title',ref.title);set('authors',(ref.authors||[]).join('; '));set('publicationDate',ref.publicationDate);set('containerTitle',ref.containerTitle);set('publisher',ref.publisher);set('pages',ref.pages);set('doi',ref.doi);set('identifier',ref.identifier);set('url',ref.url);set('citationKey',ref.citationKey);set('tags',(ref.tags||[]).join(', '));set('notes',ref.notes);}
    function renderReferenceLibrary(){
      if(!referenceLibrarySection)return;const api=referenceLibraryApi();if(!api)return;referenceLibrary=api.normalizeLibrary(referenceLibrary);citationPreferences=api.preferences(citationPreferences);const refs=referenceLibrary.references,dups=api.duplicateGroups(refs),q=String(referenceQuery||'').trim().toLowerCase();
      if(referenceCount)referenceCount.textContent=String(refs.length);if(referenceDuplicates)referenceDuplicates.textContent=String(dups.length);if(citationStyle)citationStyle.value=citationPreferences.style;if(referenceSearch&&document.activeElement!==referenceSearch)referenceSearch.value=referenceQuery;if(referenceAddSelected)referenceAddSelected.disabled=!selectedIntegratedKey;
      const filtered=refs.filter(r=>!q||[r.title,(r.authors||[]).join(' '),r.doi,r.identifier,r.citationKey,(r.tags||[]).join(' ')].join(' ').toLowerCase().includes(q));
      if(referenceList){referenceList.innerHTML='';if(!filtered.length)referenceList.innerHTML='<div class="scw-knowledge-empty-note">No references match this view.</div>';filtered.slice().sort((a,b)=>a.title.localeCompare(b.title)).forEach((ref,index)=>{const card=document.createElement('article');card.className=`scw-reference-card${selectedReferenceId===ref.id?' is-selected':''}`;const duplicate=dups.some(g=>g.references.some(x=>x.id===ref.id));card.innerHTML=`<span>${escapeHtml(ref.type.toUpperCase())}${duplicate?' · DUPLICATE CANDIDATE':''}</span><strong>${escapeHtml(ref.title)}</strong><p>${escapeHtml((ref.authors||[]).join('; ')||'Author not recorded')}${ref.publicationDate?` · ${escapeHtml(ref.publicationDate)}`:''}</p><small>${escapeHtml(ref.citationKey||'No citation key')}${ref.doi?` · DOI ${escapeHtml(ref.doi)}`:''}</small>`;const acts=document.createElement('div');acts.className='scw-knowledge-actions';const inspect=document.createElement('button');inspect.type='button';inspect.className='scw-card-action';inspect.textContent='Inspect';inspect.addEventListener('click',()=>{selectedReferenceId=ref.id;renderReferenceLibrary();renderCompositionStudio();});const edit=document.createElement('button');edit.type='button';edit.className='scw-card-action';edit.textContent='Edit';edit.addEventListener('click',()=>{selectedReferenceId=ref.id;populateReferenceForm(ref);renderReferenceLibrary();renderCompositionStudio();referenceForm?.scrollIntoView({behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'nearest'});});acts.append(inspect,edit);card.appendChild(acts);referenceList.appendChild(card);});}
      if(referenceDetail){referenceDetail.innerHTML='';const ref=refs.find(x=>x.id===selectedReferenceId);if(!ref){referenceDetail.innerHTML='<div class="scw-knowledge-empty-note">Select a reference to inspect its citation, normalized identifiers, origin links, and duplicate candidates.</div>';}else{const duplicateGroups=dups.filter(g=>g.references.some(x=>x.id===ref.id)),citation=api.format(ref,citationPreferences.style,Math.max(1,refs.indexOf(ref)+1));referenceDetail.innerHTML=`<span>CITATION PREVIEW · ${escapeHtml(citationPreferences.style.toUpperCase())}</span><strong>${escapeHtml(ref.title)}</strong><p class="scw-reference-citation">${escapeHtml(citation||'Citation cannot be formatted until more bibliographic fields are recorded.')}</p><div class="scw-retrieval-detail-group"><span>NORMALIZED IDENTIFIERS</span><em>Citation key: ${escapeHtml(ref.citationKey||'not assigned')}</em><em>DOI: ${escapeHtml(ref.doi||'not recorded')}</em><em>Identifier: ${escapeHtml(ref.identifier||'not recorded')}</em><em>Fingerprint: ${escapeHtml(api.fingerprint(ref))}</em></div><div class="scw-retrieval-detail-group"><span>CANONICAL ORIGINS</span>${(ref.origins||[]).length?ref.origins.map(o=>`<em>${escapeHtml(o.kind||'record')} · ${escapeHtml(o.projectId||'')} · ${escapeHtml(o.id||o.key||'')}</em>`).join(''):'<em>No canonical Workspace origin recorded.</em>'}</div><div class="scw-retrieval-detail-group"><span>DUPLICATE REVIEW</span>${duplicateGroups.length?`<em>${duplicateGroups[0].references.length} references share this deterministic fingerprint. Review manually; Workspace will not merge them automatically.</em>`:'<em>No duplicate candidate detected.</em>'}</div>`;const acts=document.createElement('div');acts.className='scw-reference-detail-actions';const edit=document.createElement('button');edit.type='button';edit.className='scw-button';edit.textContent='Edit';edit.addEventListener('click',()=>populateReferenceForm(ref));const copy=document.createElement('button');copy.type='button';copy.className='scw-button';copy.textContent='Copy citation';copy.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(citation);if(referenceStatus)referenceStatus.textContent='Citation copied.';}catch(_){if(referenceStatus)referenceStatus.textContent='Clipboard access unavailable; select the citation text manually.';}});const remove=document.createElement('button');remove.type='button';remove.className='scw-button';remove.textContent='Delete';remove.addEventListener('click',()=>{if(!window.confirm(`Delete reference “${ref.title}”? Project research records will not be changed.`))return;referenceLibrary.references=refs.filter(x=>x.id!==ref.id);selectedReferenceId='';persistReferenceLibrary();renderReferenceLibrary();if(referenceStatus)referenceStatus.textContent='Reference removed from the browser-local library. Canonical research was unchanged.';});acts.append(edit,copy,remove);referenceDetail.appendChild(acts);}}
    }
    function openIntegratedEntry(entry){
      if(!entry)return; state.activeProjectId=entry.projectId; const project=activeProject();
      if(entry.kind==='object'&&project){project.activeObjectId=entry.id;activeProjectMode='object';setWorkspaceView('projects');return;}
      if((entry.kind==='notebook'||entry.kind==='notebook-block')&&project){if(project.notebooks){project.notebooks.activeNotebookId=entry.notebookId||entry.id;if(entry.kind==='notebook-block'){const nb=(project.notebooks.notebooks||[]).find(n=>n.id===entry.notebookId);if(nb&&entry.sectionId)nb.activeSectionId=entry.sectionId;}}persist('Integrated Knowledge origin opened');setWorkspaceView('notebook');return;}
      if((entry.kind==='research-question'||entry.kind==='research-claim')&&project){activeProjectMode='research';persist('Integrated Knowledge research origin opened');setWorkspaceView('projects');return;}
    }
    function renderIntegratedKnowledge(){
      if(!integratedSection)return;const api=integratedApi(),search=knowledgeSearchApi();if(!api){if(integratedResults)integratedResults.innerHTML='<div class="scw-knowledge-empty-note">Integrated Knowledge helper unavailable.</div>';return;}
      const all=universalEntries(), uapi=universalSearchApi(), metrics=uapi?uapi.stats(all):api.stats(all), prefs=search?search.normalizePrefs(integratedPrefs):integratedPrefs, rows=search?search.search(all,prefs,state):api.filter(all,prefs).map(entry=>({entry,rank:{score:0,reasons:[]}})), facets=search?search.facets(all):{subtypes:[],origins:[]}; integratedPrefs=prefs;
      if(integratedTotal)integratedTotal.textContent=String(metrics.total);if(integratedObjects)integratedObjects.textContent=String(metrics.objects!==undefined?metrics.objects:(metrics.byKind?.object||0));if(integratedNotebooks)integratedNotebooks.textContent=String(metrics.notebooks!==undefined?metrics.notebooks:((metrics.byKind?.notebook||0)+(metrics.byKind?.['notebook-block']||0)));if(integratedResearch)integratedResearch.textContent=String(metrics.research!==undefined?metrics.research:((metrics.byKind?.['research-question']||0)+(metrics.byKind?.['research-claim']||0)+(metrics.byKind?.['analysis-question']||0)+(metrics.byKind?.decision||0)));if(retrievalCount)retrievalCount.textContent=String(rows.length);
      if(integratedSearch&&document.activeElement!==integratedSearch)integratedSearch.value=prefs.query;if(integratedKind)integratedKind.value=prefs.kind;if(retrievalTag&&document.activeElement!==retrievalTag)retrievalTag.value=prefs.tag;if(retrievalProvenance)retrievalProvenance.value=prefs.provenance;if(retrievalScope)retrievalScope.value=prefs.scope;if(retrievalSort)retrievalSort.value=prefs.sort;
      if(integratedProject){const current=prefs.project;integratedProject.innerHTML='<option value="all">All projects</option>';state.projects.slice().sort(projectSort).forEach(p=>{const o=document.createElement('option');o.value=p.id;o.textContent=`${p.title}${p.archivedAt?' (archived)':''}`;integratedProject.appendChild(o);});integratedProject.value=state.projects.some(p=>p.id===current)?current:'all';}
      if(retrievalSubtype){const current=prefs.subtype;retrievalSubtype.innerHTML='<option value="all">All subtypes</option>';facets.subtypes.forEach(value=>{const o=document.createElement('option');o.value=value;o.textContent=value.replaceAll('-',' ');retrievalSubtype.appendChild(o);});retrievalSubtype.value=facets.subtypes.includes(current)?current:'all';}
      if(retrievalOrigin){const current=prefs.origin;retrievalOrigin.innerHTML='<option value="all">All origins</option>';facets.origins.forEach(value=>{const o=document.createElement('option');o.value=value;o.textContent=value;retrievalOrigin.appendChild(o);});retrievalOrigin.value=facets.origins.includes(current)?current:'all';}
      renderSavedKnowledgeSearches();
      if(integratedResults){integratedResults.innerHTML='';if(!rows.length)integratedResults.innerHTML='<div class="scw-knowledge-empty-note">No local Workspace records match these retrieval fields.</div>';rows.slice(0,integratedRenderLimit).forEach(row=>{const e=row.entry, facts=search?search.provenanceFacts(e,state):{documented:false,linked:false};const card=document.createElement('article');card.className=`scw-integrated-result${selectedIntegratedKey===e.key?' is-selected':''}`;const reason=row.rank.reasons.slice(0,2).join(' · ');card.innerHTML=`<span>${escapeHtml(e.origin)} · ${escapeHtml(e.projectTitle)}</span><strong>${escapeHtml(e.title)}</strong><p>${escapeHtml(e.summary||'No summary yet.')}</p><small>${escapeHtml(e.kind.replaceAll('-',' '))}${e.subtype?` · ${escapeHtml(e.subtype)}`:''}</small><div class="scw-retrieval-badges">${facts.documented?'<em>PROVENANCE</em>':''}${facts.linked?'<em>LINKED</em>':''}</div>${reason?`<div class="scw-rank-reason">${escapeHtml(reason)}</div>`:''}`;const acts=document.createElement('div');acts.className='scw-knowledge-actions';const inspect=document.createElement('button');inspect.type='button';inspect.className='scw-card-action';inspect.textContent='Inspect';inspect.addEventListener('click',()=>{selectedIntegratedKey=e.key;renderIntegratedKnowledge();renderCompositionStudio();});const open=document.createElement('button');open.type='button';open.className='scw-card-action';open.textContent='Open origin';open.addEventListener('click',()=>openUniversalEntry(e));acts.append(inspect,open);card.appendChild(acts);integratedResults.appendChild(card);});if(integratedLoadMore){integratedLoadMore.hidden=rows.length<=integratedRenderLimit;integratedLoadMore.textContent=`Load ${Math.min(120,Math.max(0,rows.length-integratedRenderLimit))} more`;}}
      if(integratedDetail){integratedDetail.innerHTML='';const selected=all.find(e=>e.key===selectedIntegratedKey),selectedRow=rows.find(r=>r.entry.key===selectedIntegratedKey);if(!selected){integratedDetail.innerHTML='<div class="scw-knowledge-empty-note">Select a result to inspect its canonical origin, ranking reasons, provenance, and related material.</div>';}else{const con=['object','notebook','notebook-block','research-question','research-claim'].includes(selected.kind)?api.connections(selected,state):[],facts=search?search.provenanceFacts(selected,state):{sourceTitle:'',sourceUrl:'',bibliographic:false,documented:false,linked:con.length>0},related=search?search.related(selected,all,state):[];integratedDetail.innerHTML=`<span>CANONICAL ORIGIN</span><strong>${escapeHtml(selected.title)}</strong><p>${escapeHtml(selected.origin)} · ${escapeHtml(selected.projectTitle)}</p><small>${escapeHtml(selected.ref.schema)} · ${escapeHtml(selected.ref.kind)} · ${escapeHtml(selected.ref.id)}</small><div class="scw-retrieval-detail-group"><span>WHY THIS RESULT</span>${selectedRow?`<em>Deterministic retrieval score: ${Number(selectedRow.rank.score||0)}</em>`:''}${selectedRow&&selectedRow.rank.reasons.length?selectedRow.rank.reasons.map(r=>`<em>${escapeHtml(r)}</em>`).join(''):'<em>Result selected outside the current filtered result set.</em>'}</div><div class="scw-retrieval-detail-group"><span>RECORDED PROVENANCE</span>${facts.sourceTitle?`<em>${escapeHtml(facts.sourceTitle)}</em>`:''}${facts.sourceUrl?`<em>${escapeHtml(facts.sourceUrl)}</em>`:''}${facts.bibliographic?'<em>Bibliographic context recorded</em>':''}${!facts.documented?'<em>No source title, URL, or bibliographic context recorded.</em>':''}</div><div class="scw-integrated-connections"><span>EXPLICIT CONNECTIONS</span>${con.length?con.map(c=>`<em>${escapeHtml(c.relation)} · ${escapeHtml(c.source)}</em>`).join(''):'<em>No explicit connection records for this item.</em>'}</div><div class="scw-retrieval-related"><span>RELATED MATERIAL</span><div data-scw-related-list></div></div>`;const holder=integratedDetail.querySelector('[data-scw-related-list]');if(holder){if(!related.length)holder.innerHTML='<em>No related records found from explicit relationships, notebook containment, or the same recorded source.</em>';related.forEach(item=>{const b=document.createElement('button');b.type='button';b.className='scw-related-result';b.innerHTML=`<strong>${escapeHtml(item.entry.title)}</strong><small>${escapeHtml(item.reason)} · ${escapeHtml(item.source)}</small>`;b.addEventListener('click',()=>{selectedIntegratedKey=item.entry.key;renderIntegratedKnowledge();renderCompositionStudio();});holder.appendChild(b);});}}}
      renderResearchCollections(all);
      renderReferenceLibrary();
      renderCrossProjectKnowledge();
      renderResearchTasks();
    renderCollaborationArchitecture();
    ...
    function graphNodeLabel(node){const labels={'project':'Project','provenance':'Provenance','library-record':'Library record','notebook':'Notebook','notebook-block':'Notebook block','research-question':'Research question','research-claim':'Research claim','reference':'Citation reference','synthesis':'Synthesis','canvas-node':'Canvas node'};return labels[node.type]||(OBJECT_LABELS[node.type]||node.type);}
    function openGraphNode(node){if(!node)return;if(node.type==='project'){const project=state.projects.find(p=>p.id===node.projectId&&!p.archivedAt);if(!project)return;state.activeProjectId=project.id;persist('Project opened from Research Graph');render();setWorkspaceView('projects');activePanel.scrollIntoView({behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'});return;}if(node.integratedKey){const entry=integratedApi()?.derive(state).find(e=>e.key===node.integratedKey);if(entry){openIntegratedEntry(entry);return;}}if(node.type==='reference'&&node.referenceId){selectedReferenceId=node.referenceId;setWorkspaceView('research');renderIntegratedKnowledge();renderReferenceLibrary();return;}if(node.type==='synthesis'&&node.projectId){state.activeProjectId=node.projectId;const project=activeProject();if(project?.notebooks){project.notebooks.activeSynthesisId=node.synthesisId||node.entityId||null;}persist('Synthesis opened from Research Graph');setWorkspaceView('notebook');return;}if(node.objectId){openKnowledgeEntry({projectId:node.projectId,objectId:node.objectId,key:`${node.projectId}:${node.objectId}`,object:state.projects.find(p=>p.id===node.projectId)?.objects.find(o=>o.id===node.objectId)});}}
    ...
    function renderKnowledgeGraph(){if(!graphSection)return;state.knowledgeGraph=normalizeKnowledgeGraph(state.knowledgeGraph,state.projects);const graph=buildKnowledgeGraph(),prefs=state.knowledgeGraph.preferences;if(graphPathFrom&&graphPathTo){const selectedFrom=graphPathFrom.value,selectedTo=graphPathTo.value;const options=graph.nodes.slice().sort((a,b)=>String(a.label||'').localeCompare(String(b.label||''))).slice(0,800);const fill=(el,current,label)=>{el.innerHTML=`<option value="">${label}</option>`;options.forEach(n=>{const o=document.createElement('option');o.value=n.id;o.textContent=`${graphNodeLabel(n)} · ${String(n.label||'').slice(0,80)}`;el.appendChild(o);});if(options.some(n=>n.id===current))el.value=current;};fill(graphPathFrom,selectedFrom,'Choose starting node');fill(graphPathTo,selectedTo,'Choose destination node');}const projectIds=new Set(graph.nodes.filter(n=>n.type==='project').map(n=>n.projectId));if(graphSearch&&document.activeElement!==graphSearch)graphSearch.value=prefs.query;if(graphNodeType)graphNodeType.value=prefs.nodeType;if(graphRelation)graphRelation.value=prefs.relation;if(graphScope)graphScope.value=prefs.scope;if(graphDepth)graphDepth.value=String(prefs.depth);if(graphProject){const current=prefs.project;graphProject.innerHTML='<option value="all">All projects</option>';state.projects.slice().sort(projectSort).forEach(p=>{const o=document.createElement('option');o.value=p.id;o.textContent=`${p.title}${p.archivedAt?' (archived)':''}`;graphProject.appendChild(o);});graphProject.value=projectIds.has(current)?current:'all';}
      ...
      if(graphRelations){graphRelations.innerHTML='';const ledger=focus&&relationshipExplorer&&typeof relationshipExplorer.backlinks==='function'?relationshipExplorer.backlinks(graph,focus.id,prefs.relation):[];if(!focus||!ledger.length)graphRelations.innerHTML='<div class="scw-knowledge-empty-note">No relationships are visible for this focus and filter.</div>';else ledger.slice(0,80).forEach(item=>{const edge=item.edge,other=item.other;if(!other)return;const row=document.createElement('button');row.type='button';row.className='scw-graph-relation-row';const why=relationshipExplorer.edgeExplanation?relationshipExplorer.edgeExplanation(edge):edge.relation.replaceAll('-',' ');row.innerHTML=`<span>${escapeHtml(item.direction.toUpperCase())} · ${escapeHtml(edge.relation.replaceAll('-',' '))}</span><strong>${escapeHtml(other.label)}</strong><small>${escapeHtml(graphNodeLabel(other))}${other.projectTitle?` · ${escapeHtml(other.projectTitle)}`:''}<br>${escapeHtml(why)}</small>`;row.addEventListener('click',()=>{state.knowledgeGraph.selectedNodeId=other.id;touchKnowledgeGraph();persist('Knowledge Graph focus saved');renderKnowledgeGraph();});graphRelations.appendChild(row);});}
    }
    ...
    function renderActive() {
      ...
      renderLabIntegration(project);
      renderSpecialistIntegration(project);
    ...
    function render() {
      const renderStarted=window.performance&&typeof window.performance.now==='function'?window.performance.now():Date.now();
      renderIdentity();
      renderCloudRecovery();
      renderAccountSync();
      renderFilters();
      ...
      const renderEnded=window.performance&&typeof window.performance.now==='function'?window.performance.now():Date.now();
      longSessionMonitor?.markRender(renderEnded-renderStarted,'workspace-render');
    }

    /* v2.28.1 interaction safety net. Normal direct handlers remain authoritative.
    ...
    root.addEventListener('click', (event) => {
      ...
      if (target.matches('[data-scw-project-mode]')) {
        event.preventDefault();
        setProjectMode(target.dataset.scwProjectMode || 'overview');
        setWorkspaceView('projects', true);
        return;
      }
      ...
      if (target.matches('[data-scw-cancel-create]')) {
        event.preventDefault();
        if (createForm) { createForm.reset(); createForm.hidden = true; }
        return;
      }
      ...
      if (target.matches('[data-scw-new-object]')) {
        event.preventDefault();
        if (objectCreateForm) { objectCreateForm.hidden = false; const title = objectCreateForm.querySelector('input[name="title"]'); if (title) title.focus(); }
        return;
      }
      if (target.matches('[data-scw-cancel-object]')) {
        event.preventDefault();
        if (objectCreateForm) { objectCreateForm.reset(); objectCreateForm.hidden = true; }
      }
    });
    ...
    bindControl('[data-scw-new-project]', 'click', () => {
      createForm.hidden = false;
      createForm.querySelector('input[name="title"]').focus();
    });

    bindControl('[data-scw-cancel-create]', 'click', () => {
      createForm.reset();
    ...
    if (titleInput) titleInput.addEventListener('input', () => updateProject((project) => { project.title = titleInput.value.trim().slice(0, 120) || 'Untitled project'; }));
    if (titleInput) titleInput.addEventListener('change', () => { const project = activeProject(); if (project) addActivity(project, 'renamed', 'Project name updated'); persist(); render(); });
    if (descriptionInput) descriptionInput.addEventListener('input', () => updateProject((project) => { project.description = descriptionInput.value.slice(0, 600); }));
    if (statusInput) statusInput.addEventListener('change', () => updateProject((project) => { project.status = ALLOWED_STATUS.has(statusInput.value) ? statusInput.value : 'active'; addActivity(project, 'status', `Status changed to ${project.status}`); }));
    if (notesInput) notesInput.addEventListener('input', () => updateProject((project) => { project.notes = notesInput.value.slice(0, 20000); }));

    bindControl('[data-scw-pin]', 'click', () => {
    ...
    bindControl('[data-scw-duplicate]', 'click', () => {
      const project = activeProject(); if (!project) return;
    ...
    bindControl('[data-scw-export]', 'click', () => {
      const project = activeProject(); if (!project) return;
      ...
      const hardening=importExportCompatibilityApi();
      const prepared=hardening?.currentProjectExport ? hardening.currentProjectExport(portable, root.dataset.version || '0.66.0', normalizeProject) : null;
      if(prepared && (!prepared.ok || !prepared.receipt?.equivalent)){window.alert('Workspace blocked this export because the project did not survive the current round-trip compatibility check. Create a recovery export before changing the project.');return;}
      const payload = prepared?.payload || { schema: EXPORT_SCHEMA, workspaceVersion: root.dataset.version || '0.66.0', exportedAt: nowIso(), project: portable };
      downloadJson(`${safeFileName(project.title)}.sc-workspace.json`, payload);
      addActivity(project, 'exported', `Project exported as JSON${prepared?.receipt?.equivalent?' · round-trip checked':''}`); project.updatedAt = nowIso(); persist('Export recorded'); renderActive();
    });

    bindControl('[data-scw-archive]', 'click', () => {
      const project = activeProject(); if (!project) return;
    ...
    if (importFile) importFile.addEventListener('change', async () => {
      stagedProjectImport=null;
      const file = importFile.files && importFile.files[0]; if (!file) { renderProjectImportStage(); return; }
      const hardening=importExportCompatibilityApi();
      if(file.size > (hardening?.MAX_IMPORT_BYTES || 8*1024*1024)){
        stagedProjectImport={fileName:file.name,assessment:{status:'blocked',errors:['Project import files are limited to 8 MB in this local release.'],warnings:[]}};renderProjectImportStage();importFile.value='';return;
      }
      try {
        const payload=JSON.parse(await readFileTextCompat(file));
        const assessment=hardening?.assessProjectImport
          ? hardening.assessProjectImport(payload,state.projects.map(item=>item.id))
          : {status:'blocked',errors:['Import compatibility runtime is unavailable.'],warnings:[]};
        const classified=hardening?.classifyProjectPayload ? hardening.classifyProjectPayload(payload) : null;
        stagedProjectImport={fileName:file.name,payload,rawProject:classified?.ok?classified.project:null,assessment};
        renderProjectImportStage();
      } catch (err) {
        stagedProjectImport={fileName:file.name,assessment:{status:'blocked',errors:[err?.message||'Workspace could not parse this project JSON file.'],warnings:[]}};renderProjectImportStage();
      } finally { importFile.value=''; }
    });
    if(projectImportClear)projectImportClear.addEventListener('click',()=>{stagedProjectImport=null;renderProjectImportStage();});
    if(projectImportCommit)projectImportCommit.addEventListener('click',()=>{
      if(!stagedProjectImport||stagedProjectImport.assessment?.status!=='ready'||!stagedProjectImport.rawProject)return;
      const assessment=stagedProjectImport.assessment,project=normalizeProject(stagedProjectImport.rawProject);
      if(!project){window.alert('Workspace rejected the staged project during current-schema normalization. Nothing was imported.');return;}
      const sourceId=String(assessment.sourceProjectId||project.id||'');
      project.id=id('scwp');
      project.persistence=projectPersistenceTemplate();
      project.archivedAt=null;project.activeObjectId=null;project.pinned=false;project.updatedAt=nowIso();
      if(state.projects.some(item=>item.title===project.title))project.title=`${project.title} (Imported)`.slice(0,120);
      addActivity(project,'imported',`Project imported as a new local copy from ${assessment.sourceProjectSchema||'legacy schema'}${sourceId?` · source project ${sourceId}`:''}`);
      state.projects.push(project);state.activeProjectId=project.id;activeProjectMode='overview';
      const sourceSchema=assessment.sourceProjectSchema||'unknown';stagedProjectImport=null;persist(`Staged project import committed as a new local copy from ${sourceSchema}`);render();renderProjectImportStage();setWorkspaceView('projects');
    });
    if(compatibilityMatrixExport)compatibilityMatrixExport.addEventListener('click',()=>{const api=importExportCompatibilityApi();if(!api)return;downloadJson(`workspace-backward-compatibility-v${rootVersion()}.json`,api.compatibilityMatrix());});



    if(cockpitUniversalSearch)cockpitUniversalSearch.addEventListener('click',()=>{setWorkspaceView('research');setResearchSurface('search',true);window.setTimeout(()=>integratedSearch?.focus(),0);});
    ...
    if (notebookSynthesisForm) notebookSynthesisForm.addEventListener('submit',event=>{event.preventDefault();const project=notebookSelectedProject(),helper=notebookHelper();if(!project||!helper)return;project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);if(project.notebooks.syntheses.length>=helper.MAX_SYNTHESES){window.alert(`This project has reached the ${helper.MAX_SYNTHESES}-synthesis limit.`);return;}const data=new FormData(notebookSynthesisForm),kind=String(data.get('kind')||'outline'),title=String(data.get('title')||'').trim().slice(0,240)||kind.replaceAll('-',' ').replace(/\b\w/g,m=>m.toUpperCase());const refs=[...notebookSynthesisMaterial.selectedOptions].map(o=>notebookRefFromValue(project,o.value)).filter(Boolean);if(!refs.length){window.alert('Select at least one notebook block or Workspace object.');return;}const synthesis=helper.generateSynthesis?.(kind,title,refs,project.notebooks,project.objects,id,nowIso);if(!synthesis||!synthesis.selectedRefs.length){window.alert('The selected material could not be resolved.');return;}project.notebooks.syntheses.unshift(synthesis);project.notebooks.activeSynthesisId=synthesis.id;touchNotebooks(project);addActivity(project,'notebook-synthesis',`Notebook synthesis created: ${synthesis.title}`);notebookSynthesisForm.reset();persist('Notebook synthesis created');renderNotebook();});

    if(notebookPortableImport)notebookPortableImport.addEventListener('click',()=>notebookPortableFile?.click());
    if(notebookPortableFile)notebookPortableFile.addEventListener('change',async()=>{const project=notebookSelectedProject(),api=notebookPortability(),file=notebookPortableFile.files?.[0];if(!project||!api||!file)return;try{const pkg=JSON.parse(await readFileTextCompat(file)),result=await api.importAsCopy(pkg,id,nowIso);if(!result.ok)throw new Error(result.reason||'Notebook import failed.');project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);project.notebooks.notebooks.unshift(notebookHelper().notebook(result.notebook,id,nowIso));project.notebooks.activeNotebookId=result.notebook.id;const p=notebookPortabilityState(project);p.importHistory.unshift(result.history);p.importHistory=p.importHistory.slice(0,40);touchNotebooks(project);persist('Portable notebook imported as copy');renderNotebook();}catch(error){window.alert(error.message||'Notebook import failed.');}finally{notebookPortableFile.value='';}});
    if(notebookRestoreCreate)notebookRestoreCreate.addEventListener('click',async()=>{const project=notebookSelectedProject(),nb=activeNotebookFor(project),api=notebookPortability();if(!project||!nb||!api)return;const label=window.prompt('Restore point label','Before notebook changes');if(label===null)return;const point=await api.restorePoint(nb,label,'',id,nowIso),p=notebookPortabilityState(project);p.restorePoints.unshift(point);p.restorePoints=p.restorePoints.slice(0,20);touchNotebooks(project);persist('Notebook restore point created');renderNotebook();});
    if(notebookBackup)notebookBackup.addEventListener('click',async()=>{const project=notebookSelectedProject(),nb=activeNotebookFor(project),api=notebookPortability();if(!project||!nb||!api)return;try{notebookPortabilityStatus.textContent='Creating notebook backup…';const payload=await api.backupPackage(nb,{workspaceVersion:rootVersion(),projectId:project.id});const data=await cloudRequest('cloud-notebooks',{method:'POST',body:JSON.stringify(payload)});notebookPortabilityStatus.textContent=`Notebook backup saved · revision ${data?.item?.revision||1}`;}catch(error){notebookPortabilityStatus.textContent=error.message||'Notebook backup failed.';}});
    if(notebookSyncToggle)notebookSyncToggle.addEventListener('click',()=>{const project=notebookSelectedProject(),nb=activeNotebookFor(project);if(!project||!nb)return;const e=notebookSyncEnrollment(project,nb,true);e.enabled=!e.enabled;e.status=e.enabled?'not-uploaded':'disabled';e.updatedAt=nowIso();touchNotebooks(project);persist(e.enabled?'Notebook sync enabled':'Notebook sync disabled');renderNotebook();});
    if(notebookSyncCheck)notebookSyncCheck.addEventListener('click',async()=>{const project=notebookSelectedProject(),nb=activeNotebookFor(project);if(!project||!nb)return;try{const r=await evaluateNotebookSync(project,nb);notebookPortabilityStatus.textContent=`Notebook sync status: ${r.status}. No content changed.`;persist('Notebook sync status checked');renderNotebook();}catch(error){notebookPortabilityStatus.textContent=error.message||'Notebook sync status failed.';}});
    if(notebookSyncNow)notebookSyncNow.addEventListener('click',async()=>{const project=notebookSelectedProject(),nb=activeNotebookFor(project),api=notebookPortability();if(!project||!nb||!api)return;try{const r=await evaluateNotebookSync(project,nb);if(r.status==='conflict'||r.status==='remote-ahead'||r.status==='remote-missing'){notebookPortabilityStatus.textContent=`Sync stopped: ${r.status}. Use conflict-safe recovery; nothing was overwritten.`;return;}const payload=await api.syncPush(nb,r.e,{workspaceVersion:rootVersion(),projectId:project.id});const data=await cloudRequest('cloud-notebooks',{method:'POST',body:JSON.stringify(payload)});r.e.serverRevision=data.item.revision;r.e.remoteRevision=data.item.revision;r.e.lastSyncedFingerprint=data.item.notebookFingerprint;r.e.remoteFingerprint=data.item.notebookFingerprint;r.e.lastSyncedAt=nowIso();r.e.status='up-to-date';touchNotebooks(project);persist('Notebook synchronized explicitly');renderNotebook();}catch(error){notebookPortabilityStatus.textContent=error.message||'Notebook sync failed.';}});
    if(notebookSyncCloudCopy)notebookSyncCloudCopy.addEventListener('click',async()=>{const project=notebookSelectedProject(),nb=activeNotebookFor(project),api=notebookPortability();if(!project||!nb||!api)return;try{const data=await cloudRequest(`cloud-notebooks/${encodeURIComponent(nb.id)}`),cloud=data?.package?.notebook;if(!cloud)throw new Error('Cloud notebook copy is unavailable.');const copy=notebookHelper().notebook(cloud,id,nowIso);copy.id=id('nb');copy.title=(copy.title+' (Cloud copy)').slice(0,160);project.notebooks.notebooks.unshift(copy);project.notebooks.activeNotebookId=copy.id;touchNotebooks(project);persist('Cloud notebook opened as copy');renderNotebook();}catch(error){notebookPortabilityStatus.textContent=error.message||'Cloud copy could not be opened.';}});
    if(notebookSyncKeepLocal)notebookSyncKeepLocal.addEventListener('click',async()=>{const project=notebookSelectedProject(),nb=activeNotebookFor(project),api=notebookPortability();if(!project||!nb||!api)return;if(!window.confirm('Keep this local notebook as the sync head? The server revision precondition still prevents overwriting an unseen newer cloud revision.'))return;try{const r=await evaluateNotebookSync(project,nb),payload=await api.syncPush(nb,{...r.e,serverRevision:r.remote?.revision||r.e.serverRevision},{workspaceVersion:rootVersion(),projectId:project.id});const data=await cloudRequest('cloud-notebooks',{method:'POST',body:JSON.stringify(payload)});r.e.serverRevision=data.item.revision;r.e.lastSyncedFingerprint=data.item.notebookFingerprint;r.e.status='up-to-date';touchNotebooks(project);persist('Local notebook confirmed as sync head');renderNotebook();}catch(error){notebookPortabilityStatus.textContent=error.message||'Conflict resolution did not complete.';}});
    if(notebookReviewCreate)notebookReviewCreate.addEventListener('click',()=>{const project=notebookSelectedProject(),nb=activeNotebookFor(project),api=notebookReviewApi();if(!project||!nb||!api)return;const point=notebookPortabilityState(project).restorePoints.find(x=>x.id===notebookReviewBaseline?.value&&x.notebookId===nb.id);if(!point?.snapshot){window.alert('Choose a valid restore point for this notebook.');return;}const review=api.buildReview(point.snapshot,nb,{projectId:project.id,baseLabel:point.label||'Restore point',baseRevision:point.id,targetLabel:`Current · ${nb.title}`,targetRevision:nb.updatedAt},id,nowIso),g=notebookGovernanceState(project);g.reviews.unshift(review);g.reviews=g.reviews.slice(0,80);g.updatedAt=nowIso();touchNotebooks(project);addActivity(project,'notebook-change-review',`Notebook Change Review created: ${review.summary.total} change(s)`);persist('Notebook Change Review created');renderNotebook();});
    if(notebookReconcileCreate)notebookReconcileCreate.addEventListener('click',()=>{const project=notebookSelectedProject(),nb=activeNotebookFor(project),api=notebookReviewApi();if(!project||!nb||!api)return;const review=notebookReviewLatest(project,nb);if(!review){window.alert('Create a notebook Change Review first.');return;}if(nb.updatedAt!==review.target.revision){window.alert('The notebook changed after this review. Create a new Change Review before reconciling.');return;}if(!notebookReconcileAck?.checked){window.alert('Acknowledge the new-copy reconciliation boundary first.');return;}const selected=[...notebookReviewList.querySelectorAll('[data-scw-notebook-review-change]:checked')].map(x=>x.value);if(!selected.length){window.alert('Select at least one reviewed change.');return;}const point=notebookPortabilityState(project).restorePoints.find(x=>x.id===review.base.revision);if(!point?.snapshot){window.alert('The reviewed baseline restore point is no longer available.');return;}const plan=api.buildReconciliation(review,selected,{projectId:project.id,notebookId:nb.id,reviewerLabel:notebookReconcileReviewer?.value||'Workspace owner',rationale:notebookReconcileRationale?.value||''},id,nowIso),result=api.applyReconciliation(point.snapshot,nb,plan,id,nowIso);if(!result.ok){window.alert(result.message||'Notebook reconciliation could not be applied.');return;}project.notebooks.notebooks.unshift(notebookHelper().notebook(result.notebook,id,nowIso));project.notebooks.activeNotebookId=result.notebook.id;const g=notebookGovernanceState(project);g.reconciliations.unshift(result.receipt);g.reconciliations=g.reconciliations.slice(0,80);g.updatedAt=nowIso();touchNotebooks(project);addActivity(project,'notebook-reconciliation',`Reconciled notebook copy created from ${selected.length} selected change(s)`);if(notebookReconcileAck)notebookReconcileAck.checked=false;if(notebookReconcileRationale)notebookReconcileRationale.value='';persist('Reconciled notebook copy created');renderNotebook();});
    if(notebookLineageInspect)notebookLineageInspect.addEventListener('click',()=>{const project=notebookSelectedProject(),api=notebookReviewApi(),value=String(notebookLineageRef?.value||'');if(!project||!api||!value)return;const i=value.indexOf(':'),kind=value.slice(0,i),ridValue=value.slice(i+1),nb=activeNotebookFor(project);const ref={kind,id:ridValue,notebookId:kind==='block'?nb?.id||'':''};renderNotebookLineageView(api.lineage(ref,project.notebooks,project.objects));});

    if (notebookAssistanceForm) notebookAssistanceForm.addEventListener('submit',event=>{event.preventDefault();const project=notebookSelectedProject(),helper=notebookHelper();if(!project||!helper)return;project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);if(project.notebooks.assistances.length>=helper.MAX_ASSISTANCES){window.alert(`This project has reached the ${helper.MAX_ASSISTANCES}-assistance limit.`);return;}const data=new FormData(notebookAssistanceForm),question=String(data.get('question')||'').trim().slice(0,5000),title=String(data.get('title')||'').trim().slice(0,240)||String(question||'Grounded notebook question').slice(0,120);if(!question){window.alert('Enter a question to ask against the selected material.');return;}const refs=[...notebookAssistanceMaterial.selectedOptions].map(o=>notebookRefFromValue(project,o.value)).filter(Boolean).slice(0,helper.MAX_ASSISTANCE_SELECTION||48);if(!refs.length){window.alert('Select at least one notebook block or Workspace object to ground the question.');return;}const assistance=helper.prepareAssistance?.(title,question,refs,project.notebooks,project.objects,id,nowIso);if(!assistance||!assistance.selectedRefs.length){window.alert('The selected material could not be resolved.');return;}project.notebooks.assistances.unshift(assistance);project.notebooks.activeAssistanceId=assistance.id;touchNotebooks(project);addActivity(project,'notebook-assistance',`Grounded Notebook Assistance question prepared: ${assistance.title}`);notebookAssistanceForm.reset();persist('Grounded notebook question prepared');renderNotebook();});
    if (notebookCollectionForm) notebookCollectionForm.addEventListener('submit',event=>{event.preventDefault();const project=notebookSelectedProject(),helper=notebookHelper();if(!project||!helper)return;project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);if(project.notebooks.collections.length>=helper.MAX_COLLECTIONS){window.alert(`This project has reached the ${helper.MAX_COLLECTIONS}-collection limit.`);return;}const data=new FormData(notebookCollectionForm),title=String(data.get('title')||'').trim().slice(0,160);if(!title)return;const collection=helper.createCollection(title,String(data.get('description')||''),id,nowIso);project.notebooks.collections.unshift(collection);project.notebooks.activeCollectionId=collection.id;touchNotebooks(project);addActivity(project,'notebook-collection',`Notebook collection created: ${collection.title}`);notebookCollectionForm.reset();persist('Notebook collection created');renderNotebook();});
    if (notebookCollectionAssignForm) notebookCollectionAssignForm.addEventListener('submit',event=>{event.preventDefault();const project=notebookSelectedProject(),helper=notebookHelper();if(!project||!helper)return;project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);const data=new FormData(notebookCollectionAssignForm),collection=project.notebooks.collections.find(c=>c.id===String(data.get('collectionId')||'')),ref=notebookRefFromValue(project,String(data.get('itemRef')||''));if(!collection||!ref){window.alert('Choose both a collection and an item.');return;}if(!helper.addCollectionItem(collection,ref,nowIso)){window.alert('That item is already in the collection or the collection is full.');return;}project.notebooks.activeCollectionId=collection.id;touchNotebooks(project);addActivity(project,'notebook-collection-item',`Added ${ref.kind} to collection: ${collection.title}`);persist('Collection item added');renderNotebook();});
    if (notebookLinkForm) notebookLinkForm.addEventListener('submit',event=>{event.preventDefault();const project=notebookSelectedProject(),helper=notebookHelper();if(!project||!helper)return;project.notebooks=normalizeNotebooks(project.notebooks,project.objects,project.canvas);if(project.notebooks.links.length>=helper.MAX_LINKS){window.alert(`This project has reached the ${helper.MAX_LINKS}-link limit.`);return;}const data=new FormData(notebookLinkForm),source=notebookRefFromValue(project,String(data.get('sourceRef')||'')),target=notebookRefFromValue(project,String(data.get('targetRef')||'')),relation=String(data.get('relation')||'references'),note=String(data.get('note')||'');if(!source||!target){window.alert('Choose both a source and target.');return;}const link=helper.createLink(source,target,relation,note,id,nowIso);if(!link){window.alert('Choose two different items to link.');return;}const signature=`${helper.refKey(link.source)}>${link.relation}>${helper.refKey(link.target)}`;if(project.notebooks.links.some(x=>`${helper.refKey(x.source)}>${x.relation}>${helper.refKey(x.target)}`===signature)){window.alert('That explicit link already exists.');return;}project.notebooks.links.unshift(link);touchNotebooks(project);addActivity(project,'notebook-link',`Notebook link created: ${link.relation}`);notebookLinkForm.reset();persist('Notebook knowledge link created');renderNotebook();});

    if (researchQuestionForm) researchQuestionForm.addEventListener('submit', (event) => {
      event.preventDefault();
      const project = activeProject(); if (!project) return;
    ...
    if (researchSourceForm) researchSourceForm.addEventListener('submit', (event) => {
      event.preventDefault();
      const project = activeProject(); if (!project) return;
      if (project.objects.length >= MAX_OBJECTS) { window.alert(`This project has reached the ${MAX_OBJECTS}-object local limit.`); return; }
    ...
    if (researchEvidenceForm) researchEvidenceForm.addEventListener('submit', (event) => {
      event.preventDefault();
      const project = activeProject(); if (!project) return;
      if (project.objects.length >= MAX_OBJECTS) { window.alert(`This project has reached the ${MAX_OBJECTS}-object local limit.`); return; }
    ...
    if (researchClaimForm) researchClaimForm.addEventListener('submit', (event) => {
      event.preventDefault();
      const project = activeProject(); if (!project) return;
    ...
    bindControl('[data-scw-research-link-evidence]', 'click', () => {
      const project = activeProject(); if (!project) return;
    ...
    if (analysisQuestionForm) analysisQuestionForm.addEventListener('submit', (event) => {
    ...
    if (analysisDatasetForm) analysisDatasetForm.addEventListener('submit', (event)=>{
    ...
    bindControl('[data-scw-new-object]', 'click', () => {
      const project = activeProject(); if (!project) return;
    ...
    bindControl('[data-scw-cancel-object]', 'click', () => { if (objectCreateForm) { objectCreateForm.reset(); objectCreateForm.hidden = true; } });

    if (objectCreateForm) objectCreateForm.addEventListener('submit', (event) => {
      event.preventDefault();
      const project = activeProject(); if (!project) return;
      if (project.objects.length >= MAX_OBJECTS) return;
    ...
    if (objectTypeFilterInput) objectTypeFilterInput.addEventListener('change', () => { objectTypeFilter = objectTypeFilterInput.value; const project = activeProject(); if (project) renderObjects(project); });
    if (objectArchiveFilterInput) objectArchiveFilterInput.addEventListener('change', () => { objectArchiveFilter = objectArchiveFilterInput.value; const project = activeProject(); if (project) renderObjects(project); });

    if (objectTitle) objectTitle.addEventListener('input', () => updateObject((object) => { object.title = objectTitle.value.trim().slice(0, 160) || 'Untitled object'; }));
    if (objectTitle) objectTitle.addEventListener('change', () => { const project = activeProject(); const object = activeObject(); if (project && object) addActivity(project, 'object-renamed', `${OBJECT_LABELS[object.type]} renamed`); persist(); render(); });
    if (objectStatus) objectStatus.addEventListener('change', () => updateObject((object) => { object.status = OBJECT_STATUS.has(objectStatus.value) ? objectStatus.value : 'draft'; }, `Object status changed to ${objectStatus.value}`));
    if (objectSummary) objectSummary.addEventListener('input', () => updateObject((object) => { object.summary = objectSummary.value.slice(0, 1200); }));
    if (objectContent) objectContent.addEventListener('input', () => updateObject((object) => { object.content = objectContent.value.slice(0, 50000); }));
    if (objectTags) objectTags.addEventListener('change', () => updateObject((object) => { object.tags = normalizeTags(objectTags.value); objectTags.value = object.tags.join(', '); }, 'Object tags updated'));
    if (objectSourceType) objectSourceType.addEventListener('change', () => updateObject((object) => { object.provenance.sourceType = PROVENANCE_TYPES.has(objectSourceType.value) ? objectSourceType.value : 'manual'; if (!object.provenance.capturedAt) object.provenance.capturedAt = nowIso(); }, 'Object provenance updated'));
    if (objectSourceTitle) objectSourceTitle.addEventListener('input', () => updateObject((object) => { object.provenance.sourceTitle = objectSourceTitle.value.slice(0, 240); if (!object.provenance.capturedAt && object.provenance.sourceTitle) object.provenance.capturedAt = nowIso(); }));
    if (objectSourceUrl) objectSourceUrl.addEventListener('input', () => updateObject((object) => { object.provenance.sourceUrl = objectSourceUrl.value.slice(0, 2000); if (!object.provenance.capturedAt && object.provenance.sourceUrl) object.provenance.capturedAt = nowIso(); }));

    bindControl('[data-scw-object-duplicate]', 'click', () => {
    ...
    bindControl('[data-scw-object-export]', 'click', () => {
      const project = activeProject(); const object = activeObject(); if (!project || !object) return;
    ...
    bindControl('[data-scw-object-archive]', 'click', () => {
      const project = activeProject(); const object = activeObject(); if (!project || !object) return;
    ...
    if(traceExportButton) traceExportButton.addEventListener('click',()=>{const project=activeProject();if(!project)return;const payload={schema:'sc-workspace-traceability-export/1.0',workspaceVersion:rootVersion(),exportedAt:nowIso(),project:{id:project.id,title:project.title},traceability:JSON.parse(JSON.stringify(project.traceability))};downloadJson(`${safeFileName(project.title)}.traceability.json`,payload);addActivity(project,'traceability-export','Traceability package exported');persist('Traceability export recorded');});


    window.addEventListener('message',event=>{if(event.origin!==window.location.origin||event.data?.type!=='sc-workspace-notebook-assistance-response'||!event.data.packet)return;const packet=event.data.packet;try{window.sessionStorage.setItem(NOTEBOOK_ASSISTANCE_RESPONSE_KEY,JSON.stringify(packet));}catch(_){}const project=state.projects.find(p=>p.id===String(packet.project?.id||''));if(project){renderNotebookAssistance(project);renderNotebook();}});
    ...
    bindControl('[data-scw-object-delete]', 'click', () => {
      const project = activeProject(); const object = activeObject(); if (!project || !object) return;
    ...
    if(labOpen)labOpen.addEventListener('click',()=>{const project=activeProject(),ids=selectedLabObjectIds();if(!project){if(labStatus)labStatus.textContent='Choose an active project first.';return;}const handoff=createLabHandoff(project,ids),target=new URL('/lab/',window.location.origin);target.searchParams.set('sc_workspace_project',project.id);target.searchParams.set('sc_workspace_handoff',handoff.id);target.searchParams.set('sc_workspace_intent','compute');target.searchParams.set('sc_workspace_lab_workflow',String(labWorkflow?.value||'model-studio'));target.searchParams.set('sc_workspace_origin','workspace');target.searchParams.set('sc_workspace_return','1');try{window.sessionStorage.setItem(HANDOFF_KEY,JSON.stringify({schema:HANDOFF_SCHEMA,handoffId:handoff.id,projectId:project.id,objectIds:handoff.objectIds,destination:'lab',intent:'compute',createdAt:nowIso(),returnUrl:root.dataset.returnUrl||window.location.href,returnSchema:HANDOFF_RETURN_SCHEMA,returnStorageKey:HANDOFF_RETURN_KEY}));}catch(_){}persist('Lab handoff prepared');if(labStatus)labStatus.textContent=`Lab handoff ${handoff.id} prepared with ${ids.length} selected object reference(s). URL carries IDs only.`;window.open(target.toString(),'_blank');renderHandoffs(project);});
    if(labExportContext)labExportContext.addEventListener('click',()=>{const project=activeProject(),ids=selectedLabObjectIds();if(!project||!ids.length){if(labStatus)labStatus.textContent='Select at least one scientific context object before export.';return;}const handoff=createLabHandoff(project,ids),pkg=buildLabContext(project,handoff);if(!pkg){if(labStatus)labStatus.textContent='Lab integration helper is unavailable.';return;}lastLabContextPackage=pkg;downloadJson(`${safeFileName(project.title)}-${handoff.id}.sc-lab-context.json`,pkg);addActivity(project,'handoff',`Exported explicit Lab scientific context · ${handoff.id}`);persist('Lab scientific context exported');renderHandoffs(project);if(labStatus)labStatus.textContent=`Exported ${ids.length} selected object(s) for ${String(labWorkflow?.value||'model-studio')}. Nothing was uploaded automatically.`;});
    if(labExportReturnTemplate)labExportReturnTemplate.addEventListener('click',()=>{const api=labIntegrationApi();if(!api){if(labStatus)labStatus.textContent='Lab integration helper is unavailable.';return;}let pkg=lastLabContextPackage;const project=activeProject();if(!pkg&&project){const entries=(project.handoffs?.entries||[]).filter(h=>h.destination==='lab');const handoff=entries[0];if(handoff)pkg=buildLabContext(project,handoff);}const template=api.returnTemplate(pkg);if(!template){if(labStatus)labStatus.textContent='Export scientific context first, or create a Lab handoff.';return;}downloadJson(`${safeFileName(project?.title||'workspace')}-${template.handoffId}.sc-lab-return-template.json`,template);if(labStatus)labStatus.textContent='Lab return template exported.';});
    if(labImportReturn&&labImportReturnFile)labImportReturn.addEventListener('click',()=>labImportReturnFile.click());
    if(labImportReturnFile)labImportReturnFile.addEventListener('change',async()=>{const file=labImportReturnFile.files?.[0];labImportReturnFile.value='';if(!file)return;try{const api=labIntegrationApi();if(!api)throw new Error('Lab integration helper is unavailable.');const converted=api.toWorkspaceReturnPacket(JSON.parse(await file.text()));if(!converted.ok)throw new Error(converted.message);const sourceIds=[...converted.packet.sourceObjectIds||[]];const result=ingestReturnPacket(state,converted.packet,{mode:'manual',allowUnmatched:false});if(!result.ok)throw new Error(result.message);addLabTraceability(result.project,sourceIds.length?sourceIds:result.entry.objectIds,result.createdIds,result.entry.id);persist(`${result.message} Scientific traceability recorded.`);render();setWorkspaceView('projects');if(labStatus)labStatus.textContent=`${result.message} Derived-from edges were recorded from explicit source context.`;}catch(err){if(labStatus)labStatus.textContent=err?.message||'Workspace could not import that Lab scientific return.';}});
    if(specialistDestination)specialistDestination.addEventListener('change',()=>renderSpecialistIntegration(activeProject()));
    if(specialistOpen)specialistOpen.addEventListener('click',()=>{const project=activeProject(),ids=selectedSpecialistObjectIds();if(!project){if(specialistStatus)specialistStatus.textContent='Choose an active project first.';return;}const handoff=createSpecialistHandoff(project,ids),destination=handoff.destination,target=new URL(specialistRoute(),window.location.origin);target.searchParams.set('sc_workspace_project',project.id);target.searchParams.set('sc_workspace_handoff',handoff.id);target.searchParams.set('sc_workspace_intent',handoff.intent);target.searchParams.set('sc_workspace_workflow',String(specialistWorkflow?.value||''));target.searchParams.set('sc_workspace_origin','workspace');target.searchParams.set('sc_workspace_return','1');try{window.sessionStorage.setItem(HANDOFF_KEY,JSON.stringify({schema:HANDOFF_SCHEMA,handoffId:handoff.id,projectId:project.id,objectIds:handoff.objectIds,destination,intent:handoff.intent,createdAt:nowIso(),returnUrl:root.dataset.returnUrl||window.location.href,returnSchema:HANDOFF_RETURN_SCHEMA,returnStorageKey:HANDOFF_RETURN_KEY}));}catch(_){}persist(`${specialistLabel()} handoff prepared`);if(specialistStatus)specialistStatus.textContent=`${specialistLabel()} handoff ${handoff.id} prepared with ${ids.length} selected object reference(s). URL carries IDs only.`;window.open(target.toString(),'_blank');renderHandoffs(project);});
    if(specialistExportContext)specialistExportContext.addEventListener('click',()=>{const project=activeProject(),ids=selectedSpecialistObjectIds();if(!project||!ids.length){if(specialistStatus)specialistStatus.textContent='Select at least one compatible context object before export.';return;}const handoff=createSpecialistHandoff(project,ids),pkg=buildSpecialistContext(project,handoff);if(!pkg){if(specialistStatus)specialistStatus.textContent='Round-trip helper is unavailable.';return;}lastSpecialistContextPackage=pkg;downloadJson(`${safeFileName(project.title)}-${handoff.id}.${handoff.destination==='decision-studio'?'sc-decision-context':'sc-workbench-context'}.json`,pkg);addActivity(project,'handoff',`Exported explicit ${specialistLabel()} context · ${handoff.id}`);persist(`${specialistLabel()} context exported`);renderHandoffs(project);if(specialistStatus)specialistStatus.textContent=`Exported ${ids.length} selected object(s) for ${specialistLabel()}. Nothing was uploaded automatically.`;});
    if(specialistExportReturnTemplate)specialistExportReturnTemplate.addEventListener('click',()=>{const api=specialistApi();if(!api){if(specialistStatus)specialistStatus.textContent='Round-trip helper is unavailable.';return;}let pkg=lastSpecialistContextPackage;const project=activeProject(),destination=specialistDestinationKey();if((!pkg||pkg.handoff?.destination!==destination)&&project){const handoff=(project.handoffs?.entries||[]).find(h=>h.destination===destination);if(handoff)pkg=buildSpecialistContext(project,handoff);}const template=api.returnTemplate(pkg);if(!template){if(specialistStatus)specialistStatus.textContent=`Export ${specialistLabel()} context first, or create a matching handoff.`;return;}downloadJson(`${safeFileName(project?.title||'workspace')}-${template.handoffId}.${destination==='decision-studio'?'sc-decision-return-template':'sc-workbench-return-template'}.json`,template);if(specialistStatus)specialistStatus.textContent=`${specialistLabel()} return template exported.`;});
    if(specialistImportReturn&&specialistImportReturnFile)specialistImportReturn.addEventListener('click',()=>specialistImportReturnFile.click());
    if(specialistImportReturnFile)specialistImportReturnFile.addEventListener('change',async()=>{const file=specialistImportReturnFile.files?.[0];specialistImportReturnFile.value='';if(!file)return;try{const api=specialistApi();if(!api)throw new Error('Round-trip helper is unavailable.');const converted=api.toWorkspaceReturnPacket(JSON.parse(await file.text()));if(!converted.ok)throw new Error(converted.message);const sourceIds=[...converted.packet.sourceObjectIds||[]],result=ingestReturnPacket(state,converted.packet,{mode:'manual',allowUnmatched:false});if(!result.ok)throw new Error(result.message);addSpecialistTraceability(result.project,sourceIds.length?sourceIds:result.entry.objectIds,result.createdIds,result.entry.id,result.entry.destination);persist(`${result.message} Specialist traceability recorded.`);render();setWorkspaceView('projects');if(specialistStatus)specialistStatus.textContent=`${result.message} Derived-from edges were recorded from the explicit outbound context.`;}catch(err){if(specialistStatus)specialistStatus.textContent=err?.message||'Workspace could not import that specialist return.';}});
    ...
    if(interoperabilityForm)interoperabilityForm.addEventListener('submit',async(event)=>{event.preventDefault();const projectId=interoperabilityProject&&interoperabilityProject.value;const file=interoperabilityFile&&interoperabilityFile.files&&interoperabilityFile.files[0];if(!projectId){window.alert('Choose a target project first.');return;}if(!file){window.alert('Choose a file to stage.');return;}if(file.size>5*1024*1024){window.alert('For this local release, imports are limited to 5 MB per file.');return;}try{const text=await readFileTextCompat(file);const format=detectInteropFormat(file);const fingerprint=await sha256Text(text);const staged=stageExternalContent(format,text,file.name,fingerprint);const profile=staged._profile||interchangeApi()?.detectProfile(format,text,file.name)||'generic';const relationships=Array.isArray(staged._relationships)?staged._relationships:[];const importReport=staged._importReport||null;const objects=staged.slice(0,MAX_INTEROP_IMPORT_OBJECTS);if(!objects.length)throw new Error('The file did not contain importable artifacts.');stagedInteroperability={projectId,fileName:file.name,format,profile,fingerprint,objects,relationships,importReport};renderInteroperability();}catch(err){stagedInteroperability=null;renderInteroperability();window.alert(err&&err.message?err.message:'Workspace could not stage this file.');}});
    ...
    if(interoperabilityCommit)interoperabilityCommit.addEventListener('click',()=>{if(!stagedInteroperability)return;const project=state.projects.find(p=>p.id===stagedInteroperability.projectId&&!p.archivedAt);if(!project){window.alert('The target project is no longer available.');return;}const capacity=Math.max(0,MAX_OBJECTS-project.objects.length);const incoming=stagedInteroperability.objects.slice(0,capacity);if(!incoming.length){window.alert('The target project has reached its local object limit.');return;}const sourceMap=new Map();incoming.forEach(obj=>{const stagedId=obj.id,sourceId=String(obj._sourceObjectId||'');const finalId=id('scwo');obj.id=finalId;delete obj._importFingerprint;delete obj._sourceObjectId;delete obj._interchangeProfile;delete obj._interchangeMeta;const normalized=normalizeObject(obj);project.objects.unshift(normalized);sourceMap.set(stagedId,finalId);if(sourceId)sourceMap.set(sourceId,finalId);});let importedRelations=0;(Array.isArray(stagedInteroperability.relationships)?stagedInteroperability.relationships:[]).forEach(rel=>{if(project.traceability.lineage.length>=MAX_LINEAGE_RELATIONS)return;const from=sourceMap.get(rel.fromObjectId),to=sourceMap.get(rel.toObjectId);if(!from||!to||from===to)return;project.traceability.lineage.push({id:id('tl'),relation:TRACE_RELATIONS.has(rel.relation)?rel.relation:'derived-from',fromObjectId:from,toObjectId:to,note:String(rel.note||'Imported interoperability relationship').slice(0,1000),createdAt:nowIso()});importedRelations++;});if(importedRelations)touchTraceability(project);project.updatedAt=nowIso();addActivity(project,'interop-import',`Imported ${incoming.length} artifact(s)${importedRelations?` and ${importedRelations} relationship(s)`:''} from ${stagedInteroperability.fileName}`);state.interoperability.history.unshift({id:id('io'),direction:'import',format:stagedInteroperability.profile||stagedInteroperability.format,projectId:project.id,projectTitle:project.title,fileName:stagedInteroperability.fileName,fingerprint:stagedInteroperability.fingerprint,objectCount:incoming.length,at:nowIso()});state.interoperability.history=state.interoperability.history.slice(0,MAX_INTEROP_HISTORY);touchInteroperability();const message=`Imported ${incoming.length} artifact(s) as draft Workspace Objects. Review provenance before using them as evidence.`;stagedInteroperability=null;if(interoperabilityFile)interoperabilityFile.value='';persist(message);render();setWorkspaceView('interoperability');});
    if(interoperabilityExport)interoperabilityExport.addEventListener('click',async()=>{const project=state.projects.find(p=>p.id===(interoperabilityExportProject&&interoperabilityExportProject.value)&&!p.archivedAt);if(!project){window.alert('Choose a project to export.');return;}const profile=interchangeApi()?.normalizeProfile(interoperabilityExportProfile&&interoperabilityExportProfile.value||'workspace-json')||'workspace-json';let name='',objectCount=0;if(profile==='portable-project'){const pkg=await portableProjectPackage(project,{includeArchived:false,includeActivity:false,includeAi:false});name=`${safeFileName(project.title)}.sc-workspace-project.json`;downloadJson(name,pkg);objectCount=pkg.manifest.objectCount;}else{const out=interchangeApi()?.exportProject(project,profile,rootVersion());if(!out){const pkg=interchangePackage(project);name=`${safeFileName(project.title)}.sc-workspace-interchange.json`;downloadJson(name,pkg);objectCount=pkg.objects.length;}else{name=out.filename;objectCount=out.objectCount;if(typeof out.data==='string')downloadText(name,out.data,`${out.mime};charset=utf-8`);else downloadJson(name,out.data);}}state.interoperability.history.unshift({id:id('io'),direction:'export',format:profile,projectId:project.id,projectTitle:project.title,fileName:name,fingerprint:'',objectCount,at:nowIso()});state.interoperability.history=state.interoperability.history.slice(0,MAX_INTEROP_HISTORY);touchInteroperability();persist(`Interchange profile exported: ${profile}`);renderInteroperability();});
    ...
    if (cloudBackupButton) cloudBackupButton.addEventListener('click',async()=>{
      ...
      const syncEnrollmentForBackup=syncEnrollment(project.id,false);
      if(syncEnrollmentForBackup?.enabled){recordSync('backup-blocked',project,syncEnrollmentForBackup.serverRevision);cloudStatus.textContent='Manual backup is disabled for a project with active sync. Use Sync now so the revision precondition protects the cloud head.';renderCloudRecovery();return;}
      try {
    ...
    if(syncMigrationExportButton)syncMigrationExportButton.addEventListener('click',async()=>{const project=state.projects.find(item=>item.id===String(syncProject?.value||'')&&!item.archivedAt);if(!project)return;try{const pkg=await createDeviceMigrationPackage(project);downloadJson(`${safeFileName(project.title)}.sc-workspace-device-migration.json`,pkg);state.crossDeviceSync.migrations.unshift({id:id('sym'),direction:'export',sourceProjectId:project.id,outputProjectId:'',projectFingerprint:pkg.source.projectFingerprint,at:nowIso()});recordSync('migration-export',project,syncEnrollment(project.id,false)?.serverRevision||0);persist('Device-migration package exported');if(syncMigrationStatus)syncMigrationStatus.textContent='Migration package exported. It contains a project copy and sync baseline metadata, but no device identity, account profile, nonce, or automatic sync enrollment.';}catch(error){if(syncMigrationStatus)syncMigrationStatus.textContent=error.message||'Migration package export failed.';}});
    if(syncMigrationImportButton&&syncMigrationFile)syncMigrationImportButton.addEventListener('click',()=>syncMigrationFile.click());
    if(syncMigrationFile)syncMigrationFile.addEventListener('change',async()=>{const file=syncMigrationFile.files&&syncMigrationFile.files[0];if(!file)return;try{if(file.size>8*1024*1024)throw new Error('Device-migration packages are limited to 8 MB.');const pkg=JSON.parse(await readFileTextCompat(file)),assessment=await inspectDeviceMigrationPackage(pkg);if(!assessment.ok)throw new Error(assessment.reason);if(assessment.duplicate)throw new Error('This exact migration package was already imported on this device. Export a newer source project or use the existing imported copy.');const copy=cloneProject(assessment.project);copy.title=`${assessment.sourceProjectTitle} (Migrated copy)`.slice(0,120);copy.persistence=projectPersistenceTemplate();addActivity(copy,'device-migration-import',`Imported device-migration copy from source ${assessment.sourceProjectId}. Sync enrollment was not transferred.`);state.projects.unshift(copy);state.activeProjectId=copy.id;state.crossDeviceSync.migrations.unshift({id:id('sym'),direction:'import',sourceProjectId:assessment.sourceProjectId,outputProjectId:copy.id,projectFingerprint:assessment.projectFingerprint,at:nowIso()});recordSync('migration-import',copy,assessment.serverRevision);persist('Device-migration package imported as a new local copy');render();setWorkspaceView('projects',true);if(syncMigrationStatus)syncMigrationStatus.textContent='Migration package imported as a new local copy. Sync remains disabled until you explicitly enroll the new copy.';}catch(error){if(syncMigrationStatus)syncMigrationStatus.textContent=error.message||'Migration package import failed.';}finally{syncMigrationFile.value='';}});

    async function updateCrossDeviceProductionPlan(project,result){
      const api=crossDeviceProductionApi(); if(!api||!project){lastCrossDeviceProductionPlan=null;if(syncPlanState)syncPlanState.textContent='NOT CHECKED';if(syncPlanReason)syncPlanReason.textContent='Choose a project and check status to derive the continuity plan.';if(syncPlanExport)syncPlanExport.disabled=true;return null;}
      const enrollment=result?.enrollment||syncEnrollment(project.id,false)||{}, remote=result?.remote||remoteCloudRecord(project.id), localFingerprint=result?.localFingerprint||await syncProjectFingerprint(project);
      const p=api.plan({projectId:project.id,authenticated:Boolean(IDENTITY_CONFIG.authenticated),enrolled:Boolean(enrollment.enabled),localFingerprint,remoteFingerprint:String(remote?.projectFingerprint||remote?.fingerprint||enrollment.remoteFingerprint||''),baseFingerprint:String(enrollment.lastSyncedFingerprint||''),localUpdatedAt:project.updatedAt||'',remoteUpdatedAt:remote?.backedUpAt||'',serverRevision:Number(enrollment.serverRevision||0),remoteRevision:Number(remote?.revision||0),remoteExists:Boolean(remote),pendingOperation:Boolean(continuityApi()?.pendingFor?.(state.crossDeviceSync.operations,project.id))});
      lastCrossDeviceProductionPlan=p;if(syncPlanState)syncPlanState.textContent=String(p.state||'unknown').replaceAll('-',' ').toUpperCase();if(syncPlanReason)syncPlanReason.textContent=p.reason;if(syncPlanExport)syncPlanExport.disabled=false;return p;
    }
    if (syncProject) syncProject.addEventListener('change', () => { lastCrossDeviceProductionPlan=null; renderAccountSync(); const project=state.projects.find(item=>item.id===String(syncProject?.value||'')&&!item.archivedAt); updateCrossDeviceProductionPlan(project,null); });
    ...
    if (syncCheckButton) syncCheckButton.addEventListener('click', async () => {
      ...
      try {
        ...
        renderCloudRecovery(); renderAccountSync(); syncStatus.textContent = result.message; await updateCrossDeviceProductionPlan(project,result);
      } catch (error) {
    ...
    if (syncNowButton) syncNowButton.addEventListener('click', async () => {
      ...
      try {
        ...
        persist('Cross-device synchronization completed'); render(); await updateCrossDeviceProductionPlan(project,await evaluateSync(project,true));
      } catch (error) {
        const enrollment = syncEnrollment(project.id, true);
        ...
        persist('Cross-device sync state updated'); renderAccountSync(); lastCrossDeviceProductionPlan=null; if(syncPlanState)syncPlanState.textContent='RECHECK REQUIRED'; if(syncPlanReason)syncPlanReason.textContent='The last sync action did not complete cleanly. Check status again before choosing another continuity action.'; if(syncPlanExport)syncPlanExport.disabled=true; syncStatus.textContent = error.message || 'Synchronization failed.';
      } finally { syncNowButton.disabled = false; }
    });
    if(syncPlanExport)syncPlanExport.addEventListener('click',()=>{const project=state.projects.find(item=>item.id===String(syncProject?.value||'')&&!item.archivedAt),api=crossDeviceProductionApi();if(!project||!api||!lastCrossDeviceProductionPlan)return;const receipt=api.receipt(lastCrossDeviceProductionPlan,{workspaceVersion:rootVersion(),createdAt:nowIso()});downloadJson(`${safeFileName(project.title)}.sc-workspace-sync-receipt.json`,receipt);recordSync('continuity-receipt-export',project,receipt.remoteRevision||receipt.serverRevision||0);if(syncStatus)syncStatus.textContent='Exported a privacy-minimized continuity receipt. Project content, source URLs, account profile, and device identity were not included.';});
    ...
    if(collabActorForm)collabActorForm.addEventListener('submit',event=>{event.preventDefault();const api=collaborationArchitectureApi(),fd=new FormData(collabActorForm);if(!api||collaborationArchitecture.actors.length>=api.MAX_ACTORS)return;const actor=api.createActor({displayName:String(fd.get('displayName')||''),role:String(fd.get('role')||'observer')},id,nowIso);if(!actor)return;collaborationArchitecture.actors.push(actor);persistCollaborationArchitecture('Actor added locally. No account or server identity was created.');collabActorForm.reset();renderCollaborationArchitecture();});
    if(collabArchProject)collabArchProject.addEventListener('change',()=>renderCollaborationArchitecture());
    if(collabPolicyForm)collabPolicyForm.addEventListener('submit',event=>{event.preventDefault();const api=collaborationArchitectureApi(),fd=new FormData(collabPolicyForm),projectId=String(fd.get('projectId')||''),ownerActorId=String(fd.get('ownerActorId')||'');if(!api||!projectId||!ownerActorId)return;let policy=collaborationArchitecture.policies.find(p=>p.projectId===projectId);if(!policy)policy=api.createPolicy(projectId,ownerActorId,id,nowIso);else{policy=api.normalizePolicy(policy);policy.ownerActorId=ownerActorId;policy.grants=policy.grants.filter(g=>g.actorId!==ownerActorId);policy.updatedAt=nowIso();}collaborationArchitecture.policies=collaborationArchitecture.policies.filter(p=>p.projectId!==projectId);collaborationArchitecture.policies.push(policy);persistCollaborationArchitecture('Project ownership policy saved locally. This is not a server permission grant.');renderCollaborationArchitecture();});
    if(collabGrantForm)collabGrantForm.addEventListener('submit',event=>{event.preventDefault();const api=collaborationArchitectureApi(),project=activeCollabArchitectureProject(),policy=activeCollabPolicy(),fd=new FormData(collabGrantForm);if(!api||!project||!policy)return;const actorId=String(fd.get('actorId')||''),role=String(fd.get('role')||'observer');if(!actorId)return;collaborationArchitecture.policies=collaborationArchitecture.policies.map(p=>p.id===policy.id?api.setGrant(p,actorId,role,policy.ownerActorId,nowIso):p);persistCollaborationArchitecture('Role grant saved as local collaboration policy. No server access was changed.');renderCollaborationArchitecture();});
    if(collabCommentForm)collabCommentForm.addEventListener('submit',event=>{event.preventDefault();const api=collaborationArchitectureApi(),project=activeCollabArchitectureProject(),fd=new FormData(collabCommentForm);if(!api||!project||collaborationArchitecture.comments.length>=api.MAX_COMMENTS)return;const objectId=String(fd.get('objectId')||''),obj=project.objects.find(o=>o.id===objectId&&!o.archivedAt),target={kind:obj?'object':'project',projectId:project.id,objectId:obj?.id||'',objectTitle:obj?.title||''},comment=api.createComment({actorId:String(fd.get('actorId')||''),body:String(fd.get('body')||'')},target,id,nowIso);if(!comment)return;collaborationArchitecture.comments.push(comment);persistCollaborationArchitecture('Comment added around a canonical target. Source content was unchanged.');collabCommentForm.reset();renderCollaborationArchitecture();});
    if(collabProposalForm)collabProposalForm.addEventListener('submit',event=>{event.preventDefault();const api=collaborationArchitectureApi(),project=activeCollabArchitectureProject(),fd=new FormData(collabProposalForm);if(!api||!project||collaborationArchitecture.proposals.length>=api.MAX_PROPOSALS)return;const objectId=String(fd.get('objectId')||''),obj=project.objects.find(o=>o.id===objectId&&!o.archivedAt);if(!obj)return;const proposal=api.createProposal({actorId:String(fd.get('actorId')||''),kind:String(fd.get('kind')||'custom'),field:String(fd.get('field')||''),proposedValue:String(fd.get('proposedValue')||''),rationale:String(fd.get('rationale')||'')},{kind:'object',projectId:project.id,objectId:obj.id,objectTitle:obj.title},id,nowIso);if(!proposal)return;collaborationArchitecture.proposals.push(proposal);persistCollaborationArchitecture('Review proposal created. It has not been applied to canonical project content.');collabProposalForm.reset();renderCollaborationArchitecture();});
    if(collabContractPreview)collabContractPreview.addEventListener('click',()=>{const api=collaborationArchitectureApi(),project=activeCollabArchitectureProject(),policy=activeCollabPolicy();if(!api||!project||!policy){if(collabArchStatus)collabArchStatus.textContent='Choose a project and save an ownership policy first.';return;}const contract=api.buildShareableProjectContract(project,policy,collaborationArchitecture.actors,project.objects.filter(o=>!o.archivedAt).map(o=>o.id),rootVersion());if(collabContractOutput)collabContractOutput.textContent=JSON.stringify(contract,null,2);if(collabArchStatus)collabArchStatus.textContent='Contract preview contains IDs, ownership, scope, and role declarations only—no project content.';});
    if(collabContractExport)collabContractExport.addEventListener('click',()=>{const api=collaborationArchitectureApi(),project=activeCollabArchitectureProject(),policy=activeCollabPolicy();if(!api||!project||!policy){if(collabArchStatus)collabArchStatus.textContent='Choose a project and save an ownership policy first.';return;}const contract=api.buildShareableProjectContract(project,policy,collaborationArchitecture.actors,project.objects.filter(o=>!o.archivedAt).map(o=>o.id),rootVersion());downloadJson(`${safeFileName(project.title)}.sc-workspace-shareable-contract.json`,contract);if(collabArchStatus)collabArchStatus.textContent='Shareable-project contract exported. Project content was not included.';});
    if(collabArchitectureExport)collabArchitectureExport.addEventListener('click',()=>{const api=collaborationArchitectureApi();if(!api)return;downloadJson(`workspace-collaboration-architecture-${new Date().toISOString().slice(0,10)}.json`,api.exportArchitecture(collaborationArchitecture));if(collabArchStatus)collabArchStatus.textContent='Collaboration architecture exported without canonical project content.';});
    if(collabArchitectureImport)collabArchitectureImport.addEventListener('click',()=>collabArchitectureFile?.click());
    if(collabArchitectureFile)collabArchitectureFile.addEventListener('change',async()=>{const file=collabArchitectureFile.files?.[0];collabArchitectureFile.value='';if(!file)return;try{const api=collaborationArchitectureApi(),pkg=JSON.parse(await readFileTextCompat(file)),result=api.importArchitecture(pkg);if(!result.ok){if(collabArchStatus)collabArchStatus.textContent=result.message;return;}const incoming=result.architecture,by=(items,key='id')=>new Map(items.map(x=>[x[key],x]));const actors=by(collaborationArchitecture.actors),policies=by(collaborationArchitecture.policies,'projectId'),comments=by(collaborationArchitecture.comments),proposals=by(collaborationArchitecture.proposals);incoming.actors.forEach(x=>{if(!actors.has(x.id))actors.set(x.id,x);});incoming.policies.forEach(x=>{if(!policies.has(x.projectId))policies.set(x.projectId,x);});incoming.comments.forEach(x=>{if(!comments.has(x.id))comments.set(x.id,x);});incoming.proposals.forEach(x=>{if(!proposals.has(x.id))proposals.set(x.id,x);});collaborationArchitecture=api.normalizeArchitecture({actors:[...actors.values()],policies:[...policies.values()],comments:[...comments.values()],proposals:[...proposals.values()],updatedAt:nowIso()});persistCollaborationArchitecture('Collaboration architecture imported as local coordination records. No project content was created or overwritten.');renderCollaborationArchitecture();}catch(_){if(collabArchStatus)collabArchStatus.textContent='Could not read that Collaboration Architecture package.';}});
    ...
    if(collabFile)collabFile.addEventListener('change',async()=>{const file=collabFile.files&&collabFile.files[0];if(!file)return;if(file.size>8*1024*1024){window.alert('Review packages are limited to 8 MB in this local release.');collabFile.value='';return;}try{const pkg=JSON.parse(await readFileTextCompat(file)),verification=await verifyReviewPackage(pkg);stagedReviewPackage={fileName:file.name,package:pkg,verification};renderCollaboration();}catch(_){stagedReviewPackage=null;renderCollaboration();window.alert('Workspace could not stage this review package.');}finally{collabFile.value='';}});
    ...
    if(institutionalExport)institutionalExport.addEventListener('click',async()=>{const handoff=activeInstitutionalHandoff();if(!handoff)return;const project=state.projects.find(p=>p.id===handoff.projectId);if(!project)return;const validator=window.SCWorkspaceInstitutionalValidation,draft=validator?validator.handoffDraftAssessment(handoff,project):{transferAllowed:true,attention:0};if(!draft.transferAllowed){window.alert('Institutional promotion export blocked: validate the recipient, purpose, scope, and acknowledgements.');return;}try{await gateFromLatestRestore('institutional-promotion',project,async()=>{const pkg=await institutionalPromotionPackage(handoff),packageAssessment=validator?await validator.handoffPackageAssessment(pkg,state):{transferAllowed:true,attention:0,stale:false};if(!packageAssessment.transferAllowed)throw new Error('Institutional promotion package failed integrity or disclosure validation.');if((packageAssessment.attention||packageAssessment.stale)&&!window.confirm('Institutional validation found attention items. Review them before transfer. Export this frozen package anyway?'))return;const name=`${safeFileName(project.title||'workspace-project')}.sc-workspace-institutional-handoff.json`;downloadJson(name,pkg);handoff.status='exported';handoff.packageFingerprint=String(pkg.integrity?.payloadFingerprint||'');handoff.exportedAt=nowIso();handoff.updatedAt=handoff.exportedAt;state.institutional.history.unshift({id:id('ihh'),direction:'export',kind:'promotion',handoffId:handoff.id,projectId:handoff.projectId,projectTitle:project.title||'',organizationLabel:handoff.organizationLabel,fileName:name,fingerprint:handoff.packageFingerprint,status:handoff.status,objectCount:handoff.objectIds.length,at:handoff.exportedAt});state.institutional.history=state.institutional.history.slice(0,MAX_INSTITUTIONAL_HISTORY);touchInstitutional();addActivity(project,'institutional-export',`Institutional promotion package exported for ${handoff.organizationLabel}`);persist('Institutional promotion package exported');renderInstitutional();});}catch(err){window.alert(err?.message||'Institutional handoff preflight could not be opened.');}});
    ...
    if(institutionalFile)institutionalFile.addEventListener('change',async()=>{const file=institutionalFile.files&&institutionalFile.files[0];if(!file)return;if(file.size>2*1024*1024){window.alert('Institutional receipt files are limited to 2 MB.');institutionalFile.value='';return;}try{const pkg=JSON.parse(await readFileTextCompat(file)),verification=await verifyInstitutionalReceipt(pkg),hardening=window.SCWorkspaceInstitutionalValidation?await window.SCWorkspaceInstitutionalValidation.receiptAssessment(pkg,state):null;if(hardening&&!hardening.commitAllowed)verification.ok=false;stagedInstitutionalReceipt={fileName:file.name,package:pkg,verification,hardening};renderInstitutional();}catch(_){stagedInstitutionalReceipt=null;renderInstitutional();window.alert('Workspace could not stage this institutional receipt.');}finally{institutionalFile.value='';}});
    if(institutionalClear)institutionalClear.addEventListener('click',()=>{stagedInstitutionalReceipt=null;renderInstitutional();});
    if(institutionalCommit)institutionalCommit.addEventListener('click',()=>{if(!stagedInstitutionalReceipt||!stagedInstitutionalReceipt.verification.ok)return;const hardening=stagedInstitutionalReceipt.hardening;if(hardening&&hardening.requiresUnverifiedAcknowledgement&&!window.confirm('This receipt has no verifiable SHA-256 fingerprint. Commit it only if you have independently verified the institutional source. Continue?'))return;if(hardening&&hardening.chronologyAttention&&!window.confirm('This receipt timestamp predates the local export record. Commit it after reviewing the receiving-system record?'))return;const result=ingestInstitutionalReceiptPacket(state,stagedInstitutionalReceipt.package,stagedInstitutionalReceipt.fileName);if(!result.ok){window.alert(result.message);return;}state.institutional.activeHandoffId=result.handoffId;stagedInstitutionalReceipt=null;persist(result.message);render();setWorkspaceView('institutional');});
    ...
    if(shareFile)shareFile.addEventListener('change',async()=>{stagedPortableProject=null;const file=shareFile.files&&shareFile.files[0];if(!file){renderShare();return;}if(file.size>8*1024*1024){window.alert('Portable project packages are limited to 8 MB in this local release.');shareFile.value='';renderShare();return;}try{const pkg=JSON.parse(await readFileTextCompat(file)),verification=await verifyPortablePackage(pkg);stagedPortableProject={fileName:file.name,package:pkg,verification};renderShare();if(!verification.ok)window.alert(verification.message);}catch(err){stagedPortableProject=null;renderShare();window.alert(err&&err.message?err.message:'Workspace could not read this portable project.');}});
    ...
    if(lifecycleProject)lifecycleProject.addEventListener('change',()=>{if(lifecycleTarget){const project=state.projects.find(item=>item.id===lifecycleProject.value);lifecycleTarget.value=project?.lifecycle?.state||'draft';}renderProjectLifecycle();});
    if(lifecycleTarget)lifecycleTarget.addEventListener('change',renderProjectLifecycle);
    if(lifecycleForm)lifecycleForm.addEventListener('submit',(event)=>{event.preventDefault();const project=state.projects.find(item=>item.id===(lifecycleProject?.value||''));if(!project)return;const target=LIFECYCLE_STAGES.has(lifecycleTarget?.value)?lifecycleTarget.value:'draft',data=new FormData(lifecycleForm),rationale=String(data.get('rationale')||'').trim().slice(0,4000),acknowledged=data.get('acknowledged')==='on';if(!rationale){if(lifecycleStatus)lifecycleStatus.textContent='A rationale is required before recording a lifecycle milestone.';return;}if(!acknowledged){if(lifecycleStatus)lifecycleStatus.textContent='Review and acknowledge the lifecycle declaration boundary before continuing.';return;}const assessment=lifecycleAssessment(project,target),milestone=recordLifecycleMilestone(project,target,rationale,assessment);if(!milestone)return;persist(`Lifecycle milestone recorded: ${lifecycleLabel(target)}`);lifecycleForm.reset();render();setWorkspaceView('lifecycle');if(lifecycleProject)lifecycleProject.value=project.id;if(lifecycleTarget)lifecycleTarget.value=target;renderProjectLifecycle();});
    ...
    if (firstRunForm) firstRunForm.addEventListener('submit', (event) => {
      event.preventDefault();
      const data = new FormData(firstRunForm);
      const result = createFirstRunProject({title:data.get('title'),description:data.get('description'),starter:data.get('starter')});
      if (!result.ok && firstRunStatus) firstRunStatus.textContent = result.message;
    });
    if (firstRunStandard) firstRunStandard.addEventListener('click', () => {
      setWorkspaceView('projects', true);
      if (createForm) {
        createForm.hidden = false;
        const title = createForm.querySelector('input[name="title"]');
    ...
    if (cockpitOpenProject) cockpitOpenProject.addEventListener('click', () => {
      const project = state.projects.find(item => item.id === cockpitOpenProject.dataset.projectId && !item.archivedAt) || activeProject();
      if (!project) return;
      state.activeProjectId = project.id; persist('Project opened from Workspace Home'); render(); setWorkspaceView('projects', true); setProjectMode('overview');
    });
    root.querySelectorAll('[data-scw-cockpit-view]').forEach(button => button.addEventListener('click', () => setWorkspaceView(button.dataset.scwCockpitView, true)));
    cockpitModeCards.forEach(button => button.addEventListener('click', () => {
      const project = activeProject(); if (!project || button.disabled) return;
      const mode = button.dataset.scwCockpitMode || 'overview';
      setProjectMode(mode);
      setWorkspaceView('projects', true);
    }));
...
if(researchTemplateSelect)researchTemplateSelect.addEventListener('change',()=>renderResearchTemplates(activeProject()));
if(researchTemplateApply)researchTemplateApply.addEventListener('click',()=>{const project=activeProject(),template=selectedResearchTemplate();if(!project||!template)return;const result=installResearchTemplate(project,template,researchTemplateOptions());if(result.ok){persist('Research template applied');render();setProjectMode('guide');}if(researchTemplateStatus)researchTemplateStatus.textContent=result.message;});
if(researchTemplateCreateProject)researchTemplateCreateProject.addEventListener('click',()=>{const template=selectedResearchTemplate();if(!template)return;const options=researchTemplateOptions(),api=researchTemplateApi(),plan=api.plan(template,options);if(state.projects.length>=MAX_PROJECTS){window.alert('This device has reached the Workspace Project limit.');return;}const project=projectTemplate(plan.projectTitle,plan.projectDescription);state.projects.unshift(project);state.activeProjectId=project.id;const result=installResearchTemplate(project,template,options);persist('Project starter created from research template');render();setProjectMode('guide');if(researchTemplateStatus)researchTemplateStatus.textContent=`New project created. ${result.message}`;});
if(researchTemplateSaveWorkflow)researchTemplateSaveWorkflow.addEventListener('click',()=>{const project=activeProject(),run=activeWorkflowRun(project),api=researchTemplateApi();if(!project||!run||!api){if(researchTemplateStatus)researchTemplateStatus.textContent='Open an active guided workflow before saving its structure as a reusable template.';return;}const helper=notebookHelper(),activeNb=helper?project.notebooks.notebooks.find(n=>n.id===project.notebooks.activeNotebookId)||null:null;const title=window.prompt('Reusable template name',`${run.title} Template`);if(title===null)return;const template=api.templateFromWorkflow(run,activeNb,title);const added=api.addCustom(researchTemplateLibrary,template);if(!added.ok){if(researchTemplateStatus)researchTemplateStatus.textContent='Template library limit reached or the structure was invalid.';return;}researchTemplateLibrary=added.library;saveResearchTemplateLibrary();renderResearchTemplates(project);researchTemplateSelect.value=added.template.id;renderResearchTemplates(project);if(researchTemplateStatus)researchTemplateStatus.textContent='Reusable structure saved. Notes, object references, contents, findings, and completion statuses were not copied.';});
if(researchTemplateExport)researchTemplateExport.addEventListener('click',()=>{const api=researchTemplateApi();if(!api)return;downloadJson('workspace-research-templates.json',api.exportPackage(researchTemplateLibrary));if(researchTemplateStatus)researchTemplateStatus.textContent='Custom research-template library exported. Built-in templates and project content are not embedded.';});
if(researchTemplateImport)researchTemplateImport.addEventListener('click',()=>researchTemplateImportFile?.click());
if(researchTemplateImportFile)researchTemplateImportFile.addEventListener('change',async()=>{const file=researchTemplateImportFile.files?.[0],api=researchTemplateApi();if(!file||!api)return;try{const pkg=JSON.parse(await readFileTextCompat(file)),verified=api.verifyPackage(pkg);if(!verified.ok)throw new Error(verified.reason);let library=researchTemplateLibrary;for(const t of verified.library.customTemplates){const added=api.addCustom(library,{...t,id:id('rt')});if(added.ok)library=added.library;}researchTemplateLibrary=library;saveResearchTemplateLibrary();renderResearchTemplates(activeProject());if(researchTemplateStatus)researchTemplateStatus.textContent='Template package imported as new custom template copies.';}catch(_){if(researchTemplateStatus)researchTemplateStatus.textContent='Template import failed integrity or schema validation.';}researchTemplateImportFile.value='';});
    ...
    workspaceContextNavs.forEach((nav)=>nav.addEventListener('keydown',(event)=>{
      if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;
      const buttons=[...nav.querySelectorAll('[data-scw-workspace-view]')].filter(button=>!button.closest('details')||button.closest('details').open); if(!buttons.length)return;
    ...
    if(changeReconcile)changeReconcile.addEventListener('click',()=>{if(!activeChangeReview)return;setWorkspaceView('reconcile',true);populateReconciliationSelectors(changeProject?.value||'');if(reconcileProject)reconcileProject.value=changeProject?.value||'';populateReconciliationSelectors(reconcileProject?.value||'');if(reconcileBase)reconcileBase.value=changeBase?.value||'';if(reconcileTarget)reconcileTarget.value=changeTarget?.value||'current';if(reconcileLoad)reconcileLoad.disabled=!reconcileProject?.value||!reconcileBase?.value||reconcileBase.value===reconcileTarget?.value;activeReconciliation=null;renderReconciliation();});
    if(reconcileProject)reconcileProject.addEventListener('change',()=>{activeReconciliation=null;populateReconciliationSelectors(reconcileProject.value);renderReconciliation();});
    if(reconcileBase)reconcileBase.addEventListener('change',()=>{activeReconciliation=null;if(reconcileLoad)reconcileLoad.disabled=!reconcileProject?.value||!reconcileBase.value||reconcileBase.value===reconcileTarget?.value;renderReconciliation();});
    if(reconcileTarget)reconcileTarget.addEventListener('change',()=>{activeReconciliation=null;if(reconcileLoad)reconcileLoad.disabled=!reconcileProject?.value||!reconcileBase?.value||reconcileBase.value===reconcileTarget.value;renderReconciliation();});
    if(reconcileLoad)reconcileLoad.addEventListener('click',()=>{const ctx=buildActiveReconciliation();if(!ctx){if(reconcileStatus)reconcileStatus.textContent='Unable to build this comparison.';return;}renderReconciliation();});
    if(reconcileSelectAll)reconcileSelectAll.addEventListener('click',()=>{if(!activeReconciliation)return;activeReconciliation.selected=new Set(window.SCWorkspaceReconciliation.allChanges(activeReconciliation.review).map(item=>item.key));renderReconciliation();});
    if(reconcileClear)reconcileClear.addEventListener('click',()=>{if(!activeReconciliation)return;activeReconciliation.selected.clear();renderReconciliation();});
    const refreshReconciliationCreate=()=>{if(reconcileCreate)reconcileCreate.disabled=!(reconcileAck?.checked&&activeReconciliation?.result?.canCreate&&String(reconcileReviewer?.value||'').trim()&&String(reconcileRationale?.value||'').trim());};
    if(reconcileAck)reconcileAck.addEventListener('change',refreshReconciliationCreate);
    if(reconcileReviewer)reconcileReviewer.addEventListener('input',refreshReconciliationCreate);
    if(reconcileRationale)reconcileRationale.addEventListener('input',refreshReconciliationCreate);
    if(reconcileExport)reconcileExport.addEventListener('click',()=>{if(!activeReconciliation?.plan)return;downloadJson(`${safeFileName(activeReconciliation.project.title)}-reconciliation-plan-${new Date().toISOString().slice(0,10)}.json`,activeReconciliation.plan);if(reconcileStatus)reconcileStatus.textContent='Reconciliation plan exported. No project state was modified.';});
    if(reconcileCreate)reconcileCreate.addEventListener('click',async()=>{const ctx=activeReconciliation,receiptHelper=window.SCWorkspaceReconciliationReceipt;if(!ctx?.result?.canCreate||!reconcileAck?.checked||!receiptHelper)return;const reviewerLabel=String(reconcileReviewer?.value||'').trim().slice(0,160),rationale=String(reconcileRationale?.value||'').trim().slice(0,4000);if(!reviewerLabel||!rationale){if(reconcileStatus)reconcileStatus.textContent='Record a decision-maker label and rationale before creating the reconciled copy.';return;}const copy=cloneProject(ctx.result.candidate);copy.title=`${ctx.project.title} (Reconciled)`.slice(0,120);const receiptDoc=objectTemplate('document','Reconciliation Decision Receipt');receiptDoc.status='ready';receiptDoc.tags=['reconciliation','decision-receipt','audit'];receiptDoc.provenance={sourceType:'manual',sourceTitle:'Workspace reconciliation decision receipt',sourceUrl:'',capturedAt:nowIso()};copy.objects.unshift(receiptDoc);const all=window.SCWorkspaceReconciliation.allChanges(ctx.review);let receipt=receiptHelper.buildReceipt({id:id('rcr'),createdAt:nowIso(),sourceProjectId:ctx.project.id,sourceProjectTitle:ctx.project.title,outputProjectId:copy.id,outputProjectTitle:copy.title,receiptDocumentObjectId:receiptDoc.id,reviewerLabel,rationale,base:ctx.baseMeta,target:ctx.targetMeta,changeReviewSummary:ctx.review.summary,availableChanges:all,selectedChanges:ctx.plan.selected,blockedChanges:ctx.result.blockers||[],dependencyValidation:'passed',humanAcknowledged:true});const fingerprint=await sha256Text(receiptHelper.fingerprintPayload(receipt));receipt=receiptHelper.withFingerprint(receipt,fingerprint);receiptDoc.summary=`Auditable reconciliation receipt: ${receipt.decision.acceptedCount} accepted, ${receipt.decision.declinedCount} declined.`;receiptDoc.content=receiptHelper.toMarkdown(receipt);addActivity(copy,'reconciled',`Created from ${ctx.plan.selectedChangeCount} explicitly selected change${ctx.plan.selectedChangeCount===1?'':'s'} · ${ctx.baseMeta.label} → ${ctx.targetMeta.label} · receipt ${receipt.id}`);state.projects.unshift(copy);state.activeProjectId=copy.id;recordReconciliation(ctx,copy,receipt);activeReconciliation=null;persist('Reconciled project copy and decision receipt created');render();setWorkspaceView('projects');setProjectMode('overview');});
    ...
    if(integratedSearch)integratedSearch.addEventListener('input',()=>{integratedRenderLimit=120;integratedPrefs.query=integratedSearch.value.slice(0,320);renderIntegratedKnowledge();});
    if(integratedKind)integratedKind.addEventListener('change',()=>{integratedRenderLimit=120;integratedPrefs.kind=integratedKind.value;renderIntegratedKnowledge();});
    if(integratedProject)integratedProject.addEventListener('change',()=>{integratedRenderLimit=120;integratedPrefs.project=integratedProject.value;renderIntegratedKnowledge();});
    if(retrievalSubtype)retrievalSubtype.addEventListener('change',()=>{integratedRenderLimit=120;integratedPrefs.subtype=retrievalSubtype.value;renderIntegratedKnowledge();});
    if(retrievalTag)retrievalTag.addEventListener('input',()=>{integratedRenderLimit=120;integratedPrefs.tag=retrievalTag.value.slice(0,80);renderIntegratedKnowledge();});
    if(retrievalOrigin)retrievalOrigin.addEventListener('change',()=>{integratedRenderLimit=120;integratedPrefs.origin=retrievalOrigin.value;renderIntegratedKnowledge();});
    if(retrievalProvenance)retrievalProvenance.addEventListener('change',()=>{integratedRenderLimit=120;integratedPrefs.provenance=retrievalProvenance.value;renderIntegratedKnowledge();});
    if(retrievalScope)retrievalScope.addEventListener('change',()=>{integratedRenderLimit=120;integratedPrefs.scope=retrievalScope.value;renderIntegratedKnowledge();});
    if(retrievalSort)retrievalSort.addEventListener('change',()=>{integratedRenderLimit=120;integratedPrefs.sort=retrievalSort.value;renderIntegratedKnowledge();});
    if(integratedLoadMore)integratedLoadMore.addEventListener('click',()=>{integratedRenderLimit=Math.min(600,integratedRenderLimit+120);renderIntegratedKnowledge();});
    if(saveSearchButton)saveSearchButton.addEventListener('click',()=>{const api=knowledgeSearchApi();if(!api)return;const name=String(window.prompt('Name this saved search:','Research search')||'').trim();if(!name)return;const stamp=nowIso();savedKnowledgeSearches.unshift(api.savedSearch({name,preferences:integratedPrefs,createdAt:stamp,updatedAt:stamp},()=>id('ss')));savedKnowledgeSearches=savedKnowledgeSearches.slice(0,40);const ok=persistSavedKnowledgeSearches();renderSavedKnowledgeSearches();if(searchStatus)searchStatus.textContent=ok?'Saved locally in this browser.':'Could not save in browser storage.';});
    if(savedSearchSelect)savedSearchSelect.addEventListener('change',()=>{const item=savedKnowledgeSearches.find(x=>x.id===savedSearchSelect.value);if(deleteSearchButton)deleteSearchButton.disabled=!item;if(item){applyIntegratedPrefs(item.preferences);if(searchStatus)searchStatus.textContent=`Loaded “${item.name}”.`;}});
    if(deleteSearchButton)deleteSearchButton.addEventListener('click',()=>{const sid=savedSearchSelect?.value||'';if(!sid)return;savedKnowledgeSearches=savedKnowledgeSearches.filter(x=>x.id!==sid);persistSavedKnowledgeSearches();renderSavedKnowledgeSearches();if(searchStatus)searchStatus.textContent='Saved search deleted from this browser.';});

    if(libraryContinuityOpen)libraryContinuityOpen.addEventListener('click',()=>window.open('/knowledge-libraries/','_blank'));
    if(libraryContinuityImport)libraryContinuityImport.addEventListener('click',()=>libraryContinuityImportFile?.click());
    if(libraryContinuityImportFile)libraryContinuityImportFile.addEventListener('change',async()=>{const file=libraryContinuityImportFile.files?.[0];libraryContinuityImportFile.value='';if(!file)return;try{const api=libraryContinuityApi(),pkg=api.normalizePackage(JSON.parse(await file.text()));if(!pkg.ok){if(libraryContinuityStatus)libraryContinuityStatus.textContent=pkg.message;return;}libraryContinuityItems=api.merge(libraryContinuityItems,pkg.items);saveLibraryContinuity();renderLibraryContinuity();if(libraryContinuityStatus)libraryContinuityStatus.textContent=`Staged ${pkg.items.length} Library record(s) locally. Nothing was added to a project automatically.`;}catch(_){if(libraryContinuityStatus)libraryContinuityStatus.textContent='Could not read that Knowledge Library continuity package.';}});
    if(libraryContinuityClear)libraryContinuityClear.addEventListener('click',()=>{if(!window.confirm('Clear staged Knowledge Library context from this browser? Workspace project sources already created will not be changed.'))return;libraryContinuityItems=[];saveLibraryContinuity();renderLibraryContinuity();if(libraryContinuityStatus)libraryContinuityStatus.textContent='Staged Library context cleared from this browser.';});
    window.addEventListener('sc-library-workspace-continuity',event=>{const api=libraryContinuityApi(),pkg=api?.normalizePackage(event.detail);if(!pkg?.ok)return;libraryContinuityItems=api.merge(libraryContinuityItems,pkg.items);saveLibraryContinuity();renderLibraryContinuity();if(libraryContinuityStatus)libraryContinuityStatus.textContent=`Received ${pkg.items.length} same-origin Library record(s). Explicit project promotion is still required.`;});    if(clearSearchButton)clearSearchButton.addEventListener('click',()=>{applyIntegratedPrefs({query:'',kind:'all',subtype:'all',project:'all',tag:'',origin:'all',provenance:'all',scope:'active',sort:'relevance'});if(searchStatus)searchStatus.textContent='Retrieval fields cleared.';});
    root.querySelectorAll('[data-scw-dynamic-view]').forEach(button=>button.addEventListener('click',()=>{const api=researchCollectionsApi(),search=knowledgeSearchApi();if(!api||!search)return;activeResearchCollectionId='';activeResearchViewId='';integratedPrefs=api.builtinCriteria(button.dataset.scwDynamicView,integratedPrefs,search);dynamicGroupBy='project';dynamicDensity='compact';if(researchCollectionStatus)researchCollectionStatus.textContent=`Applied ${button.textContent.trim()} dynamic view. Current project and scope lens preserved.`;renderIntegratedKnowledge();}));
    if(saveCollectionButton)saveCollectionButton.addEventListener('click',()=>{const api=researchCollectionsApi(),search=knowledgeSearchApi();if(!api||!search)return;const name=String(window.prompt('Name this smart collection:','Research collection')||'').trim();if(!name)return;const description=String(window.prompt('Optional collection description:','')||'').trim();const stamp=nowIso();const item=api.smartCollection({name,description,criteria:integratedPrefs,createdAt:stamp,updatedAt:stamp},()=>id('rc'),search);researchCollections.unshift(item);researchCollections=researchCollections.slice(0,api.MAX_COLLECTIONS);activeResearchCollectionId=item.id;activeResearchViewId='';const ok=persistResearchCollections();if(researchCollectionStatus)researchCollectionStatus.textContent=ok?'Smart collection saved locally. Membership will be recalculated from canonical records.':'Could not save the collection in browser storage.';renderIntegratedKnowledge();});
    if(smartCollectionSelect)smartCollectionSelect.addEventListener('change',()=>{const item=researchCollections.find(x=>x.id===smartCollectionSelect.value);activeResearchCollectionId=item?.id||'';activeResearchViewId='';if(item){integratedPrefs=knowledgeSearchApi()?.normalizePrefs(item.criteria)||item.criteria;if(researchCollectionStatus)researchCollectionStatus.textContent=`Loaded “${item.name}”. Membership recalculated now.`;renderIntegratedKnowledge();}else renderResearchCollectionControls();});
    if(deleteCollectionButton)deleteCollectionButton.addEventListener('click',()=>{if(!activeResearchCollectionId)return;researchCollections=researchCollections.filter(x=>x.id!==activeResearchCollectionId);activeResearchCollectionId='';persistResearchCollections();if(researchCollectionStatus)researchCollectionStatus.textContent='Smart collection definition deleted from this browser. Canonical records were not changed.';renderIntegratedKnowledge();});
    if(researchViewGroup)researchViewGroup.addEventListener('change',()=>{dynamicGroupBy=researchViewGroup.value;activeResearchViewId='';renderIntegratedKnowledge();});
    if(researchViewDensity)researchViewDensity.addEventListener('change',()=>{dynamicDensity=researchViewDensity.value;activeResearchViewId='';renderIntegratedKnowledge();});
    if(saveViewButton)saveViewButton.addEventListener('click',()=>{const api=researchCollectionsApi(),search=knowledgeSearchApi();if(!api||!search)return;const name=String(window.prompt('Name this research view:','Research view')||'').trim();if(!name)return;const stamp=nowIso();const item=api.researchView({name,criteria:integratedPrefs,presentation:{groupBy:dynamicGroupBy,density:dynamicDensity,limit:80},createdAt:stamp,updatedAt:stamp},()=>id('rv'),search);researchViews.unshift(item);researchViews=researchViews.slice(0,api.MAX_VIEWS);activeResearchViewId=item.id;activeResearchCollectionId='';const ok=persistResearchViews();if(researchCollectionStatus)researchCollectionStatus.textContent=ok?'Research view saved locally.':'Could not save the view in browser storage.';renderIntegratedKnowledge();});
    if(researchViewSelect)researchViewSelect.addEventListener('change',()=>{const item=researchViews.find(x=>x.id===researchViewSelect.value);activeResearchViewId=item?.id||'';activeResearchCollectionId='';if(item){integratedPrefs=knowledgeSearchApi()?.normalizePrefs(item.criteria)||item.criteria;dynamicGroupBy=item.presentation.groupBy;dynamicDensity=item.presentation.density;if(researchCollectionStatus)researchCollectionStatus.textContent=`Loaded view “${item.name}”.`;renderIntegratedKnowledge();}else renderResearchCollectionControls();});
    if(deleteViewButton)deleteViewButton.addEventListener('click',()=>{if(!activeResearchViewId)return;researchViews=researchViews.filter(x=>x.id!==activeResearchViewId);activeResearchViewId='';persistResearchViews();if(researchCollectionStatus)researchCollectionStatus.textContent='Saved view deleted from this browser.';renderIntegratedKnowledge();});
    if(citationStyle)citationStyle.addEventListener('change',()=>{citationPreferences.style=citationStyle.value;citationPreferences.updatedAt=nowIso();persistCitationPreferences();renderReferenceLibrary();if(referenceStatus)referenceStatus.textContent='Citation style preference saved locally.';});
    if(referenceSearch)referenceSearch.addEventListener('input',()=>{referenceQuery=referenceSearch.value.slice(0,240);renderReferenceLibrary();});
    if(referenceFormClear)referenceFormClear.addEventListener('click',()=>clearReferenceForm());
    if(referenceForm)referenceForm.addEventListener('submit',(event)=>{event.preventDefault();const api=referenceLibraryApi();if(!api)return;const fd=new FormData(referenceForm),existing=referenceLibrary.references.find(x=>x.id===String(fd.get('id')||'')),stamp=nowIso(),draft={id:String(fd.get('id')||''),type:String(fd.get('type')||'other'),title:String(fd.get('title')||''),authors:String(fd.get('authors')||''),publicationDate:String(fd.get('publicationDate')||''),containerTitle:String(fd.get('containerTitle')||''),publisher:String(fd.get('publisher')||''),pages:String(fd.get('pages')||''),doi:String(fd.get('doi')||''),identifier:String(fd.get('identifier')||''),url:String(fd.get('url')||''),citationKey:String(fd.get('citationKey')||''),tags:String(fd.get('tags')||'').split(',').map(x=>x.trim()).filter(Boolean),notes:String(fd.get('notes')||''),origins:existing?.origins||[],createdAt:existing?.createdAt||stamp,updatedAt:stamp};let ref=api.normalizeReference(draft,()=>id('ref'));if(!ref.title)return;if(!ref.citationKey)ref.citationKey=api.uniqueCitationKey(ref,referenceLibrary.references.filter(x=>x.id!==ref.id));if(existing)referenceLibrary.references=referenceLibrary.references.map(x=>x.id===ref.id?ref:x);else referenceLibrary.references.unshift(ref);referenceLibrary.references=referenceLibrary.references.slice(0,api.MAX_REFERENCES);selectedReferenceId=ref.id;const ok=persistReferenceLibrary();populateReferenceForm(ref);renderReferenceLibrary();const group=api.duplicateGroups(referenceLibrary.references).find(g=>g.references.some(x=>x.id===ref.id));if(referenceStatus)referenceStatus.textContent=ok?(group?'Reference saved. A deterministic duplicate candidate was detected; review it manually.':'Reference saved to the browser-local library.'):'Could not save the reference in browser storage.';});
    if(referenceAddSelected)referenceAddSelected.addEventListener('click',()=>{const api=referenceLibraryApi(),ik=integratedApi();if(!api||!ik)return;const entry=derivedIntegratedEntries(ik).find(e=>e.key===selectedIntegratedKey);if(!entry){if(referenceStatus)referenceStatus.textContent='Select a research result first.';return;}let ref=api.referenceFromEntry(entry,()=>id('ref'));if(!ref.title){if(referenceStatus)referenceStatus.textContent='The selected record does not contain enough recorded bibliographic context to create a reference.';return;}const exact=referenceLibrary.references.find(x=>api.fingerprint(x)===api.fingerprint(ref));if(exact){selectedReferenceId=exact.id;renderReferenceLibrary();if(referenceStatus)referenceStatus.textContent='A reference with the same deterministic fingerprint already exists. Workspace did not create another copy.';return;}ref.citationKey=api.uniqueCitationKey(ref,referenceLibrary.references);referenceLibrary.references.unshift(ref);referenceLibrary.references=referenceLibrary.references.slice(0,api.MAX_REFERENCES);selectedReferenceId=ref.id;persistReferenceLibrary();renderReferenceLibrary();if(referenceStatus)referenceStatus.textContent='Reference created from recorded provenance on the selected canonical research result.';});
    if(referenceExport)referenceExport.addEventListener('click',()=>{const api=referenceLibraryApi();if(!api)return;downloadJson(`workspace-reference-library-${new Date().toISOString().slice(0,10)}.json`,api.exportPackage(referenceLibrary,citationPreferences));if(referenceStatus)referenceStatus.textContent='Reference library exported as portable JSON.';});
    if(referenceImport)referenceImport.addEventListener('click',()=>referenceImportFile?.click());
    if(referenceImportFile)referenceImportFile.addEventListener('change',()=>{const file=referenceImportFile.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const api=referenceLibraryApi();try{const pkg=JSON.parse(String(reader.result||''));if(!api||pkg?.schema!==api.EXPORT_SCHEMA)throw new Error('Unsupported reference package.');const imported=api.normalizeLibrary(pkg.library),fps=new Map((pkg.integrity?.fingerprints||[]).map(x=>[String(x.id),String(x.fingerprint)]));if(imported.references.some(r=>fps.has(r.id)&&fps.get(r.id)!==api.fingerprint(r)))throw new Error('Reference fingerprint verification failed.');const merged=[...referenceLibrary.references];imported.references.forEach(r=>{if(!merged.some(x=>api.fingerprint(x)===api.fingerprint(r))){if(!r.citationKey)r.citationKey=api.uniqueCitationKey(r,merged);merged.push(r);}});referenceLibrary=api.normalizeLibrary({references:merged.slice(0,api.MAX_REFERENCES),updatedAt:nowIso()});citationPreferences=api.preferences(pkg.preferences||citationPreferences);persistReferenceLibrary();persistCitationPreferences();renderReferenceLibrary();if(referenceStatus)referenceStatus.textContent='Reference library imported. Exact fingerprint duplicates were not copied.';}catch(err){if(referenceStatus)referenceStatus.textContent=err?.message||'Reference library import failed.';}finally{referenceImportFile.value='';}};reader.readAsText(file);});
    if(compositionNew)compositionNew.addEventListener('click',()=>{const api=compositionApi();if(!api||compositionLibrary.drafts.length>=api.MAX_DRAFTS)return;const draft=api.normalizeDraft({title:`Research composition ${compositionLibrary.drafts.length+1}`,projectId:state.activeProjectId||''},()=>id('cmp'));compositionLibrary.drafts.unshift(draft);activeCompositionId=draft.id;activeCompositionSectionId='';persistCompositionLibrary();renderCompositionStudio();if(compositionStatus)compositionStatus.textContent='New browser-local composition created.';});
    if(compositionDraftSelect)compositionDraftSelect.addEventListener('change',()=>{activeCompositionId=compositionDraftSelect.value||'';activeCompositionSectionId=activeComposition()?.sections[0]?.id||'';renderCompositionStudio();});
    if(compositionDelete)compositionDelete.addEventListener('click',()=>{const d=activeComposition();if(!d)return;if(!window.confirm('Delete this browser-local composition draft? Canonical research and Workspace Documents are not affected.'))return;compositionLibrary.drafts=compositionLibrary.drafts.filter(x=>x.id!==d.id);activeCompositionId=compositionLibrary.drafts[0]?.id||'';activeCompositionSectionId=activeComposition()?.sections[0]?.id||'';persistCompositionLibrary();renderCompositionStudio();if(compositionStatus)compositionStatus.textContent='Composition draft deleted. Canonical research was unchanged.';});
    [compositionTitle,compositionAbstract].forEach(el=>{if(el)el.addEventListener('input',()=>{const d=activeComposition();if(!d)return;if(el===compositionTitle)d.title=el.value.trim().slice(0,240)||'Untitled research document';else d.abstract=el.value.slice(0,5000);d.updatedAt=nowIso();persistCompositionLibrary();renderCompositionStudio();});});
    if(compositionStatusInput)compositionStatusInput.addEventListener('change',()=>{const d=activeComposition();if(!d)return;d.status=compositionStatusInput.value;d.updatedAt=nowIso();persistCompositionLibrary();renderCompositionStudio();});
    if(compositionSectionForm)compositionSectionForm.addEventListener('submit',event=>{event.preventDefault();const api=compositionApi(),d=activeComposition();if(!api||!d||d.sections.length>=api.MAX_SECTIONS)return;const fd=new FormData(compositionSectionForm),sec=api.normalizeSection({kind:String(fd.get('kind')||'custom'),title:String(fd.get('title')||'Section'),body:String(fd.get('body')||'')},()=>id('csec'));d.sections.push(sec);d.updatedAt=nowIso();activeCompositionSectionId=sec.id;compositionSectionForm.reset();persistCompositionLibrary();renderCompositionStudio();if(compositionStatus)compositionStatus.textContent='Section added. Research inputs and citations remain explicit attachments.';});
    if(compositionAttachResearch)compositionAttachResearch.addEventListener('click',()=>{const api=compositionApi(),ik=integratedApi(),d=activeComposition(),sec=activeCompositionSection();if(!api||!ik||!d||!sec)return;const entry=derivedIntegratedEntries(ik).find(x=>x.key===selectedIntegratedKey);if(!entry){if(compositionStatus)compositionStatus.textContent='Select a research result first.';return;}const next=api.addSource(sec,entry);d.sections=d.sections.map(x=>x.id===sec.id?next:x);d.updatedAt=nowIso();persistCompositionLibrary();renderCompositionStudio();if(compositionStatus)compositionStatus.textContent='Canonical research pointer attached to the selected section.';});
    if(compositionAttachReference)compositionAttachReference.addEventListener('click',()=>{const api=compositionApi(),d=activeComposition(),sec=activeCompositionSection(),ref=referenceLibrary.references.find(x=>x.id===selectedReferenceId);if(!api||!d||!sec||!ref){if(compositionStatus)compositionStatus.textContent='Select a Citation Library reference first.';return;}const next=api.addCitation(sec,ref);d.sections=d.sections.map(x=>x.id===sec.id?next:x);d.updatedAt=nowIso();persistCompositionLibrary();renderCompositionStudio();if(compositionStatus)compositionStatus.textContent='Citation Library reference attached to the selected section.';});
    if(compositionExport)compositionExport.addEventListener('click',()=>{const api=compositionApi(),d=activeComposition();if(!api||!d)return;downloadJson(`${safeFileName(d.title)}-composition.json`,{schema:api.EXPORT_SCHEMA,exportedAt:nowIso(),library:api.normalizeLibrary({drafts:[d]}),integrity:{algorithm:'fnv1a32-composition-fingerprint',drafts:[{id:d.id,fingerprint:api.fingerprint(d)}]}});if(compositionStatus)compositionStatus.textContent='Composition exported with deterministic integrity receipt.';});
    if(compositionImport)compositionImport.addEventListener('click',()=>compositionImportFile?.click());
    if(compositionImportFile)compositionImportFile.addEventListener('change',()=>{const file=compositionImportFile.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const api=compositionApi();try{const pkg=JSON.parse(String(reader.result||''));if(!api||pkg?.schema!==api.EXPORT_SCHEMA)throw new Error('Unsupported composition package.');const incoming=api.normalizeLibrary(pkg.library),fps=new Map((pkg.integrity?.drafts||[]).map(x=>[String(x.id),String(x.fingerprint)]));if(incoming.drafts.some(d=>fps.has(d.id)&&fps.get(d.id)!==api.fingerprint(d)))throw new Error('Composition fingerprint verification failed.');incoming.drafts.forEach(d=>{d.id=id('cmp');d.title=`${d.title} — imported copy`.slice(0,240);d.materializedDocumentId='';d.createdAt=nowIso();d.updatedAt=d.createdAt;compositionLibrary.drafts.unshift(d);});compositionLibrary.drafts=compositionLibrary.drafts.slice(0,api.MAX_DRAFTS);activeCompositionId=compositionLibrary.drafts[0]?.id||'';activeCompositionSectionId=activeComposition()?.sections[0]?.id||'';persistCompositionLibrary();renderCompositionStudio();if(compositionStatus)compositionStatus.textContent='Composition imported as a new browser-local copy.';}catch(err){if(compositionStatus)compositionStatus.textContent=err?.message||'Composition import failed.';}finally{compositionImportFile.value='';}};reader.readAsText(file);});
    if(compositionMaterialize)compositionMaterialize.addEventListener('click',()=>{const api=compositionApi(),refApi=referenceLibraryApi(),d=activeComposition(),project=activeProject();if(!api||!d){return;}if(!project){if(compositionStatus)compositionStatus.textContent='Open a project before creating a Workspace Document.';return;}const payload=api.documentPayload(d,referenceLibrary,refApi?refApi.format:null,citationPreferences.style),obj=objectTemplate('document',payload.title);obj.summary=payload.summary;obj.content=payload.content;obj.status=payload.status;obj.tags=payload.tags;obj.provenance=payload.provenance;project.objects.unshift(obj);project.activeObjectId=obj.id;project.updatedAt=nowIso();d.materializedDocumentId=obj.id;d.projectId=project.id;d.updatedAt=nowIso();addActivity(project,'composition-materialized',`Composition materialized as Document: ${obj.title}`);persistCompositionLibrary();persist('Composition materialized as Workspace Document');render();renderCompositionStudio();if(compositionStatus)compositionStatus.textContent='Workspace Document created explicitly. Source research and the browser-local composition remain unchanged.';});
    if(crossProjectTarget)crossProjectTarget.addEventListener('change',()=>renderCrossProjectKnowledge());
    if(crossProjectAdd)crossProjectAdd.addEventListener('click',()=>{const api=crossProjectApi(),ik=integratedApi();if(!api||!ik)return;const entry=derivedIntegratedEntries(ik).find(e=>e.key===selectedIntegratedKey),targetProjectId=String(crossProjectTarget?.value||'');if(!entry){if(crossProjectStatus)crossProjectStatus.textContent='Select a canonical research result first.';return;}if(!targetProjectId||targetProjectId===entry.projectId){if(crossProjectStatus)crossProjectStatus.textContent='Choose a different target project. Cross-project references cannot point back into the source project.';return;}const result=api.addReference(crossProjectKnowledge,{targetProjectId,source:entry.ref,relation:String(crossProjectRelation?.value||'references'),note:String(crossProjectNote?.value||'')},()=>id('xpk'));crossProjectKnowledge=result.ledger;if(result.added){persistCrossProjectKnowledge();if(crossProjectNote)crossProjectNote.value='';if(crossProjectStatus)crossProjectStatus.textContent='Cross-project reference recorded. No canonical content was copied or changed.';renderCrossProjectKnowledge();renderKnowledgeGraph();}else if(crossProjectStatus)crossProjectStatus.textContent=result.reason==='duplicate'?'That explicit cross-project relationship already exists.':result.reason==='limit'?'The browser-local cross-project reference limit has been reached.':'Could not create the cross-project reference.';});
    ...
    if(groundedForm)groundedForm.addEventListener('submit',event=>{event.preventDefault();const api=groundedResearchApi(),ik=integratedApi();if(!api||!ik)return;if(!groundedScopeKeys.length){if(groundedStatus)groundedStatus.textContent='Add at least one Integrated Knowledge result to the grounding scope.';return;}if(groundedResearchLibrary.sessions.length>=api.MAX_SESSIONS){if(groundedStatus)groundedStatus.textContent='The browser-local grounded request limit has been reached.';return;}const fd=new FormData(groundedForm),question=String(fd.get('question')||'').trim();if(!question){if(groundedStatus)groundedStatus.textContent='Enter a grounded research question.';return;}const session=api.prepareRequest({title:String(fd.get('title')||''),task:String(fd.get('task')||'general-question'),question,selectedKeys:groundedScopeKeys},derivedIntegratedEntries(ik),id,nowIso);if(!session.grounding.length){if(groundedStatus)groundedStatus.textContent='The selected grounding records could not be resolved.';return;}groundedResearchLibrary.sessions.unshift(session);groundedResearchLibrary.sessions=groundedResearchLibrary.sessions.slice(0,api.MAX_SESSIONS);activeGroundedSessionId=session.id;groundedScopeKeys=[];persistGroundedResearchLibrary();groundedForm.reset();renderGroundedResearchAssistant();if(groundedStatus)groundedStatus.textContent=`Frozen request prepared with ${session.grounding.length} grounding record${session.grounding.length===1?'':'s'}.`;});
    if(groundedDelete)groundedDelete.addEventListener('click',()=>{if(!activeGroundedSessionId)return;groundedResearchLibrary.sessions=groundedResearchLibrary.sessions.filter(s=>s.id!==activeGroundedSessionId);activeGroundedSessionId=groundedResearchLibrary.sessions[0]?.id||'';persistGroundedResearchLibrary();renderGroundedResearchAssistant();if(groundedStatus)groundedStatus.textContent='Grounded request removed from this browser. Canonical research was unchanged.';});
    if(groundedExportRequest)groundedExportRequest.addEventListener('click',()=>{const api=groundedResearchApi(),session=activeGroundedSession();if(!api||!session)return;downloadJson(`${safeFileName(session.title)}.sc-workspace-grounded-research-request.json`,api.exportRequest(session));if(groundedStatus)groundedStatus.textContent='Grounded request packet exported.';});
    if(groundedCopy)groundedCopy.addEventListener('click',async()=>{const api=groundedResearchApi(),session=activeGroundedSession();if(!api||!session)return;const body=api.promptMarkdown(session);try{await navigator.clipboard.writeText(body);if(groundedStatus)groundedStatus.textContent='Grounded prompt copied.';}catch(_){downloadText(`${safeFileName(session.title)}.grounded-prompt.md`,body,'text/markdown;charset=utf-8');if(groundedStatus)groundedStatus.textContent='Clipboard unavailable; grounded prompt exported instead.';}});
    if(groundedSaveResponse)groundedSaveResponse.addEventListener('click',()=>{const api=groundedResearchApi(),session=activeGroundedSession();if(!api||!session)return;const result=api.applyResponse(session,groundedResponse?.value||'',groundedResponseSource?.value||'manual',nowIso);if(!result.ok){if(groundedStatus)groundedStatus.textContent=result.message;renderGroundedResearchAssistant();return;}groundedResearchLibrary.sessions=groundedResearchLibrary.sessions.map(s=>s.id===session.id?result.record:s);persistGroundedResearchLibrary();renderGroundedResearchAssistant();if(groundedStatus)groundedStatus.textContent=`Cited draft saved with ${result.record.citations.length} validated grounding citation${result.record.citations.length===1?'':'s'}.`;});
    if(groundedExportResponse)groundedExportResponse.addEventListener('click',()=>{const api=groundedResearchApi(),session=activeGroundedSession();if(!api||!session?.response)return;downloadJson(`${safeFileName(session.title)}.sc-workspace-grounded-research-response.json`,api.exportResponse(session));if(groundedStatus)groundedStatus.textContent='Grounded response draft exported for review.';});
    if(groundedReview)groundedReview.addEventListener('click',()=>{const session=activeGroundedSession();if(!session?.response)return;session.status='reviewed';session.reviewedAt=nowIso();session.updatedAt=session.reviewedAt;persistGroundedResearchLibrary();renderGroundedResearchAssistant();if(groundedStatus)groundedStatus.textContent='Grounded draft marked reviewed. No canonical Document has been created.';});
    if(groundedReject)groundedReject.addEventListener('click',()=>{const session=activeGroundedSession();if(!session?.response)return;session.status='rejected';session.reviewedAt=nowIso();session.updatedAt=session.reviewedAt;persistGroundedResearchLibrary();renderGroundedResearchAssistant();if(groundedStatus)groundedStatus.textContent='Grounded draft rejected. Grounding packet remains available for audit.';});
    if(groundedTargetProject)groundedTargetProject.addEventListener('change',()=>renderGroundedResearchAssistant());
    if(groundedMaterialize)groundedMaterialize.addEventListener('click',()=>{const api=groundedResearchApi(),session=activeGroundedSession(),project=state.projects.find(p=>p.id===String(groundedTargetProject?.value||''));if(!api||!session||session.status!=='reviewed'||!project)return;if(project.objects.length>=MAX_OBJECTS){if(groundedStatus)groundedStatus.textContent=`The target project has reached the ${MAX_OBJECTS}-object local limit.`;return;}const payload=api.materializationPayload(session),obj=objectTemplate('document',payload.title);obj.summary=payload.summary;obj.content=payload.content.slice(0,50000);obj.status=payload.status;obj.tags=normalizeTags(payload.tags);obj.provenance=payload.provenance;project.objects.unshift(obj);project.activeObjectId=obj.id;project.updatedAt=nowIso();session.status='materialized';session.materializedDocumentId=obj.id;session.updatedAt=nowIso();addActivity(project,'grounded-research-document',`Grounded Research Assistant II reviewed draft materialized: ${session.title}`);persistGroundedResearchLibrary();persist('Grounded research draft materialized as Workspace Document');render();renderGroundedResearchAssistant();if(groundedStatus)groundedStatus.textContent='Reviewed grounded draft materialized explicitly. Frozen grounding packet and canonical source records remain unchanged.';});
    if(integratedOpenNotebook)integratedOpenNotebook.addEventListener('click',()=>setWorkspaceView('notebook'));
    if(integratedOpenKnowledge)integratedOpenKnowledge.addEventListener('click',()=>setWorkspaceView('knowledge'));
    if(integratedOpenProject)integratedOpenProject.addEventListener('click',()=>setWorkspaceView('projects'));
    if(graphPathTrace)graphPathTrace.addEventListener('click',()=>{const graph=buildKnowledgeGraph();if(!relationshipExplorer||typeof relationshipExplorer.shortestPath!=='function')return;activeGraphPath=relationshipExplorer.shortestPath(graph,graphPathFrom?.value||'',graphPathTo?.value||'',5);if(graphPath){graphPath.innerHTML='';if(!activeGraphPath.found){graphPath.innerHTML='<div class="scw-knowledge-empty-note">No explicit path was found within five hops.</div>';}else{activeGraphPath.nodes.forEach((node,index)=>{const row=document.createElement('div');row.className='scw-graph-path-step';row.innerHTML=`<span>${String(index+1).padStart(2,'0')}</span><strong>${escapeHtml(node.label)}</strong><small>${escapeHtml(graphNodeLabel(node))}${index<activeGraphPath.edges.length?` · ${escapeHtml(activeGraphPath.edges[index].relation.replaceAll('-',' '))} →`:''}</small>`;graphPath.appendChild(row);});}}if(graphPathStatus)graphPathStatus.textContent=activeGraphPath.found?`Explicit path found: ${activeGraphPath.hops} hop${activeGraphPath.hops===1?'':'s'}.`:'No explicit path found within five hops.';});
    if(graphPathClear)graphPathClear.addEventListener('click',()=>{activeGraphPath=null;if(graphPathFrom)graphPathFrom.value='';if(graphPathTo)graphPathTo.value='';if(graphPath)graphPath.innerHTML='<div class="scw-knowledge-empty-note">Choose two nodes to trace a path through recorded Workspace relationships.</div>';if(graphPathStatus)graphPathStatus.textContent='Path cleared.';});
    if(graphExport)graphExport.addEventListener('click',()=>{const graph=buildKnowledgeGraph();if(!relationshipExplorer||typeof relationshipExplorer.snapshot!=='function')return;const snap=relationshipExplorer.snapshot(graph,{workspaceVersion:'1.8.0',selectedNodeId:state.knowledgeGraph?.selectedNodeId||'',filters:state.knowledgeGraph?.preferences||{},path:activeGraphPath});downloadJson('workspace-knowledge-graph-snapshot.json',snap);if(graphPathStatus)graphPathStatus.textContent='Graph snapshot exported. Canonical object bodies were not copied into the snapshot.';});
    ...
    savedKnowledgeSearches=loadSavedKnowledgeSearches();
    libraryContinuityItems=loadLibraryContinuity(); renderLibraryContinuity();
    researchCollections=loadResearchCollections();
    researchViews=loadResearchViews();
    ...
    collaborationArchitecture=loadCollaborationArchitecture();
    ...
    activeCompositionId=compositionLibrary.drafts[0]?.id||'';
    activeCompositionSectionId=activeComposition()?.sections[0]?.id||'';
    renderSavedKnowledgeSearches();
    ...
    persist();
    renderProjectImportStage();
    renderBackwardCompatibilityMatrix();
    render();

    [
      '[data-scw-workspace-view]', '[data-scw-project-mode]', '[data-scw-cockpit-view]',
      '[data-scw-research-route]', '[data-scw-filter]', '[data-scw-new-project]',
      '[data-scw-cancel-create]', '[data-scw-import-project]', '[data-scw-new-object]',
      '[data-scw-cancel-object]', '[data-scw-pin]', '[data-scw-duplicate]',
      '[data-scw-export]', '[data-scw-archive]', '[data-scw-delete]',
      '[data-scw-object-duplicate]', '[data-scw-object-export]', '[data-scw-object-archive]',
      '[data-scw-object-delete]'
    ].forEach((selector) => root.querySelectorAll(selector).forEach((element) => { element.dataset.scwBound = '1'; }));

    root.dataset.scwRuntimeReady = '1';
    root.dataset.scwRuntimeState = bindingIssues.length ? 'ready-with-missing-optional-controls' : 'ready';
    interactionRuntime.initializedRoots += 1;
    interactionRuntime.ready = interactionRuntime.initializedRoots > 0;
    interactionRuntime.lastReadyAt = new Date().toISOString();
    interactionRuntime.audit = function(){
      const roots = [...document.querySelectorAll('[data-sc-workspace]')];
      const visibleButtons = roots.flatMap((workspaceRoot) => [...workspaceRoot.querySelectorAll('button:not([hidden]), a:not([hidden])')].filter((element) => {
        const style = window.getComputedStyle(element);
        return style.display !== 'none' && style.visibility !== 'hidden';
      }));
      const candidates = visibleButtons.filter((element) => [...element.attributes].some((attribute) => attribute.name.startsWith('data-scw-')));
      const unbound = candidates.filter((element) => element.dataset.scwBound !== '1' && !element.disabled).map((element) => ({
        text: String(element.textContent||'').trim().slice(0,120),
        attributes: [...element.attributes].filter((attribute) => attribute.name.startsWith('data-scw-')).map((attribute) => attribute.name)
      }));
      return {version:INTERACTION_RUNTIME_VERSION, ready:this.ready, roots:roots.length, initializedRoots:this.initializedRoots, issues:[...this.issues], unhandledErrors:[...this.unhandledErrors], visibleActionCandidates:candidates.length, unbound};
    };
  }
  ...
  function boot() {
    ...
    if (globalThis.SCWorkspaceFieldUse && typeof globalThis.SCWorkspaceFieldUse.boot === 'function') globalThis.SCWorkspaceFieldUse.boot(window);
  }
...
/* v1.0.1 — GA Field Stabilization & Production Evidence Closure */

/* v2.0.4 visual regression/theme isolation marker; no routing or data behavior changes. */

/* Workspace v2.4.0 — Dataset, Model & Execution Run Registry */

/* Workspace v2.6.0 — Reproducible Execution Environments & Dependency Manifests */
...
// v2.14.0 Julia Simulation & Numerical Modeling Runtime

// v2.15.0 Predictive Analytics & Machine Learning Runtime

// v2.16.0 Native Arrow / Parquet Cross-Language Exchange

/* v2.17.0 Reproduction & Cross-Runtime Verification */

window.SCWorkspaceForecastingRuntime={version:'2.21.0',operations:['naive','seasonal-naive','linear-trend','exponential-smoothing','holt-winters','arima','backtest','evaluate'],arbitraryCodeExecution:false};

window.SCWorkspaceUncertaintyRuntime={version:'2.21.0',operations:['monte-carlo-weighted-sum','bootstrap-interval','latin-hypercube','correlated-normal','empirical-summary','rank-correlation-sensitivity','variance-contribution-linear','scenario-envelope'],arbitraryCodeExecution:false};

window.SCWorkspaceOptimizationRuntime={version:'2.21.0',operations:['quadratic-box','linear-box','grid-search','random-search','coordinate-descent','pareto-weighted-sum','robust-scenario-rank','parameter-sweep-rank'],arbitraryCodeExecution:false};


/* Workspace v2.22.0 — Robust Decision Optimization & Pareto Analysis */
window.SCWorkspaceDecisionAnalysis={version:'2.22.0',bounded:true};
...
/* Workspace v2.24.1 — Interface Shell & Version Alignment Repair */
window.SCWorkspaceInterfaceShell = Object.freeze({
  schema: 'sc-workspace-interface-shell/2.24.1',
  workspaceVersion: '2.24.1',
  assetLineageRestoredFrom: '2.21.0',
  backendAuthorityBaseline: '2.24.0',
  singleActiveWorkspaceSurface: true,
  historicalReleaseSurfacesPrimaryNavigation: false
});
(function(){
  'use strict';
  function boot(){
    document.querySelectorAll('[data-sc-workspace]').forEach(function(root){
      root.dataset.scwInterfaceShellReady='1';
      root.dataset.scwInterfaceShellVersion='2.24.1';
      /* Fail closed visually: only one application section may be exposed before/after routing. */
      var sections=[].slice.call(root.querySelectorAll('[data-scw-workspace-section]'));
      var visible=sections.filter(function(s){return !s.hidden;});
      if(visible.length!==1 || (visible[0] && visible[0].dataset.scwWorkspaceSection!=='start')){
        sections.forEach(function(section){ section.hidden=section.dataset.scwWorkspaceSection!=='start'; });
      }
      /* Analyze is a primary route into the active project's analysis mode. */
      root.querySelectorAll('[data-scw-primary-analyze]').forEach(function(button){
        button.addEventListener('click',function(){
          setTimeout(function(){
            var analysis=root.querySelector('[data-scw-project-mode="analysis"]');
            if(analysis && !analysis.disabled) analysis.click();
          },0);
        });
      });
    });
...
/* Workspace v2.25.0 — Command & Query API thin-client contract */
window.SCWorkspaceCommandQuery = Object.freeze({
  schema: 'sc-workspace-command-query-client/1.0',
  workspaceVersion: '2.25.0',
  profilePath: '/v1/command-query',
  commandPath: '/v1/commands/execute',
  queryPath: '/v1/queries/execute',
  workspaceOverviewPath: '/v1/read-models/workspace-overview',
  backendAuthoritative: true,
  browserCommandAuthority: false,
  queriesMutate: false
});

;window.SCWorkspaceNotebookOrchestration=Object.freeze({
  version:"2.30.0",
  schema:"sc-workspace-notebook-orchestration/1.0",
  backendAuthoritative:true,
  browserSchedulesDependencies:false,
  endpoints:Object.freeze({profile:"/v1/notebook-orchestration",plans:"/v1/notebook-execution-plans",receipts:"/v1/notebook-orchestration-receipts"})
});

/* v2.27.0 — Reproducible Scientific Study Packages */
window.SCWorkspaceScientificStudyPackages=Object.freeze({version:"2.30.0",backendAuthoritative:true,manifestSchema:"sc-workspace-scientific-study-package/1.0",deterministicManifest:true,artifactSnapshots:true,integrity:"SHA-256",arbitraryCodeExecution:false});


/* Workspace v2.28.0 — Declarative Visualization Specification API */
window.SCWorkspaceVisualizationSpecifications=Object.freeze({
  version:"2.30.0",
  schema:"sc-workspace-visualization-spec/1.0",
  rendererContract:"sc-workspace-renderer-contract/1.0",
  backendAuthoritative:true,
  rendererNeutral:true,
  browserDefinesAnalyticalMeaning:false,
  endpoints:Object.freeze({
    profile:"/v1/visualization-specs/profile",
    specs:"/v1/visualization-specs",
    receipts:"/v1/visualization-spec-receipts"
  }),
