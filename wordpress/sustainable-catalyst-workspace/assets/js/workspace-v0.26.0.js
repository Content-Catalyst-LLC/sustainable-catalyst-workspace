(() => {
  ...
  function reconciliationTemplate() { return { schema: RECONCILIATION_SCHEMA, history: [], updatedAt: nowIso() }; }
  ...
  function init(root) {
    ...
    if(reconcileAck)reconcileAck.addEventListener('change',()=>{if(reconcileCreate)reconcileCreate.disabled=!(reconcileAck.checked&&activeReconciliation?.result?.canCreate);});
    ...
    if(reconcileCreate)reconcileCreate.addEventListener('click',()=>{const ctx=activeReconciliation;if(!ctx?.result?.canCreate||!reconcileAck?.checked)return;const copy=cloneProject(ctx.result.candidate);copy.title=`${ctx.project.title} (Reconciled)`.slice(0,120);addActivity(copy,'reconciled',`Created from ${ctx.plan.selectedChangeCount} explicitly selected change${ctx.plan.selectedChangeCount===1?'':'s'} · ${ctx.baseMeta.label} → ${ctx.targetMeta.label}`);state.projects.unshift(copy);state.activeProjectId=copy.id;recordReconciliation(ctx,copy);activeReconciliation=null;persist('Reconciled project copy created');render();setWorkspaceView('projects');setProjectMode('overview');});
