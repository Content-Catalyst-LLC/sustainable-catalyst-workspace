(() => {
  ...
  function rootVersion(){ const el=document.querySelector('[data-sc-workspace]'); return el&&el.dataset.version?el.dataset.version:'0.19.0'; }
  ...
  function init(root) {
    ...
    function renderIdentity() {
      ...
      if (identityNote) identityNote.textContent = authenticated ? 'You are signed in, but v0.19.0 does not upload or synchronize Workspace Projects; handoff returns remain local to this browser unless you explicitly export them. Export/import remains the cross-device portability path.' : 'Sign-in establishes the identity boundary only. Project sync and server-side project storage remain disabled.';
    ...
    function setWorkspaceView(view) {
      workspaceView = ['projects','knowledge','graph','activity','interoperability','collaboration','institutional','share'].includes(view) ? view : 'projects';
    ...
    function renderKnowledgeCollectionDetail(index) {
      ...
      const exp=document.createElement('button');exp.type='button';exp.className='scw-card-action';exp.textContent='Export JSON';exp.addEventListener('click',()=>{const payload={schema:KNOWLEDGE_COLLECTION_EXPORT_SCHEMA,workspaceVersion:root.dataset.version||'0.19.0',exportedAt:nowIso(),collection:JSON.parse(JSON.stringify(collection)),objects:collection.items.map(ref=>{const e=index.find(x=>x.projectId===ref.projectId&&x.objectId===ref.objectId);return e?{project:{id:e.projectId,title:e.projectTitle},object:JSON.parse(JSON.stringify(e.object))}:null;}).filter(Boolean)};downloadJson(`${safeFileName(collection.title)}.sc-workspace-knowledge.json`,payload);});
    ...
    root.querySelector('[data-scw-export]').addEventListener('click', () => {
      ...
      const payload = { schema: EXPORT_SCHEMA, workspaceVersion: root.dataset.version || '0.19.0', exportedAt: nowIso(), project: portable };
    ...
    importFile.addEventListener('change', () => {
      ...
      reader.onload = () => {
        try {
          ...
          window.alert('Workspace could not import this file. Use a Workspace project JSON export from v0.2.0 through v0.19.0, or a compatible future release.');
    ...
    root.querySelector('[data-scw-object-export]').addEventListener('click', () => {
      const project = activeProject(); const object = activeObject(); if (!project || !object) return;
      const payload = { schema: OBJECT_EXPORT_SCHEMA, workspaceVersion: root.dataset.version || '0.19.0', exportedAt: nowIso(), projectId: project.id, object: JSON.parse(JSON.stringify(object)) };
    ...
    if(institutionalExport)institutionalExport.addEventListener('click',async()=>{const handoff=activeInstitutionalHandoff();if(!handoff)return;try{const pkg=await institutionalPromotionPackage(handoff),project=state.projects.find(p=>p.id===handoff.projectId),name=`${safeFileName(project?.title||'workspace-project')}.sc-workspace-institutional-handoff.json`;downloadJson(name,pkg);handoff.status='exported';handoff.packageFingerprint=String(pkg.integrity?.payloadFingerprint||'');handoff.exportedAt=nowIso();handoff.updatedAt=handoff.exportedAt;state.institutional.history.unshift({id:id('ihh'),direction:'export',kind:'promotion',handoffId:handoff.id,projectId:handoff.projectId,projectTitle:project?.title||'',organizationLabel:handoff.organizationLabel,fileName:name,fingerprint:handoff.packageFingerprint,status:handoff.status,objectCount:handoff.objectIds.length,at:handoff.exportedAt});state.institutional.history=state.institutional.history.slice(0,MAX_INSTITUTIONAL_HISTORY);touchInstitutional();if(project)addActivity(project,'institutional-export',`Institutional promotion package exported for ${handoff.organizationLabel}`);persist('Institutional promotion package exported');renderInstitutional();}catch(err){window.alert(err?.message||'Institutional handoff package could not be exported.');}});
