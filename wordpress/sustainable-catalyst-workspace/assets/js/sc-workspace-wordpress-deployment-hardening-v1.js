(function(root,factory){
  ...
  const PREVIOUS_RELEASE='1.2.0';
  const ROLLBACK_RELEASE='1.2.0';
  const EXPECTED_SCRIPT='workspace-v1.3.0.js';
  const EXPECTED_STYLE='workspace-v1.3.0.css';
  const checks=Object.freeze([
    ['root-version','Workspace runtime version','The rendered Workspace root must report the current v1.3.0 release.'],
    ...
    ['current-script','Current cumulative JavaScript','The page must load workspace-v1.3.0.js rather than an older cumulative shell.'],
    ['current-style','Current cumulative stylesheet','The page must load workspace-v1.3.0.css rather than an older cumulative stylesheet.'],
    ['asset-query','WordPress asset cache version','Current cumulative assets must carry the v1.0.0 WordPress version query when query strings are present.'],
  ...
  const manualItems=Object.freeze([
    ['upload-metadata','WordPress replacement metadata','Before replacement, verify WordPress shows Uploaded Version 1.0.0 and the expected author/requirements.'],
    ...
    ['rest-health','REST health identity','Verify /wp-json/sc-workspace/v1/health reports version 1.0.0 and the deployment-hardening contract is reachable.'],
    ...
    ['rollback-rehearsal','v1.2.0 rollback rehearsal','Verify the bundled v0.84.0 WordPress rollback artifact can be restored and existing browser-local projects remain readable.']
  ...
  function assess(workspaceRoot,options={}){
    ...
    out.push(finding('release-stage','Release stage',['release-candidate','general-availability','ga-stabilization','workspace-home','universal-search','library-continuity'].includes(releaseStage),`Workspace stage is ${releaseStage||'unset'}; expected release-candidate, general-availability, ga-stabilization, workspace-home, universal-search, or library-continuity.`));
