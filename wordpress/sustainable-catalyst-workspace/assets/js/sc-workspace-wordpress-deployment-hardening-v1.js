(function(root,factory){
  ...
  const EXPECTED_SCRIPT='workspace-v1.14.0.js';
  const EXPECTED_STYLE='workspace-v1.14.0.css';
  const checks=Object.freeze([
    ['root-version','Workspace runtime version','The rendered Workspace root must report the current v1.14.0 release.'],
    ...
    ['current-script','Current cumulative JavaScript','The page must load workspace-v1.14.0.js rather than an older cumulative shell.'],
    ['current-style','Current cumulative stylesheet','The page must load workspace-v1.14.0.css rather than an older cumulative stylesheet.'],
  ...
  const manualItems=Object.freeze([
    ['upload-metadata','WordPress replacement metadata','Before replacement, verify WordPress shows Uploaded Version 1.13.0 and the expected author/requirements.'],
    ...
    ['rest-health','REST health identity','Verify /wp-json/sc-workspace/v1/health reports version 1.13.0 and the deployment-hardening contract is reachable.'],
    ...
    ['rollback-rehearsal','v1.13.0 rollback rehearsal','Verify the bundled v1.10.0 WordPress rollback artifact can be restored and existing browser-local projects remain readable.']
