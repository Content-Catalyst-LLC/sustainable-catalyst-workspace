(function(root,factory){
  ...
  const PREVIOUS_RELEASE='2.0.3';
  const ROLLBACK_RELEASE='2.0.3';
  const EXPECTED_SCRIPT='workspace-v2.0.4.js';
  const EXPECTED_STYLE='workspace-v2.0.4.css';
  const checks=Object.freeze([
    ['root-version','Workspace runtime version','The rendered Workspace root must report the current v2.0.4 release.'],
    ...
    ['current-script','Current cumulative JavaScript','The page must load workspace-v2.0.4.js rather than an older cumulative shell.'],
    ['current-style','Current cumulative stylesheet','The page must load workspace-v2.0.4.css rather than an older cumulative stylesheet.'],
  ...
  const manualItems=Object.freeze([
    ['upload-metadata','WordPress replacement metadata','Before replacement, verify WordPress shows Uploaded Version 2.0.4 and the expected author/requirements.'],
    ...
    ['rest-health','REST health identity','Verify /wp-json/sc-workspace/v1/health reports version 2.0.4 and the deployment-hardening contract is reachable.'],
    ...
    ['rollback-rehearsal','v2.0.3 rollback rehearsal','Verify the bundled v2.0.3 WordPress rollback artifact can be restored and existing browser-local projects remain readable.']
