(function(root,factory){
  ...
  const RELEASE_VERSION='0.81.0';
  const PREVIOUS_RELEASE='0.80.0';
  const EXPECTED_SCRIPT='workspace-v0.81.0.js';
  const EXPECTED_STYLE='workspace-v0.81.0.css';
  const checks=Object.freeze([
    ['root-version','Workspace runtime version','The rendered Workspace root must report the current v0.81.0 release.'],
    ...
    ['current-script','Current cumulative JavaScript','The page must load workspace-v0.81.0.js rather than an older cumulative shell.'],
    ['current-style','Current cumulative stylesheet','The page must load workspace-v0.81.0.css rather than an older cumulative stylesheet.'],
    ['asset-query','WordPress asset cache version','Current cumulative assets must carry the v0.81.0 WordPress version query when query strings are present.'],
  ...
  const manualItems=Object.freeze([
    ['upload-metadata','WordPress replacement metadata','Before replacement, verify WordPress shows Uploaded Version 0.81.0 and the expected author/requirements.'],
    ...
    ['rest-health','REST health identity','Verify /wp-json/sc-workspace/v1/health reports version 0.81.0 and the deployment-hardening contract is reachable.'],
    ...
    ['rollback-rehearsal','v0.80.0 rollback rehearsal','Verify the bundled v0.80.0 WordPress rollback artifact can be restored and existing browser-local projects remain readable.']
  ...
  function parseAsset(doc,kind,expected){
    ...
    const stale=nodes.map(node=>String(node.getAttribute?.(attr)||node[attr]||'')).filter(value=>/workspace-v0\.(?:7\d|80)\.0\.(?:js|css)/.test(value)&&!value.includes(expected));
  ...
  function assess(workspaceRoot,options={}){
    ...
    out.push(finding('release-stage','Release Candidate stage',releaseStage==='release-candidate',`Workspace stage is ${releaseStage||'unset'}; expected release-candidate.`));
  ...
  function checklist(version){return{schema:CHECKLIST_SCHEMA,generatedAt:stamp(),workspaceVersion:String(version||RELEASE_VERSION).slice(0,40),rollbackRelease:PREVIOUS_RELEASE,items:manualItems.map(x=>({...x,state:'not-verified'})),instructions:'Perform these checks on the deployed WordPress Release Candidate. A green automated gate does not close the manual items.',governance:{manual:true,automaticCompletion:false,automaticSubmission:false,projectDataMutation:false}};}
  function contract(){return{schema:SCHEMA,reportSchema:REPORT_SCHEMA,checklistSchema:CHECKLIST_SCHEMA,releaseVersion:RELEASE_VERSION,previousRelease:PREVIOUS_RELEASE,releaseCandidate:true,featureFreeze:true,storageSchemaVersion:35,projectSchema:'sc-workspace-project/20.0',projectExportSchema:'sc-workspace-project-export/20.0',schemaMigrationRequired:false,safeBootstrapGuard:true,activationPreflight:true,boundedDeploymentHistory:true,mixedVersionBrowserDetection:true,versionedAssetFilenames:true,rollbackArtifactRequired:true,automaticCachePurge:false,automaticRollback:false,automaticProjectMigration:false,projectDataRead:false,projectDataMutation:false,telemetry:false};}
  return Object.freeze({SCHEMA,REPORT_SCHEMA,CHECKLIST_SCHEMA,RELEASE_VERSION,PREVIOUS_RELEASE,EXPECTED_SCRIPT,EXPECTED_STYLE,checks,manualItems,assess,report,checklist,contract});
});
