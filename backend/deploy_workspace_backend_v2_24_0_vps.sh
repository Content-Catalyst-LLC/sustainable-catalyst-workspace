...
ZIP_PATH="${1:-/tmp/sustainable-catalyst-workspace-backend-v2.24.0.zip}"
BASE="/opt/sustainable-catalyst"
NEW="$BASE/sustainable-catalyst-workspace-backend-v2.24.0"

[[ -f "$ZIP_PATH" ]] || { echo "ERROR: backend ZIP not found: $ZIP_PATH" >&2; exit 1; }
ENV_SOURCE=""
for candidate in \
  "$BASE/sustainable-catalyst-workspace-backend-v2.24.0/.env" \
...
assert d['ok'] is True and d['version']=='2.24.0', d
...
PYAUTHDEL
echo 'PASS: Workspace backend v2.24.0 API + authoritative domain state + mutation receipts + inherited scientific runtime fabric are running.'
