...
def test_typed_contract_includes_all_sync_routes():
    item=client_profile(app.openapi()); assert item['workspaceVersion'] in {'2.31.0','2.32.0'}; assert item['typedEndpointCount']==len(TYPED_ENDPOINTS) and item['typedEndpointCount']>=20; assert item['missingOpenApiOperations']==[]
...
def test_health_advertises_local_first_protocol():
    h=health(); assert h['version'] in {'2.31.0','2.32.0'}; assert h['localFirstSynchronizationProtocol'] is True; assert h['offlineOutboxAuthoritative'] is False; assert h['syncAutomaticSemanticMerge'] is False
