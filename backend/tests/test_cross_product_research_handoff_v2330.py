...
def test_typed_contract_exposes_handoff_routes():
    item=client_profile(app.openapi()); assert item['workspaceVersion']=='2.34.0'; assert item['typedEndpointCount']==len(TYPED_ENDPOINTS)==34; assert item['typedEndpoints']['handoffCreate']['path']=='/v1/handoffs'; assert item['requestSchemas']['handoffCreate']=='sc-workspace-research-handoff-request/1.0'; assert item['missingOpenApiOperations']==[]

def test_health_advertises_handoff_fabric_without_generic_destination_mutation():
    h=health(); assert h['version']=='2.34.0'; assert h['crossProductResearchHandoffFabric'] is True; assert h['handoffRevisionPinning'] and h['handoffFingerprintPinning']; assert h['handoffGenericDestinationMutation'] is False
